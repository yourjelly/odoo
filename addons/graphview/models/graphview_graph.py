...
class GraphviewGraph(models.Model):
    ...
    def create_ondelete_dep(self):
        ...
        if ondelete_graph := self.env.ref('graphview.ondelete', raise_if_not_found=False):
            ondelete_graph.write(data)
        else:
            ondelete_graph = self.create(data)
            self.env['ir.model.data'].create({
                'name': 'ondelete',
                'model': 'graphview.graph',
                'res_id': ondelete_graph.id,
                'module': 'graphview',
                'noupdate': True
            })
