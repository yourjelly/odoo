...
class TestMailResend(TestMailCommon):
    ...
    def test_mail_resend_workflow(self):
        ...
        self._reset_bus()
        data = list()
        data.append((self.cr.dbname, 'res.partner', self.partner_admin.id))
        if self.user_partner != self.partner_admin:
            data.append((self.cr.dbname, 'res.partner', self.user_partner.id))
