...
class TestMailMessageAccess(MessageAccessCommon):
    ...
    #  - document-based (write or create, using _get_mail_message_access and _mail_post_access)
    ...
    def test_access_unlink_portal(self):
        """ Read unlink check for portal users. """
        for msg, msg_vals, should_crash, reason in [
            # document based
            (self.record_portal.message_ids[0], {}, True, 'W Access on record but unlink limited'),
    ...
    #   - author
    #   - recipients / notified
    #   - document-based (write or create)
    # ------------------------------------------------------------
