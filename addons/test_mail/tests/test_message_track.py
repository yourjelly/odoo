...
class TestTracking(TestMailCommon):
    ...
    def test_message_track_no_subtype(self):
        ...
        with self.mock_mail_gateway():
            self.record.write(
                {"name": "Test2", "customer_id": customer.id,}
            )
            self.flush_tracking()
    ...
    def test_message_track_template(self):
        ...
        with self.mock_mail_gateway():
            self.record.write(
                {"name": "Test2", "customer_id": self.user_admin.partner_id.id,}
            )
            self.flush_tracking()
...
class TestTrackingMonetary(TestMailMultiCompanyCommon):
    def setUp(self):
        ...
        record = (
            ...
            .with_user(self.user_employee)
            .with_context(self._test_context)
            .create({"company_id": self.user_employee.company_id.id,})
        )
        self.flush_tracking()
        self.record = record.with_context(mail_notrack=False)
