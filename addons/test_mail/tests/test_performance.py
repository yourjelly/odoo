...
class TestMailPerformance(BaseMailPerformance):
    def setUp(self):
        ...
        self.env["mail.performance.thread"].create(
            [
                {"name": "Object 0", "value": 0, "partner_id": self.res_partner_3.id,},
                {"name": "Object 1", "value": 10, "partner_id": self.res_partner_3.id,},
                {"name": "Object 2", "value": 20, "partner_id": self.res_partner_4.id,},
                {
                    "name": "Object 3",
                    "value": 30,
    ...
    def test_write_mail_simple(self):
        ...
        with self.assertQueryCount(__system__=1, emp=1):
            rec.write(
                {"name": "Test2", "email_from": "test@test.com",}
            )
...
class TestMailAPIPerformance(BaseMailPerformance):
    def setUp(self):
        ...
        self.customer = (
            ...
            .with_context(self._quick_create_ctx)
            .create({"name": "Test Customer", "email": "customer.test@example.com",})
        )
        self.user_test = (
            self.env["res.users"]
            .with_context(self._quick_create_ctx)
    ...
    def test_mail_composer(self):
        ...
        with self.assertQueryCount(__system__=2, emp=2):
            composer = (
                ...
                .create(
                    {"body": "<p>Test Body</p>", "partner_ids": [(4, customer_id)],}
                )
            )
...
class TestMailComplexPerformance(BaseMailPerformance):
    def setUp(self):
        ...
        self.env["ir.config_parameter"].sudo().set_param(
            "mail.catchall.alias", "test-catchall"
        )
        self.env["ir.config_parameter"].sudo().set_param(
            "mail.bounce.alias", "test-bounce"
        )
    ...
    def test_complex_tracking_subscription_subtype(self):
        ...
        with self.assertQueryCount(__system__=75, emp=75):
            rec.write(
                {"name": "Test2", "container_id": self.container.id,}
            )
    ...
    def test_complex_tracking_subscription_write(self):
        ...
        container2 = (
            ...
            .with_context(mail_create_nosubscribe=True)
            .create(
                {"name": "Test Container 2", "customer_id": False, "alias_name": False,}
            )
        )
    ...
    def test_complex_tracking_template(self):
        ...
        with self.assertQueryCount(__system__=28, emp=29):
            rec.write(
                {"name": "Test2", "customer_id": customer_id, "user_id": self.env.uid,}
            )
                ...
                [
                    {
                        ...
                        "attachment_ids": [
                            (0, 0, {"name": "test file 0 - %d" % j, "datas": "data",})
                            for j in range(2)
                        ],
                        "notification_ids": [
                            (
                    ...
                    },
                    {
                        ...
                        "attachment_ids": [
                            (0, 0, {"name": "test file 1 - %d" % j, "datas": "data",})
                            for j in range(2)
                        ],
                        "notification_ids": [
                            (
...
class TestMailHeavyPerformancePost(BaseMailPerformance):
    def setUp(self):
        ...
        self.customer = (
            self.env["res.partner"]
            .with_context(self._quick_create_ctx)
            .create({"name": "customer", "email": "customer@example.com",})
        )
        self.record = (
            self.env["mail.test.container"]
            .with_context(mail_create_nosubscribe=True)
        ...
        self.partner_follower = (
            ...
            .with_context(self._quick_create_ctx)
            .create(
                {"name": "partner_follower", "email": "partner_follower@example.com",}
            )
        )
        ...
        self.partner = (
            self.env["res.partner"]
            .with_context(self._quick_create_ctx)
            .create({"name": "partner", "email": "partner@example.com",})
        )
        self.vals = [
            {
