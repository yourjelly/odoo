...
class TestActivityRights(TestActivityCommon):
    ...
    def test_activity_security_user_noaccess_manual(self):
        ...
        read_group_result = (
            self.env["mail.activity"]
            .with_user(self.user_employee)
            .read_group([("id", "=", test_activity.id)], ["summary"], ["summary"],)
        )
        ...
        with patch.object(
            ...
            with self.assertRaises(exceptions.AccessError):
                self.env["mail.activity"].with_user(self.user_employee).read_group(
                    [("id", "=", test_activity.id)], ["summary"], ["summary"],
                )
...
class TestActivityFlow(TestActivityCommon):
    ...
    def test_activity_summary_sync(self):
        ...
        email_activity_type = ActivityType.create(
            {"name": "email", "summary": "Email Summary",}
        )
    ...
    def test_action_feedback_attachment(self):
        ...
        partner = self.env["res.partner"].create({"name": "Tester",})
...
class TestActivityMixin(TestActivityCommon):
    ...
    def setUpClass(cls):
        ...
        cls.user_utc = mail_new_test_user(cls.env, name="User UTC", login="User UTC",)
        cls.user_utc.tz = "UTC"

        cls.user_australia = mail_new_test_user(
            cls.env, name="user Australia", login="user Australia",
        )
