...
QUnit.module("test_mail", {}, function () {
    ...
    QUnit.skipRefactoring(
        ...
        async function (assert) {
            ...
            const pyEnv = await startServer();
            const mailTestTrackAllId1 = pyEnv["mail.skipRefactoring.track.all"].create({
                float_field: 1,
            });
            const { click } = await this.start({ res_id: mailTestTrackAllId1 });
    ...
    QUnit.skipRefactoring(
        ...
        async function (assert) {
            ...
            await testUtils.fields.editAndTrigger(
                document.querySelector("div[name=date_field] .o_datepicker .o_datepicker_input"),
                "12/14/2018",
                ["change"]
            );
            await click(".o_form_button_save");
        ...
        async function (assert) {
            ...
            await testUtils.fields.editAndTrigger(
                document.querySelector(
                    "div[name=datetime_field] .o_datepicker .o_datepicker_input"
                ),
                "12/14/2018 13:42:28",
                ["change"]
            );
            ...
            const savedRecord = pyEnv
                .getData()
                ["mail.skipRefactoring.track.all"].records.find(
                    ({ id }) => id === mailTestTrackAllId1
                );
