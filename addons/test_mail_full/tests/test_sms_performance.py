...
class TestSMSPerformance(BaseMailPerformance, sms_common.SMSCase):
    def setUp(self):
        ...
        self.user_employee.write(
            {"login": "employee", "country_id": self.env.ref("base.be").id,}
        )
    ...
    def test_message_sms_record_1_partner(self):
        ...
        with self.mockSMSGateway(sms_allow_unlink=True), self.assertQueryCount(
            employee=24
        ):  # test_mail_enterprise: 19
            messages = record._message_sms(body="Performance Test", partner_ids=pids,)
    ...
    def test_message_sms_record_default(self):
        ...
        with self.mockSMSGateway(sms_allow_unlink=True), self.assertQueryCount(
            ...
            messages = record._message_sms(body="Performance Test",)
...
class TestSMSMassPerformance(BaseMailPerformance, sms_common.MockSMS):
    def setUp(self):
        ...
        self.user_employee.write(
            {"login": "employee", "country_id": be_country_id,}
        )
        self.admin = self.env.user
        self.admin.write(
        ...
        for x in range(50):
            ...
            records += (
                self.env["mail.test.sms"]
                .with_context(**self._quick_create_ctx)
                .create({"name": "Test_%s" % (x), "customer_id": partners[x].id,})
            )
    ...
    def test_composer_mass_active_domain(self):
        composer = (
            ...
            .create({"body": self._test_body, "mass_keep_log": False,})
        )
    ...
    def test_composer_mass_active_domain_w_log(self):
        composer = (
            ...
            .create({"body": self._test_body, "mass_keep_log": True,})
        )
