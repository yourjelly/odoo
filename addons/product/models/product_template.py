...
class ProductTemplate(models.Model):
    ...
    def _compute_template_field_from_variant_field(self, fname, default=False):
        ...
        """
        mapping = {}
        variant_to_read = self.env['product.product']
        for template in self:
            variant_count = len(template.product_variant_ids)
            if variant_count == 1:
                mapping[template] = template.product_variant_ids
                variant_to_read |= template.product_variant_ids
        ...
        variant_to_read.read([fname])
        for template, variant in mapping.items():
            template[fname] = variant[fname]
