...
class ResCompany(models.Model):
    ...
    def _activate_or_create_pricelists(self):
        ...
        if self.env.user._has_group('product.group_product_pricelist'):
    ...
    def write(self, vals):
        ...
        enabled_pricelists = self.env.user._has_group('product.group_product_pricelist')
        res = super(
            ResCompany, self.with_context(disable_company_pricelist_creation=True)
        ).write(vals)
        if not enabled_pricelists and self.env.user._has_group('product.group_product_pricelist'):
