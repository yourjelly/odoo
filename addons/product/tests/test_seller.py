...
class TestSeller(TransactionCase):
    ...
    def test_20_sellers_company(self):
        ...
        names = self.product_consu.with_context(partner_id=self.asustec.id,).name_get()
        ...
        names = self.product_consu.with_context(
            partner_id=self.asustec.id, company_id=company_a.id,
        ).name_get()
        ref = set([x[1] for x in names])
        ...
        names = self.product_consu.with_context(
            partner_id=self.asustec.id, company_id=company_b.id,
        ).name_get()
        ref = set([x[1] for x in names])
        self.assertEqual(len(names), 2, "2 vendor references should have been found")
        self.assertEqual(
