    ...
    def test_update_variant_with_nocreate(self):
        ...
        template.attribute_line_ids = [
            (
                0,
                0,
                {"attribute_id": self.size.id, "value_ids": [(6, 0, self.size_S.ids)],},
            )
        ]
        self.assertEqual(len(template.product_variant_ids), 1)
...
class TestVariantsImages(common.TestProductCommon):
    def setUp(self):
        ...
        self.template = self.env["product.template"].create({"name": "template",})

        color_values = self.env["product.attribute.value"].create(
            [
                {"name": color, "attribute_id": product_attribute.id, "sequence": i,}
...
class TestVariantsArchive(common.TestProductCommon):
    ...
    def setUp(self):
        ...
        color_values = self.env["product.attribute.value"].create(
            [
                {"name": n, "sequence": i, "attribute_id": self.pa_color.id,}
        ...
        size_values = self.env["product.attribute.value"].create(
            [
                {"name": n, "sequence": i, "attribute_id": self.pa_size.id,}
    ...
    def test_dynamic_attributes_archiving(self):
        ...
        color_values = ProductAttributeValue.create(
            [
                {"name": n, "sequence": i, "attribute_id": pa_color.id,}
                for i, n in enumerate(["white", "black"])
            ]
        )
        ...
        size_values = ProductAttributeValue.create(
            [
                {"name": n, "sequence": i, "attribute_id": pa_size.id,}
                for i, n in enumerate(["s", "m"])
            ]
        )
        ...
        material_values = ProductAttributeValue.create(
            [{"name": "Wood", "sequence": 1, "attribute_id": pa_material.id,}]
        )
    ...
    def _get_add_all_attributes_command(self):
        return [
            (0, 0, {"attribute_id": pa.id, "value_ids": [(6, 0, pa.value_ids.ids)],})
    ...
    def _assert_2color_x_0size(self, variants=None):
        ...
        self.assertEqual(len(variants), 2)
        self._assert_required_combinations(
            variants, required_values=[self.ptav_color_white, self.ptav_color_black,]
        )
