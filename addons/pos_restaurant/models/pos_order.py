...
class PosOrder(models.Model):
    ...
    def remove_from_ui(self, server_ids):
        tables = self.env['sale.order'].search([('id', 'in', server_ids)]).table_id
    ...
    def sync_from_ui(self, orders):
        ...
        if self.env.context.get('table_ids'):
            order_ids = [order['id'] for order in result['sale.order']]
            ...
            if len(table_orders) > 0:
                ...
                result['sale.order'].extend(table_orders.read(table_orders._load_pos_data_fields(config_id), load=False))
                ...
                result['sale.order.line'].extend(table_orders.lines.read(table_orders.lines._load_pos_data_fields(config_id), load=False))
