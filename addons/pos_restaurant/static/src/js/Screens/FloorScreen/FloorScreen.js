odoo.define('pos_restaurant.FloorScreen', function (require) {
    ...
    class FloorScreen extends PosComponent {
        ...
        constructor() {
            ...
            this.state = useState({ selectedTableId: false });
        ...
        _onCreateTable() {
            this.env.actionHandler({ name: 'actionCreateTable', args: [this.props.activeFloor] });
        }
        _onDuplicateTable() {
            this.env.actionHandler({ name: 'actionDuplicateTable', args: [this.env.model.getSelectedTable()] });
        }
        ...
        async _changeShape() {
            ...
            await this.env.actionHandler({ name: 'actionUpdateTable', args: [selectedTable, { shape: newShape }] });
        }
        async _renameTable() {
            const selectedTable = this.env.model.getSelectedTable();
            if (!selectedTable) return;
            ...
            if (newName !== selectedTable.name) {
                await this.env.actionHandler({ name: 'actionUpdateTable', args: [selectedTable, { name: newName }] });
            }
        }
        async _setTableColor({ detail: color }) {
            const selectedTable = this.env.model.getSelectedTable();
            await this.env.actionHandler({ name: 'actionUpdateTable', args: [selectedTable, { color }] });
        }
        ...
        async _deleteTable() {
            ...
            if (!confirmed) return;
            await this.env.actionHandler({ name: 'actionDeleteTable', args: [selectedTable] });
        }
