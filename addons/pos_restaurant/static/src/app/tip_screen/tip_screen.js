...
export class TipScreen extends Component {
    ...
    async validateTip() {
        ...
        if (!amount) {
            await this.pos.data.write("sale.order", [serverId], { is_tipped: true, tip_amount: 0 });
            this.goNextScreen();
            return;
        }
        ...
        const serverTipLine = await this.pos.data.create("sale.order.line", [serializedTipLine]);
        await this.pos.data.write("sale.order", [serverId], {
            is_tipped: true,
