...
export class AnalyticDistribution extends Component {
    ...
    /**
     * Computes the totals for each account, grouped by plan
     * @returns {Object}
     */
    accountTotalsByPlan() {
        const accountTotals = {};
    ...
    planIsComplete(total) {
        return formatFloat(total * 100, this.decimalPrecision) === formatFloat(100, this.decimalPrecision);
    }
    ...
    async lineChanged(record, changes, line) {
        ...
        if (changes.percentage != line.percentage) {
            ...
            line.percentage = record.data[this.props.amount_field] / this.props.record.data[this.props.amount_field];
        }
    }
    ...
    // actions
    addLine() {
        let maxMandatory = 0, maxOptional = 0, hasMandatoryPlan = false;
        ...
        const remainder = 1 - (hasMandatoryPlan ? maxMandatory : (maxOptional || noPlanTotal));
        const lineToAdd = {
            id: this.nextId++,
