...
class PurchaseBillMatch(models.Model):
    ...
    def _compute_display_name(self):
        for line in self:
            line.display_name = line.pol_id.display_name or line.aml_id.product_id.display_name or line.aml_id.display_name
    ...
    def action_match_lines(self):
        ...
        for po_line in self.pol_id:
            if len(self.pol_id.filtered(lambda l: l.product_id == po_line.product_id)) > 1:
                raise UserError(_("More than 1 Purchase Order line has the same product: %(product)s", product=po_line.product_id.display_name))
            matching_bill_lines = self.aml_id.filtered(lambda l: l.product_id == po_line.product_id and not l.purchase_line_id)
    ...
    def action_add_to_po(self):
        ...
        context = {
            ...
            'dialog_size': 'medium',
            'default_display_add_button': bool(self.aml_id.product_id),
        }
