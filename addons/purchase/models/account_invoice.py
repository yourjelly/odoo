...
class AccountMove(models.Model):
    ...
    purchase_order_description = fields.Char(compute='_compute_origin_po_count')
    purchase_status = fields.Selection(
        selection=[
            ('no', "Purchase Order not required or partially matched"),
            ('matched', "All lines linked"),
        ],
    ...
    def _compute_purchase_status(self):
        for move in self:
            all_lines_matched = all(bool(il.purchase_line_id) for il in move.invoice_line_ids.filtered(lambda l: l.display_type == 'product'))
            move.purchase_status = 'matched' if all_lines_matched else 'no'

    @api.depends('line_ids.purchase_line_id')
    def _compute_origin_po_count(self):
        for move in self:
            po_count = len(move.line_ids.purchase_line_id.order_id)
            move.purchase_order_count = po_count
            if po_count == 1:
                move.purchase_order_description = move.line_ids.purchase_line_id.order_id.display_name
            elif po_count > 1:
                move.purchase_order_description = po_count
            else:
                move.purchase_order_description = False

    def action_purchase_matching(self):
        return self.partner_id.open_purchase_matching()
