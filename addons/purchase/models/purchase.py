...
class PurchaseOrder(models.Model):
    ...
    READONLY_STATES = ('purchase', 'done', 'cancel')
    ...
    date_order = fields.Datetime('Order Deadline', required=True, readonly=[('state', 'in', READONLY_STATES)], index=True, copy=False, default=fields.Datetime.now,
        help="Depicts the date within which the Quotation should be confirmed and converted into a purchase order.")
    date_approve = fields.Datetime('Confirmation Date', readonly=True, index=True, copy=False)
    partner_id = fields.Many2one('res.partner', string='Vendor', required=True, readonly=[('state', 'in', READONLY_STATES)], change_default=True, tracking=True, domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]", help="You can find a vendor by its Name, TIN, Email or Internal Reference.")
    dest_address_id = fields.Many2one('res.partner', domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]", string='Dropship Address', readonly=[('state', 'in', READONLY_STATES)],
        help="Put an address if you want to deliver directly from the vendor to the customer. "
             "Otherwise, keep empty to deliver to your own company.")
    currency_id = fields.Many2one('res.currency', 'Currency', required=True, readonly=[('state', 'in', READONLY_STATES)],
    ...
    order_line = fields.One2many('purchase.order.line', 'order_id', string='Order Lines', readonly=[('state', 'in', ['cancel', 'done'])], copy=True)
    ...
    incoterm_id = fields.Many2one('account.incoterms', 'Incoterm', readonly=[('state', '=', 'done')], help="International Commercial Terms are a series of predefined commercial terms used in international transactions.")

    product_id = fields.Many2one('product.product', related='order_line.product_id', string='Product')
    user_id = fields.Many2one(
        'res.users', string='Buyer', index=True, tracking=True,
        default=lambda self: self.env.user, check_company=True)
    company_id = fields.Many2one('res.company', 'Company', required=True, index=True, readonly=[('state', 'in', READONLY_STATES)], default=lambda self: self.env.company.id)
