...
var SalePortalSidebar = PortalSidebar.extend({
    ...
    _generateMenu: function () {
        var self = this,
            ...
            $saleHistory = this.$el.find('li#portal_sale_history');
        ...
        _.each(this.spyWatched.find("h1, h2"), function (el) {
            ...
            switch (el.tagName.toLowerCase()) {
                case "h1":
                    ...
                    var newLI = $("<li>").append($('<a href="#' + id + '"/>').text(text));
                    // due to customize show option, when we used customize menu to hide history of sale
                    lastLI = $saleHistory.length ? newLI.insertBefore($saleHistory) : newLI.appendTo($bsSidenav);
                    lastUL = false;
                    break;
                case "h2":
