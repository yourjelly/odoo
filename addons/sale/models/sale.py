...
class SaleOrder(models.Model):
    ...
    def _create_invoices(self, grouped=False, final=False):
            ...
            for line in order.order_line:
                ...
                if line.qty_to_invoice > 0 or (line.qty_to_invoice < 0 and final):
                    if pending_section:
                        ...
                        pending_section = None
                invoice_vals['line_ids'].append((0, 0, line._prepare_invoice_line()))

            if not invoice_vals['line_ids']:
        ...
        out_refund_vals_list = []
        if final:
            out_invoice_vals_list = []
            for invoice_vals in invoice_vals_list:
                if sum(l[2]['quantity'] * l[2]['price_unit'] for l in invoice_vals['line_ids']) < 0:
                    for l in invoice_vals['line_ids']:
                        l[2]['quantity'] = -l[2]['quantity']
                    invoice_vals['type'] = 'out_refund'
                    out_refund_vals_list.append(invoice_vals)
                else:
        ...
        # Create invoices.
        invoices = self.env['account.move'].with_context(type='out_invoice', assist_move_creation=True).create(out_invoice_vals_list)
        refunds = self.env['account.move'].with_context(type='out_refund', assist_move_creation=True).create(out_refund_vals_list)
