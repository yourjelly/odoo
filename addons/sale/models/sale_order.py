...
class SaleOrder(models.Model):
    ...
    def onchange_partner_id(self):
        ...
        if not self.env.context.get("not_self_saleperson") or not self.team_id:
            values["team_id"] = (
                self.env["crm.team"]
                .with_context(default_team_id=self.partner_id.team_id.id)
    ...
    def onchange_user_id(self):
        if self.user_id:
            self.team_id = (
                self.env["crm.team"]
                .with_context(default_team_id=self.team_id.id)
...
- You should modify the invoicing policy of your product: Open the product, go to the "Sales tab" and modify invoicing policy from "delivered quantities" to "ordered quantities".
    ...
    def message_post(self, **kwargs):
        ...
        return super(
            SaleOrder, self.with_context(mail_post_autofollow=True)
        ).message_post(**kwargs)
