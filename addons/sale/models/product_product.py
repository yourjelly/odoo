    ...
    def _inverse_sol_qty(self):
        ...
        if sale_order:
            ...
            if qty > 0 and not sale_order_line:  # positive quantity and no line -> create one
                ...
                sol._compute_name()  # computing the name after it so the sol is already created
                                     # and linked to a product
    ...
    def _compute_price_unit(self):
        ...
        if sale_order:
            for product in self:
                ...
                if len(sale_order_line) == 1:
                    price = sale_order_line.with_company(sale_order.company_id)._get_display_price()
