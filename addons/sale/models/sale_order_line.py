...
class SaleOrderLine(models.Model):
    ...
    def _compute_discount(self):
        test = self.env.user.has_group('product.group_discount_per_so_line')
        for line in self:
            if not (line.product_id and line.product_uom and
                    line.order_id.partner_id and line.order_id.pricelist_id and
                    line.order_id.pricelist_id.discount_policy == 'without_discount') and test:
                line.discount = 0.0
            else:
                line.discount = 0.0
                ...
                if new_list_price != 0:
                    if line.order_id.pricelist_id.currency_id != currency:
                        ...
                        new_list_price = currency._convert(
                            ...
                            line.order_id.company_id or self.env.company, line.order_id.date_order or fields.Date.today())
                    discount = (new_list_price - price) / new_list_price * 100
                    if (discount > 0 and new_list_price > 0) or (discount < 0 and new_list_price < 0):
