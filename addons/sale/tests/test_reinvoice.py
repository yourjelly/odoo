...
class TestReInvoice(TestCommonSaleNoChart):
    ...
    def setUpClass(cls):
        ...
        cls.AccountMove = cls.env['account.move'].with_context(type='in_invoice', assist_move_creation=True, mail_notrack=True, mail_create_nolog=True)
    ...
    def test_at_cost(self):
        ...
        invoice_b = self.AccountMove.create({
            'partner_id': self.partner_customer_usd.id,
            'journal_id': self.journal_purchase.id,
            'line_ids': [
                (0, 0, {
