...
class TestSaleOrder(TestCommonSaleNoChart):
    ...
    def test_cost_invoicing(self):
        ...
        inv = self.env['account.move'].with_context(type='in_invoice', assist_move_creation=True).create({
            'partner_id': self.partner_customer_usd.id,
            'line_ids': [
                (0, 0, {'product_id': serv_cost.id, 'quantity': 2, 'analytic_account_id': so.analytic_account_id.id}),
            ],
        })
        inv.post()
