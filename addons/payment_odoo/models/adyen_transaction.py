...
class AdyenTransaction(models.Model):
    ...
    partner_state_id = fields.Many2one(
        related="payment_transaction_id.partner_state_id"
    )
    partner_country_id = fields.Many2one(
        related="payment_transaction_id.partner_country_id"
    )
    ...
    def _handle_payment_notification(self, notification_data, payment_tx):
        ...
        if float_is_zero(
            payment_tx.fees, precision_digits=payment_tx.currency_id.decimal_places
        ) and not float_is_zero(
            tx_sudo.fees, precision_digits=tx_sudo.fees_currency_id.decimal_places
        ):
            amount = tx_sudo.fees_currency_id._convert(
                tx_sudo.fees,
                payment_tx.currency_id,
                self.env.company,
                tx_sudo.date or fields.Date.today(),
            )
    ...
    def _create_missing_tx(self, account_id, transaction, **kwargs):
        ...
        if tx.description and not tx.payment_transaction_id:
            payment_tx = self.env["payment.transaction"].search(
                [("reference", "=", tx.description), ("provider", "=", "odoo")]
            )
            if payment_tx:
                tx.payment_transaction_id = payment_tx.id

                if (
                    not transaction.get("disputePspReference")
                    and tx.merchant_amount > 0
                ):
                    tx.total_amount = payment_tx.amount
                    tx.currency_id = payment_tx.currency_id

                    fees_amount = tx.total_amount - tx.merchant_amount
                    tx.fees = payment_tx.currency_id._convert(
                        fees_amount,
                        tx.fees_currency_id,
                        self.env.company,
                        tx.date or fields.Date.today(),
                    )
