...
class PaymentTransaction(models.Model):
    ...
    adyen_transaction_ids = fields.One2many(
        "adyen.transaction", "payment_transaction_id"
    )
        ...
        data = {
            ...
            "metadata": {
                ...
                "notification_url": urls.url_join(
                    base_url, OdooController._notification_url
                ),
    ...
    def _send_payment_request(self):
        ...
        if not self.token_id:
            raise UserError(
                "Odoo Payments: " + _("The transaction is not linked to a token.")
            )
    ...
    def _get_tx_from_feedback_data(self, provider, data):
        ...
        reference = data.get("merchantReference") or data.get("additionalData", {}).get(
            "merchantReference"
        )
        if not reference:
    ...
    def _odoo_tokenize_from_feedback_data(self, data):
        ...
        payment_method_type = next(
            pm["type"] for pm in payment_methods if pm["id"] == acquirer_ref
        )
