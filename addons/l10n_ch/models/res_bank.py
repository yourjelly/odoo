...
class ResPartnerBank(models.Model):
    ...
    def _get_qr_code_url(
        ...
        if qr_method == "ch_qr":
            qr_code_vals = self._l10n_ch_get_qr_vals(
                amount,
                currency,
                debtor_partner,
                free_communication,
            ...
            return (
                "/report/barcode/?type=%s&value=%s&width=%s&height=%s&quiet=1&mask=ch_cross"
                % (
                    "QR",
    ...
    def _check_for_qr_code_errors(
        ...
        if qr_method == "ch_qr":
            ...
            if debtor_partner and not _partner_fields_set(debtor_partner):
                return _(
                    "The partner the QR-code must have a complete postal address (street, zip, city and country)."
                )
