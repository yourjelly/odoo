...
test("show spreadsheet in readonly mode", async function () {
    const odooEnv = await makeSpreadsheetMockEnv();
    const model = await createModelWithDataSource({ odooEnv });
    await addGlobalFilter(model, THIS_YEAR_GLOBAL_FILTER);
    data = await freezeOdooData(model);
    const fixture = await mountPublicSpreadsheet(odooEnv, "dashboardDataUrl", "spreadsheet");
...
test("show dashboard in dashboard mode when there are global filters", async function () {
    ...
    await addGlobalFilter(model, THIS_YEAR_GLOBAL_FILTER);
    data = await freezeOdooData(model);
    const fixture = await mountPublicSpreadsheet(odooEnv, "dashboardDataUrl", "dashboard");
    const filterButton = fixture.querySelector(".o-public-spreadsheet-filter-button");
