...
QUnit.test("Cannot open see records on the main PIVOT.TABLE cell", async function (assert) {
    const { env, model } = await createSpreadsheetWithPivot();
...
QUnit.test(
    "Cannot open see records on the empty PIVOT.TABLE cell below the main cell",
...
QUnit.test("Can see records on PIVOT.TABLE cells", async function (assert) {
    const actions = [];
    const fakeActionService = {
        start: (env) => ({
            doAction: (actionRequest, options = {}) => {
    ...
    async function checkCells(cells) {
        ...
        // sheet where the pivot is made of a single PIVOT.TABLE formula.
        for (const [xc, formula] of Object.entries(cells)) {
            ...
            // action on the second sheet, on PIVOT.TABLE
    ...
    const data_cells = {
        ...
        B3: '=PIVOT(1,"probability","bar","false","foo",1)',
        // B4 is an non-empty value
        B4: '=PIVOT(1,"probability","bar","true","foo",1)',
        // B5 is a column group total value
        B5: '=PIVOT(1,"probability","foo",1)',
        // F3 is a row group total value
        F3: '=PIVOT(1,"probability","bar","false")',
        // F5 is the total
        F5: '=PIVOT(1,"probability")',
    };
    ...
    setCellContent(model, "A3", `=PIVOT.TABLE("1",,,FALSE)`, "42");
