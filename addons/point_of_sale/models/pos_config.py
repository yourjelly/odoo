...
class AccountBankStmtCashWizard(models.Model):
            ...
            if self.env.context.get("balance", False) == "start":
                ...
                vals["cashbox_lines_ids"] = [
                    [
                        0,
                        0,
                        {"coin_value": line.coin_value, "number": 0, "subtotal": 0.0},
                    ]
                    for line in lines
                ]
        return vals
    ...
    def set_default_cashbox(self):
        self.ensure_one()
        current_session = self.env["pos.session"].browse(
            self.env.context["pos_session_id"]
        )
...
class PosConfig(models.Model):
    ...
    show_allow_invoicing_alert = fields.Boolean(
        ...
        for pos_config in self:
            ...
            if session:
                ...
                if session[0]["cash_register_id"]:
                    ...
                    pos_config.last_session_closing_cashbox = (
                        self.env["account.bank.statement"]
                        .browse(session[0]["cash_register_id"][0])
                        .cashbox_end_id
                    )
                else:
    ...
    def _check_session_state(self):
        ...
        if open_session:
            raise ValidationError(
                _(
                    "You are not allowed to change the cash control status while a session is already opened."
                )
            )
        ...
        if self.cash_control and self.payment_method_ids:
            for method in self.payment_method_ids:
                if method.is_cash_count and (
                    not method.cash_journal_id.loss_account_id
    ...
    def _check_currencies(self):
        ...
        if any(
            self.payment_method_ids.filtered(lambda pm: pm.is_cash_count).mapped(
                ...
                not in (self.company_id.currency_id | pm.cash_journal_id.currency_id)
            )
        ):
            raise ValidationError(
                _(
    ...
    def _check_payment_method_receivable_accounts(self):
        ...
        invalid_payment_methods = self.payment_method_ids.filtered(
            lambda method: not method.receivable_account_id
        )
