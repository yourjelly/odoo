...
class PosSession(models.Model):
    ...
    def action_pos_session_open(self):
        ...
        for session in self.filtered(
            lambda session: session.state in ("new_session", "opening_control")
        ):
            values = {}
        ...
        for session in self:
            ...
            if not session.config_id.cash_control:
                return session.action_pos_session_close(
                    balancing_account, amount_to_balance
                )
    ...
    def action_pos_session_close(self, balancing_account=False, amount_to_balance=0):
        ...
        if (
            self.cash_control
            and abs(self.cash_register_difference)
            ...
            try:
                ...
                if sudo:
                    data = (
                        ...
                        ._create_account_move(balancing_account, amount_to_balance)
                    )
                else:
                    raise e
    ...
    def _create_cash_statement_lines_and_cash_move_lines(self, data):
        ...
        for statement in self.statement_ids:
            split_cash_statement_lines[statement] = BankStatementLine.create(
                split_cash_statement_line_vals[statement]
            )
            combine_cash_statement_lines[statement] = BankStatementLine.create(
                combine_cash_statement_line_vals[statement]
            )
    ...
    def _create_invoice_receivable_lines(self, data):
        ...
        for partner, amounts in invoice_receivables.items():
            ...
            invoice_receivable_vals[commercial_partner].append(
    ...
    def _reconcile_account_move_lines(self, data):
        ...
        for statement in self.statement_ids:
            ...
            all_lines = (
                ...
                .filtered(lambda aml: aml.account_id.internal_type == "receivable")
            ...
            lines_by_account = [
                all_lines.filtered(lambda l: l.account_id == account)
        ...
        for account_id in order_account_move_receivable_lines:
            (
                ...
                | invoice_receivable_lines.get(
                    account_id, self.env["account.move.line"]
                )
            ).reconcile()
    ...
    def _get_split_receivable_vals(self, payment, amount, amount_converted):
        partial_vals = {
            ...
            ._find_accounting_partner(payment.partner_id)
            .id,
    ...
    def _get_statement_line_vals(
        self, statement, receivable_account, amount, date=False, partner=False
    ):
        return {
            ...
            "partner_id": partner
            and self.env["res.partner"]._find_accounting_partner(partner).id,
        }
    ...
    def _get_related_account_moves(self):
        ...
        non_reconcilable_lines = session_move.line_ids.filtered(
        ...
        cash_move_lines = self.env["account.move.line"].search(
