...
class PosMakeInvoice(models.TransientModel):
    ...
    def create_invoices(self):
        ...
        if any(
            float_round(order_id.amount_paid - order_id.amount_total, precision_rounding=self.pos_order_ids.currency_id.rounding) != 0
            for order_id in self.pos_order_ids
        ):
            raise UserError("Some orders are not fully paid.")
