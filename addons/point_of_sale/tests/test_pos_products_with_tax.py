...
class TestPoSProductsWithTax(TestPoSCommon):
    ...
    def setUp(self):
        ...
        self.product1 = self.create_product(
            "Product 1", self.categ_basic, 10.0, 5.0, tax_ids=self.taxes["tax7"].ids,
        )
        self.product2 = self.create_product(
            "Product 2", self.categ_basic, 20.0, 10.0, tax_ids=self.taxes["tax10"].ids,
        )
    ...
    def test_orders_no_invoiced(self):
        ...
        orders = []
        orders.append(
            self.create_ui_order_data([(self.product1, 10), (self.product2, 5)])
        )
        orders.append(
            self.create_ui_order_data([(self.product2, 7), (self.product3, 4)])
        )
        orders.append(
        ...
        sales_lines = session_move.line_ids.filtered(
            lambda line: line.account_id == self.sale_account
        )
        self.assertAlmostEqual(
            sum(sales_lines.mapped("balance")),
            -628.18,
            msg="Sales line balance should be equal to untaxed orders amount.",
        )
        ...
        self.assertAlmostEqual(
            receivable_line_bank.balance,
            230.25,
            msg="Bank receivable should be equal to the total bank payments.",
        )

        receivable_line_cash = session_move.line_ids.filtered(
            lambda line: self.cash_pm.name in line.name
        )
        self.assertAlmostEqual(
        ...
        self.assertTrue(
            ...
            "Cash receivable line should be fully-reconciled.",
        )
    ...
    def test_orders_with_invoiced(self):
        ...
        invoiced_order = self.pos_session.order_ids.filtered(
            lambda order: "09876-098-0987" in order.pos_reference
        )
        self.assertEqual(
        ...
        receivable_line = session_move.line_ids.filtered(
            lambda line: line.account_id
            in self.receivable_account
            + self.env["account.account"].search([("name", "=", "Account Receivable")])
            and line.name == "From invoiced orders"
        )
        self.assertAlmostEqual(receivable_line.balance, -426.09)
        self.assertTrue(
            receivable_line.full_reconcile_id,
            msg="Receivable line for invoices should be fully reconciled.",
        )

        pos_receivable_line_bank = session_move.line_ids.filtered(
            lambda line: self.bank_pm.name in line.name
            and line.account_id == self.bank_pm.receivable_account_id
        )
    ...
    def test_return_order(self):
        ...
        make_payment = (
            ...
            .create({"payment_method_id": self.cash_pm.id, "amount": -104.01,})
        )
        make_payment.check()
        self.assertEqual(
        ...
        self.assertFalse(
            receivable_line_bank,
            msg="There should be no bank receivable line because no bank payment made.",
        )
        ...
        self.assertFalse(
            receivable_line_cash,
            msg="There should be no cash receivable line because it is combined with the original cash payment.",
        )
