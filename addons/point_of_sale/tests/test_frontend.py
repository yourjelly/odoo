...
class TestPointOfSaleHttpCommon(AccountTestInvoicingHttpCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        attribute = env["product.attribute"].create({"name": "add 2",})
        attribute_value = env["product.attribute.value"].create(
            {"name": "add 2", "attribute_id": attribute.id,}
        )
        ...
        chair_color_attribute = env["product.attribute"].create(
            {"name": "Color", "display_type": "color", "create_variant": "no_variant",}
        )
        ...
        chair_legs_metal = env["product.attribute.value"].create(
            {"name": "Metal", "attribute_id": chair_legs_attribute.id,}
        )
        chair_legs_wood = env["product.attribute.value"].create(
            {"name": "Wood", "attribute_id": chair_legs_attribute.id,}
        )
        ...
        chair_fabrics_leather = env["product.attribute.value"].create(
            {"name": "Leather", "attribute_id": chair_fabrics_attribute.id,}
        )
        ...
        fixed_pricelist = env["product.pricelist"].create(
            {
                "name": "Fixed",
                "item_ids": [
                    (0, 0, {"compute_price": "fixed", "fixed_price": 1,}),
                    (
                        0,
                        0,
                        {
        ...
        env["product.pricelist"].create(
            {
                ...
                "item_ids": [
                    ...
                    (0, 0, {"compute_price": "fixed", "fixed_price": 2,}),
                ],
            }
        )

        product_category_3 = env["product.category"].create(
            {"name": "Services", "parent_id": env.ref("product.product_category_1").id,}
        )
        ...
        res_partner_18 = env["res.partner"].create(
            {"name": "Lumber Inc", "is_company": True,}
        )
        ...
        env["ir.property"]._set_default(
            "property_product_pricelist", "res.partner", public_pricelist,
        )
...
class TestUi(TestPointOfSaleHttpCommon):
    def test_01_pos_basic_order(self):
        ...
        self.main_pos_config.write(
            {"iface_tipproduct": True, "tip_product_id": self.tip.id,}
        )
