...
class TestPoSMultipleReceivableAccounts(TestPoSCommon):
    ...
    def test_01_invoiced_order_from_other_customer(self):
        ...
        def _before_closing_cb():
            ...
            self.assertEqual(len(self.pos_session.order_ids.filtered(lambda order: order.account_move != False)), 1, 'There should only be one invoiced order.')
    ...
    def test_02_all_orders_invoiced_mixed_customers(self):
        ...
        def _before_closing_cb():
            ...
            # check if there is one invoiced order
            self.assertEqual(len(self.pos_session.order_ids.filtered(lambda order: order.account_move != False)), 3, 'All orders should be invoiced.')
