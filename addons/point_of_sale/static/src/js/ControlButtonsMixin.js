odoo.define('point_of_sale.ControlButtonsMixin', function (require) {
    ...
    const ControlButtonsMixin = (x) => {
        ...
        class Extended extends x {
            get controlButtons() {
                if (!sorted) {
                    // This is the price to pay in removing the declaration of referred
                    // control buttons from the manifest. We need to sort the control buttons
                    // at the first render of the component that uses this mixin.
                    sortControlButtons();
                    sorted = true;
                }
