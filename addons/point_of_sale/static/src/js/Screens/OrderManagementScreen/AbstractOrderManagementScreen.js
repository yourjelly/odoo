odoo.define('point_of_sale.AbstractOrderManagementScreen', function (require) {
    'use strict';
    ...
    class AbstractOrderManagementScreen extends PosComponent {
        ...
        mounted() {
            ...
            setTimeout(() => this.env.actionHandler({ name: 'actionsetNPerPage', args: [val] }), 0);
        }
        async _onClickOrder(event) {
            await this.env.actionHandler({ name: 'actionSelectOrder', args: [event.detail] });
        }
        get selectedClient() {
        ...
        _onSearch([searchTerm, key]) {
            ...
            if ((key && key === 'Enter') || !key) {
                ...
                this.env.actionHandler({ name: 'actionSearch', args: [domain] });
            }
        }
        /**
         * E.g. 1
