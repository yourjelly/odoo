...
import { EditableTable } from "@point_of_sale/app/floor_screen/editable_table";
import { EditBar } from "@point_of_sale/app/floor_screen/edit_bar";
import { Table } from "@point_of_sale/app/floor_screen/table";
...
export class FloorScreen extends Component {
    ...
    static template = "point_of_sale.FloorScreen";
    ...
    async _save(table) {
        ...
        if (table.id) {
            await this.orm.write("pos.table", [table.id], tableCopy);
        } else {
            const tableId = await this.orm.create("pos.table", [tableCopy]);
    ...
    async _renameFloor(floorId, newName) {
        await this.orm.call("pos.floor", "rename_floor", [floorId, newName]);
    }
    get activeFloor() {
    ...
    async addFloor() {
        ...
        const floor = await this.orm.call("pos.floor", "create_from_ui", [
            newName,
            "#ACADAD",
            this.pos.config.id,
        ]);
    ...
    async duplicateTableOrFloor() {
        if (this.selectedTables.length == 0) {
            ...
            const newFloor = await this.orm.call("pos.floor", "create_from_ui", [
                newFloorName,
    ...
    async setFloorColor(color) {
        ...
        await this.orm.write("pos.floor", [this.activeFloor.id], {
    ...
    async deleteFloorOrTable() {
        if (this.selectedTables.length == 0) {
            ...
            await this.orm.call("pos.floor", "deactivate_floor", [
        ...
        originalSelectedTableIds.forEach(async (id) => {
            const response = await this.orm.call("pos.table", "are_orders_still_in_draft", [
                id,
            ]);
            if (!response) {
                ...
                this.orm.write("pos.table", [id], { active: false });
