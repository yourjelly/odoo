...
export class ProductConfiguratorPopup extends Component {
    ...
    computeProductProduct() {
        ...
        const activeCombinations = product.product_variant_ids?.map((prd) => [
            ...prd.product_template_variant_value_ids.map((ptav) => ptav.id),
        ]);
        const isAvailable = activeCombinations.some(
            (ac) => JSON.stringify(ac) === JSON.stringify(formattedPayload.attribute_value_ids)
        );
