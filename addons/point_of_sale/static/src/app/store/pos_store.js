...
export class PosStore extends Reactive {
    ...
    taxIdsToTaxesKey(taxIds){
        return taxIds.sort().map(x => x.toString()).join("-");
    }

    getTaxIdsAfterFiscalPosition(taxIds, fiscalPosition){
        const fpId = fiscalPosition.id;
        const taxesKey = this.taxIdsToTaxesKey(taxIds);
        if(!this.taxes_computation.fiscal_position[fpId].hasOwnProperty(taxesKey)){
            return taxIds;
        }
        const newTaxesKey = this.taxes_computation.fiscal_position[fpId][taxesKey];
        if(!newTaxesKey){
            return [];
        }
        const newTaxIds = newTaxesKey.split("-").map(x => parseInt(x));
        return newTaxIds;
    }
