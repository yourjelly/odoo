odoo.define('point_of_sale.test_popups', function(require) {
    ...
    QUnit.test('ConfirmPopup', async function(assert) {
        ...
        const root = await mount(Root, { env, target });
        ...
        testUtils.dom.click(target.querySelector('.confirm'));
        await testUtils.nextTick();
        ...
        promResponse = root.showPopup('ConfirmPopup', {});
        await testUtils.nextTick();
        testUtils.dom.click(target.querySelector('.cancel'));
        await testUtils.nextTick();
        userResponse = await promResponse;
        ...
        await testUtils.nextTick();
        assert.strictEqual(target.querySelector('.title').innerText.trim(), 'Are you sure?');
        assert.strictEqual(target.querySelector('.body').innerText.trim(), 'Are you having fun?');
        assert.strictEqual(target.querySelector('.confirm').innerText.trim(), 'Hell Yeah!');
        assert.strictEqual(
            target.querySelector('.cancel').innerText.trim(),
            'Are you kidding me?'
        );
    });
    ...
    QUnit.test('NumberPopup', async function(assert) {
        ...
        await testUtils.nextTick();
        testUtils.dom.triggerEvent(target.querySelector('.confirm'), 'mousedown');
        await testUtils.nextTick();
        userResponse = await promResponse;
        assert.strictEqual(userResponse.confirmed, true);
        ...
        await testUtils.nextTick();
        testUtils.dom.triggerEvent(target.querySelector('.cancel'), 'mousedown');
        await testUtils.nextTick();
        userResponse = await promResponse;
        assert.strictEqual(userResponse.confirmed, false);
        ...
        await testUtils.nextTick();
        let nodes = Array.from(target.querySelectorAll('button'));
        ...
        assert.strictEqual(target.querySelector('.title').innerText.trim(), 'Are you sure?');
        assert.strictEqual(target.querySelector('.confirm').innerText.trim(), 'Hell Yeah!');
        assert.strictEqual(target.querySelector('.cancel').innerText.trim(), 'Are you kidding me?');
        testUtils.dom.triggerEvent(target.querySelector('.confirm'), 'mousedown');
        await testUtils.nextTick();
        userResponse = await promResponse;
        assert.strictEqual(userResponse.confirmed, true);
    ...
    QUnit.test('EditListPopup', async function(assert) {
        ...
        await testUtils.nextTick();
        testUtils.dom.click(target.querySelector('.fa-trash-o'));
        await testUtils.nextTick();
        testUtils.dom.click(target.querySelector('.confirm'));
        await testUtils.nextTick();
        userResponse = await promResponse;
