...
class PaymentTransaction(models.Model):
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        if self.provider_code != 'razorpay' or self.tokenize:
            return res
    ...
    def _razorpay_tokenize_from_notification_data(self, notification_data):
        ...
        payment_response = self.provider_id._razorpay_make_request(
            endpoint=f"payments/{notification_data['id']}", payload={'id': notification_data['id']}, method="GET"
        )
        _logger.info(
        ...
        token = self.env['payment.token'].create({
            ...
            'payment_details': payment_response['card']['last4'],
            'razorpay_customer_id': notification_data['customer_id'],
            'partner_id': self.partner_id.id,
            'provider_ref': notification_data['token_id'],
        })
        self.write({
            'token_id': token,
            'tokenize': False,
    ...
    def _send_payment_request(self):
        ...
        recurring_payment_payload = {
            ...
            'currency': self.currency_id.name,
            'order_id': order_response['id'],
            'customer_id': self.token_id.razorpay_customer_id,
