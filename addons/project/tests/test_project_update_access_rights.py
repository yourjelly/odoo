...
class TestProjectUpdateAccessRights(TestProjectCommon):
    ...
    def test_project_update_user_project_owner_cannot_write(self):
        ...
        with self.assertRaises(
            AccessError,
            msg="%s, who owns the project, should not be able to write in the project update"
            % self.env.user.name,
        ):
            self.project_update_1.with_user(self.env.user).name = "Test write"
    ...
    def test_project_update_user_project_owner_no_unlink(self):
        ...
        with self.assertRaises(
            AccessError,
            msg="%s should not be able to unlink the project update even if project owner"
            % self.env.user.name,
        ):
            self.project_update_1.with_user(self.env.user).unlink()
