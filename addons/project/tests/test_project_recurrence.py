...
class TestProjectrecurrence(TransactionCase):
    ...
    def test_recurrence_cron_repeat_after_subsubtasks(self):
        ...
        child_task_2 = self.env['project.task'].create({
            'name': 'Child task 2 that have recurrence',
            'parent_id': parent_task.id,
        })
        grand_child_task_1 = self.env['project.task'].create({
            'name': 'Grandchild task 1 (recurrent)',
            'parent_id': child_task_2.id,
        })
        grand_child_task_2 = self.env['project.task'].create({
            'name': 'Grandchild task 2',
            'parent_id': child_task_2.id,
        })
        grand_child_task_3 = self.env['project.task'].create({
            'name': 'Grandchild task 3',
            'parent_id': grand_child_task_2.id,
        })
        grand_child_task_4 = self.env['project.task'].create({
            'name': 'Grandchild task 4',
            'parent_id': grand_child_task_3.id,
        })
        self.env['project.task'].create({
            'name': 'Grandchild task 5',
            'parent_id': grand_child_task_4.id,
        })
        with freeze_time("2020-01-01"):
            with Form(child_task_2.with_context(tracking_disable=True)) as task_form:
            ...
            with Form(grand_child_task_1.with_context(tracking_disable=True)) as subtask_form:
            ...
            self.assertTrue(child_task_2.recurrence_id)
            self.assertEqual(child_task_2.recurrence_id.next_recurrence_date, date(2020, 1, 15))
