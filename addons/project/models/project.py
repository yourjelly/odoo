...
class Project(models.Model):
    ...
    def _get_tasks_analysis_counts(self, created=False, updated=False):
        ...
        thirty_days_ago = datetime.combine(
            fields.Date.context_today(self) + timedelta(days=-30), datetime.min.time()
        )
        for t in tasks:
...
class Task(models.Model):
    ...
    repeat_on_month = fields.Selection(
        [("date", "Date of the Month"), ("day", "Day of the Month"),],
        default="date",
        compute="_compute_repeat",
        readonly=False,
    )
    ...
    repeat_on_year = fields.Selection(
        [("date", "Date of the Year"), ("day", "Day of the Year"),],
        default="date",
        compute="_compute_repeat",
        readonly=False,
    )
    ...
    def _check_recurring_task(self):
        if self.filtered(lambda task: task.parent_id and task.recurring_task):
            raise ValidationError(
                _(
                    "A sub-task cannot be a recurring task. Please consider making its parent task, a recurring task."
                )
            )
    ...
    def rating_apply(self, rate, token=None, feedback=None, subtype_xmlid=None):
        return super(Task, self).rating_apply(
