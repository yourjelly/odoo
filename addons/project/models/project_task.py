...
class Task(models.Model):
    ...
    def fields_get(self, allfields=None, attributes=None):
        ...
        if not self.env.user._has_group('base.group_portal'):
            return fields
    ...
    def _get_view_cache_key(self, view_id=None, view_type='form', **options):
        ...
        return key + (self.env.user._has_group('base.group_portal'),)
    ...
    def _ensure_fields_are_accessible(self, fields, operation='read', check_group_user=True):
        ...
        if fields and (not check_group_user or self.env.user._has_group('base.group_portal')) and not self.env.su:
    ...
    def _determine_fields_to_fetch(self, field_names, ignore_when_in_cache=False):
        if not self.env.su and self.env.user._has_group('base.group_portal'):
    ...
    def check_field_access_rights(self, operation, field_names):
        ...
        elif not field_names and not self.env.su and self.env.user._has_group('base.group_portal'):
            valid_names = self.SELF_READABLE_FIELDS
            return [
    ...
    def create(self, vals_list):
        ...
        is_portal_user = self.env.user._has_group('base.group_portal')
        if is_portal_user:
    ...
    def write(self, vals):
        ...
        partner_ids = []
        if self.env.user._has_group('base.group_portal') and not self.env.su:
    ...
    def _mail_get_message_subtypes(self):
        ...
        if len(self) == 1:
            ...
            if ((self.project_id and not self.project_id.allow_task_dependencies)\
                or (not self.project_id and not self.env.user._has_group('project.group_project_task_dependencies')))\
    ...
    def action_project_sharing_view_parent_task(self):
        if self.parent_id.project_id != self.project_id and self.env.user._has_group('base.group_portal'):
