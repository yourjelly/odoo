...
class Project(models.Model):
    ...
    def _create_analytic_account(self):
        for project in self:
            company_id = False if int(self.env['ir.config_parameter'].sudo().get_param("project.share_analytic_account")) else project.company_id.id  # TODO can't we always take the company of the project?
            project_plan, _other_plans = self.env['account.analytic.plan']._get_all_plans()
            analytic_account = self.env['account.analytic.account'].create({
                'name': project.name,
