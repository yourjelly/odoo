    ...
    QUnit.module("Views", (hooks) => {
        ...
        QUnit.test("check if default mode is line chart and line chart is stacked for burndown chart", async function (assert) {
            ...
            const burndownChart = await makeView(
                { serverData },
                { modelName: "burndown_chart", type: "burndown_chart" }
            );
            ...
            assert.ok(burndownChart.getScaleOptions().yAxes.every(y => y.stacked), "the stacked property in y axes should be true when the stacked is enabled in line chart");
            assert.ok(burndownChart.getElementOptions().line.fill, "The fill property should be true to add backgroundColor in line chart.");

            const actualDatasets = [];
            const expectedDatasets = [];
            const keysToEvaluate = ["backgroundColor", "borderColor", "originIndex", "pointBackgroundColor"];
            const datasets = burndownChart.chart.data.datasets;
        ...
        QUnit.test("check if it is classic line chart when stacked prop is false in line chart", async function (assert) {
            ...
            assert.notOk(burndownChart.getScaleOptions().yAxes.every(y => y.stacked), "the y axes should have a stacked property set to false since the stacked property in line chart is false.");
            assert.notOk(burndownChart.getElementOptions().line.fill, "The fill property should be false since the stacked property is false.");
