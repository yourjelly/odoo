...
class ReportProjectTaskBurndownChart(models.AbstractModel):
    ...
    def _where_calc(self, domain, active_test=True):
        ...
        simple_date_groupby_sql, __ = self._read_group_groupby(main_query, f"date:{interval}")
    ...
    def _read_group_select(self, query, aggregate_spec) -> "tuple(str, list[str])":
        if aggregate_spec == '__count':
            return f'SUM("{self._table}"."__count")', []
        return super()._read_group_select(query, aggregate_spec)
