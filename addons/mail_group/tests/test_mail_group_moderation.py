...
class TestMailGroupModeration(TestMailListCommon):
    ...
    def test_moderation_rule_api(self):
        ...
        message_1, message_2, message_3 = self.env["mail.group.message"].create(
            [
                ...
                {"email_from": '"xSs" <xss@teST.com>', "mail_group_id": mail_group.id,},
                {"email_from": '"Bob" <bob@teST.com>', "mail_group_id": mail_group.id,},
            ]
        )
    ...
    def test_moderation_rule_security(self):
        ...
        with self.assertRaises(
            ...
            self.env["mail.group.moderation"].with_user(user_portal).browse(
                self.moderation.ids
            ).email
