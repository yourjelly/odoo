...
class MailGroupMessage(models.Model):
    ...
    def _inverse_group_message_parent_id(self):
        ...
        for message in self:
            message.mail_message_id.parent_id = (
                message.group_message_parent_id.mail_message_id
            )
    ...
    def create(self, values_list):
        for vals in values_list:
            if not vals.get("mail_message_id"):
                vals.update(
                    {"res_id": vals.get("mail_group_id"), "model": "mail.group",}
                )
    ...
    def action_moderate_accept(self):
        ...
        self.write(
            {"moderation_status": "accepted", "moderator_id": self.env.uid,}
        )
    ...
    def action_moderate_reject(self):
        self._assert_moderable()
        self.write(
            {"moderation_status": "rejected", "moderator_id": self.env.uid,}
        )
    ...
    def _create_moderation_rule(self, status):
        ...
        self.env["mail.group.moderation"].create(
            [
                {"email": email, "mail_group_id": mail_group_id, "status": status,}
    ...
    def _moderate_send_reject_email(self, subject, comment):
        for message in self:
            ...
            body_html = tools.append_content_to_html(
                "<div>%s</div>" % tools.ustr(comment), message.body, plaintext=False
            )
