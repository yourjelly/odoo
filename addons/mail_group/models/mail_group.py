...
class MailGroup(models.Model):
    ...
    def _get_default_image(self):
        image_path = modules.get_resource_path(
    ...
    image_128 = fields.Image(
        "Image", max_width=128, max_height=128, default=_get_default_image
    )
    # Messages
    ...
    can_manage_members = fields.Boolean(
        ...
        compute="_compute_can_manage_members",
    )
    ...
    def action_send_guidelines(self, members=None):
        ...
        template = self.env.ref(
            "mail_group.mail_group_send_guidelines", raise_if_not_found=False
        )
        if not template:
            raise UserError(
                _(
                    'View "mail_group.mail_group_send_guidelines" was not found. No email has been sent. Please contact an administrator to fix this issue.'
                )
            )
        ...
        banned_emails = (
            ...
            .sudo()
            .search([("status", "=", "ban"), ("mail_group_id", "=", self.id),])
            .mapped("email")
        )
    ...
    def _join_group(self, email, partner_id=None):
        ...
        if existing_member:
            raise UserError(
                _('The email "%s" is already registered in this group.', email)
            )
