...
class TestWorkEntry(TestPayslipBase):
    ...
    def test_approve_multiple_day_work_entry(self):
        ...
        work_entry.action_validate(work_entry.ids)
    ...
    def test_duplicate_global_work_entry_to_attendance(self):
        ...
        benef = self.env['hr.work.entry'].create({
            'name': '1',
            'employee_id': self.richard_emp.id,
    ...
    def test_create_work_entry_leave(self):
        ...
        self.assertEqual(calendar_leave.work_entry_type_id, benef.work_entry_type_id, "It should have the same work_entry type")

    def test_validate_conflict_work_entry(self):
        start = self.to_datetime_tz('2015-11-01 09:00:00')
        end = self.to_datetime_tz('2015-11-01 13:00:00')
        benef1 = self.env['hr.work.entry'].create({
            'name': '1',
            'employee_id': self.richard_emp.id,
            'work_entry_type_id': self.env.ref('hr_payroll.work_entry_type_attendance').id,
            'contract_id': self.richard_emp.contract_id.id,
        ...
        benef2 = self.env['hr.work.entry'].create({
            'name': '2',
            'employee_id': self.richard_emp.id,
            'work_entry_type_id': self.env.ref('hr_payroll.work_entry_type_attendance').id,
            'contract_id': self.richard_emp.contract_id.id,
        ...
        self.assertFalse(benef1.action_validate(benef1.ids), "It should not validate work_entries conflicting with others")
    ...
    def test_validate_non_approved_leave_work_entry(self):
        ...
        self.assertFalse(benef1.action_validate(benef1.ids),"It should not validate work_entries conflicting with non approved leaves")
        self.assertTrue(benef1.display_warning)
    ...
    def test_validate_undefined_work_entry(self):
        ...
        self.assertFalse(benef1.action_validate(benef1.ids),"It should not validate work_entries without a type")
    ...
    def test_approve_leave_work_entry(self):
        ...
        new_leave_benef = self.env['hr.work.entry'].search([
        ...
        new_benef = self.env['hr.work.entry'].search([
            ('date_start', '=', Datetime.to_datetime('2015-11-02 09:00:01')),
            ('date_stop', '=', end),
            ('work_entry_type_id.is_leave', '=', False)
        ])

        self.assertTrue(new_benef, "It should have created a work_entry for the last two days")
        self.assertTrue(new_leave_benef, "It should have created a leave work_entry for the first day")

        self.assertTrue(benef.action_validate((new_benef | new_leave_benef).ids), "It should be able to validate the work_entries")
    ...
    def test_time_week_leave_work_entry(self):
        ...
        leave_benef = self.env['hr.work.entry'].create({
            'name': '1leave',
            'employee_id': self.richard_emp.id,
            'contract_id': self.richard_emp.contract_id.id,
            'work_entry_type_id': self.work_entry_type_leave.id,
