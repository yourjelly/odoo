...
class AccountMoveSend(models.Model):
    ...
    enable_xml = fields.Boolean(compute='_compute_send_mail_extra_fields')
    checkbox_label = fields.Char(compute='_compute_checkbox_label')  # label for the checkbox_xml field
    checkbox_xml = fields.Boolean(compute='_compute_checkbox_xml', store=True, readonly=False)

    @api.model
    def _get_default_enable_xml(self, move):
        return not move.invoice_pdf_report_id and move.is_sale_document() and move.partner_id.edi_format
    ...
    def _compute_checkbox_label(self):
        for wizard in self:
            code_to_label = dict(wizard.move_ids.partner_id._fields['edi_format'].selection)
            codes = wizard.move_ids.partner_id.mapped('edi_format')
            if any(codes):
                wizard.checkbox_label = ", ".join([code_to_label[c] for c in codes])
            else:
                wizard.checkbox_label = False

    def _compute_send_mail_extra_fields(self):
        # EXTENDS 'account'
        super()._compute_send_mail_extra_fields()
        for wizard in self:
            wizard.enable_xml = any([self._get_default_enable_xml(m) for m in wizard.move_ids])

    @api.depends('checkbox_xml')
    def _compute_mail_attachments_widget(self):
        # EXTENDS 'account' - add depends
        super()._compute_mail_attachments_widget()

    @api.depends('enable_xml')
    def _compute_checkbox_xml(self):
        for wizard in self:
            wizard.checkbox_xml = True
    ...
    def _get_linked_attachments(self, move):
        # EXTENDS 'account'
        return super()._get_linked_attachments(move) + move.xml_id

    def _get_placeholder_mail_attachments_data(self, move):
        # EXTENDS 'account'
        results = super()._get_placeholder_mail_attachments_data(move)

        if self.enable_xml and self.checkbox_xml:
            builder, _options = move.partner_id._get_edi_builder()
    ...
    def _prepare_document(self, invoice):
        ...
        if self.checkbox_xml and self._get_default_enable_xml(invoice):
            builder, options = invoice.partner_id._get_edi_builder()
            ...
            results['xml_attachment_values'] = {
                'name': filename,
                'raw': xml_content,
                'mimetype': 'application/xml',
                'res_model': invoice._name,
                'res_id': invoice.id,
                'res_field': 'xml_file', # Binary field
            }
            results['xml_options'] = {
                'edi_format': invoice.partner_id.edi_format,
                'builder': builder,
                'options': options,
            }
    ...
    def _postprocess_document(self, invoice, prepared_data):
        ...
        # Add PDF to XML
        if 'xml_options' in prepared_data and prepared_data['xml_options']['edi_format'] != 'facturx':
        ...
        if 'xml_options' in prepared_data and prepared_data['xml_options']['edi_format'] == 'facturx':
            xml_facturx = prepared_data['xml_attachment_values']['raw']
        else:
        ...
        # PDF-A.
        if 'xml_options' in prepared_data \
                and prepared_data['xml_options']['options'].get('facturx_pdfa') \
    ...
    def _postprocess_invoice_ubl_xml(self, invoice, prepared_data):
        # Add PDF to XML
        tree = etree.fromstring(prepared_data['xml_attachment_values']['raw'])
        ...
        prepared_data['xml_attachment_values']['raw'] = b"<?xml version='1.0' encoding='UTF-8'?>\n" \
    ...
    def _link_document(self, invoice, prepared_data):
        ...
        attachment_vals = prepared_data.get('xml_attachment_values')
        if attachment_vals:
            self.env['ir.attachment'].create(attachment_vals)
            invoice.invalidate_model(fnames=['xml_id', 'xml_file'])
