...
class PosPaymentMethod(models.Model):
    ...
    adyen_account_id = fields.Many2one(
        "adyen.account", related="company_id.adyen_account_id"
    )
    ...
    def onchange_use_payment_terminal(self):
        for payment_method in self:
            if (
                payment_method.use_payment_terminal == "odoo_adyen"
                and payment_method.adyen_terminal_id
            ):
                payment_method.adyen_terminal_identifier = (
                    payment_method.adyen_terminal_id.terminal_uuid
                )
    ...
    def _proxy_adyen_request_odoo_proxy(self, data, operation):
        try:
            ...
            return company_sudo.adyen_account_id._adyen_rpc(
                "v1/%s" % operation,
                {
                    "request_data": data,
