...
class AdyenAccount(models.Model):
    ...
    def action_order_terminal(self):
        ...
        onboarding_url = (
            ...
            .sudo()
            .get_param("adyen_platforms.onboarding_url")
        )
        return {
            "type": "ir.actions.act_url",
            "target": "new",
            "url": url_join(
                onboarding_url, "order_terminals?store_uuids=%s" % store_uuids
            ),
        }
...
class AdyenStore(models.Model):
    ...
    terminal_ids = fields.One2many(
        "adyen.terminal", "store_id", string="Payment Terminals", readonly=True
    )
    ...
    def create(self, values):
        ...
        response = adyen_account_id._adyen_rpc(
            "v1/create_store", self._format_data(values)
        )
        stores = response["accountHolderDetails"]["storeDetails"]
        created_store = next(
            store
            for store in stores
            if store["storeReference"] == values.get("store_reference")
        )
    ...
    def unlink(self):
        for store_id in self:
            store_id.adyen_account_id._adyen_rpc(
                "v1/close_stores",
                {
    ...
    def _format_data(self, values):
        ...
        state_id = (
            self.env["res.country.state"].browse(values.get("state_id"))
        ...
        return {
            ...
            "accountHolderDetails": {
                "storeDetails": [
                    {
                        "storeReference": values.get("store_reference")
                        or self.store_reference,
                        ...
                        "fullPhoneNumber": values.get("phone_number")
...
class AdyenTerminal(models.Model):
    ...
    def _sync_adyen_terminals(self):
        for adyen_store_id in self.env["adyen.store"].search([]):
            response = adyen_store_id.adyen_account_id._adyen_rpc(
                "v1/connected_terminals", {"store": adyen_store_id.store_uuid,}
            )
            terminals_in_db = set(
                self.search([("store_id", "=", adyen_store_id.id)]).mapped(
                    "terminal_uuid"
                )
            )
