...
class WebsiteForum(WebsiteProfile):
    ...
    def forum(self, **kwargs):
        ...
        if len(forums) == 1:
            return http.redirect('/forum/%s' % slug(forums[0]), code=302)
    ...
    def question(self, forum, question, **post):
        ...
        if question.parent_id:
            ...
            return http.redirect(redirect_url, 301)
        filters = 'question'
    ...
    def question_edit_answer(self, forum, question, **kwargs):
        for record in question.child_ids:
            if record.create_uid.id == request.uid:
                answer = record
                break
        return http.redirect("/forum/%s/post/%s/edit" % (slug(forum), slug(answer)))
    ...
    def question_close(self, forum, question, **post):
        ...
        return http.redirect("/forum/%s/question/%s" % (slug(forum), slug(question)))
    ...
    def question_reopen(self, forum, question, **kwarg):
        question.reopen()
        return http.redirect("/forum/%s/%s" % (slug(forum), slug(question)))
    ...
    def forum_post(self, forum, **post):
        ...
        if not user.email or not tools.single_email_re.match(user.email):
            return http.redirect("/forum/%s/user/%s/edit?email_required=1" % (slug(forum), request.session.uid))
    ...
    def post_create(self, forum, post_parent=None, **post):
        ...
        if request.env.user.forum_waiting_posts_count:
            return http.redirect("/forum/%s/ask" % slug(forum))
        ...
        return http.redirect("/forum/%s/%s" % (slug(forum), post_parent and slug(post_parent) or new_question.id))
    ...
    def post_delete(self, forum, post, **kwargs):
        ...
        post.unlink()
        if question:
            http.redirect("/forum/%s/%s" % (slug(forum), slug(question)))
        return http.redirect("/forum/%s" % slug(forum))
    ...
    def open_partner(self, forum, partner_id=0, **post):
        if partner_id:
            ...
            if partner and partner.user_ids:
                return http.redirect("/forum/%s/user/%d" % (slug(forum), partner.user_ids[0].id))
        return http.redirect("/forum/%s" % slug(forum))
    ...
    def view_user_forum_profile(self, forum, user_id, forum_origin, **post):
        return http.redirect('/profile/user/' + str(user_id) + '?forum_id=' + str(forum.id) + '&forum_origin=' + str(forum_origin))
