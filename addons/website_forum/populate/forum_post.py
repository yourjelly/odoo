...
class Post(models.Model):
    ...
    def _populate_factories(self):
        ...
        def create_answers(iterator, *args):
            for values in iterator:
                answers = [
                    fields.Command.create({
                        'name': f"reply to {values['name']}",
                        'forum_id': values['forum_id']}
                    ) for _ in range(populate.random.choices(*zip(*QA_WEIGHTS.items()))[0])]
                yield {'child_ids': answers, **values}

        return [
            ('name', populate.constant('post_{counter}')),
            ('forum_id', populate.randomize(forum_ids)),
            ('child_ids', create_answers),
            ('bump_date', populate.randomize([
                fields.Datetime.now(),
