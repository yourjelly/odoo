...
class Forum(models.Model):
    ...
    def _set_default_faq(self):
        self.faq = (
            self.env["ir.ui.view"]
            ._render_template("website_forum.faq_accordion", {"forum": self})
            .decode("utf-8")
        )
...
class Post(models.Model):
    ...
    def validate(self):
        for post in self:
            ...
            post.write(
                {"state": "active", "active": True, "moderator_id": self.env.user.id,}
            )
            post.post_notification()
        return True
    ...
    def flag(self):
        ...
        for post in self:
            ...
            if post.state == "flagged":
                ...
                post.write(
                    {"state": "flagged", "flag_user_id": self.env.user.id,}
                )
                res.append(
                    post.can_moderate
    ...
    def _notify_record_by_inbox(
        ...
        if msg_vals.get("message_type", message.message_type) == "comment":
            return
        return super(Post, self)._notify_record_by_inbox(
