...
class ProductProduct(models.Model):
    ...
    manufactured_num_products = fields.Float(
    ...
    def _compute_product_margin_fields_values(self):
        ...
        sqlquery = """
            SELECT 
              cpc.product_id, 
              SUM(cpc.qty_producing) AS quantity_produced, 
              SUM(
                COALESCE(
                  (
            ...
            FROM 
              (
                SELECT 
                  ...
                  mrp.PRODUCT_ID, 
                  mrp.qty_producing, 
                  SUM(cr.cost_components) AS total_components_cost 
                FROM 
                  ...
                  JOIN (
                        ...
                        sm.quantity * COALESCE(
                          (
                            ...
                            OR um.st_uom = um.prd_uom THEN COALESCE(
                              svsp.new_standard_price, sp.standard_price, 
                              0
                    ...
                    WHERE 
                      sm.scrapped = 'false'
                  ) AS cr ON cr.raw_m_p = mrp.id 
                where 
                mrp.state = 'done' 
                AND mrp.product_id IN %s 
                AND mrp.date_start BETWEEN %s 
                AND %s 
                AND mrp.date_finished BETWEEN %s 
                AND %s 
              ...
              JOIN (
                ...
                WHERE 
                mp.product_id IN %s 
                AND mp.date_start BETWEEN %s 
                AND %s 
                AND mp.date_finished BETWEEN %s 
                AND %s 
                AND mp.company_id = %s 
                GROUP BY 
                  mp.id
              ) AS opc ON cpc.id = opc.id 
            GROUP BY 
              cpc.product_id
        """
        ...
        for prodt_id, quantity_produced, grand_total_cost in result_out:
            product = self.env["product.product"].browse(prodt_id)
            if product:
                manufactured_prds.append(prodt_id)
                res[prodt_id]["total_cost"] += grand_total_cost
                res[prodt_id]["manufactured_num_products"] = quantity_produced
                res[prodt_id]["total_margin"] -= grand_total_cost
                res[prodt_id]["total_margin_rate"] -= (
                    res[prodt_id].get("turnover", 0.0)
                    and grand_total_cost * 100 / res[prodt_id].get("turnover", 0.0)
                    or 0.0
                )
