...
class EventTypeMail(models.Model):
    ...
    interval_type = fields.Selection([
        ...
        ('before_event_neg', 'After the event started'),
        # event based: end date
        ('after_event', 'After the event ended'),
        ('after_event_neg', 'Before the event ends')],
...
class EventMail(models.Model):
    ...
    def _compute_scheduled_date(self):
        for scheduler in self:
            ...
            elif scheduler.interval_type in ('before_event', 'before_event_neg'):
    ...
    def execute(self):
        ...
        for scheduler in self._filter_template_ref():
            if scheduler.interval_type == 'after_sub':
                ...
                if scheduler.scheduled_date <= now and (scheduler.interval_type not in ('before_event', 'before_event_neg') or scheduler.event_id.date_end > now):
