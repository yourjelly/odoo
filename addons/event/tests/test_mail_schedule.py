...
class TestMailSchedule(TestEventCommon):
    ...
    def test_00_event_mail_schedule(self):
        ...
        self.assertEqual(len(schedulers), 1, 'event: wrong scheduler creation')
        self.assertEqual(schedulers[0].scheduled_date, (event_date_begin + relativedelta(days=-1)).strftime(tools.DEFAULT_SERVER_DATETIME_FORMAT), 'event: incorrect scheduled date')
        ...
        mails = self.env['mail.mail'].search([('subject', 'ilike', 'TestEventMail'), ('date', '>=', now.strftime(tools.DEFAULT_SERVER_DATETIME_FORMAT))], order='date DESC', limit=3)
