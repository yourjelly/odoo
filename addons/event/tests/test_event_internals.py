...
class TestEventData(TestEventCommon):
    ...
    def test_event_date_computation(self):
        ...
        self.assertEqual(
            registration.get_date_range_str(), u"on Mar 1, 2020, 11:00:00 AM"
        )
    ...
    def test_event_configuration_mails_from_type(self):
        ...
        event_type_mails = self.env["event.type"].create(
            {"name": "Type Mails", "auto_confirm": False}
        )
        ...
        event_type_tickets.write(
            {
                "event_type_ticket_ids": [
                    Command.clear(),
                    Command.create({"name": "Default Ticket", "seats_max": 10,}),
                ]
            }
        )
        event = self.env["event.event"].create(
        ...
        event.write(
            {
                "event_ticket_ids": [
                    Command.clear(),
                    Command.create({"name": "Registration Ticket", "seats_max": 10,}),
                ]
            }
        )
...
class TestEventRegistrationData(TestEventCommon):
    ...
    def test_registration_partner_sync(self):
        ...
        event.write({"registration_ids": [(0, 0, {"partner_id": customer.id,})]})
        ...
        event.write(
            {
                "registration_ids": [
                    (0, 0, {"name": "Nibbler In Space", "phone": test_phone,})
                ]
            }
        )
        new_reg = event.registration_ids.sorted()[0]
    ...
    def test_registration_partner_sync_company(self):
        ...
        company = (
            ...
            .sudo()
            .create({"name": "Customer Company", "is_company": True, "type": "other",})
        )
        ...
        event.write(
            {
                "event_ticket_ids": [
                    (5, 0),
                    (0, 0, {"name": "First Ticket", "seats_max": 30,}),
                    (
                        0,
                        0,
