...
class ChooseDeliveryCarrier(models.TransientModel):
    ...
    available_carrier_ids = fields.Many2many(
        "delivery.carrier", string="Available Carriers", compute='_compute_available_carrier'
    )
    ...
    def _onchange_order_id(self, types=('fixed')):
        if self.carrier_id and self.order_id.delivery_set and self.delivery_type not in types:
            vals = self._get_shipment_rate()
            if vals.get('error_message'):
                warning = {
    ...
    def _compute_available_carrier(self):
        for rec in self:
            ...
            rec.available_carrier_ids = carriers.available_carriers(
                rec.order_id.partner_shipping_id
