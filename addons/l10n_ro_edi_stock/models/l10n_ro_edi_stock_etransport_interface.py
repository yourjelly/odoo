...
class L10nRoEdiStockETransportInterface(models.Model):
    ...
    l10n_ro_edi_stock_document_load_id = fields.Char(compute='_compute_current_document_data', string="eTransport Id")
    l10n_ro_edi_stock_document_uit = fields.Char(compute='_compute_current_document_data', string="eTransport UIT")

    l10n_ro_edi_stock_state = fields.Selection(selection=DOCUMENT_STATES, compute='_compute_current_document_data', string="eTransport Status")
    ...
    def _reset_location_data(self, location: str):
        for interface in self:
            setattr(interface, f'l10n_ro_edi_stock_{location}_bcp', False)
            setattr(interface, f'l10n_ro_edi_stock_{location}_customs_office', False)
            setattr(interface, f'l10n_ro_edi_stock_{location}_state_id', False)
            setattr(interface, f'l10n_ro_edi_stock_{location}_zip', False)
            setattr(interface, f'l10n_ro_edi_stock_{location}_city', False)
            setattr(interface, f'l10n_ro_edi_stock_{location}_street', False)
            setattr(interface, f'l10n_ro_edi_stock_{location}_other_info', False)
    ...
    def _check_fetch_values(self, errors=None) -> list:
        ...
        match self.l10n_ro_edi_stock_state:
            case 'stock_sending_failed':
                if not self._get_last_stock_validated_document():
    ...
    def _create_document_stock_sent(self, values: dict[str, object]):
        self.ensure_one()
    ...
    def _create_document_stock_sending_failed(self, values: dict[str, object]):
        self.ensure_one()
        return self.env['l10n_ro.edi.stock.document'].create({
            'interface_id': self.id,
    ...
    def _create_document_stock_validated(self, values: dict[str, object]):
        self.ensure_one()
        return self.env['l10n_ro.edi.stock.document'].create({
            'interface_id': self.id,
    ...
    def _get_failed_documents(self):
        self.ensure_one()
        return self.l10n_ro_edi_stock_document_ids.filtered(lambda doc: doc.state == 'stock_sending_failed')

    def _get_stock_sent_and_failed_documents(self):
        self.ensure_one()
        return self.l10n_ro_edi_stock_document_ids.filtered(lambda doc: doc.state in ('stock_sending_failed', 'stock_sent'))

    def _get_last_stock_validated_document(self):
        self.ensure_one()
        sent_documents = self.l10n_ro_edi_stock_document_ids.filtered(lambda doc: doc.state == 'stock_validated').sorted()
    ...
    def _send_etransport_document(self, send_type: str):
        ...
        if errors := self._validate_data():
            ...
            if send_type == 'amend':
                last_sent_document = self._get_last_stock_validated_document()
                document_values |= {'load_id': last_sent_document.load_id, 'uit': last_sent_document.uit}

            self._create_document_stock_sending_failed(document_values)
            return
        ...
        if 'error' in result:
            ...
            self._get_stock_sent_and_failed_documents().unlink()

            content = result['content']
            uit = content['UIT'] if send_type == 'send' else self._get_last_stock_validated_document().uit
            self._create_document_stock_sent({'load_id': content['index_incarcare'], 'uit': uit})

    def _fetch_document_status(self):
        session = requests.Session()
        documents_to_delete = self.env['l10n_ro.edi.stock.document']
        to_fetch = self.filtered(lambda inter: inter.l10n_ro_edi_stock_state == 'stock_sent')

        for interface in to_fetch:
            current_sending_document = interface.l10n_ro_edi_stock_document_ids.filtered(lambda doc: doc.state == 'stock_sent')[0]

            if errors := interface._check_fetch_values():
                documents_to_delete |= interface._get_failed_documents()
                interface._create_document_stock_sending_failed({'message': '\n'.join(errors), 'load_id': current_sending_document.load_id, 'uit': current_sending_document.uit})
                continue
            ...
            if 'error' in result:
                documents_to_delete |= interface._get_failed_documents()
                interface._create_document_stock_sending_failed({'message': result['error'], 'load_id': current_sending_document.load_id, 'uit': current_sending_document.uit})
            else:
                documents_to_delete |= interface._get_stock_sent_and_failed_documents()
                match state := result['content']['stare']:
                    case 'ok':
                        interface._create_document_stock_validated({'load_id': current_sending_document.load_id, 'uit': current_sending_document.uit})
                    case 'in prelucrare':
                        # Document is still being sent
                        interface._create_document_stock_sent({'load_id': current_sending_document.load_id, 'uit': current_sending_document.uit})
                    case 'XML cu erori nepreluat de sistem':
                        interface._create_document_stock_sending_failed({'message': _("XML contains errors."), 'load_id': current_sending_document.load_id, 'uit': current_sending_document.uit})
                    case _:
