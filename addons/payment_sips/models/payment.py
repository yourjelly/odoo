...
class TxSips(models.Model):
    ...
    def _sips_form_validate(self, data):
        ...
        if status in self._sips_valid_tx_status:
            msg = 'ref: %s, got valid response [%s], set as done.' % (self.reference, status)
            _logger.info(msg)
            ...
            msg = 'ref: %s, got response [%s], set as cancel.' % (self.reference, status)
            data.update(state_message=msg)
            self.write(data)
            self._set_transaction_cancel()
        elif status in self._sips_wait_tx_status:
            msg = 'ref: %s, got wait response [%s], set as cancel.' % (self.reference, status)
            data.update(state_message=msg)
            self.write(data)
            self._set_transaction_cancel()
        elif status in self._sips_refused_tx_status:
            msg = 'ref: %s, got refused response [%s], set as cancel.' % (self.reference, status)
            data.update(state_message=msg)
            self.write(data)
            self._set_transaction_cancel()
        elif status in self._sips_pending_tx_status:
            msg = 'ref: %s, got pending response [%s], set as pending.' % (self.reference, status)
            data.update(state_message=msg)
            self.write(data)
            self._set_transaction_pending()
        elif status in self._sips_cancel_tx_status:
            msg = 'ref: %s, got cancel response [%s], set as cancel.' % (self.reference, status)
            data.update(state_message=msg)
            self.write(data)
            self._set_transaction_cancel()
        else:
            msg = 'ref: %s, got  unrecognized response [%s], set as error.' % (self.reference, status)
            data.update(state_message=msg)
            self.write(data)
            self._set_transaction_cancel()
