...
class L10nItDeclarationOfIntent(models.Model):
    ...
    def _compute_consumed_amount(self):
        if not self.ids:
            ...
            return
        domain = [
            ('l10n_it_edi_doi_declaration_of_intent_id', 'in', self.ids),
            ('state', '=', 'posted'),
            ('move_type', 'in', self.env['account.move'].get_invoice_types()),
        ]
        groups = self.env['account.move']._read_group(domain, ['l10n_it_edi_doi_declaration_of_intent_id'], ['amount_total_signed:sum'])
    ...
    def _compute_committed_amount(self):
        ...
        group = self.env['sale.order']._read_group(domain, ['l10n_it_edi_doi_declaration_of_intent_id'], ['amount_to_invoice:array_agg'])
        treated = self.env[self._name]
        for declaration, amount_to_invoice_agg in group:
