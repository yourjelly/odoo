...
class Survey(http.Controller):
    ...
    def survey_access_code_check(self, access_code, **post):
        ...
        """
        survey = self._get_survey_by_access_code(access_code)
        if survey:
    ...
    def survey_start_short(self, survey_short_token):
        ...
        survey = self._get_survey_by_access_code(survey_short_token)
        if survey:
            return werkzeug.utils.redirect("/survey/start/%s" % survey.access_token)

        return werkzeug.utils.redirect("/")

    def _get_survey_by_access_code(self, survey_short_token):
