...
class SurveyUser_InputLine(models.Model):
    ...
    def _compute_answer_score(self):
        ...
        """
        relevant_user_inputs = self.env['survey.user_input'].browse()
        if any(u.survey_id.session_speed_rating and u.is_session_answer for u in self.user_input_id):
            relevant_user_inputs = self.env['survey.user_input'].search([('id', 'in', self.user_input_id.ids)])

        # default and non-scored questions
        for line in self:
            is_create = not line.answer_score
            ...
            if is_create and answer_score > 0:
                user_input = line.user_input_id if line.user_input_id in relevant_user_inputs else False
                session_speed_rating = user_input and user_input.is_session_answer and user_input.survey_id.session_speed_rating
                if session_speed_rating and line.question_id.is_time_limited:
                    max_score_delay = 2
