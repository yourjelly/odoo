...
class OnboardingStep(models.Model):
    ...
    panel_step_done_text = fields.Html(
        'Text to show when step is completed', default='Step Completed!<br/>Click here to review')
    ...
    state = fields.Selection(
        ONBOARDING_STEP_STATES, compute='_compute_state', store=False)
    state_ids = fields.One2many('base.onboarding.step.state', 'step_id', string='Step States Records')
    ...
    def _compute_panel_step_properties(self):
        for step in self:
            step.panel_step_properties = json_safe.dumps({
                ...
                'done_icon': f"'{step.panel_step_done_icon}'",
    ...
    def _compute_state(self):
        step_state_model = self.env['base.onboarding.step.state']
        for step in self:
            step.state = step_state_model.search_or_create(step).state
