...
class TestSequenceMixin(AccountTestInvoicingCommon):
    ...
    def test_journal_sequence_groupby_compute(self):
        ...
        journals = self.env["account.journal"].create(
            [
                {"name": f"Journal{i}", "code": f"J{i}", "type": "general",}
                for i in range(2)
            ]
        )
    ...
    def test_sequence_concurency(self):
        ...
        with self.env.registry.cursor() as cr0, self.env.registry.cursor() as cr1, self.env.registry.cursor() as cr2:
            ...
            journal = env0["account.journal"].create(
                {"name": "concurency_test", "code": "CT", "type": "general",}
            )
    ...
    def test_sequence_deletion(self):
        ...
        journal = self.env["account.journal"].create(
            {"name": "Test sequences - deletion", "code": "SEQDEL", "type": "general",}
        )

        move_1_1 = self.create_move(
