...
class TestReconciliation(AccountingTestCase):
    ...
    def create_invoice(self, type='out_invoice', invoice_amount=50, currency_id=None):
        ...
        invoice = self.env['account.move'].with_context(type=type, assist_move_creation=True).create(invoice_vals)
        invoice.post()
        return invoice
    ...
    def test_partial_reconcile_currencies_01(self):
        ...
        invoice_a = self.env['account.move'].with_context(type='in_invoice', assist_move_creation=True).create({
            'partner_id': self.partner_agrolait_id,
            'currency_id': self.currency_usd_id,
            'invoice_date': '%s-07-01' % time.strftime('%Y'),
            'date': '%s-07-01' % time.strftime('%Y'),
        ...
        invoice_b = self.env['account.move'].with_context(type='in_invoice', assist_move_creation=True).create({
            'partner_id': self.partner_agrolait_id,
            'currency_id': self.currency_usd_id,
    ...
    def create_invoice_partner(self, type='out_invoice', invoice_amount=50, currency_id=None, partner_id=False, payment_term_id=False):
        move = self.env['account.move'].with_context(type=type, assist_move_creation=True).create({
    ...
    def test_partial_reconcile_currencies_02(self):
        ...
        invoice_cust_1 = self.env['account.move'].with_context(type='out_invoice', assist_move_creation=True).create({
            'partner_id': self.partner_agrolait_id,
        ...
        invoice_cust_1.post()
        aml = invoice_cust_1.mapped('line_ids').filtered(
    ...
    def test_multiple_term_reconciliation_opw_1906665(self):
        ...
        invoice.js_assign_outstanding_line(payment.move_line_ids.filtered('credit').id)

        receivable_lines = invoice.line_ids.filtered(lambda line: line.account_id == self.account_rcv).sorted('date_maturity')[0]
    ...
    def test_reconciliation_cash_basis_fx_01(self):
        ...
        cash_basis_move1 = cash_basis_moves.filtered(
            lambda m: m.amount_total == 123936.31)
