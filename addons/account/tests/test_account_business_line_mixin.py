...
class TestAccountBusinessLineMixin(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.percent_tax_10_1 = cls.env['account.tax'].create({
            'name': 'percent_tax_10_1',
            'amount_type': 'percent',
            'amount': 10,
            'sequence': 10,
        })
        cls.percent_tax_10_2 = cls.env['account.tax'].create({
            'name': 'percent_tax_10_2',
            'amount_type': 'percent',
            'amount': 10,
            'sequence': 10,
        })
    ...
    def run(self, result=None):
        ...
        if self.__class__ == TestAccountBusinessLineMixin:
            return
    ...
    def assertTaxesComputation(self, business_object, expected_amount_untaxed, expected_amount_tax):
        totals = self._get_totals(business_object)

        if 'amount_untaxed' in totals:
            self.assertAlmostEqual(totals['amount_untaxed'], expected_amount_untaxed)
        if 'amount_tax' in totals:
            self.assertAlmostEqual(totals['amount_tax'], expected_amount_tax)
        if 'amount_total' in totals:
            self.assertAlmostEqual(totals['amount_total'], expected_amount_untaxed + expected_amount_tax)
    ...
    def test_multi_taxes_friendly_amounts(self):
        ...
        '''
        business_object = self._create_business_object([
        ...
        self.assertTaxesComputation(business_object, 5000.0, 3300.0)

    def test_multi_taxes_round_per_line(self):
        ...
        self.assertTaxesComputation(business_object, 997.93, 440.33)

    def test_multi_taxes_round_globally_1(self):
        ...
        self.assertTaxesComputation(business_object, 997.93, 440.32)

    def test_multi_taxes_round_globally_2(self):
        '''Test the computation of taxes with decimals using the 'round_globally' method.
        ...
        self.assertTaxesComputation(business_object, 257.09, 45.25)

    def test_multi_taxes_round_globally_3(self):
        '''Test the computation of taxes with decimals using the 'round_globally' method.
        The tax amounts shouldn't be aggregated together because the taxes are not the same.
        line 1          10.0 - (10.0 / 1.2) = 1.6667
        line 2          10.0 - (10.0 / 1.4) = 2.8561
        amount_untaxed  8.33 + 7.14 = 15.47
        amount_tax      ROUND(1.6667) + ROUND(2.8561) = 1.67 + 2.86 = 4.53
        '''
        self.env.company.tax_calculation_rounding_method = 'round_globally'
        business_object = self._create_business_object([
            {'price_unit': 10.0, 'tax_ids': self.percent_tax_20_price_incl.ids},
            {'price_unit': 10.0, 'tax_ids': self.percent_tax_40_price_incl_affect_base.ids},
        ])
        self.assertTaxesComputation(business_object, 15.47, 4.53)

    def test_multi_taxes_round_globally_4(self):
        '''Test the computation of taxes with decimals using the 'round_globally' method.
        The tax amounts shouldn't be aggregated together because the taxes are not the same.
        tax line 1      (0.05 * 0.10) = 0.005
        tax line 2      (0.05 * 0.10) = 0.005
        amount_untaxed  0.05
        amount_tax      ROUND(0.005) + ROUND(0.005) = 0.01 + 0.01 = 0.02
        '''
        self.env.company.tax_calculation_rounding_method = 'round_globally'
        business_object = self._create_business_object([
            {'price_unit': 0.05, 'tax_ids': (self.percent_tax_10_1 + self.percent_tax_10_2).ids},
        ])
        self.assertTaxesComputation(business_object, 0.05, 0.02)
