...
class TestAccountMoveMisc(AccountTestInvoicingCommon):
    ...
    def test_add_followers_on_post(self):
        ...
        existing_partners = self.env['res.partner'].create([
            {'name': 'Jean', 'company_id': company.id},
            {'name': 'Paulus'},
        ])
    ...
    def test_invoice_change_period_accrual(self):
        ...
        accrual_lines = self.env['account.move'].browse(wizard_res['domain'][0][2]).line_ids.sorted(lambda line: (line.account_id, -line.balance))
        revenue_account_id = self.company_data['default_account_revenue'].id
        accrual_account_id = wizard.revenue_accrual_account.id
        self.assertRecordValues(accrual_lines, [
            {'amount_currency': -1200.0,    'debit': 0.0,   'credit': 600.0,    'account_id': revenue_account_id,   'reconciled': False},
            {'amount_currency': 1200.0,     'debit': 600.0, 'credit': 0.0,      'account_id': accrual_account_id,   'reconciled': True},
            {'amount_currency': 1200.0,     'debit': 600.0, 'credit': 0.0,      'account_id': revenue_account_id,   'reconciled': False},
            {'amount_currency': -1200.0,    'debit': 0.0,   'credit': 600.0,    'account_id': accrual_account_id,   'reconciled': True},
        ])
    ...
    def test_journal_override_sequence_regex(self):
        ...
        next.post()
        self.assertEqual(next.name, '00000876-G 0003')

        next.journal_id.sequence_override_regex = r'^(?P<prefix1>)(?P<seq>\d*)(?P<suffix>.*)$'
        next.name = '/'
        next._compute_name()
        self.assertEqual(next.name, '00000877-G 0002')
    ...
    def test_journal_sequence_ordering(self):
        ...
        # Ordered by date
        self.assertEqual(copies[0].name, 'XMISC/2019/00001')
        self.assertEqual(copies[1].name, 'XMISC/2019/00002')
        self.assertEqual(copies[2].name, 'XMISC/2019/00003')
        self.assertEqual(copies[3].name, 'XMISC/2019/00004')
        self.assertEqual(copies[4].name, 'XMISC/2019/00005')
        self.assertEqual(copies[5].name, 'XMISC/2019/00006')
    ...
    def test_invoice_zero_percent_tax(self):
