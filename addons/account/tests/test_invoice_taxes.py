...
class TestInvoiceTaxes(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.percent_tax_1 = cls.env["account.tax"].create(
            {"name": "21%", "amount_type": "percent", "amount": 21, "sequence": 10,}
        )
        ...
        cls.percent_tax_2 = cls.env["account.tax"].create(
            {"name": "12%", "amount_type": "percent", "amount": 12, "sequence": 30,}
        )
    ...
    def test_group_of_taxes(self):
        ...
        """
        invoice = self._create_invoice(
            [(121, self.group_tax), (100, self.percent_tax_2),]
        )
        invoice.action_post()
        self.assertRecordValues(
            invoice.line_ids.filtered("tax_line_id").sorted(lambda x: x.price_unit),
    ...
    def test_division_tax(self):
        ...
        self.assertRecordValues(
            invoice.line_ids.filtered("tax_line_id"),
            [{"name": sale_tax.name, "tax_base_amount": 0.0, "balance": -100,}],
        )
        ...
        self.assertRecordValues(
            invoice.line_ids.filtered("tax_line_id"),
            [{"name": sale_tax.name, "tax_base_amount": 0.0, "balance": -200,}],
        )
