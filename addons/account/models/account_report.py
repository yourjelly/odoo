...
class AccountReportLine(models.Model):
    ...
    def _build_sum_formula_for_sum_children_aggregation(self, expression_label):
        self.ensure_one()
        all_terms = []
        expressions_to_gather = self.children_ids.expression_ids.filtered(lambda x: x.total == expression_label)
        for expr in expressions_to_gather:
            if expr.report_line_id.code:
                # Use the existing code to refer to the dependency
                all_terms.append(f"{expr.report_line_id.code}.{expr.total}")
            else:
                # Generate a forced code for the line; so that it can still be referenced
                all_terms.append(f"_no_code_{expr.report_line_id.id}.{expr.total}")
...
class AccountReportExpression(models.Model):
        ...
        while to_expand:
            ...
            for candidate_expr in to_expand:
                ...
                for line_code, expr_labels in labels_by_code.items():
                    # line_code might have been generated by the sum_children feature, in which case it contains the line id, but not its code
                    forced_code_match = re.match("_no_code_(?P<report_line_id>[0-9]*)", line_code)
                    if forced_code_match:
                        report_line_domain_term =('report_line_id', '=', int(forced_code_match.group('report_line_id')))
                    else:
                        ('report_line_id.code', '=', line_code)

                    dependency_domain = [report_line_domain_term, ('total', 'in', tuple(expr_labels))] + cross_report_domain
    ...
    def _get_aggregation_terms_details(self):
        ...
        for expression in self:
            ...
            expression_formula = expression.formula
            if re.match(expression_formula, 'sum_children(.*,.*)'):
                expression_formula = expression.report_line_ids._build_sum_formula_for_sum_children_aggregation(expression.total)

            expression_terms = re.split('[-+/*]', expression_formula.replace(' ', '')) # TODO une constante pour les opérateurs admis à l'aggrégation ?
