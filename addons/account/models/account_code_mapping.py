...
class AccountCodeMapping(models.Model):
    ...
    def _get_records(self, record_keys):
        ...
        new_records = self.browse(new_record_ids)

        self.env.cache.update(new_records, self._fields['account_id'], [account_id for account_id, _ in missing_keys])
        self.env.cache.update(new_records, self._fields['company_id'], [company_id for _, company_id in missing_keys])
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            # Since `account.code` may be protected at this point (if it was previously written on by `account.create()`)
            # writing on `code` might not do anything. Therefore we write on `code_store`.
            account.check_access('write')
            account.with_company(company.root_id).sudo().code_store = vals['code']

            # Since 'code' may be protected at this point, it might not get invalidated by the write on `code_store`, so
            # we need to invalidate it manually.
            account.invalidate_recordset(fnames=['code'])
