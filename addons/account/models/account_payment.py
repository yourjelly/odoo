...
class account_abstract_payment(models.AbstractModel):
    ...
    def default_get(self, fields):
        ...
        invoices = self.env['account.move'].browse(active_ids)\
            .filtered(lambda move: move.type in ('out_invoice', 'out_refund', 'in_invoice', 'in_refund', 'out_receipt', 'in_receipt'))
        ...
        multi = any(inv.commercial_partner_id != invoices[0].commercial_partner_id
            ...
            or inv.invoice_account_id != invoices[0].invoice_account_id
            or inv.invoice_partner_bank_id != invoices[0].invoice_partner_bank_id
            for inv in invoices)
        ...
        rec.update({
            ...
            'communication': ' '.join([ref for ref in invoices.mapped('ref') if ref]),
    ...
    def _onchange_partner_id(self):
        if not self.multi and self.invoice_ids and self.invoice_ids[0].invoice_partner_bank_id:
    ...
    def _compute_payment_amount(self, invoices=None, currency=None):
        ...
        for res in query_res:
            ...
            if move_currency == currency and move_currency != company.currency_id:
                total += MAP_INVOICE_TYPE_PAYMENT_SIGN[res['type']] * res['residual_currency']
            elif move_currency == currency == company.currency_id:
                total += MAP_INVOICE_TYPE_PAYMENT_SIGN[res['type']] * res['residual']
            else:
                amount_total = MAP_INVOICE_TYPE_PAYMENT_SIGN[res['type']] * res['residual']
                total += move_currency._convert(amount_total, currency, company, self.payment_date or fields.Date.today())
        return total
...
class account_register_payments(models.TransientModel):
    ...
    def _groupby_invoices(self):
        ...
        for inv in self.invoice_ids:
            ...
            account_id = inv.line_ids.filtered(lambda line: line._is_invoice_payment_term_line())[0].account_id.id
            invoice_type = MAP_INVOICE_TYPE_PARTNER_TYPE[inv.type]
            recipient_account =  inv.invoice_partner_bank_id
            key = (partner_id, account_id, invoice_type, recipient_account)
            if not key in results:
                results[key] = self.env['account.move']
            results[key] += inv
        return results
    ...
    def _prepare_payment_vals(self, invoices):
        ...
        bank_account = self.multi and invoices[0].invoice_partner_bank_id or self.partner_bank_account_id
        pmt_communication = self.show_communication_field and self.communication \
                            or self.group_invoices and ' '.join([inv.ref or inv.name for inv in invoices]) \
                            or invoices[0].ref # in this case, invoices contains only one element, since group_invoices is False
        values = {
...
class account_payment(models.Model):
    ...
    invoice_ids = fields.Many2many('account.move', 'account_invoice_payment_rel', 'payment_id', 'invoice_id', string="Invoices", copy=False, readonly=True, help="""Technical field containing the invoices for which the payment has been generated.
    ...
    def default_get(self, fields):
        ...
        if invoice_defaults and len(invoice_defaults) == 1:
            ...
            rec['payment_type'] = invoice['type'] in ('out_invoice', 'in_refund', 'out_receipt') and 'inbound' or 'outbound'
            rec['partner_type'] = MAP_INVOICE_TYPE_PARTNER_TYPE[invoice['type']]
            rec['partner_id'] = invoice['partner_id'] and invoice['partner_id'][0]
        ...
        for payment in self:
            ...
            if payment.currency_id == company_currency:
                ...
                balance = payment.currency_id._convert(
                    counterpart_amount, company_currency, payment.company_id, payment.payment_date)
                write_off_balance = payment.currency_id._convert(
                    write_off_amount, company_currency, payment.company_id, payment.payment_date)
            ...
            if payment.payment_type == 'transfer':
                ...
                if payment.destination_journal_id.currency_id:
                    counterpart_amount = payment.currency_id._convert(
                        counterpart_amount, payment.destination_journal_id.currency_id, payment.company_id, payment.payment_date)
    ...
    def post(self):
        ...
        """
        AccountMove = self.env['account.move'].with_context(type='misc')
        for rec in self:
