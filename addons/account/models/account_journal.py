...
class AccountJournal(models.Model):
    ...
    def _compute_edi_format_ids(self):
        ...
        for journal in self:
            ...
            # The existing edi formats that are already in use so we can't remove them.
    ...
    def write(self, vals):
        ...
        if vals.get('edi_format_ids'):
            ...
            if documents.filtered(lambda d: d.edi_format_id._needs_web_services()):
                raise UserError(
                    _('Cannot deactivate (%s) on this journal because not all documents are synchronized',
