...
class AccountAnalyticDefault(models.Model):
    ...
    date_start = fields.Date(
        string="Start Date", help="Default start date for this Analytic Account."
    )
    date_stop = fields.Date(
        string="End Date", help="Default end date for this Analytic Account."
    )

    @api.constrains("analytic_id", "analytic_tag_ids")
    def _check_account_or_tags(self):
        if any(
            not default.analytic_id and not default.analytic_tag_ids for default in self
        ):
