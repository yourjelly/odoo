...
class AccountLock_Exception(models.Model):
    ...
    def _search_state(self, operator, value):
        if operator not in ('in', 'not in') or set(value).difference(['revoked', 'expired', 'active']):
        ...
        if 'active' in value:
            domain |= Domain('active', '=', True) & (Domain('end_time', '=', False) | Domain('end_datetime', '>=', self.env.cr.now()))
        if operator == 'not in':
            domain = ~domain
        return domain
