...
class AccountChartTemplate(models.Model):
    ...
    def try_loading(self, company=False, install_demo=True):
        ...
        if not company.chart_template_id and not self.existing_accounting(company):
            ...
            if install_demo and self.env.ref("base.module_account").demo:
                self.with_context(
                    default_company_id=company.id, allowed_company_ids=[company.id],
                )._create_demo_data()
    ...
    def _load(self, sale_tax_rate, purchase_tax_rate, company):
        ...
        if not company.account_journal_suspense_account_id:
            company.account_journal_suspense_account_id = self._create_liquidity_journal_suspense_account(
...
class AccountTaxRepartitionLineTemplate(models.Model):
    ...
    def get_repartition_line_create_vals(self, company):
        ...
        for record in self:
            ...
            tags_to_add += record.plus_report_line_ids.mapped("tag_ids").filtered(
                lambda x: not x.tax_negate
            )
            tags_to_add += record.minus_report_line_ids.mapped("tag_ids").filtered(
