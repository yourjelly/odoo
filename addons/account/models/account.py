...
class AccountAccountTag(models.Model):
    ...
    tax_report_line_ids = fields.Many2many(string="Parent Tax Report Lines", comodel_name='account.tax.report.line', relation='account_tax_report_line_tags_rel', help="The tax report lines for wich this tag was generated")
...
class AccountTaxReportLine(models.Model):
    ...
    def create(self, vals):
        ...
        if tag_name:
            vals['children_tag_ids'] = self._get_tags_create_vals(tag_name, vals.get('country_id'))
    ...
    def write(self, vals):
        ...
        if 'tag_name' in vals:
            if vals['tag_name']:
                ...
                without_tags = self.filtered(lambda x: not x.children_tag_ids)
                for record in without_tags:
                    import pdb; pdb.set_trace()
                    record.write({'children_tag_ids': self._get_tags_create_vals(record.tag_name, record.country_id.id)})

                # We update the name of every tag linked to other lines
                to_update = self - without_tags
                minus_child_tags = to_update.mapped('children_tag_ids').filtered(lambda x: x.tax_negate)
                minus_child_tags.write({'name': '-' + vals['tag_name']})

                plus_child_tags = to_update.mapped('children_tag_ids').filtered(lambda x: not x.tax_negate)
                plus_child_tags.write({'name': '+' + vals['tag_name']})

            else:
                # tag_name was set empty, so we remove the tags
                self._delete_tags(self.mapped('children_tag_ids.id'))
    ...
    def _delete_tags(self, tag_ids_to_delete):
        ...
        repartition_lines.write({'tag_ids': [(2, tag_id, 0) for tag_id in tag_ids_to_delete]})
