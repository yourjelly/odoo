...
class AccountMove(models.Model):
    ...
    def _aggregate_tax_details(self):
        self.ensure_one()
        all_lines = self.line_ids.filtered(lambda line: not line.display_type)
        base_lines = all_lines.filtered(lambda line: not line._get_tax_repartition_line())

        tax_details_list = []
        for line in base_lines:
            for tax_detail in line.tax_detail_ids:
        ...
        return all_lines._prepare_diff_tax_lines(tax_details_list)
    ...
    def _recompute_tax_lines(self, recompute_tax_base_amount=False):
        ...
        if not recompute_tax_base_amount:
            for line_diff in base_lines._prepare_diff_base_lines():
                update_vals = line_diff['update']
                line_ids_commands.append(Command.update(line_diff['source_line'].id, {
                    'tax_exigible': update_vals['tax_exigible'],
                    'tax_tag_ids': update_vals['tag_ids'],
                }))

        # Update the tax lines.

        for diff_vals in self._aggregate_tax_details():
...
class AccountMoveLine(models.Model):
    ...
    def _compute_tax_details(self):
        tax_lines = self.filtered(lambda line: not line.display_type and line.tax_repartition_line_id)
        base_lines = self.filtered(lambda line: not line.display_type and not line.tax_repartition_line_id)
        null_lines = self.filtered('display_type')
        ...
        # Tax lines.
        # Ensure each tax line is exactly the sum of its related tax details.

        # Map each tax line with those originator tax details.
        tax_details_per_tax_line = defaultdict(lambda: self.env['account.tax.detail'])
        for move in tax_lines.move_id:
            for diff_vals in move._aggregate_tax_details():
                if diff_vals['command'][0] != Command.UPDATE:
                    continue
                for tax_detail_vals in diff_vals['source_dicts']:
                    tax_details_per_tax_line[diff_vals['source_line']] |= tax_detail_vals['source_record']
        ...
        for line in tax_lines:
            ...
            tax_details = tax_details_per_tax_line[line]

            # Remove the previously generated tax details.
            for tax_detail in line.tax_detail_ids:
                tax_detail_ids_commands.append(Command.delete(tax_detail.id))

            if not tax_details:
                continue

            # Create a new rounding tax detail.
            delta_balance = line.balance - sum(tax_details.mapped('tax_amount'))
            delta_amount_currency = line.amount_currency - sum(tax_details.mapped('tax_amount_currency'))
            if not float_is_zero(delta_balance, precision_digits=pd1) or not float_is_zero(delta_amount_currency, precision_digits=pd2):
                tax_detail_ids_commands.append(Command.create({
                    'account_id': line.account_id.id,
                    'tax_amount': float_round(delta_balance, precision_digits=pd1),
                    'tax_amount_currency': float_round(delta_amount_currency, precision_digits=pd2),
                    'tax_base_amount': tax_details[0].tax_base_amount if tax_details else 0.0,
                    'tax_base_amount_currency': tax_details[0].tax_base_amount_currency if tax_details else 0.0,
                    'tax_ids': [(6, 0, line.tax_ids.ids)],
                    'tag_ids': [(6, 0, line.tax_tag_ids.ids)],
                    'tax_repartition_line_id': line.tax_repartition_line_id.id,
                    'tax_id': tax_details[0].tax_id.id,
                }))
