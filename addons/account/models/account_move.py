...
class AccountMove(models.Model):
    ...
    def _get_reconciled_info_JSON_values(self):
        ...
        reconciled_vals = []
        for amount, counterpart_line in self._get_reconciled_invoices_partials():
    ...
    def _get_reconciled_invoices_partials(self):
        ...
        :return A list of tuple (amount, invoice_line).
        '''
        self.ensure_one()
        ...
        for partial in pay_term_lines.matched_debit_ids:
            invoice_partials.append((partial.credit_amount_currency, partial.debit_move_id))
        for partial in pay_term_lines.matched_credit_ids:
            invoice_partials.append((partial.debit_amount_currency, partial.credit_move_id))
...
class AccountMoveLine(models.Model):
    ...
    def _update_accounting_fields_from_price_subtotal(self):
        self.ensure_one()
        ...
        if not currency.is_zero(self.amount_currency - self.price_subtotal):
            amount_currency = self.price_subtotal * sign
            balance = currency._convert(
                amount_currency,
                company_currency,
                company,
    ...
    def _onchange_amount_currency(self):
        for line in self:
            ...
            if not line.move_id.is_invoice(include_receipts=True):
                balance = line.currency_id._convert(line.amount_currency, company.currency_id, company, line.move_id.date or fields.Date.today())
                line.debit = balance if balance > 0.0 else 0.0
                line.credit = -balance if balance < 0.0 else 0.0
                continue
    ...
    def _create_reconciliation_partials(self):
        ''' Create the partials on the current journal items to perform the reconciliation.
        ...
        return self.env['account.partial.reconcile'].create(partials_vals_list)

    def _create_reconciliation_exchange_difference_move(self, tax_cash_basis_values_per_move):
        ...
        def add_line_to_transfer_vals(line, add_or_default=False):
            # Helper to add the move line balance to the transfer journal entries to fix using the right
            # computed grouping key.
            ...
            if add_or_default:
                transfer_account_vals_to_fix.setdefault(grouping_key, {
                    **grouping_dict,
                    'balance': 0.0,
                })
            if grouping_key in transfer_account_vals_to_fix:
                transfer_account_vals_to_fix[grouping_key]['balance'] += line.balance

        # Map to retrieve the balance of the tax accounts to fix for each line.
        transfer_account_vals_to_fix = {}

        # Group by all lines by transfer account that need to be balanced but this exchange difference journal entry.
        for move_values in tax_cash_basis_values_per_move.values():
            ...
            for line in move_values['to_process_lines']:
                ...
                if not line.account_id.reconcile:
                    ...
                    add_line_to_transfer_vals(line, add_or_default=True)

            if not is_exchange_diff_needed:
                continue

            for line in move_values['cash_basis_moves'].line_ids:
                add_line_to_transfer_vals(line)

        # Create extra journal items on the exchange difference journal entry.
        for values in transfer_account_vals_to_fix.values():

            account = self.env['account.account'].browse(values['account_id'])
            if account.company_id.currency_id.is_zero(values['balance']):
                continue
            ...
            exchange_diff_move_vals['line_ids'] += [
                (0, 0, {
                    ...
                    'debit': -values['balance'] if values['balance'] < 0.0 else 0.0,
                    'credit': values['balance'] if values['balance'] > 0.0 else 0.0,
                    'amount_currency': 0.0,
                    'tax_base_amount': values.get('tax_base_amount', 0.0),
                    'account_id': values['account_id'],
                    'currency_id': values['currency_id'],
                    'partner_id': values['partner_id'],
                    'tax_repartition_line_id': values.get('tax_repartition_line_id'),
                    'analytic_account_id': values['analytic_account_id'],
                    'analytic_tag_ids': [(6, 0, values['analytic_tag_ids'])],
                    'tax_tag_ids': [(6, 0, values.get('tax_tag_ids', []))],
                    'tax_ids': [(6, 0, values.get('tax_ids', []))],
                    'sequence': sequence,
                }),
                (0, 0, {
                    'name': _('Currency exchange rate difference (cash basis)'),
    ...
    def reconcile(self):
        ...
        if is_full_needed:
            ...
            if self._context.get('no_exchange_difference'):
                ...
                exchange_move = involved_lines._create_reconciliation_exchange_difference_move(tax_cash_basis_values_per_move)
                if exchange_move:
