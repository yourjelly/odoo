...
class AccountMove(models.Model):
    ...
    def _onchange_invoice_date(self):
        if self.invoice_date:
            ...
            if (
                ...
                and self.date != self.invoice_date
    ...
    def _compute_invoice_taxes_by_group(self):
        ...
        for move in self:
            ...
            for line in tax_lines:
                ...
                res[line.tax_line_id.tax_group_id]["amount"] += (
    ...
    def _validate_taxes_country(self):
        ...
        for record in self:
            if (
                record.line_ids.tax_ids
                and record.line_ids.tax_ids.country_id != record.tax_country_id
            ):
                raise ValidationError(
                    _(
    ...
    def _unlink_forbid_parts_of_chain(self):
        ...
        if not self._context.get("force_delete") and any(
            move.name != "/" and not move._is_last_from_seq_chain() for move in self
        ):
            raise UserError(
                _(
                    "You cannot delete this entry, as it has already consumed a sequence number and is not the last one in the chain. Probably you should revert it instead."
                )
            )
            ...
            for move, reverse_move in zip(self, reverse_moves):
                lines = move.line_ids.filtered(
                    lambda x: (
                        ...
                        or x.account_id.internal_type == "liquidity"
                    )
                ...
                for line in lines:
                    ...
                    (line + counterpart_lines).with_context(
    ...
    def action_view_reverse_entry(self):
        ...
        if len(reverse_entries) == 1:
            action.update(
                {"view_mode": "form", "res_id": reverse_entries.id,}
            )
        else:
            action.update(
    ...
    def generate_qr_code(self):
        ...
        rslt = self.partner_bank_id.build_qr_code_url(
...
class AccountMoveLine(models.Model):
    ...
    def _get_computed_price_unit(self):
        ...
        if self.product_uom_id and self.product_uom_id != product_uom:
            product_price_unit = product_uom._compute_price(
                product_price_unit, self.product_uom_id
            )
        ...
        if product_taxes and fiscal_position:
            ...
            if set(product_taxes.ids) != set(product_taxes_after_fp.ids):
                ...
                if any(tax.price_include for tax in flattened_taxes_before_fp):
                    ...
                    product_price_unit = company_currency.round(
    ...
    def _onchange_amount_currency(self):
        for line in self:
            ...
            balance = line.currency_id._convert(
                line.amount_currency, company.currency_id, company, line.move_id.date
            )
    ...
    def _compute_amount_residual(self):
        ...
        for line in self:
            if line.id and (
                ...
                line.reconciled = line.company_currency_id.is_zero(
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if currency_id == move.company_id.currency_id.id:
                ...
                vals.update(
                    {"currency_id": currency_id, "amount_currency": balance,}
                )
            else:
            ...
            for move_id, modified_lines in move_initial_values.items():
                ...
                for line in self.filtered(lambda l: l.move_id.id == move_id):
                    ...
                    if tracking_value_ids:
                        ...
                        tmp.update({"error": True, "field_error": field_name})
                    else:
                        continue
