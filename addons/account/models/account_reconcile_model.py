...
class AccountReconcileModelLineCommon(models.AbstractModel):
    _name = "account.reconcile.model.line.common"
    _description = "Common between account.reconcile.model.line & account.reconcile.model.line.template"
    ...
    amount_string = fields.Char(
        string="Amount",
        default='100', store=True, readonly=False,
        compute='_compute_amount_string',
    ...
    * From Label: There is no need for regex delimiter, only the regex is needed. For instance if you want to extract the amount from\nR:9672938 10/07 AX 9415126318 T:5L:NA BRT: 3358,07 C:\nYou could enter\nBRT: ([\d,]+)""")
    force_tax_included = fields.Boolean(
        ...
        compute='_compute_force_tax_included',
    ...
    def _compute_amount_string(self):
        for line in self:
            if line.amount_type == 'percentage':
                line.amount_string = '100'
            elif line.amount_type == 'regex':
                line.amount_string = '([\d,]+)'
            else:
                line.amount_string = ''
...
class AccountReconcileModelLineTemplate(models.Model):
    ...
    _inherit = 'account.reconcile.model.line.common'
    _description = "Reconcile Model Line Template"

    model_id = fields.Many2one(comodel_name='account.reconcile.model.template')
...
class AccountReconcileModelLine(models.Model):
    ...
    # ==== Stored fields ====
    model_id = fields.Many2one(comodel_name='account.reconcile.model', readonly=True)
    ...
    @api.depends('tax_ids')
    def _compute_force_tax_included(self):
        for line in self:
            if len(line.tax_ids) != 1:
                line.force_tax_included = False
            else:
                line.force_tax_included = line.force_tax_included
    ...
    @api.constrains('amount_string', 'amount_type', 'amount')
...
class AccountReconcileModelCommon(models.AbstractModel):
    _name = "account.reconcile.model.common"
    _description = "Common between account.reconcile.model & account.reconcile.model.template"
    ...
    rule_type = fields.Selection(
        ...
        string="Type",
        default='writeoff_button', required=True)
...
class AccountReconcileModelTemplate(models.Model):
    ...
    _inherit = 'account.reconcile.model.common'
    ...
    line_ids = fields.One2many(
        comodel_name='account.reconcile.model.line.template',
