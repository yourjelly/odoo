...
class AccountReconcileModel(models.Model):
    ...
    def _get_write_off_move_lines_dict(self, st_line, open_balance_vals):
        ...
        if not self.account_id or st_line.company_currency_id.is_zero(residual_amount):
            return []

        if self.amount_type == 'percentage':
            balance = residual_amount * (self.amount / 100.0)
        elif self.amount_type == "regex":
            match = re.search(self.amount_from_label_regex, st_line.payment_ref)
            if match:
                balance = copysign(float(re.sub(r'\D' + self.decimal_separator, '', match.group(1)).replace(self.decimal_separator, '.')) * (1 if residual_amount > 0.0 else -1), residual_amount)
            else:
                balance = 0
        else:
            balance = self.amount * (1 if residual_amount > 0.0 else -1)

        lines_vals_list = []

        writeoff_line = {
            'name': self.label or st_line.payment_ref,
            'balance': balance,
        ...
        if self.has_second_line and self.second_account_id:
            remaining_balance = balance - sum(aml['balance'] for aml in lines_vals_list)
            if self.second_amount_type == 'percentage':
                ...
                match = re.search(self.second_amount_from_label_regex, st_line.payment_ref)
                if match:
            ...
            second_writeoff_line = {
                'name': self.second_label or st_line.payment_ref,
            ...
            lines_vals_list.append(second_writeoff_line)
            ...
            if self.second_tax_ids:
                ...
                if self.force_second_tax_included:
                    tax = tax[0].with_context(force_price_include=True)
                lines_vals_list += self._get_taxes_move_lines_dict(tax, second_writeoff_line)
        ...
        for rule in self:
            ...
            SELECT
                ...
                (
                    ...
                    AND
                    TRIM(move.invoice_payment_ref) = TRIM(st_line.payment_ref)
