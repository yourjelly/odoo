...
class AccountReconcileModel(models.Model):
    ...
    def _get_write_off_move_lines_dict(self, st_line, residual_balance):
        ...
        if self.rule_type == "invoice_matching" and (
            not self.match_total_amount or (self.match_total_amount_param == 100)
        ):
            return []
    ...
    def _prepare_reconciliation(self, st_line, aml_ids=[], partner=None):
        ...
        write_off_amount = (
            max(aml_total_residual, -st_line_residual_before, key=abs)
            + st_line_residual_before
            + st_line_residual
        )
        ...
        for partner_mapping in self.partner_mapping_line_ids:
            ...
            match_narration = (
                re.match(
                    ...
                    html2text.html2text(st_line.narration or "").rstrip(),
                )
    ...
    def _get_invoice_matching_rule_result(
        ...
        if candidate_currencies == {st_line_currency.id}:
            for candidate in candidates:
                residual_amount = (
                    candidate["aml_currency_id"]
                    and candidate["aml_amount_residual_currency"]
                    or candidate["aml_amount_residual"]
                )
        ...
        if (
            not disable_bypass and priorities & {1, 2}
        ) or self._check_rule_propositions(st_line, candidates):
            rslt = {
                "model": self,
    ...
    def _check_rule_propositions(self, statement_line, candidates):
        ...
        for reconciliation_vals in reconciliation_overview:
            ...
            if line_vals["currency_id"]:
                ...
                line_residual_after_reconciliation -= (
        ...
        reconciled_percentage = (
            100
            - abs(line_residual_after_reconciliation) / abs(residual_difference) * 100
