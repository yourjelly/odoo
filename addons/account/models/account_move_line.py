...
class AccountMoveLine(models.Model):
    ...
    def _manually_modified(self, fname):
        if not self.env.context.get('check_has_changed'):
            new_line = self.new(origin=self._origin).with_context(check_has_changed=True)
            current_value = new_line[fname]
            self._fields[fname].compute_value(new_line)
            computed_value = new_line[fname]
    ...
    def _compute_name(self):
        ...
        for line in self.filtered(lambda l: l.move_id.inalterable_hash is False):
            if line._manually_modified('name'):
                continue
