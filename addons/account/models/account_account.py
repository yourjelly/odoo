...
class AccountGroup(models.Model):
    ...
    def _adapt_parent_account_group(self, company=None):
        ...
        if not company_ids:
            return
        cond = SQL('child.company_id IN %s', tuple(company_ids))
        self.flush_model()
        query = """
            WITH relation AS (
        ...
        if updated_record:
            ...
            # self.env.cr.execute('ANALYZE account_group') to check, could be usefull on some cases if number of update was high
