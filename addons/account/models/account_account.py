...
def get_account_type_domain(field_names):
    domains = []
    for parent_value, valid_values in get_account_type_value_at_level(len(field_names) - 1):
        domains.append([(field_names[len(field_names) - 1], '=', parent_value), (field_names[len(field_names) - 2], 'in', tuple(valid_values))])
...
class AccountAccount(models.Model):
    ...
    main_type = fields.Selection(
        selection=get_account_type_selection(0),
        string="Type")
    sub_type = fields.Selection(
        selection=get_account_type_selection(1),
        string="Sub Type",
        store=True, readonly=False, copy=True,
        compute='_compute_sub_type',
        domain=str(get_account_type_domain(['main_type', 'sub_type'])))
    sub_sub_type = fields.Selection(
        selection=get_account_type_selection(1),
        string="Sub Sub Type",
        store=True, readonly=False, copy=True,
        compute='_compute_sub_sub_type',
        domain=str(get_account_type_domain(['main_type', 'sub_type', 'sub_sub_type'])))
