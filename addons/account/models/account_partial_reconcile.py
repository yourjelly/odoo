            ...
            for partial_values in move_values['partials']:
                ...
                for caba_treatment, line in move_values['to_process_lines']:
                    ...
                    if caba_treatment == 'tax':
                        ...
                        if analytics:
                            res = {}
                            for key, percents in analytic_for_taxes.items():
                                analytic_distribution, amount = key
                                analytic_distribution = eval(analytic_distribution)
                                for percent in percents:
                                    amount_tax = amount*percent/100
                                    ratio = 1.0
                                    if amount != 0:
                                        ratio = abs(amount_tax / move.amount_tax)
                                    if analytic_distribution:
                                        for analytic_id, analytic_percent in analytic_distribution.items():
                        ...
                        cb_line_vals['analytic_distribution'] = {}
                        if analytics:
                            if not (str(line.analytic_distribution), line.balance) in analytic_for_taxes:
                                analytic_for_taxes[(str(line.analytic_distribution), line.balance)] = []
                            analytic_for_taxes[(str(line.analytic_distribution), line.balance)].extend(line.tax_ids.mapped('amount'))
                            ratio = 1.0
