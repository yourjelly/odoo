...
class AccountBusinessLineMixin(models.AbstractModel):
    ...
    def _get_company(self):
        # TO BE OVERRIDDEN
        return self.env['res.company'] or self.env.company

    def _get_currency(self):
        # TO BE OVERRIDDEN
        return self.env['res.currency'] or self._get_company().currency_id
    ...
    def _compute_line_tax_details(self):
        ...
        for tax_vals in taxes_res['taxes']:
            tax_vals['grouping_dict'] = self._get_tax_base_line_grouping_key(tax_vals)
        return taxes_res
    ...
    def _prepare_diff_tax_lines(self):
        ...
        for line in base_lines:
            taxes_res = line._compute_line_tax_details()

            for tax_res in taxes_res['taxes']:
                map_key = _serialize_python_dictionary(tax_res['grouping_dict'])
                ...
                base_line_map[map_key]['tax_base_amount'] += tax_res['base']
                base_line_map[map_key]['tax_amount'] += tax_res['amount']
        ...
        for map_key, agg_tax_detail in base_line_map.items():
            currency = self.env['res.currency'].browse(agg_tax_detail['currency_id'])
            res['amount_tax'] += currency.round(agg_tax_detail['tax_amount'])
