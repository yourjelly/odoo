...
class AccountBusinessLineMixin(models.AbstractModel):
    ...
    def _prepare_tax_details(self):
        ...
        :return: A list of python dictionaries containing:
            ...
            * tax_details:  A list of python dictionary containing:
                ...
                * <The values returned by _get_tax_detail_from_base_lineget_tax_grouping_key_from_base_line')
    ...
    def _prepare_diff_base_lines(self):
        ''' Compute a list of python dictionaries used to update the base lines according the newly computed taxes.

        :return: A list of python dictionaries containing:
            * source_line:  The business line record.
            * update:       A dictionary containing:
                * tax_exigible: Must this base line appear in the tax report.
                * tag_ids:      The tax grids for this base line.
        '''
        base_lines = self.filtered(lambda line: not line._get_tax_repartition_line())
        res = []
        for line in base_lines:
            is_refund_document = line._is_refund_document()
            tax_reps_field = 'refund_repartition_line_ids' if is_refund_document else 'invoice_repartition_line_ids'
            tax_reps = line.tax_ids.flatten_taxes_hierarchy()\
                .mapped(tax_reps_field)\
                .filtered(lambda x: x.repartition_type == 'base')
            res.append({
                'source_line': line,
                'update': {
                    'tax_exigible': all(tax.tax_exigibility == 'on_invoice' for tax in line.tax_ids),
                    'tag_ids': [Command.set(tax_reps.tag_ids.ids)],
                },
            })
        return res
