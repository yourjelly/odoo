...
class ResPartnerBank(models.Model):
    ...
    def _check_allow_out_payment(self):
        ...
        for bank in self:
            if bank.allow_out_payment:
                if not self.env.user._has_group('account.group_validate_bank_account'):
    ...
    def _compute_user_has_group_validate_bank_account(self):
        user_has_group_validate_bank_account = self.env.user._has_group('account.group_validate_bank_account')
        for bank in self:
    ...
    def write(self, vals):
        ...
        if 'allow_out_payment' in vals and not self.env.user._has_group('account.group_validate_bank_account'):
