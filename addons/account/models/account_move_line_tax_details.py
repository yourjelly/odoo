...
class AccountMoveLine(models.Model):
    ...
    def _get_query_tax_details(self, table_references, search_condition, fallback=True) -> SQL:
        ...
        :param table_references:    The query to inject after the FROM as SQL object.
        :param search_condition:    The query to inject in the WHERE clause as SQL object.
        :param fallback:            Fallback on an approximated mapping if the mapping failed.
        :return:                    A tuple <query, params>.
        """
        ...
        for group_tax in group_taxes:
            ...
            group_taxes_query_list.append(SQL(f'WHEN tax.id = %s THEN ARRAY[{children_taxes_in_query}]',
                                              group_tax.id,
                                              *children_taxes.ids))
