...
class AccountPaymentRegister(models.TransientModel):
    ...
    def _compute_writeoff_account_id(self):
        for wizard in self:
            ...
            if wizard.can_edit_wizard:
                batch_result = wizard._get_batches()[0]
                cash_discount_accounts_ids = [wizard.company_id.account_journal_cash_discount_income_id.id,
                                             wizard.company_id.account_journal_cash_discount_expense_id.id]
                if wizard.writeoff_account_id and wizard.writeoff_account_id.id not in cash_discount_accounts_ids:
                    return
                if batch_result['payment_values']['early_discount_pay_term']:
                    wizard.writeoff_account_id = self.get_early_pay_cash_discount_account(batch_result['payment_values'])

    def get_early_pay_cash_discount_account(self, payment_values):
        for wizard in self:
            if payment_values['payment_type'] == 'inbound':
                writeoff_account_id = wizard.company_id.account_journal_cash_discount_income_id.id
            else:
                writeoff_account_id = wizard.company_id.account_journal_cash_discount_expense_id.id
        return writeoff_account_id

    @api.depends('payment_difference_handling')
    def _compute_writeoff_label(self):
        for wizard in self:
            if not wizard.writeoff_label:
    ...
    def _create_payment_vals_from_batch(self, batch_result):
        ...
        early_pay_applicable = batch_values['discounted_amount'] and self.early_pay_discount_toggle_button
        payment_vals = {
            'date': self.payment_date,
            'amount': batch_values['source_amount_currency'] if not early_pay_applicable else batch_values['discounted_amount'],
        ...
        if early_pay_applicable:
            payment_vals['write_off_line_vals'] = {
                ...
                'account_id': self.get_early_pay_cash_discount_account(payment_vals),
            }
        return payment_vals
