...
class AccountPaymentRegister(models.TransientModel):
    ...
    move_ids = fields.Many2many('account.move', 'account_payment_register_move_rel', 'wizard_id', 'move_id',
        string="Journal entries", readonly=True, copy=False,)
    ...
    wizard_mode = fields.Selection(selection=[
        ('single_move', 'Single Journal Entry'),
    ...
    def _get_move_batch_key(self, move):
        ''' Method used to give the "key" grouping the moves in batches.
        This key is expressed as a dictionary. At the end, an account.payment will be
        generated for each batch.
        :param move:    An account.move record.
        :return:        A python dictionary that will be a grouping key after serialization.
        '''
        lines = move.line_ids.filtered(lambda line: line.account_internal_type in ('receivable', 'payable'))
        ...
        if lines[0].account_internal_type == 'receivable':
    ...
    def _get_wizard_values_from_batch(self, batch_result):
        ...
        batch_moves = batch_result['moves']
        lines = batch_moves.line_ids.filtered(lambda line: line.account_internal_type in ('receivable', 'payable'))
    ...
    def _get_batch_communication(self, batch_result):
        ...
        '''
        lines = batch_result['moves'].line_ids.filtered(lambda line: line.account_internal_type in ('receivable', 'payable'))
        return ' '.join(label for label in lines.mapped('name') if label)
    ...
    def _compute_from_moves(self):
        ...
        for pay in self:
            ...
            if len(batches) == 1:
                ...
                if len(batch_result['moves']) == 1:
                    pay.wizard_mode = 'single_move'
                else:
                    pay.wizard_mode = 'single_batch'
            else:
                # == Multiple batches: The wizard is not editable  ==
                pay.update({
                    'company_id': batches[0]['moves'][0].company_id.id,
                    'partner_id': False,
                ...
                if all(len(batch_result['moves']) == 1 for batch_result in batches):
    ...
    def _compute_communication(self):
        for pay in self:
            if pay.wizard_mode in ('single_move', 'single_batch'):
    ...
    def _compute_payment_method_fields(self):
        for pay in self:
            if pay.payment_type == 'inbound':
                ...
                pay.hide_payment_method = len(pay.journal_id.inbound_payment_method_ids) <= 1
            else:
                pay.available_payment_method_ids = pay.journal_id.outbound_payment_method_ids
                pay.hide_payment_method = len(pay.journal_id.outbound_payment_method_ids) <= 1
    ...
    def default_get(self, fields_list):
        ...
        if 'move_ids' in fields_list and 'move_ids' not in res:
            ...
            # Keep moves having a residual amount to pay.
            for move in moves:
                ...
                if not available_lines:
                    raise UserError(_(
                        "You can't register a payment for the %s journal entry because there is not residual amount to "
                        "pay landing on a receivable/payable account."
                    ) % move.name)
    ...
    def _create_payments(self):
        ...
        to_reconcile = []
        if self.wizard_mode == 'single_move' or (self.wizard_mode == 'single_batch' and self.group_payment):
            payment_vals = self._create_payment_vals_from_wizard()
            payment_vals_list = [payment_vals]
            to_reconcile.append(batches[0]['moves'])
        else:
            # Don't group payments: Create one batch per move.
            if not self.group_payment:
                new_batches = []
                for batch_result in batches:
                    for move in batch_result['moves']:
                        new_batches.append({
                            **batch_result,
                            'moves': move,
                        })
            ...
            for batch_result in batches:
                ...
                to_reconcile.append(batch_result['moves'])

        payments = self.env['account.payment'].create(payment_vals_list)
        payments.action_post()

        domain = [('account_internal_type', 'in', ('receivable', 'payable')), ('reconciled', '=', False)]
        for payment, moves in zip(payments, to_reconcile):
            payment_lines = payment.line_ids.filtered_domain(domain)
            for account in payment_lines.account_id:
                (payment_lines + moves.line_ids)\
