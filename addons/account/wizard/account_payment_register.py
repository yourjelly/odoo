...
class AccountPaymentRegister(models.TransientModel):
    ...
    def _get_line_batch_key(self, line):
        ...
        res = {
            ...
            "partner_bank_id": (
                line.move_id.partner_bank_id
                or line.partner_id.commercial_partner_id.bank_ids[:1]
            ).id,
        ...
        if line.move_id.partner_bank_id and line.move_id.is_inbound():
            journal = self._search_journal(
                line.move_id.company_id.id,
                res["currency_id"],
                line.move_id.partner_bank_id.id,
            )
            if journal:
    ...
    def _search_journal(self, company_id, currrency_id, partner_bank_id=None):
        ...
        if currrency_id:
            journal = self.env["account.journal"].search(
                domain + [("currency_id", "=", currrency_id)], limit=1
            )
        if not journal:
    ...
    def _get_batches(self):
        ...
        for line in lines:
            ...
            batches.setdefault(
                serialized_key,
                {"key_values": batch_key, "lines": self.env["account.move.line"],},
            )
    ...
    def _compute_journal_id(self):
        for wizard in self:
            if wizard.journal_id:
                ...
                wizard.journal_id = self._search_journal(
                    wizard.company_id.id, wizard.source_currency_id.id
                )
    ...
    def action_create_payments(self):
        ...
        if len(payments) == 1:
            action.update(
                {"view_mode": "form", "res_id": payments.id,}
            )
        else:
            action.update(
                {"view_mode": "tree,form", "domain": [("id", "in", payments.ids)],}
            )
        return action
