...
class AutomaticEntryWizard(models.TransientModel):
        ...
        for record in self:
            ...
            if record.action == 'change_period':
                for aml in record.move_line_ids:
                    lock_date_message = aml.move_id._get_lock_date_message(aml.date)
    ...
    def _get_move_dict_vals_change_period(self):
        ...
        def get_lock_safe_date(aml):
            ...
            return reference_move._get_accounting_date(aml.date, aml.journal_id, tax_closing_types=aml.move_id._get_impacted_tax_closing_types())
    ...
    def _do_action_change_period(self, move_vals):
        ...
        for move in self.move_line_ids.move_id:
            ...
            accrual_move = created_moves[1:].filtered(
                lambda m: m.date == m._get_accounting_date(move.date, move.journal_id, tax_closing_types=move._get_impacted_tax_closing_types())
            )
