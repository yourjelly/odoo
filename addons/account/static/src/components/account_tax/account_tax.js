...
export class AccountTaxJson extends Component {
    static template = "account.AccountTaxJson";
    static components = {
        TagsList,
        Record,
        Field,
    ...
    async willStart() {
        if (this.editingRecord) {
            ...
            await this.fetchAllTaxes(this.props);
        }
    ...
    //             await this.fetchAllTaxes({ record });
    //         }
    ...
    async fetchAllTaxes(props) {
        this.allTaxes = await this.orm.call("account.move.line", "get_account_tax_json", [], {id: props.record.evalContext.id});
    }
...
export const accountTaxJson = {
    component: AccountTaxJson,
...
registry.category("fields").add("account_tax_json", accountTaxJson);
