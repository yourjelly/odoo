...
var payment_method = require('point_of_sale.electronic_payment_method');
...
var MercuryPaymentMethod = payment_method.AbstractPaymentMethod.extend({
    ...
    retry_mercury_transaction: function (def, response, retry_nr, can_connect_to_server, callback, args) {
        ...
        if (retry_nr < self.server_retries) {
            ...
            if (response) {
                message = "Error " + response.error + ": " + self.lookUpCodeTransaction["TimeoutError"][response.error] + "<br/>" + response.message;
            } else {
        ...
        mercury_transaction.call('do_payment', [transaction], undefined, {timeout: self.server_timeout_in_ms}).then(function (data) {
            ...
            if (response.status === 'Approved') {
                ...
                if (response.message === "AP*" && self._does_credit_payment_line_exist(response.authorize, decodedMagtek['number'],
                    ...
                    // todo jov: this should automagically be fixed
                    // when using the electronic payment information
                    // thing
    ...
    // don't do anything when creating a paymentline
...
pos_model.PosModel = pos_model.PosModel.extend({
    after_load_server_data: function () {
        ...
        this.cashregisters.forEach(function (cashregister) {
            ...
            if (journal.code === "BNK2") { // todo jov: this sucks, should just check Mercury flag from backend
                var payment_method = new MercuryPaymentMethod(self, journal.id);
                self.electronic_payment_methods.push(payment_method);
            }
        });

        return _modelproto.after_load_server_data.apply(this, arguments);
    },
...
PaymentScreenWidget.include({
    ...
    credit_action_callback: function (parsed_barcode_result) {
        var online_payment_journals = this.pos.getOnlinePaymentJournals();

        // todo jov: i don't really care about this
        // online_payment_journal stuff, we should probably just get
        // rid of it.
        if (online_payment_journals.length === 1) {
            parsed_barcode_result.journal_id = online_payment_journals[0].item;
            this.pos.current_paymentline_electronic_pay(parsed_barcode_result);
        } 
    },
    show: function () {
        this._super();

        // todo jov: same as above, we should get rid of it or clean it up
        if (this.pos.getOnlinePaymentJournals().length !== 0) {
            this.pos.barcode_reader.set_action_callback('credit', _.bind(this.credit_action_callback, this));
        }            
    },
