...
function describeWhyStepFailed(step) {
    ...
    if (step.skip_trigger && !stepState.skipTriggerFound) {
        return `Skip trigger element cannot be found in DOM : ${step.skip_trigger}}`;
    } else if (step.extra_trigger && !stepState.extraTriggerFound) {
        return `Extra trigger element cannot be found in DOM : ${step.extra_trigger}}`;
    } else if (!stepState.triggerFound) {
        return `Trigger element cannot be found in DOM : ${step.trigger}}`;
    } else if (step.alt_trigger && !stepState.altTriggerFound) {
        return `Alt(ernative) trigger element cannot be found in DOM : ${step.alt_trigger}}`;
