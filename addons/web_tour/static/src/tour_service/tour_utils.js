...
export class RunningTourActionHelper {
    ...
    selectByIndex = (index, selector) => {
        const element = this.focus(selector);
        const value = hoot.queryAll(`option`, { root: this.currentActiveElement }).at(index)?.value;
        if (value) {
            hoot.select(value, { target: this.currentActiveElement });
    ...
    selectByLabel = (contains, selector) => {
        this.focus(selector);
        const values = hoot.queryAll(`option:contains(${contains})`, { root: this.currentActiveElement })
