...
class TestKarmaTrackingCommon(common.TransactionCase):
    ...
    def test_consolidation_monthly(self):
        ...
        consolidated = Tracking.search(
            [("user_id", "=", self.test_user_2.id), ("consolidated", "=", True),]
        )
        self.assertEqual(len(consolidated), 2)
        self.assertEqual(
        ...
        current_user_trackings = Tracking.search([("user_id", "=", self.test_user.id),])
