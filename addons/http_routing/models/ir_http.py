...
def url_lang(path_or_uri, lang_code=None):
    ...
    if not url.netloc and not url.scheme and (url.path or force_lang):
        ...
        lang_url_codes = [url_code for _, url_code, _, _ in Lang.get_available()]
...
def is_multilang_url(local_url, lang_url_codes=None):
    ...
    if not lang_url_codes:
        lang_url_codes = [url_code for _, url_code, _, _ in request.env['res.lang'].get_available()]
...
class IrHttp(models.AbstractModel):
    ...
    def _add_dispatch_parameters(cls, func):
        ...
        if request.routing_iteration == 1:
            ...
            lang_codes = [code for code, _, _, _ in Lang.get_available()]
