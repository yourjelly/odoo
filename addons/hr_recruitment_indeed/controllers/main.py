...
class HrIndeedRecruitment(http.Controller):
    ...
    def _prepare_indeed_apply_data(self, jobs):
        ...
        indeed_job_url = quote(f"{request.env.user.get_base_url()}/jobs")
        indeed_job_url = indeed_job_url.replace('/', '%2F')
        job_feed_data = []
        for job in jobs:
...
<source>%s</source>"""%(content)
    ...
    def handle_callback(self):
        try:
            ...
            json_data = json.loads(indeed_data)
            _logger.warning(_("Recieved data: %s") % json_data)
