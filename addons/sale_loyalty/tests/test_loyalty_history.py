...
class TestLoyatyhistory(TestSaleCouponCommon):

    def setUp(self):
        super().setUp()
        self.partner_a = self.env['res.partner'].create({'name': 'Jean Jacques'})
        ...
        self.loyalty_card = self.env['loyalty.card'].create({
            'program_id': self.loyalty_program.id, 'partner_id': self.partner_a.id, 'points': 2
        })
    ...
    def test_add_loyalty_history_line_with_reward(self):
        ...
        history_records = len(coupon_applied.history_ids.filtered(lambda history: history.order_id.id == order.id))
    ...
    def test_add_loyalty_history_line_without_reward(self):
        ...
        history_records = self.loyalty_card.history_ids.filtered(lambda history: history.order_id.id == order.id)
        self.assertEqual(history_records.issued, 0.0,
