...
class ProductProduct(models.Model):
    ...
    def action_compute_bom_days(self):
        ...
        for product in self:
            bom_data = self.env['report.mrp.report_bom_structure'].with_context(minimized=True, without_stock=True)._get_bom_data(bom_by_products[product], warehouse)
            availability_delay = bom_data.get('availability_delay')
