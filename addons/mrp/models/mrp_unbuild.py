...
class MrpUnbuild(models.Model):
    ...
    product_id = fields.Many2one(
        ...
        required=True, readonly=[('state', '=', 'done')])
    company_id = fields.Many2one(
        'res.company', 'Company',
        default=lambda s: s.env.company,
        required=True, index=True, readonly=[('state', '=', 'done')])
...
""",
    ...
    location_id = fields.Many2one(
        ...
        required=True, readonly=[('state', '=', 'done')], help="Location where the product you want to unbuild is.")
    location_dest_id = fields.Many2one(
        'stock.location', 'Destination Location',
        domain="[('usage','=','internal'), '|', ('company_id', '=', False), ('company_id', '=', company_id)]",
        check_company=True,
        compute='_compute_location_id', store=True, precompute=True,
        required=True, readonly=[('state', '=', 'done')], help="Location where you want to send the components resulting from the unbuild order.")
