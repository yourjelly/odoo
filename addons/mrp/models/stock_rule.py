    ...
    def _run_manufacture(self, procurements):
        ...
        for company_id, productions_values in productions_values_by_company.items():
            ...
            for production in productions:
                # TODO? Voir si c'est pas mieux de passer la valeur des champs directement dans _prepare_procurement_values
                parent_workorder_id = self.env.context.get('parent_mrp_production_id', False)
                if parent_workorder_id:
                    # If this workorder is generated by an another workorder, we must set its
                    # date_planned_finished equals to its parent date_planned_start.
                    import pdb; pdb.set_trace()
                    parent_workorder = self.env['mrp.production'].browse(parent_workorder_id)
                    if parent_workorder.exists():
                        production.write({
                            'date_deadline': parent_workorder.date_deadline - timedelta(hours=1),
                            'date_start_wo': parent_workorder.date_deadline - timedelta(hours=1),
                            'date_planned_start': parent_workorder.date_planned_start - timedelta(hours=1),
                            'date_planned_finished': parent_workorder.date_deadline,
                        })
