...
class ProcurementGroup(models.Model):
    ...
    @api.model
    def _get_all_manufacturing_rule(self, product_id, location_id, values):
        # We need to find exact manufacturing rules...
        result = False
        location = location_id
        while (not result) and location:
            result = self._search_rule(product_id, values, [('location_id', '=', location.id), ('action', '=', 'manufacture')], limit=None)
...
class ProcurementRule(models.Model):
    ...
    def _run_manufacture(self, product_id, product_qty, product_uom, location_id, name, origin, values):
        ...
        current_rule = self
        bom = current_rule._get_matching_bom(product_id, values)
        if not bom:
            rules = ProcGroup._get_all_manufacturing_rule(product_id, location_id, values) - current_rule
            for rule in rules:
                bom = rule._get_matching_bom(product_id, values)
                if bom:
                    current_rule = rule
                    break;
            if not bom:
        ...
        production = ProductionSudo.create(current_rule._prepare_mo_vals(product_id, product_qty, product_uom, location_id, name, origin, values, bom))
    ...
    def _prepare_mo_vals(self, product_id, product_qty, product_uom, location_id, name, origin, values, bom):
        return {
            ...
            'location_src_id': self.location_src_id.id or self.picking_type_id.location_id.id or location_id.id,
