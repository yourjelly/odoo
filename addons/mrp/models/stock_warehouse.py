...
class StockWarehouse(models.Model):
    ...
    def _create_or_update_manufacturing_picking_types(self):
        ...
        if not self.manu_type_id:
            vals = self._get_picking_type_vals('manu_only', color, location_id, location_dest_id)
            vals.update({'name': 'Manufacture', 'active': self.manufacture_to_resupply})
        ...
        for field_name, values in pycompat.items(picking_types):
    ...
    def _update_name_and_code(self, name=False, code=False):
        ...
        for wh in self:
            if wh.manufacture_pull_id and name:
                wh.manufacture_pull_id.write({'name': wh.manufacture_pull_id.name.replace(wh.name, name, 1)})
        return res
