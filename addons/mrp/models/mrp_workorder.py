...
class MrpWorkorder(models.Model):
    ...
    def _onchange_expected_duration(self):
        self.duration_expected = self._get_duration_expected(self.workcenter_id)
    ...
    def _get_duration_expected(self, workcenter):
        self.ensure_one()
        if not self.workcenter_id or not self.operation_id:
            return False
        ...
        cycle_number = float_round(qty_production / workcenter.capacity, precision_digits=0, rounding_method='UP')
        return workcenter.time_start + workcenter.time_stop + cycle_number * time_cycle * 100.0 / workcenter.time_efficiency
