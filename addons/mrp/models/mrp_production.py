...
class MrpProduction(models.Model):
    ...
    def _compute_components_availability(self):
        ...
        productions = self.filtered(
            lambda mo: mo.state not in ["cancel", "draft", "done"]
        )
        ...
        for production in productions:
            ...
            if any(
                float_compare(
                    ...
                    move.product_qty,
                    move.product_id.uom_id.rounding,
                )
                == -1
            ...
            production.json_popover = json.dumps(
                {
                    ...
                    if production.delay_alert_date
                    else False,
                    "late_elements": [
                        {
                            "id": late_document.id,
    ...
    def _compute_state(self):
        ...
        for production in self:
            if not production.state:
                ...
                and all(move.state == "cancel" for move in production.move_raw_ids)
            ):
    ...
    def _compute_show_lock(self):
        for order in self:
            order.show_lock = (
                self.env.user.has_group("mrp.group_locked_by_default")
    ...
    def _onchange_bom_id(self):
        if not self.product_id and self.bom_id:
            self.product_id = (
                ...
                or self.bom_id.product_tmpl_id.product_variant_ids[0]
            )
                ...
                if move_finished_values.get("byproduct_id") in moves_byproduct_dict:
                    ...
                    list_move_finished += [
                        (
                            1,
                            moves_byproduct_dict[
                                move_finished_values["byproduct_id"]
                            ].id,
    ...
    def write(self, vals):
        ...
        for production in self:
            ...
            if (
                not production.bom_id.operation_ids
    ...
    def action_view_mrp_production_childs(self):
        ...
        if len(mrp_production_ids) == 1:
            action.update(
                {"view_mode": "form", "res_id": mrp_production_ids[0],}
            )
        else:
            action.update(
                {
    ...
    def _post_inventory(self, cancel_backorder=False):
        for order in self:
            ...
            if not finish_moves.quantity_done:
                finish_moves.quantity_done = float_round(
    ...
    def button_mark_done(self):
        ...
        if len(backorders) == 1:
            action.update(
                {"view_mode": "form", "res_id": backorders[0].id,}
            )
        else:
            action.update(
                {
    ...
    def _check_sn_uniqueness(self):
        ...
        for move in self.move_finished_ids:
            ...
            for move_line in move.move_line_ids:
                ...
                co_prod_move_lines = self.move_finished_ids.move_line_ids.filtered(
                ...
                if duplicates:
                    ...
                    if not (
                        duplicates_unbuild and duplicates - duplicates_unbuild == 0
                    ):
