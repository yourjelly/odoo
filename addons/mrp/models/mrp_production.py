...
class MrpProduction(models.Model):
    ...
    def _get_default_is_locked(self):
        return not self.env.user._has_group('mrp.group_unlocked_by_default')
    ...
    def _compute_show_lock(self):
        for order in self:
            order.show_lock = order.state == 'done' or (
                not self.env.user._has_group('mrp.group_unlocked_by_default')
    ...
    def _compute_show_allocation(self):
        ...
        if not self.env.user._has_group('mrp.group_mrp_reception_report'):
            return
        for mo in self:
    ...
    def button_mark_done(self):
        ...
        if not backorders:
            ...
            elif self.env.user._has_group('mrp.group_mrp_reception_report'):
    ...
    def _get_autoprint_done_report_actions(self):
        ...
        for print_format in productions_to_print.picking_type_id.mapped('mrp_product_label_to_print'):
            ...
            if print_format == 'pdf':
                ...
                clean_action(action, self.env)
                report_actions.append(action)
        if self.env.user._has_group('mrp.group_mrp_reception_report'):
            ...
            if reception_labels_to_print:
                ...
                if moves_to_print:
                    ...
                    clean_action(action, self.env)
                    report_actions.append(action)
        if self.env.user._has_group('stock.group_production_lot'):
    ...
    def action_open_label_type(self):
        ...
        if self.env.user._has_group('stock.group_production_lot') and move_line_ids.lot_id:
