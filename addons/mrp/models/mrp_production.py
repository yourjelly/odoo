...
class MrpProduction(models.Model):
    ...
    source_production_id = fields.Many2one('mrp.production', string="Source Production", related="procurement_group_id.source_production_id")
    child_production_count = fields.Integer("Number of Childs Manufacturing", compute='_compute_child_production_count')

    @api.depends('procurement_group_id.target_production_ids')
    def _compute_child_production_count(self):
        for production in self:
            production.child_production_count = len(production.procurement_group_id.target_production_ids)
    ...
    def create(self, values):
        ...
        mo = super(MrpProduction, self).create(values)
        self.env['procurement.group'].browse(values['procurement_group_id']).write({'target_production_ids': [(4, mo.id)]})
        return mo
    ...
    def action_view_child_production(self):
        ...
        return {
            ...
            'domain': [('id', 'in', self.procurement_group_id.target_production_ids.ids)],
            'res_model': 'mrp.production',
            'type': 'ir.actions.act_window',
            'view_mode': 'tree,form',
        }

    def action_view_source_production(self):
        self.ensure_one()
        return {
            'name': _('MO generated by %s' % self.name),
            'domain': [('id', 'in', self.source_production_id)],
            'res_model': 'mrp.production',
            'type': 'ir.actions.act_window',
            'view_mode': 'tree,form',
        }
