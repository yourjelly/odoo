    ...
    def _compute_show_allocation(self):
        ...
        for mo in self:
            if not mo.picking_type_id:
                return
            lines = mo.move_finished_ids.filtered(lambda m: m.product_id.type == 'consu' and m.product_id.is_trackable and m.state != 'cancel')
            if lines:
    ...
    def button_mark_done(self):
        ...
        if not backorders:
            if self.env.context.get('from_workorder'):
                ...
                lines = mos_to_show.move_finished_ids.filtered(lambda m: m.product_id.type == 'consu' and m.product_id.is_trackable and m.state != 'cancel' and m.picked and not m.move_dest_ids)
                if lines:
