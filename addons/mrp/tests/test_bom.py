...
class TestBoM(TestMrpCommon):
    ...
    def test_10_variants(self):
        ...
        test_bom_l1 = self.env["mrp.bom.line"].create(
            {"bom_id": test_bom.id, "product_id": self.product_2.id, "product_qty": 2,}
        )
        ...
        tmp_picking_type = self.env["stock.picking.type"].create(
            {
                ...
                .create({"code": "mrp.production", "name": "tmp_production_sequence",})
                .id,
            }
        )
    ...
    def test_20_bom_report(self):
        ...
        self.assertEqual(
            float_compare(report_values["lines"]["total"], 46.21, precision_digits=2),
            0,
            "Product Bom Price is not correct",
        )
        self.assertEqual(
            float_compare(
                report_values["lines"]["total"] / 11.0, 4.20, precision_digits=2
            ),
            0,
        ...
        operation_cost = float_round(
            10 / 60 * 10, precision_digits=2
        ) * 2 + float_round(6 / 60 * 10, precision_digits=2)
        # Both needs 2 operation cycle
        self.assertEqual(
            report_values_12["lines"]["operations_cost"],
            report_values_22["lines"]["operations_cost"],
        )
        self.assertEqual(report_values_22["lines"]["operations_cost"], operation_cost)
        report_values_23 = self.env["report.mrp.report_bom_structure"]._get_report_data(
            bom_id=bom_crumble.id, searchQty=23, searchVariant=False
        )
        operation_cost = float_round(
            15 / 60 * 10, precision_digits=2
        ) * 2 + float_round(9 / 60 * 10, precision_digits=2)
        ...
        # Operation cost is the sum of operation line.
        operation_cost = float_round(10 / 60 * 10, precision_digits=2) + float_round(
            30 / 60 * 20, precision_digits=2
        )
        self.assertEqual(
            float_compare(
                report_values["lines"]["operations_cost"],
    ...
    def test_21_bom_report_variant(self):
        ...
        self.car = self.env["product.template"].create({"name": "Car",})
    ...
    def test_replenishment(self):
        ...
        manufacturing_route = self.env["stock.location.route"].search(
