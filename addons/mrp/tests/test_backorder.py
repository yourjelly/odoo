...
class TestMrpWorkorderBackorder(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.finished1 = cls.env["product.product"].create(
            {"name": "finished1", "type": "product",}
        )
        cls.compfinished1 = cls.env["product.product"].create(
            {"name": "compfinished1", "type": "product",}
        )
        cls.compfinished2 = cls.env["product.product"].create(
            {"name": "compfinished2", "type": "product",}
        )
        cls.workcenter1 = cls.env["mrp.workcenter"].create({"name": "workcenter1",})
        cls.workcenter2 = cls.env["mrp.workcenter"].create({"name": "workcenter2",})
