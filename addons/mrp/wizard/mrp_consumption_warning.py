...
class MrpConsumptionWarning(models.TransientModel):
    ...
    mrp_production_issue_count = fields.Integer(compute="_compute_mrp_production_issue_count")
    mrp_consumption_warning_line_ids = fields.One2many('mrp.consumption.warning.line', 'mrp_consumption_warning_id')

    @api.depends("mrp_consumption_warning_line_ids.mrp_production_id")
    def _compute_mrp_production_issue_count(self):
        for wizard in self:
            wizard.mrp_production_issue_count = len(wizard.mrp_consumption_warning_line_ids.mrp_production_id)
