...
class ReportMoOverview(models.AbstractModel):
    ...
    def _get_mo_summary(self, production, components, current_mo_cost, current_real_cost, remaining_cost_share):
        ...
        return {
            ...
            'quantity_free': product.uom_id._compute_quantity(max(product.free_qty, 0), production.product_uom_id) if product.is_trackable else False,
            'quantity_on_hand': product.uom_id._compute_quantity(product.qty_available, production.product_uom_id) if product.is_trackable else False,
    ...
    def _format_component_move(self, production, move_raw, replenishments, replenish_data, level, index):
        ...
        component = {
            ...
            'quantity_free': product.uom_id._compute_quantity(max(product.free_qty, 0), move_raw.product_uom) if product.is_trackable else False,
            'quantity_on_hand': product.uom_id._compute_quantity(product.qty_available, move_raw.product_uom) if product.is_trackable else False,
    ...
    def _get_component_receipt(self, product, move, warehouse, replenishments, replenish_data):
        ...
        if not product.is_trackable or move.state == 'done':
    ...
    def _get_replenishment_lines(self, production, move_raw, replenish_data, level, current_index):
        ...
        if product.is_trackable and production.state not in ('done', 'cancel')\
