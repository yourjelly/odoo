...
class ReportBomStructure(models.AbstractModel):
    ...
    def _compute_current_production_capacity(self, bom_data):
        ...
        for comp in bom_data.get('components', []):
            if not comp['product'].is_trackable or float_is_zero(comp['base_bom_line_qty'], precision_digits=comp['uom'].rounding):
                continue
    ...
    def _get_components_closest_forecasted(self, lines, line_quantities, parent_bom, product_info, parent_product, ignore_stock=False):
        ...
        for line in lines:
            ...
            if (not product.is_trackable or
    ...
    def _get_quantities_info(self, product, bom_uom, product_info, parent_bom=False, parent_product=False):
        quantities_info = {
            'free_qty': max(product.uom_id._compute_quantity(product.free_qty, bom_uom), 0) if product.is_trackable else 0,
            'on_hand_qty': product.uom_id._compute_quantity(product.qty_available, bom_uom) if product.is_trackable else 0,
            'stock_loc': 'in_stock',
        }
