...
class TestConvertBack(common.TransactionCase):
    ...
    def field_rountrip_result(self, field, value, expected, custom_type=False):
        ...
        type = 'data-oe-field' if custom_type else 'data-oe-type'
        model = 'ir.qweb.field.' + element.get(type, '')
    ...
    def field_roundtrip(self, field, value, expected=False, custom_type=False):
        expected = expected if expected else value
        self.field_rountrip_result(field, value, expected, custom_type)
    ...
    def test_float_time(self):
        self.field_roundtrip('float_time', '42:0', 42.0, True)
