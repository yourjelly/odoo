    ...
    return _.defaults({}, data, {
            ...
            render_template: function (args) {
                if (args[0] === 'web_editor.snippets') {
                    return '<h2 id="snippets_menu">Add blocks</h2>' +
                        ...
                        '    </div>' +
                        // '    <div data-js="background_position" data-selector=".test_option_all">' +
        ...
        'ir.attachment': {
            fields: {
                ...
                datas_fname: { string: "datas_fname", type: 'char' },
                mimetype: { string: "mimetype", type: 'char' },
                checksum: { string: "checksum", type: 'char' },
                url: { string: "url", type: 'char' },
                type: { string: "type", type: 'char' },
                res_id: { string: "res_id", type: 'integer' },
                res_model: { string: "res_model", type: 'char' },
...
var keydown = function (key, $editable, options) {
    ...
    if (typeof key === 'string') {
        keyPress.key = key;
        keyPress.keyCode = +_.findKey(keyboardMap, function (k) {return k === key;});
    } else {
