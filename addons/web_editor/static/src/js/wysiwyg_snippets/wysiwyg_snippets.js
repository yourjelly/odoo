...
Wysiwyg.include({
     ...
     **/
    destroy: function () {
        // Necessary for operations on $editable:
        // if (this._summernote) {
        //     var $editable = $(this._summernote.layoutInfo.editable);
        // }
        this._super();
    ...
    isEditableNode: function (node) {
        ...
        if (this.snippets && node.tagName === 'DIV' && !node.contentEditable) {
            ...
            if (
                !_.find(node.childNodes, function (node) {
...
$.fn.extend({
    ...
    selectContent: function () {
        if (this.length) {
            ...
            if (
                !range.sc.tagName && range.so === 0 &&
                range.sc.textContent[range.so] === '\u200B'
            ) {
                range.so += 1;
            }
            if (
                !range.ec.tagName && range.eo === range.ec.textContent.length &&
                range.ec.textContent[range.eo - 1] === '\u200B'
            ) {
                range.eo -= 1;
            }
            range.select();
