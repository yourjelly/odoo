...
var FileWidget = SearchableMediaWidget.extend({
    ...
    _save: async function () {
        ...
        if (img.autoOptimized) {
            const removeIfUnused = JSON.parse(media.dataset.removeIfUnused || '[]');
            media.dataset.removeIfUnused = JSON.stringify(removeIfUnused.concat([img.id]));
        }
        media.dataset.currentlyUsed = img.id;
