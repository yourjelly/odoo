...
var HelperPlugin = AbstractPlugin.extend({
    ...
    deleteBetween: function (pointA, pointB) {
        ...
        var point = {
            node: this.firstLeaf(next),
            offset: 0
        };
        if (nodes.length > 1 || nodes.length && !dom.isText(nodes[0])) {
            point = this.removeEmptyInlineNodes({
                node: this.firstLeaf(next),
                offset: 0
            });
        }
    ...
    insertTextInline: function (text) {
        ...
        // If selection on single BR: add a zero-width char after it before removing it
        var selected = this.getSelectedNodes();
        var zwChar = this.document.createTextNode('\u200B');
        if (
            !range.isCollapsed() &&
            selected.length === 1 && selected[0].innerHTML &&
            this.getRegex('emptyElemWithBR').test(selected[0].innerHTML)
        ) {
            selected[0].appendChild(zwChar);
        }
        range = range.deleteContents();
        zwChar.textContent = "";

        // If range is text: split the text at offset
        if (dom.isText(range.sc)) {
        ...
        // If range is on an unbreakable
        if (this.options.isUnbreakableNode(range.sc)) {
            // If has child: move to that child
            if (range.sc.firstElementChild) {
                range.sc = range.ec = range.sc.firstElementChild;
                range.so = range.eo = 0;
            } else {
                // Otherwise: append a P and set range on it
                var blankP = this.document.createElement('p');
                $(range.sc).prepend(blankP);
                range.sc = range.ec = blankP;
                range.so = range.eo = 0;
            }
        }
        // If range on element
        if (range.sc.tagName) {
            // If void element: insert before
            if (dom.isVoid(range.sc)) {
                $(range.sc).before(textNode);
            } else {
                // If non-void element: append text node to it
                $(range.sc).append(textNode);
            }
        } else if (!range.sc.parentNode || this.options.isUnbreakableNode(range.sc.parentNode)) {
            // If range on text and is child of unbrekable or orphan: wrap in a p
            var wrapperP = this.document.createElement('p');
            wrapperP.appendChild(textNode);
            $(range.sc).after(wrapperP);
        } else {
            // In any other case: just insert the text node after range
