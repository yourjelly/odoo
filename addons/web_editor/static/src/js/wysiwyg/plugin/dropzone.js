...
var DropzonePlugin = Plugins.dropzone.extend({
    ...
    _onDrop: function (e) {
        ...
        if (!this._canDropHere(dataTransfer)) {
            console.warn('Cannot drop files here.');
            return;
        }

        if (dataTransfer.getData('text/html')) {
            return this._dropHTML(dataTransfer.getData('text/html'));
            ...
            if (!images.length || images.length < dataTransfer.files.length) {
                console.warn('Images are the only file types that can be dropped.');
            }
            if (images.length) {
                return this._dropImages(images);
            }
        }
    },
    /**
