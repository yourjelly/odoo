...
Wysiwyg.include({
    ...
    willStart: function () {
        ...
        return this.defAsset
            ...
            .then(this._super.bind(this)).then(function () {
                var _summernoteMaster = $.summernote;
                var _summernoteSlave = self.$iframe[0].contentWindow._summernoteSlave;
                _summernoteSlave.options = _.extend({}, _summernoteMaster.options, {
                    modules: _summernoteSlave.options.modules,
                });
                self._enableBootstrapInIframe();
            });
    },
    /**
     * @override
    ...
    _loadIframe: function () {
        ...
        this.$iframe.on('load', function onLoad(ev) {
            ...
            self.defAsset.then(function (asset) {
                if (_avoidDoubleLoad !== avoidDoubleLoad) {
