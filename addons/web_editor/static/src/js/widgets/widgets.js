...
var ColorPickerWidget = Widget.extend({
    ...
    _updateUI: function () {
        ...
        var top = (100 - this.colors.lightness) * 2;
        var left = this.colors.saturation * 2;
    ...
    _onMouseMovePicker: function (ev) {
        ...
        var $container = this.$('.o_color_pick_area');
        var top = Math.round(ev.clientY - $container.offset().top);
        var left = Math.round(ev.clientX - $container.offset().left);
        var saturation = Math.round(left/2);
        var lightness = Math.round((200 - top)/2);
