...
function getCSSRules(doc) {
    ...
    const agg = [];
    const other = [...cssRules];
    while (other.length) {
        const rule = other.shift();
        let adaptedRule = {...rule};
        for (const otherRule of other) {
            if (otherRule !== rule && rule.specificity === otherRule.specificity && Object.keys(rule.style).length === Object.keys(otherRule.style).length && Object.keys(rule.style).every(key => key in otherRule.style && rule.style[key] === otherRule.style[key])) {
                if (rule.selector !== otherRule.selector) {
                    adaptedRule.selector = `${adaptedRule.selector},${otherRule.selector}`;
                }
                other.splice(other.indexOf(otherRule), 1);
            }
        }
        agg.push(adaptedRule);
    }
    agg.sort((a, b) => a.specificity - b.specificity);
    console.timeEnd('getCSSRules');
    return agg;
}
