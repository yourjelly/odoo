...
export class HtmlField extends Component {
        ...
        useEffect(() => {
            (async () => {
                ...
                if (this.props.readonly) {
                    ...
                    if (codeViewEl) {
                        codeViewEl.value = this.htmlValue;
                    }
                }
            })();
        });
    ...
    get htmlValue () {
        return this._textToHtml(String(this.props.value || ''));
    }
    get markupValue () {
        return markup(this.htmlValue);
    }
    get showIframe () {
    ...
    _isDirty() {
        return !this.props.readonly && this.htmlValue !== this.getEditingValue();
    }
    _getCodeViewEl() {
                ...
                cwindow.document
                    ...
                    .write(
                        ...
                        '<head>' +
                            ...
                            asset.cssLibs.map((cssLib) => {
                                return '<link type="text/css" rel="stylesheet" href="' + cssLib + '"/>';
                            }).join('\n') + '\n' +
                            asset.cssContents.map((cssContent) => {
