...
var summernote = require('summernote/summernote'); // wait that summernote is loaded
var _t = core._t;
...
var editor = $.summernote.options.modules.editor;
...
editor.insertTable = function (event, editor, layoutInfo, sDim) {
...
editor.tab = function (event, editor, layoutInfo, outdent) {
    var $editable = layoutInfo.editable();
...
editor.untab = function (event, editor, layoutInfo) {
    return $.summernote.pluginEvents.tab(event, editor, layoutInfo, true);
};
editor.up = function (event, editor, layoutInfo) {
    var r = range.create();
...
editor.down = function (event, editor, layoutInfo) {
    var r = range.create();
    var node = dom.firstChild(r.sc.childNodes[r.so] || r.sc);
...
editor.enter = function (event, editor, layoutInfo) {
    var $editable = layoutInfo.editable();
...
editor.visible = function (event, editor, layoutInfo) {
    var $editable = layoutInfo.editable();
...
editor.delete = function (event, editor, layoutInfo) {
    var $editable = layoutInfo.editable();
...
editor.undo = function ($popover) {
    if(!$popover.attr('disabled')) $popover.data('NoteHistory').undo();
};
editor.redo = function ($popover) {
...
editor.currentStyle = function (target) {
