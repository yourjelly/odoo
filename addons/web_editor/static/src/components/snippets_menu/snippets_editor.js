...
export class SnippetEditor extends Component {
    ...
    getOptionTarget(option) {
        ...
        //if (!matched && typeof option.selector === "function") {
        //    matched = option.selector.is(this.props.target);
        //}
        //if (!matched && typeof option.selector === "string") {
        //    matched = $target.is(option.selector);
        //}
        //if (typeof option.target === "function") {
        //    target = option.target(this.props.target);
        //    if (!target) {
        //        matched = false;
        //    }
        //}
        //if (typeof option.target === "string") {
        //    $target = $target.find(option.target);
        //    if (!$target.length) {
        //        matched = false;
        //    }
        //}
        ...
        //    matched = matched && !option.exclude(this.props.target);
        //}
        //if (typeof option.exclude === "string") {
        //    matched = matched && !$target.is(option.exclude);
        //}
    ...
    initializeOptions() {
        ...
        for (const [optionName, optionDef] of registry.category("snippets_options").getEntries()) {
            ...
            if (target && optionDef.template) {
                ...
                const option = {
                    ...
                    props: {
                        ...
                        // Pass the original element so that options can hook to it.
                        target: target,
                        updateOverlay: () => {
                            this.cover();
                        },
     ...
     * @param {boolean} [show]
     */
    _toggleVisibilityStatus(show) {
        if (show === undefined) {
            show = !this.isTargetVisible();
        }
        if (show) {
            delete this.props.target.dataset.invisible;
        } else {
            this.props.target.dataset.invisible = '1';
        }
        return show;
    }
    /**
     * Lock the target behind the mutex, this ensures that only
     * one action uses the target at a time. And that it is almost
     * always in a correct state.
     * @param {Function} callback
     * @param {HTMLElement} target
     * @param {boolean} [ignoreMutex=false] - bypass the mutex
     * @returns {*}
     */
    useTarget(callback, target, ignoreMutex = false) {
        if (ignoreMutex) {
            return callback(target);
        }
        return this.props.snippetEditionRequest(() => callback(target));
    }
    /**
     *
     * @param name
    ...
    onDragAndDropStart() {
        ...
        const closestFormEl = this.props.target.closest('form');
        ...
        if (this.props.target.classList.contains('s_table_of_content')) {
        ...
        // such cases.
        if (this.props.target.classList.contains('s_website_form_field')) {
    ...
    onDragAndDropEnd({ x, y }, dropzone) {
        ...
        if (!this.dragState.dropped) {
            ...
            const { nearest } = this.props.target.ownerDocument.defaultView.$;
            let $el = nearest({x, y}, '.oe_drop_zone:not(.disabled)', {container: body}).first();
            if ($el.length) {
                $el.after($target);
    ...
    async removeSnippet() {
        ...
        await this.props.renderChildSnippets(this.props.target);
        ...
        await this.env.snippetEditionRequest(async () => {
            await Promise.all(this.options.map(opt => opt.props.events.onRemove()));
