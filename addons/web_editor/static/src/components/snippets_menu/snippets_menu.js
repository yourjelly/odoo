...
import { callForEachChildSnippet } from "@web_editor/utils/hooks";
...
export class SnippetsMenu extends Component {
    ...
    static components = { SnippetEditor }
    ...
    setup() {
        this.$body = $(this.props.document.body);
        ...
        useEffect(
            (editable) => {
                ...
                // TODO: find a native way to do the line below
        ...
        useEffect(
            (saving) => {
                if (saving) {
                    this.env.mutex.getUnlockedDef().then(async () => {
                        // Editors did their cleanForSave, we can now destroy them
    ...
    async cleanForSave() {
        ...
        // Wait for the SnippetsEditors cleanForSave, the call is handled by the
        // wysiwyg.
     ...
     * @param target
     */
    createSnippetEditorProps(target) {
        if (!target) {
            return;
        }
        ...
        if (existingEditor) {
            existingEditor.showOptions = true;
    ...
    renderChildSnippets(snippet) {
        ...
        const proms = Array.from($snippet.add(globalSelector.all($snippet))).map((target) => {
            const prop = this.createSnippetEditorProps(target);
    ...
    computeSnippetTemplates(snippetsHTML) {
        ...
        for (const panel of panels) {
            ...
            const section = { id: "", name: "", header: "", snippets: [] };
            ...
            this.snippetSections.push(section);
        }
        ...
        for (const option of snippetOptionsRegistry.getAll()) {
            ...
            Object.assign(option, {
                ...
                "drop-in":
                    option.dropIn &&
                    this.computeSelectorFunctions(option.dropIn, "", false, noCheck),
            });
        ...
        snippetsHTML
            .querySelectorAll(".o_not_editable")
    ...
    getDragAndDropOptions(){
        return {
            ...
            onDragStart: ({ element }) => {
                ...
                element.querySelector(".oe_snippet_thumbnail").classList.add("o_we_already_dragging");
                ...
                // grep: NO_DOUBLE_TOC
                if (baseBody.classList.contains("s_table_of_content")) {
                    $selectorChildren = $selectorChildren.filter(
                        (i, el) => !el.closest(".s_table_of_content")
                    );
                }
            ...
            onDragEnd: ({ x, y }) => {
                ...
                if (
                    ...
                    const $el = doc.defaultView.$.nearest(point, selector, container).first();
                    if ($el.length) {
