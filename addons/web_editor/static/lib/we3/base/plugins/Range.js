...
var BaseRange = class extends we3.AbstractPlugin {
     ...
     * @param {Boolean} [options.moveLeft]
     * @param {Boolean} [options.moveRight]
     */
    _computeSetRange (points, options) {
        ...
        points = this._moveToAfterFirstBR(points);

        this._didRangeChange = this._willRangeChange(points);
        var focusedNodeID = this._willFocusNodeChange(points);
        ...
        if (this._focusedNodeID !== focusedNodeID) {
            this._isChangeFocusNode = true;
            this._focusedNodeID = focusedNodeID;
        } else {
            this._isChangeFocusNode = false;
        }
    ...
    _moveToAfterFirstBR (points) {
        ...
        if (!archNode.isBR() || !prev || prev.isBR() || !next || !next.isBR()) {
            return points;
        }
        return {
            scID: prev.id,
    ...
    _setRange (range) {
        ...
        var isChangeFocusNode = this._isChangeFocusNode;
    ...
    /**
     * Return the focus node id if will change once set to the given points.
     *
     * @private
     * @param {Object} points
     * @param {Number} points.scID
