...
we3.ArchNode = class {
     ...
     * @param {int} offset
     * @param {object} [options]
     * @param {boolean} [options.removeEmpty] true to remove generated empty nodes
     * @returns {ArchNode|undefined}
     */
    split (offset, options) {
        ...
        this.after(archNode);
        if (options && options.removeEmpty && !this.isText()) {
    ...
    unwrap () {
        ...
        // clean up: remove generated empty nodes
        start.removeIfEmpty()
        var prev = this.previousSibling();
        if (prev) {
            prev.removeIfEmpty();
        }
        // clean up: delete new edges if needed
        ...
        var first = this.firstLeaf();
        var last = this.lastLeaf();
        first.deleteEdge(true, deleteEdgeOptions);
