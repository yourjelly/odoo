    ...
    static _createTestingVirtualNode (archNode) {
        ...
        if (matches) {
            ...
            matches.forEach(function (match) {
                if (match === regex.rangeCollapsedMarker) {
                    fragment.append(new TestNode(archNode.params, null, null, regex.rangeStartMarker));
                    fragment.append(new TestNode(archNode.params, null, null, regex.rangeEndMarker));
                } else if (match && match.length) {
                    if (match === regex.rangeStartMarker || match === regex.rangeEndMarker) {
