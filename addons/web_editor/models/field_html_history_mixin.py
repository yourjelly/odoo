...
class HtmlHistoryMixin(models.AbstractModel):
# class HtmlHistoryMixin(models.Model):
    _name = "field.html.history.mixin"
    _description = "Field html History"

    history_diff = fields.One2many('field.html.history.diff', 'res_id', "History diffs", auto_join=True)
        ...
        if history_field_name and history_field_name in vals:
            ...
            if self.id and isinstance(new_body_str, str):
                ...
                if isinstance(old_body_str, str) and new_body_str != old_body_str:
                    ...
                    self.env['field.html.history.diff'].create({
    ...
    def unlink(self):
        ...
        self.env['field.html.history.diff'].search([('res_id', 'in', self.ids)]).unlink()
        return super().unlink()
    ...
    def action_related_history(self):
        ...
        return {
            ...
            'domain': [('id', 'in', self.history_diff.ids)]
        }
    # ------------------------------------------------------------
    # HISTORY Restoring
    # ------------------------------------------------------------
