...
class TestSMSComposerComment(SMSCommon, TestSMSRecipients):
    ...
    def test_composer_comment_field_w_numbers(self):
        with self.with_user('employee'):
            composer = self.env['sms.composer'].with_context(
                active_model='mail.test.sms', active_id=self.test_record.id,
                default_number_field_name='phone_nbr',
            ).create({
                'body': self._test_body,
                'numbers': ','.join(self.random_numbers),
            })
        ...
        self.assertSMSNotification([
            {'partner': self.test_record.customer_id, 'number': self.test_record.phone_nbr},
    ...
    def test_composer_comment_field_w_template(self):
        ...
        self.assertSMSNotification([{'partner': self.test_record.customer_id, 'number': self.test_record.phone_nbr}], 'Dear %s this is an SMS.' % self.test_record.display_name, messages)
