...
class Employee(models.Model):
    ...
    def _create_future_public_holidays_timesheets(self, employees):
        ...
        global_leaves_wo_calendar = self.env['resource.calendar.leaves'].search([('calendar_ids', '=', False), ('date_from', '>=', today)])
        for employee in employees:
            if not employee.active:
                continue
            # First we look for the global time off that are already planned after today
            global_leaves = employee.resource_calendar_id.leave_ids.filtered(lambda l: l.date_from >= today) + global_leaves_wo_calendar
