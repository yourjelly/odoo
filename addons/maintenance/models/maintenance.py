...
class MaintenanceTeam(models.Model):
    ...
    def _compute_todo_requests(self):
        for team in self:
            ...
            team.todo_request_count_date = len(
                team.todo_request_ids.filtered(lambda e: e.schedule_date != False)
            )
            team.todo_request_count_high_priority = len(
                team.todo_request_ids.filtered(lambda e: e.priority == "3")
            )
            team.todo_request_count_block = len(
                team.todo_request_ids.filtered(lambda e: e.kanban_state == "blocked")
            )
