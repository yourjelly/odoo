...
class Repair(models.Model):
    ...
    def _create_invoices(self, group=False):
        ...
        for repair in repairs:
            ...
            if not journal:
                raise UserError(
                    _('Please define an accounting sales journal for the %s company.') % self.company_id.id)
            ...
            for operation in repair.operations.filtered(lambda op: op.type == 'add'):
                ...
                invoice_vals['line_ids'].append((0, 0, invoice_line_vals))
        ...
        self.env['account.move'].with_context(type='out_invoice', assist_move_creation=True).create(invoices_vals_list)
