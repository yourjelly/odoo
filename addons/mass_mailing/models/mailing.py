...
class MassMailing(models.Model):
    ...
    filter_id = fields.Many2one('mailing.saved.filters', string='Filters', tracking=True)
    is_favorite = fields.Boolean(store=True, readonly=False, compute='_compute_is_favorite')
    ...
    def _compute_is_favorite(self):
        for record in self:
            record.is_favorite = (record.mailing_domain == record.filter_id.mailing_domain)
