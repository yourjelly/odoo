...
class TestMassMailing(models.TransientModel):
    ...
    def send_mail_test(self):
        ...
        if record:
            ...
            body = markupsafe.Markup(mailing._render_field('body_html', record.ids, post_process=True)[record.id])
            preview = mailing._render_field('preview', record.ids, post_process=True)[record.id]
            full_body = tools.prepend_preview(body, preview)
            subject = mailing._render_field('subject', record.ids)[record.id]
        else:
            full_body = tools.prepend_preview(mailing.body_html, mailing.preview)
