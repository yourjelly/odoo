...
class TestUTMConsistencyMassMailing(TestUTMCommon, MassMailCommon):
    ...
    def test_utm_consistency(self):
        ...
        utm_source_mailing = self.env.ref('utm.utm_source_mailing')
        self.assertEqual(mailing.source_id, utm_source_mailing)

        # the medium is automatically assigned to the global record "Email"
        utm_source_mailing = self.env.ref('utm.utm_medium_email')
        self.assertEqual(mailing.medium_id, utm_source_mailing)
