...
class TestMailingContactToList(MassMailCommon):
    ...
    def test_mailing_contact_to_list(self):
        contacts = self.env["mailing.contact"].create(
            [
                {"name": "Contact %02d", "email": "contact_%02d@test.example.com",}
                for x in range(30)
            ]
        )
        ...
        mailing = self.env["mailing.list"].create({"name": "Contacts Agregator",})
        ...
        mailing2 = self.env["mailing.list"].create({"name": "Contacts Sublimator",})
                ...
                [
                    {
                        ...
                        "subscription_list_ids": [
                            (
                                0,
                                0,
                                {"list_id": self.mailing_list_1.id, "opt_out": True,},
                            ),
                            (
                                0,
                                0,
                                {"list_id": self.mailing_list_2.id, "opt_out": True,},
                            ),
                        ],
                    }
                    for x in range(0, 5)
