odoo.define("mass_mailing.unsubscribe", function (require) {
  ...
  session.load_translations().then(function () {
    if (email != "" && email != undefined) {
      ajax
        .jsonRpc("/mailing/blacklist/check", "call", {
          email: email,
  ...
  $("#unsubscribe_form").on("submit", function (e) {
    ...
    ajax
      .jsonRpc("/mail/mailing/unsubscribe", "call", {
      ...
      .then(function (result) {
        if (result == "unauthorized") {
          ...
          $("#subscription_info").html(
            _t("An error occurred. Your changes have not been saved, try again later.")
          );
  ...
  $("#button_add_blacklist").click(function (e) {
    ...
    ajax
      .jsonRpc("/mailing/blacklist/add", "call", {
        email: email,
        mailing_id: mailing_id,
        res_id: res_id,
        token: token,
  ...
  $("#button_remove_blacklist").click(function (e) {
    ...
    ajax
      .jsonRpc("/mailing/blacklist/remove", "call", {
        email: email,
        mailing_id: mailing_id,
        res_id: res_id,
        token: token,
