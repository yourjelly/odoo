...
class TestEmployee(TestHrCommon):
    ...
    def test_hierarchy_read(self):
        ...
        self.assertDictEqual(result[0], {'id': self.employee_paul.id, 'parent_id': False, '__child_ids__': [self.employee_georges.id, self.employee_pierre.id]})

        result = HrEmployee.hierarchy_read([('id', '=', self.employee_georges.id)], ['id'], 'parent_id')
        self.assertEqual(len(result), 3 + employee_count)
        for emp in employees:
            emp_dict = {'id': emp.id, 'parent_id': emp.parent_id.id and (emp.parent_id.id, emp.parent_id.display_name)}
        ...
        result = HrEmployee.hierarchy_read([('id', '=', self.employee_georges.id)], ['id'], 'parent_id')
        self.assertEqual(len(result), 2 + employee_count)
        self.assertIn({
            'id': self.employee_georges.id,
            'parent_id': self.employee_georges.parent_id.id and (self.employee_georges.parent_id.id, self.employee_georges.parent_id.display_name),
            '__child_ids__': self.employee_pierre.ids,
            '__focus__': True,
        }, result)
        self.assertIn({
            'id': self.employee_paul.id,
            'parent_id': self.employee_paul.parent_id.id and (self.employee_paul.parent_id.id, self.employee_paul.parent_id.display_name),
        }, result)
