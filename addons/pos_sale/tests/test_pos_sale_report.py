...
class TestPoSSaleReport(TestPoSCommon):
    ...
    def test_weight_and_volume_product_variant(self):
        ...
        self.env['sale.order'].sync_from_ui([order])
    ...
    def test_different_shipping_address(self):
        ...
        order_ids = self.env['sale.order'].sync_from_ui([data])

        move_id = self.env['account.move'].browse(order_ids['sale.order'][0]['account_move'])
