...
class PortalShare(models.TransientModel):
    ...
    def action_send_mail(self):
        ...
        if (
            hasattr(active_record, "access_token")
            and active_record.access_token
            or not signup_enabled
        ):
        ...
        for partner in self.partner_ids:
            share_link = active_record.get_base_url() + active_record._get_share_url(
        ...
        for partner in self.partner_ids - partner_ids:
            ...
            share_link = partner._get_signup_url_for_action(
                action="/mail/view", res_id=self.res_id, model=self.model
            )[partner.id]
            saved_lang = self.env.lang
            self = self.with_context(lang=partner.lang)
            template = self.env.ref("portal.portal_share_template", False)
