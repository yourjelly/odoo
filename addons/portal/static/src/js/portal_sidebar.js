odoo.define('portal.portal_sidebar', function (require) {
'use strict';
...
var NavigationSpyMenu = Widget.extend({
    ...
    generateMenu: function () {
        var self = this;
        // reset ids
        $("[id^=portal_header_], [id^=portal_]", this.spy_watched).attr("id", "");
        ...
        _.each(this.spy_watched.find("h1, h2"), function (el) {
            ...
            switch (el.tagName.toLowerCase()) {
                case "h1":
                    id = self.setElementId('portal_header_', el);
                    ...
                    break;
                case "h2":
                    id = self.setElementId('portal_', el);
                    text = self.extractText($(el));
                    if (!text) {
                        break;
                    }
...
nav_menu.setElement($('[data-id="portal_sidebar"]'));
