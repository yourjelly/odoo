...
class PortalMixin(models.AbstractModel):
    ...
    def _portal_ensure_token(self):
        ...
        if hasattr(self, 'is_public') and not self.is_public:
            self.sudo().access_token = False
    ...
    def _check_token(self, token):
        """Checks the access token on a given record and returns True if it's valid.

        :param token: (str) The token to check
        :return: (ool, bool) True if the token is valid
        """
        self.ensure_one()
        record_sudo = self.sudo()
        return token and record_sudo.access_token and consteq(record_sudo.access_token, token)
