...
class ResPartner(models.Model):
    ...
    def _assert_user_email_uniqueness(self):
        ...
        if not email:
            raise UserError(_('Email %(email)s of contact %(partner_name)s is invalid.', email, self.name))
        ...
        if user:
            raise UserError(_('Email %(email)s is already used by an existing user.', user.email))
    ...
    def _portal_send_user_invitation_email(self, welcome_message=False):
        ...
        portal_url = self.with_context(signup_force_type_in_url='', lang=self.lang)._get_signup_url_for_action()[self.id]
        self.signup_prepare()
    ...
    def action_grant_portal_access(self, welcome_message=False):
        ...
        self.with_context(active_test=True)._portal_send_user_invitation_email(welcome_message)
