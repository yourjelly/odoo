...
will update the cost of every lot/serial number in stock."),
    ...
    def _prepare_out_svl_vals(self, quantity, company, lot_id=False):
        ...
        fifo_vals = self._run_fifo(abs(quantity), company, lot_id=lot_id)
    ...
    def _get_fifo_candidates_domain(self, company, lot_id=False):
        return [
            ("product_id", "=", self.id),
            ("remaining_qty", ">", 0),
    ...
    def _get_fifo_candidates(self, company, lot_id=False):
        candidates_domain = self._get_fifo_candidates_domain(company, lot_id=lot_id)
        return self.env["stock.valuation.layer"].sudo().search(candidates_domain)

    def _run_fifo(self, quantity, company, lot_id=False):
        self.ensure_one()

        # Find back incoming stock valuation layers (called candidates here) to value `quantity`.
        qty_to_take_on_candidates = quantity
        candidates = self._get_fifo_candidates(company, lot_id=lot_id)
            ...
            if product.lot_valuated:
                if float_compare(product.quantity_svl, 0, precision_rounding=product.uom_id.rounding) > 0:
                    for lot in product.stock_valuation_layer_ids.filtered(lambda l: l.remaining_qty).lot_id:
                        svsl_vals = product._prepare_out_svl_vals(lot.quantity_svl, self.env.company, lot_id=lot)
                    ...
                    for lot in product.stock_valuation_layer_ids.filtered(lambda l: l.remaining_qty).lot_id:
                        svsl_vals = product._prepare_in_svl_vals(abs(lot.quantity_svl), lot.value_svl / lot.quantity_svl, lot_id=lot)
                        svsl_vals['description'] = description + svsl_vals.pop('rounding_adjustment', '')
                        svsl_vals['company_id'] = self.env.company.id
                        empty_stock_svl_list.append(svsl_vals)
            else:
        ...
        for product in self:
            ...
            for lot, qty in lot_by_product[product].items():
                if float_compare(quantity_svl, 0, precision_rounding=rounding) > 0:
                    ...
                    svl_vals = product._prepare_in_svl_vals(qty_to_remove, price_unit, lot_id=lot)

                else:
                    svl_vals = product._prepare_out_svl_vals(abs(quantity_svl), self.env.company, lot_id=lot)
