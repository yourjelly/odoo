...
class ValuationReconciliationTestCommon(AccountTestInvoicingCommon):
    ...
    def _change_pickings_date(self, pickings, date):
        pickings.mapped('move_ids').write({'date': date})
        pickings.mapped('move_ids.account_move_ids').write({'name': '/', 'state': 'draft'})
        pickings.mapped('move_ids.account_move_ids').write({'date': date})
