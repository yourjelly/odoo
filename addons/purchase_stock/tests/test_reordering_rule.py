...
class TestReorderingRule(TransactionCase):
    ...
    def test_procure_not_default_partner(self):
        ...
        default_vendor = self.env["res.partner"].create({"name": "Supplier A",})
        secondary_vendor = self.env["res.partner"].create({"name": "Supplier B",})
        ...
        product = self.env["product.product"].create(
            {
                ...
                "uom_id": uom_unit.id,
                "default_code": "A",
                "route_ids": [(6, 0, [route_mto.id, purchase_route.id,])],
            }
        )
