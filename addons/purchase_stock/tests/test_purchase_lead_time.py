...
class TestPurchaseLeadTime(PurchaseTestCommon):
    def test_00_product_company_level_delays(self):
        ...
        schedule_date = datetime.combine(
            order_date + timedelta(days=self.product_1.seller_ids.delay), time.min
        ).replace(tzinfo=None, hour=12)
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            schedule_date + timedelta(days=company.po_lead),
            "Deadline date of should be equal to: schedule date of purchase order + lead_po.",
        )
    ...
    def test_01_product_level_delay(self):
        ...
        # Make procurement request from product_1's form view, create procurement and check it's state
        date_planned1 = fields.Datetime.to_string(
            fields.datetime.now() + timedelta(days=10)
        )
        ...
        date_planned2 = fields.Datetime.to_string(
            fields.datetime.now() + timedelta(days=10)
        )
        ...
        schedule_date_2 = datetime.combine(
            order_date + timedelta(days=self.product_2.seller_ids.delay), time.min
        ).replace(tzinfo=None, hour=12)
        self.assertEqual(
        ...
        self.assertEqual(
            purchase2.order_line[1].date_planned,
            po_schedule_date,
    ...
    def test_02_product_route_level_delays(self):
        ...
        order_date = fields.Datetime.from_string(date_planned) - timedelta(
            days=self.product_1.seller_ids.delay + rule_delay + company.po_lead
        )
        self.assertEqual(
            purchase.date_order,
            order_date,
    ...
    def test_merge_po_line_3(self):
        ...
        self.t_shirt.seller_ids.write(
            {"product_name": "Vendor Name", "product_code": "Vendor Code",}
        )
        partner = self.t_shirt.seller_ids[:1].name
        t_shirt = self.t_shirt.with_context(lang=partner.lang, partner_id=partner.id,)
