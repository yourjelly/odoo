...
class TestPurchaseOrder(ValuationReconciliationTestCommon):
    ...
    def test_03_po_return_and_modify(self):
        ...
        po1.write({"order_line": [(1, po1.order_line[0].id, {"product_qty": 15}),]})
        ...
        company_b = self.env["res.company"].create(
            {
                ...
                .search([("id", "!=", company_a.currency_id.id),], limit=1)
                .id,
            }
        )
        self.env.user.write(
    ...
    def test_06_on_time_rate(self):
        ...
        self.env.user.write(
            {"company_id": company_a.id, "company_ids": [(6, 0, [company_a.id])],}
        )
        po = self.env["purchase.order"].create(self.po_vals)
