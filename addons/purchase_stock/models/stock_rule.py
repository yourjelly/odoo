...
class StockRule(models.Model):
    ...
    def _get_lead_days(self, product, **values):
        ...
        seller = (
            ...
            or product.with_company(buy_rule.company_id)._select_seller()
        )
        ...
        if supplier_delay and not bypass_delay_description:
            delay_description += (
                ...
                % (_("Vendor Lead Time"), supplier_delay, _("day(s)"))
            )
        security_delay = buy_rule.picking_type_id.company_id.po_lead
        if not bypass_delay_description:
            delay_description += (
                '<tr><td>%s</td><td class="text-right">+ %d %s</td></tr>'
                % (_("Purchase Security Lead Time"), security_delay, _("day(s)"))
            )
        days_to_purchase = buy_rule.company_id.days_to_purchase
        if not bypass_delay_description:
            delay_description += (
                '<tr><td>%s</td><td class="text-right">+ %d %s</td></tr>'
                % (_("Days to Purchase"), days_to_purchase, _("day(s)"))
            )
        return (
    ...
    def _merge_procurements(self, procurements_to_merge):
        ...
        for procurements in procurements_to_merge:
            ...
            values.update(
                {"move_dest_ids": move_dest_ids, "orderpoint_id": orderpoint_id,}
            )
    ...
    def _make_po_get_domain(self, company_id, values, partner):
        ...
        if values.get("orderpoint_id"):
            procurement_date = fields.Date.to_date(
                values["date_planned"]
            ) - relativedelta(days=int(values["supplier"].delay) + company_id.po_lead)
            delta_days = int(
