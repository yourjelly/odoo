...
from odoo.addons.hw_drivers.controllers.driver import iot_devices, get_ip, get_odoo_server_url, home
...
class IoTboxHomepage(web.Home):
    ...
    def index(self):
        if (os.path.isfile('/home/pi/wifi_network.txt') == False or os.path.isfile(os.path.join(home(), 'odoo-remote-server.conf')) == False) and access_point():
    ...
    def list_drivers(self):
        drivers_list = []
        for driver in os.listdir(os.path.join(home(), "odoo/addons/hw_drivers/drivers")):
    ...
    def load_drivers(self):
        ...
        if server:
            ...
            if resp and resp.data:
                ...
                zip_file.extractall(os.path.join(home(), "odoo/addons/hw_drivers/drivers"))
    ...
    def connect_to_wifi(self, essid, password, persistent=False):
        ...
        subprocess.check_call([os.path.join(home(), 'odoo/addons/point_of_sale/tools/posbox/configuration/connect_to_wifi.sh'), essid, password, persistent])
        server = get_odoo_server_url()
        res_payload = {
    ...
    def clear_wifi_configuration(self):
        os.system(os.path.join(home(), 'odoo/addons/point_of_sale/tools/posbox/configuration/clear_wifi_configuration.sh'))
    ...
    def clear_server_configuration(self):
        os.system(os.path.join(home(), 'odoo/addons/point_of_sale/tools/posbox/configuration/clear_server_configuration.sh'))

        return "<meta http-equiv='refresh' content='0; url=http://" + get_ip() + ":8069'>"

    @http.route('/drivers_clear', type='http', auth='none', cors='*', csrf=False)
    def clear_drivers_list(self):
        os.system(os.path.join(home(), 'odoo/addons/point_of_sale/tools/posbox/configuration/clear_drivers_list.sh'))
    ...
    def connect_to_server(self, token, iotname):
        ...
        token = token.split('|')[1]
        reboot = 'reboot'
        subprocess.check_call([os.path.join(home(), 'odoo/addons/point_of_sale/tools/posbox/configuration/connect_to_server.sh'), url, iotname, token, reboot])
    ...
    def step_by_step_configure(self, token, iotname, essid, password, persistent=False):
        if token:
            ...
            url = ''
        subprocess.check_call([os.path.join(home(), 'odoo/addons/point_of_sale/tools/posbox/configuration/connect_to_server_wifi.sh'),url, iotname, token, essid, password, persistent])
        return url
    ...
    def enable_ngrok(self, auth_token):
        if subprocess.check_call(['pgrep', 'ngrok']) == 1:
