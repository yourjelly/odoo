...
class IoTboxHomepage(odoo.addons.web.controllers.main.Home):
    ...
    def get_homepage_data(self):
        ...
        for device in iot_devices:
            iot_device.append({
                                'name': iot_devices[device].get_name() + ' : ' + str(iot_devices[device].value),
                                'type': iot_devices[device].get_type(),
                                'message': iot_devices[device].get_identifier() + iot_devices[device].get_message()
                                })
    ...
    def index(self):
        if (os.path.isfile('/home/pi/wifi_network.txt') == False or os.path.isfile('/home/pi/odoo-remote-server.conf') == False) and get_ip() == '10.11.12.1':
