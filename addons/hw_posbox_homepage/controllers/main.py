...
class IoTboxHomepage(Home):
    ...
    def list_handlers(self):
        drivers_list = []
        driver_list = helpers.list_file_by_os(os.listdir(get_resource_path('hw_drivers', 'iot_handlers/drivers')))
        interface_list = helpers.list_file_by_os(os.listdir(get_resource_path('hw_drivers', 'iot_handlers/interfaces')))
    ...
    def connect_to_server(self, token, iotname):
        if token:
            ...
            helpers.connect_server(url, token, db_uuid, enterprise_code)
        else:
            url = helpers.get_odoo_server_url()
            token = helpers.get_token()
        if iotname != 'check name here':
            # call .sh file to rename iot
            print('test')
        else:
