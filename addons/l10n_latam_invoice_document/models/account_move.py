...
class AccountMove(models.Model):
    ...
    l10n_latam_tax_ids = fields.One2many(
        compute="_compute_l10n_latam_amount_and_taxes", comodel_name="account.move.line"
    )
    ...
    def _compute_l10n_latam_manual_document_number(self):
        ...
        for rec in recs_with_journal_id:
            rec.l10n_latam_manual_document_number = self._is_manual_document_number(
                rec.journal_id
            )
    ...
    def _compute_l10n_latam_amount_and_taxes(self):
        ...
        for invoice in recs_invoice:
            ...
            currencies = invoice.line_ids.filtered(
                lambda x: x.currency_id == invoice.currency_id
            ).mapped("currency_id")
            included_taxes = (
                invoice.l10n_latam_document_type_id
                and invoice.l10n_latam_document_type_id._filter_taxes_included(
                    tax_lines.mapped("tax_line_id")
                )
            )
            if not included_taxes:
                ...
                included_invoice_taxes = tax_lines.filtered(
                    lambda x: x.tax_line_id in included_taxes
                )
                ...
                l10n_latam_amount_untaxed = invoice.amount_untaxed + sign * sum(
    ...
    def _compute_l10n_latam_document_type(self):
        for rec in self.filtered(lambda x: x.state == "draft"):
            ...
            document_types = (
                document_types.filtered(lambda x: x.internal_type == "debit_note")
                if debit_note
                else document_types
            )
    ...
    def _compute_invoice_taxes_by_group(self):
        report_or_portal_view = (
            "commit_assetsbundle" in self.env.context
