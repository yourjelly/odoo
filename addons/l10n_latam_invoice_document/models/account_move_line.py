...
class AccountMoveLine(models.Model):
    ...
    l10n_latam_price_unit = fields.Float(
    ...
    l10n_latam_price_net = fields.Float(
        compute="compute_l10n_latam_prices_and_taxes", digits="Product Price"
    )
    ...
    def compute_l10n_latam_prices_and_taxes(self):
        for line in self:
            ...
            included_taxes = (
                ...
                and invoice.l10n_latam_document_type_id._filter_taxes_included(
                    line.tax_ids
                )
            )
            ...
            price_digits = 10 ** self.env["decimal.precision"].precision_get(
                "Product Price"
            )
            if not included_taxes:
                price_unit = line.tax_ids.with_context(
                    round=False, force_sign=invoice._get_tax_force_sign()
                ).compute_all(
                ...
                l10n_latam_price_unit = (
                    included_taxes.with_context(
                        force_sign=invoice._get_tax_force_sign()
                    ).compute_all(
                        line.price_unit * price_digits,
                        invoice.currency_id,
                        1.0,
                ...
                l10n_latam_price_subtotal = included_taxes.with_context(
                    force_sign=invoice._get_tax_force_sign()
                ).compute_all(
                    price,
                    invoice.currency_id,
