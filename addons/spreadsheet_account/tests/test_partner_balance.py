...
class SpreadsheetAccountingFunctionsTest(AccountTestInvoicingCommon):
    ...
    def test_partner_balance_no_account_codes(self):
        ...
        partner_balance = self.env['account.account'].spreadsheet_fetch_partner_balance([
            {
                'partner_ids': [str(self.move_23.partner_id.id)],
                'date_from': {
                    'range_type': 'year',
                    'year': 2023,
                },
    ...
    def test_partner_balance_daily(self):
        ...
        partner_balance = self.env['account.account'].spreadsheet_fetch_partner_balance([
            {
                'partner_ids': [str(self.move_22.partner_id.id)],
                'date_from': {
                    'range_type': 'day',
                    'year': 2022,
                    'month': 10,
    ...
    def test_partner_filter(self):
        ...
        partner_balance = self.env['account.account'].spreadsheet_fetch_partner_balance([
            {
                'partner_ids': [str(self.move_23.partner_id.id), str(self.move_23_partner_b.partner_id.id)],
                'date_from': {
                    'range_type': 'year',
                    'year': 2023,
                },
