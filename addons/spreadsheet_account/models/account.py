...
class AccountAccount(models.Model):
    ...
    def _get_all_lines(self, args_list, fields, timeline, accounts):
        ...
        # Get all include_initial_balance account lines from all the past moves
        period = timeline[0]
        ...
        domain = [
            *self.env['account.move.line']._check_company_domain(company_ids),
            ('account_id', 'in', include_initial_balance_account_ids),
        ...
        for period in timeline[1:]:
            ...
            domain = [
                *self.env['account.move.line']._check_company_domain(company_ids),
                ('account_id', 'in', account_ids_in_period.ids),
                ('parent_state', 'in', ('draft', 'posted')),
                ('date', '>=', period[0]),
                ('date', '<=', period[1]),
            ]
