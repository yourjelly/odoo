...
class HrEmployeeCV(Controller):
    ...
    def print_employee_cv(self, employee_ids='', color_primary='#666666', color_secondary='#666666', **post):
        if not request.env.user._has_group('base.group_user') or not employee_ids or re.search("[^0-9|,]", employee_ids):
            return request.not_found()

        ids = [int(s) for s in employee_ids.split(',')]
        employees = request.env['hr.employee'].browse(ids)
        if not request.env.user._has_group('hr.group_hr_user') and employees.ids != request.env.user.employee_id.ids:
            return request.not_found()
