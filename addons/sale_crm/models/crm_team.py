            ...
            if lead_dups_dict.order_ids \
               and len(lead_dups_dict.order_ids.company_id + lead_dups_dict.company_id) > 1:
                ...
                if len(leads_compatible_with_head_opportunity) >= 2:
                    ...
                    values['leads_dups_dict'].update({new_leads or lead: leads_compatible_with_head_opportunity})
                    opportunities = values['different_company'] = leads_with_so_different_company
                body = _('Leads (%s) could not be merged and have been skipped because their Sales Orders (%s) belong to different companies.',
                         ', '.join(opportunities.mapped('name')), ", ".join(opportunities.order_ids.mapped('name')))
                opportunities._print_merging_leads_failed(body)
                if len(leads_compatible_with_head_opportunity) < 2:
                    return {
