...
class L10nInAccountWithhold(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        if active_model not in ('account.move', 'account.payment') or not active_ids:
            raise UserError(_("Withholds must be created from an Invoice or a Payment."))
        active_record = self.env[active_model].browse(active_ids)
        result['reference'] = active_record.name
    ...
    @api.depends('tds_group_id')
    def _compute_type_tax_use(self):
        for wizard in self:
            type_tax_use = False
            if wizard.related_move_id:
                if wizard.related_move_id.move_type == 'in_invoice':
                    type_tax_use = 'purchase'
                elif wizard.related_move_id.move_type == 'out_invoice':
                    type_tax_use = 'sale'
            elif wizard.related_payment_id:
                if wizard.related_payment_id.payment_type == 'inbound':
                    type_tax_use = 'sale'
                elif wizard.related_payment_id.payment_type == 'outbound':
                    type_tax_use = 'purchase'
    ...
    def _compute_withhold_type(self):
        for wizard in self:
            if wizard.related_move_id:
                wizard.withhold_type = 'in_withhold' if wizard.related_move_id.move_type in ('in_invoice', 'in_receipt') else 'out_withhold'
            else:
    ...
    def _prepare_withhold_header(self):
        ...
        vals = {
            ...
            'l10n_in_withholding_move_id': self.related_move_id.id or self.related_payment_id.move_id.id,
        }
        return vals
    ...
    def _prepare_withhold_move_lines(self, withholding_account_id):
        ...
        for line in self.withhold_line_ids:
            debit = line.base if self.withhold_type == 'in_withhold' else 0.0
            credit = 0.0 if self.withhold_type == 'in_withhold' else line.base
        ...
        debit = 0.0 if self.withhold_type == 'in_withhold' else total_amount
        credit = total_amount if self.withhold_type == 'in_withhold' else 0.0
        ...
        debit = total_tax if self.withhold_type == 'in_withhold' else 0.0
        credit = 0.0 if self.withhold_type == 'in_withhold' else total_tax
    ...
    def _validate_withhold_data_on_post(self, withholding_account_id):
        ...
        if sum(line.base for line in self.withhold_line_ids) > self.amount:
