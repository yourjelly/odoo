...
class L10nEsEdiTbaiDocument(models.Model):
    ...
    def _get_lines_values(self, base_lines, rate, is_refund):
        ...
        def _get_base_line_price_total(base_line):
        ...
        for base_line in base_lines:
            ...
            if not any(t.l10n_es_type == 'sujeto_isp' for t in base_line['taxes']):
                total = _get_base_line_price_total(base_line)
            else:
    ...
    def get_tax_details(self, base_lines, filter_invl_to_apply=None):
        ...
        def full_filter_invl_to_apply(base_line):
            return (
                not all(t == 'ignore' for t in base_line['taxes'].flatten_taxes_hierarchy().mapped('l10n_es_type'))
                and filter_invl_to_apply(base_line) if filter_invl_to_apply else True
            )
        ...
        to_process = []
        base_lines_full_filtered = [base_line for base_line in base_lines if full_filter_invl_to_apply(base_line)]
