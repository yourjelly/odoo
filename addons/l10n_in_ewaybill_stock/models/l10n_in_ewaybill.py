...
class Ewaybill(models.Model):
    ...
    is_fiscal_position_invisible = fields.Boolean(compute="_compute_is_fiscal_position_invisible")
    ...
    def _compute_is_fiscal_position_invisible(self):
        """
        Make fiscal field invisible if a Sales Order (SO) or Purchase Order (PO) exists.

        This function checks the presence of a Sales Order (SO) or Purchase Order (PO) in the system.
        If either a SO or PO exists, the fiscal field will be made invisible.
        `_l10n_in_get_fiscal_position` will return False in this case.

        However, if no SO or PO exists, the fiscal field will be visible.
        If a fiscal position is not set on the SO or PO, the field will still be invisible,
        returning an empty recordset.

        Example:
        - self.env['account.fiscal.position'] == False (output will be False)
        - self.env['account.fiscal.position'] == account.fiscal.position() (output will be True)
        """
        for ewaybill in self:
