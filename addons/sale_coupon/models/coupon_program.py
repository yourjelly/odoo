...
class CouponProgram(models.Model):
    ...
    def _compute_order_count(self):
        for program in self:
            program.order_count = self.env["sale.order.line"].search_count(
    ...
    def _filter_not_ordered_reward_programs(self, order):
        ...
        for program in self:
            if program.reward_type == "product" and not order.order_line.filtered(
                ...
                and not order.order_line.filtered(
                    lambda line: line.product_id
                    in program.discount_specific_product_ids
                )
            ):
                continue
            programs |= program
