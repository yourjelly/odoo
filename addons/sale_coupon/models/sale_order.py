...
class SaleOrder(models.Model):
    ...
    def _get_paid_order_lines(self):
        ...
        return self.order_line.filtered(lambda x: not x.is_not_a_product_line() or x.product_id in free_reward_product)

    def _get_base_order_lines(self, program):
        """ Returns the sale order lines not linked to the given program.
        """
        return self.order_line.filtered(lambda x: not x.is_not_a_product_line() or (x.is_reward_line and x.product_id != program.discount_line_product_id))
    ...
    def _get_cheapest_line(self):
        ...
        return min(self.order_line.filtered(lambda x: not x.is_not_a_product_line() and x.price_reduce > 0), key=lambda x: x['price_reduce'])
