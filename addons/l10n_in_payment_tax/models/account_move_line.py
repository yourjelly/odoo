...
class AccountMoveLine(models.Model):
    ...
    is_advance_payment_line = fields.Boolean("is_advance_payment_line")
    ...
    def _get_payment_tax_reverse_moves_line_vals(self, partial_line, payment_line_with_tax):
        ...
        compute_all_res = payment_line_with_tax.payment_id.l10n_in_tax_ids.compute_all(base_amount, currency=partial_line.company_currency_id)
    ...
    def _get_payment_tax_reverse_moves_vals(self, partial_line, payment_line_with_tax):
        default_journal = self.env['account.journal'].search([
            ('company_id', '=', partial_line.company_id.id),
            ('type', '=', 'general')
            ])
        return {
            'move_type': 'entry',
            ...
            'ref': 'Advanced Payment Tax Adjustment agains reconcile of %s - %s'%(partial_line.credit_move_id.move_id.name, partial_line.debit_move_id.move_id.name),
            ...
            'l10n_in_advanced_payment_tax_origin_move_id': payment_line_with_tax.payment_id.move_id.id,
        }
