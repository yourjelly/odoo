...
class TestDefaultTeam(TestSalesCommon):
    ...
    def setUpClass(cls):
        ...
        cls.team_c2 = cls.env["crm.team"].create(
            {"name": "C2 Team1", "sequence": 1, "company_id": cls.company_2.id,}
        )
        cls.team_sequence = cls.env["crm.team"].create(
            {"name": "Team LowSequence", "sequence": 0, "company_id": False,}
        )
    ...
    def test_default_team_fallback(self):
        ...
        self.user_sales_leads.write(
            {"company_ids": [(4, self.company_2.id)], "company_id": self.company_2.id,}
        )
