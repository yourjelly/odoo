...
class Project(models.Model):
    ...
    def _get_revenues_items_from_sol(self, domain=None, with_action=True):
        ...
        display_sol_action = with_action and len(self) == 1 and self.env.user._has_group('sales_team.group_sale_salesman')
        revenues_dict = {}
        ...
        if sale_line_read_group:
            ...
            if downpayment_amount_invoiced:
                ...
                if with_action and (
                        self.env.user._has_group('sales_team.group_sale_salesman_all_leads') or
    ...
    def _get_stat_buttons(self):
        ...
        if self.env.user._has_group('sales_team.group_sale_salesman_all_leads'):
            self_sudo = self.sudo()
            buttons.append({
                'icon': 'dollar',
        ...
        if self.env.user._has_group('account.group_account_readonly'):
            self_sudo = self.sudo()
            buttons.append({
