...
class TestNestedTaskUpdate(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        product = cls.env["product.product"].create(
            {"name": "Prepaid Consulting", "type": "service",}
        )
        cls.order_line = cls.env["sale.order.line"].create(
            {"name": "Order line", "product_id": product.id, "order_id": sale_order.id,}
        )
    ...
    def test_creating_subtask_user_id_on_parent_dont_go_on_child(self):
        parent = self.env["project.task"].create(
            {"name": "parent", "user_id": self.user.id}
        )
        child = self.env["project.task"].create(
            {"name": "child", "parent_id": parent.id, "user_id": False}
        )
    ...
    def test_write_user_id_on_parent_dont_write_on_child(self):
        ...
        child = self.env["project.task"].create(
            {"name": "child", "user_id": False, "parent_id": parent.id}
        )
        self.assertFalse(child.user_id)
    ...
    def test_writing_on_parent_with_multiple_tasks(self):
        parent = self.env["project.task"].create(
            {"name": "parent", "user_id": False, "partner_id": self.partner.id}
        )
        children_values = [
            {"name": "child%s" % i, "user_id": False, "parent_id": parent.id}
            for i in range(5)
        ]
