...
class SaleProjectCustomerPortal(ProjectCustomerPortal):
    ...
    def _task_get_searchbar_groupby(self):
        ...
        values['sale_order'] = {'input': 'sale_order', 'label': _('Sales Order'), 'order': 8}
        values['sale_line'] = {'input': 'sale_line', 'label': _('Sales Order Item'), 'order': 9}
    ...
    def _task_get_searchbar_inputs(self):
        ...
        values['sale_order'] = {'input': 'sale_order', 'label': _('Search in Sales Order'), 'order': 8}
        values['sale_line'] = {'input': 'sale_line', 'label': _('Search in Sales Order Item'), 'order': 9}
        values['invoice'] = {'input': 'invoice', 'label': _('Search in Invoice'), 'order': 10}
        return dict(sorted(values.items(), key=lambda item: item[1]["order"]))
