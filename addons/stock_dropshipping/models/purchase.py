...
class PurchaseOrderLine(models.Model):
    ...
    def _prepare_stock_moves(self, picking):
        ...
        for re in res:
            re['sale_line_ids'] = [(4, sol_id, None) for sol_id in self.sale_line_ids.ids]
        return res
...
class StockRule(models.Model):
    ...
    def _get_procurements_to_merge_groupby(self, procurement):
        ...
        """
        return procurement.values.get('sale_line_ids'), super(StockRule, self)._get_procurements_to_merge_groupby(procurement)

    @api.model
    def _get_procurements_to_merge_sorted(self, procurement):
        return procurement.values.get('sale_line_ids'), super(StockRule, self)._get_procurements_to_merge_sorted(procurement)
    ...
    def _prepare_purchase_order_line(self, product_id, product_qty, product_uom, company_id, values, po):
        ...
        sale_line_ids = values.get('sale_line_ids', False)
        if sale_line_ids:
            res['sale_line_ids'] = [(4, sol_id, None) for sol_id in sale_line_ids.ids]
        return res
