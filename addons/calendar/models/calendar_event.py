...
class Meeting(models.Model):
    ...
    videocall_location = fields.Char(
        "Join Video Call", default=_default_videocall_location
    )
    # visibility
        ...
        vals_list = [
            dict(
                ...
                attendee_ids=self._attendees_values(
                    vals.get("partner_ids", self_partner_id)
                ),
            )
            for vals in vals_list
        ]
    ...
    def write(self, values):
        ...
        if (
            any(
                [
                    values.get(key)
                    for key in self.env["calendar.event"]._get_time_fields()
                ]
            )
            or "alarm_ids" in values
        ):
            update_alarms = True
