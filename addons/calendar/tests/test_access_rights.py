...
class TestAccessRights(TransactionCase):
    ...
    def test_private_read_name(self):
        event = self.create_event(
            self.john, privacy="private", name="my private event",
        )
        self.assertEqual(
            self.read_event(self.john, event, "name"),
            "my private event",
            "Owner should be able to read the event",
        )
        self.assertEqual(
            self.read_event(self.raoul, event, "name"),
            "Busy",
            "Private value should be obfuscated",
        )
        with self.assertRaises(AccessError):
            self.read_event(self.portal, event, "name")

    def test_private_other_field(self):
        event = self.create_event(self.john, privacy="private", location="in the Sky",)
        self.assertEqual(
    ...
    def test_private_and_public(self):
        private = self.create_event(
            self.john, privacy="private", location="in the Sky",
        )
        public = self.create_event(self.john, privacy="public", location="In Hell",)
    ...
    def test_read_group_private(self):
        event = self.create_event(self.john)
        with self.assertRaises(AccessError):
            self.env["calendar.event"].with_user(self.raoul).read_group(
                [("id", "=", event.id)], fields=["name"], groupby="name"
            )
