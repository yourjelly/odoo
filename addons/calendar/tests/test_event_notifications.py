...
class TestEventNotifications(TransactionCase, MailCase, CronMixinCase):
    ...
    def test_message_set_inactive_invite(self):
        ...
        with self.assertNoNotifications():
            self.event.write(
                {"partner_ids": [(4, self.partner.id)], "active": False,}
            )
    ...
    def test_bus_notif(self):
        ...
        with patch.object(fields.Datetime, "now", lambda: now):
            with self.assertBus(
                [(self.env.cr.dbname, "calendar.alarm", self.partner.id)]
            ):
            ...
            notif = (
                self.env["calendar.alarm_manager"].with_user(self.user).get_next_notif()
            )
