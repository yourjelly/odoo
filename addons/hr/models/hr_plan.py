...
class HrPlanActivityType(models.Model):
    ...
    _sql_constraints = [
        (
            "check_deadline_days",
            "CHECK (COALESCE(deadline_days) >= 0)",
...
class HrPlan(models.Model):
    ...
    trigger_onboarding = fields.Selection(
        [("manual", "Manual"), ("employee_creation", "Employee Creation"),],
        compute="_compute_triggers",
        inverse="_inverse_triggers",
        required=True,
        readonly=False,
    )
    trigger_offboarding = fields.Selection(
        [("manual", "Manual"), ("employee_archive", "Archived Employee"),],
        compute="_compute_triggers",
        inverse="_inverse_triggers",
        required=True,
        readonly=False,
