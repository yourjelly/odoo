...
class TestSelfAccessProfile(TestHrCommon):
    ...
    def test_access_my_profile(self):
        ...
        view_infos = james.get_view(view_id=view.id)
    ...
    def test_profile_view_fields(self):
        ...
        view_infos = self.env['res.users'].with_user(user_all_groups).get_view(view_id=view.id)
        ...
        view_infos = self.env['res.users'].with_user(user).get_view(view_id=view.id)
        fields = [el.get('name') for el in etree.fromstring(view_infos['arch']).xpath('//field[not(ancestor::field)]')]
    ...
    def test_access_my_profile_toolbar(self):
        ...
        view = self.env.ref('hr.res_users_view_form_profile')
        available_actions = james.get_view(view_id=view.id, toolbar=True)['toolbar']['action']
        ...
        view = self.env.ref('hr.res_users_view_form_profile')
        available_actions = john.get_view(view_id=view.id, toolbar=True)['toolbar']['action']
