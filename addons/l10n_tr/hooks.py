...
def _load_neighborhood(cr, registry):
    ...
    Even with the faster CSVs, it would take +60 seconds to load it with
    ...
    _logger.info(f'loading l10n_tr/data/l10n_tr.neighborhood.csv')
    ...
    for line in csv_file:
        ...
        if row[1] not in areas:
            _logger.error(f'Error while loading Neighborhoods, matching area {row[0]} not found for {row[1]}.')
            raise MissingError(_('Error while loading Neighborhoods, Area with id: {} not defined.').format(row[1]))
    ...
    cr.execute(
        """INSERT INTO ir_model_data
           ...
           SELECT concat('l10n_tr_neighborhood_', id), id, 'l10n_tr', 'l10n_tr.neighborhood.csv', 't'
           FROM l10n_tr_neighborhood""")
    _logger.info(f'total of {count} Turkish neighborhoods loaded.')
