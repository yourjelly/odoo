...
class Quiz(models.Model):
    ...
    event_track_id = fields.Many2one(
        "event.track", compute="_compute_event_track_id", readonly=True, store=True
    )
    ...
    def _compute_event_track_id(self):
        for quiz in self:
            quiz.event_track_id = (
                quiz.event_track_ids[0] if quiz.event_track_ids else False
            )
...
class QuizQuestion(models.Model):
    ...
    def _check_answers_integrity(self):
        for question in self:
            if (
                len(question.answer_ids.filtered(lambda answer: answer.awarded_points))
                != 1
            ):
                raise ValidationError(
                    _('Question "%s" must have 1 correct answer', question.name)
                )
