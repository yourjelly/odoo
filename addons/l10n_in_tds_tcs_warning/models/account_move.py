...
class AccountMove(models.Model):
    ...
    def _get_sections_aggregate_sum_by_pan(self, section_alert, commercial_partner_id):
        ...
        default_domain = [
            ('account_id.l10n_in_tds_tcs_section', '=', section_alert.id),
    ...
    @api.depends('state', 'l10n_in_withhold_move_ids')
    def _compute_l10n_in_tcs_tds_warning(self):
        def _group_by_section_alert(invoice_line_ids):
            group_by_lines = {}
            line_ids = invoice_line_ids.filtered(
                lambda l: (
                    l.balance != 0
                    and l.account_id.l10n_in_tds_tcs_section
                    and l.account_id.l10n_in_tds_tcs_section not in l.tax_ids.mapped('l10n_in_section_id')
                )
            )
            for line in line_ids:
                group_key = line.account_id.l10n_in_tds_tcs_section
        ...
        def _is_section_applicable(section_alert, threshold_sums, invoice_currency_rate, lines):
            if section_alert.is_aggregate_limit:
                aggregaete_period_key = section_alert.consider_amount == 'total_amount' and 'price_total' or 'balance'
                aggregaete_total = threshold_sums.get(section_alert.aggregate_period, {}).get(aggregaete_period_key)
                if aggregaete_total > section_alert.aggregate_limit:
                    return True
            return (
        ...
        for move in self:
            if move.country_code == 'IN' and move.state == 'posted':
                ...
                for section_alert, lines in _group_by_section_alert(move.invoice_line_ids).items():
                    if (
                        move._l10n_in_is_warning_applicable(section_alert)
                ...
                if warning_sections and move.move_type in ['in_invoice', 'out_invoice']:
                    move.l10n_in_tcs_tds_warning = warning_sections._get_warning_messege()
                else:
