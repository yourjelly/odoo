...
export class WebsocketWorker {
    ...
    _onClientMessage(client, { action, data }) {
        switch (action) {
            ...
            case 'force_update_channels':
                return this._updateChannels({ force: true });
        }
    }
    ...
    */
    ...
    _updateChannels({ force = false } = {}) {
        const allTabsChannels = [...new Set([].concat.apply([], Array.from(this.channelsByClient.values())))].sort();
        const shouldUpdateChannelSubscription = JSON.stringify(allTabsChannels) !== this.lastChannelSubscription;
        if (force || shouldUpdateChannelSubscription) {
            this.lastChannelSubscription = JSON.stringify(allTabsChannels);
