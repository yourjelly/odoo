...
from ..websocket import CloseCode, InvalidStateException, acquire_cursor
...
class ImDispatch(threading.Thread):
    def __init__(self):
        ...
        self._max_workers = (
            int(max_workers) if (max_workers := os.getenv('ODOO_BUS_DISPATCH_MAX_WORKERS')) else None
        )
            ...
            with ThreadPoolExecutor(max_workers=self._max_workers) as executor:
                while not stop_event.is_set():
                    if sel.select(TIMEOUT):
                        ...
                        # relay notifications to websockets that have subscribed
                        ...
                        for channel in channels:
                            ...
                            websockets.update(related_ws)
                        self._ws_waiting.update(websockets)
                        for websocket in websockets:
                            executor.submit(self._dispatch_notifications, websocket=websocket)
