...
class WebsocketConnectionHandler:
    ...
    def _serve_forever(cls, websocket, db, httprequest, version):
        ...
        if version != cls._VERSION:
            # Close the connection from an outdated worker. We can't use a custom
            # close code because it would be considered successful, preventing
            # exponential reconnect backoff. This would cause old workers to
            # reconnect frequently, putting pressure on the server. Clean closes
            # don't trigger reconnections, assuming they are intentional. The reason
            # indicates to the origin worker not to reconnect, preventing old
            # workers from lingering after updates.
            websocket.disconnect(CloseCode.CLEAN, "OUTDATED")
