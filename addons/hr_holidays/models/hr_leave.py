...
class HolidaysRequest(models.Model):
    ...
    holiday_status_id = fields.Many2one(
        ...
        domain="""[
            ('company_id', 'in', [employee.company_id, False]),
            '|',
    ...
    employee_id = fields.Many2one(
        ...
        tracking=True, domain=lambda self: self._get_employee_domain())
    ...
    _sql_constraints = [
        ('type_value', 'CHECK(employee_id IS NOT NULL)',
    ...
    def _compute_from_employee_id(self):
        for holiday in self:
            holiday.manager_id = holiday.employee_id.leave_manager_id
            if holiday.holiday_status_id.requires_allocation == 'no':
                continue
            if not holiday.employee_id:
                holiday.holiday_status_id = False
                continue
            if holiday.employee_id.user_id != self.env.user\
                    and holiday._origin.employee_id != holiday.employee_id\
                    and not holiday.holiday_status_id.with_context(employee_id=holiday.employee_id.id).has_valid_allocation:
                holiday.holiday_status_id = False
...
""",
    ...
    def onchange(self, values, field_names, fields_spec):
        ...
        if values and 'employee_id' in fields_spec and 'employee_id' not in self._context:
            employee_id = self._context.get('default_employee_id')\
                or self._context.get('employee_id')\
    ...
    def create(self, vals_list):
        ...
        fast_create = self._context.get('leave_fast_create')
        for values in vals_list:
            if 'resource_id' not in values:
                values['resource_id'] = self.env['hr.employee'].browse(values['employee_id']).resource_id.id
            # if 'date_from' not in values:
            #     # TODO BEDO
            #     values['date_from']
            # if 'date_to' not in values:
            #     # TODO BEDO
            #     values['date_to']
            if not fast_create:
    ...
    def _validate_leave_request(self):
        ...
        meeting_holidays = self.filtered(lambda l: l.holiday_status_id.create_calendar_meeting)
            ...
            if not is_manager and state != 'confirm':
                ...
                if state == 'draft':
                    ...
                    if (state == 'validate' and val_type == 'manager')\
                            and self.env.user != holiday.employee_id.leave_manager_id\
                            and not is_officer:
                        raise UserError(_('You must be %s\'s Manager to approve this leave', holiday.employee_id))
