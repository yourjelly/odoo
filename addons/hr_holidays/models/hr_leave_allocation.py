...
class HolidaysAllocation(models.Model):
    ...
    manager_id = fields.Many2one(
        "hr.employee", compute="_compute_from_employee_id", store=True, string="Manager"
    )
    notes = fields.Html(
        "Reasons",
        readonly=True,
    ...
    interval_number = fields.Integer(
        "Number of unit between two intervals",
        compute="_compute_from_holiday_status_id",
        store=True,
        readonly=False,
        states={
    ...
    _sql_constraints = [
        ...
        (
            "number_per_interval_check",
            "CHECK(number_per_interval > 0)",
            "The number per interval should be greater than 0",
        ),
        (
            "interval_number_check",
    ...
    def _update_accrual(self):
        ...
        for holiday in holidays:
            ...
            values["nextcall"] = (
                holiday.nextcall if holiday.nextcall else today
            ) + delta
    ...
    def _compute_can_approve(self):
        for allocation in self:
            try:
                if (
                    allocation.state == "confirm"
                    and allocation.validation_type == "both"
                ):
    ...
    def _compute_from_employee_id(self):
        ...
        for holiday in self:
            ...
            if (
                ...
                not holiday.holiday_status_id and not holiday._origin.holiday_status_id
            ):
    ...
    def _compute_from_holiday_status_id(self):
        for allocation in self:
            ...
            # set default values
            if (
                not allocation.interval_number
                and not allocation._origin.interval_number
            ):
                allocation.interval_number = 1
            if (
                not allocation.number_per_interval
                and not allocation._origin.number_per_interval
            ):
                allocation.number_per_interval = 1
            if (
                not allocation.unit_per_interval
                and not allocation._origin.unit_per_interval
            ):
    ...
    def action_draft(self):
        ...
        self.write(
            {"state": "draft", "first_approver_id": False, "second_approver_id": False,}
        )
    ...
    def action_approve(self):
        ...
        if any(holiday.state != "confirm" for holiday in self):
            raise UserError(
                _(
                    'Allocation request must be confirmed ("To Approve") in order to approve it.'
                )
            )
    ...
    def _check_approval_update(self, state):
        ...
        for holiday in self:
            ...
            if (state == "validate1" and val_type == "both") or (
                ...
                if (
                    self.env.user == holiday.employee_id.leave_manager_id
                    and self.env.user != holiday.employee_id.user_id
                ):
                    continue
                manager = (
            ...
            if state == "validate" and val_type == "both":
                if not is_officer:
                    raise UserError(
                        _(
                            "Only a Time off Approver can apply the second approval on allocation requests."
                        )
                    )
