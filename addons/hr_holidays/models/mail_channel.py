...
class Channel(models.Model):
    ...
    def channel_info(self, extra_info=False):
            ...
            if 'direct_partner' in channel_info:
                for direct_partner in channel_info['direct_partner']:
                    if 'leave' in direct_partner['im_status']:
                        partners_on_leave += direct_partner['id']
        if partners_on_leave:
            ...
            messages = dict(((status['id'], res) for res in self.env.cr.dictfetchall()))
            for channel_info in channel_infos:
                if 'direct_partner' in channel_info:
                    for direct_partner in channel_info['direct_partner']:
                        if 'leave' in direct_partner['im_status']:
                            direct_partner['out_of_office_date_end'] = messages.get(direct_partner['id'], {}).get('date_to')
                            direct_partner['out_of_office_message'] = messages.get(direct_partner['id'], {}).get('out_of_office_message')
