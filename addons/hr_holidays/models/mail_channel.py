...
class Channel(models.Model):
    ...
    def _get_channel_partner_info(self, all_partners, direct_partners):
        partner_infos = super(Channel, self)._get_channel_partner_info(
            all_partners, direct_partners
        )
        # only search for leave out_of_office_date_end if im_status is on leave
        partners_on_leave = [
            partner_id
            for partner_id in direct_partners.ids
            if "leave" in partner_infos[partner_id]["im_status"]
        ]
        if partners_on_leave:
            now = fields.Datetime.now()
            self.env.cr.execute(
                """SELECT res_users.partner_id as partner_id, hr_leave.date_to as date_to
                                FROM res_users
                                ...
                                AND res_users.partner_id in %s""",
                (now, now, tuple(partners_on_leave)),
            )
            out_of_office_infos = dict(
                ((res["partner_id"], res) for res in self.env.cr.dictfetchall())
            )
            for partner_id, out_of_office_info in out_of_office_infos.items():
                partner_infos[partner_id][
                    "out_of_office_date_end"
                ] = out_of_office_info["date_to"]
