...
class HolidaysType(models.Model):
    ...
    def _model_sorting_key(self, leave_type):
        ...
        return (
            ...
            leave_type.allocation_type == "fixed" and remaining,
            leave_type.allocation_type == "fixed_allocation" and remaining,
            taken,
        )
    ...
    def _check_validity_dates(self):
        for leave_type in self:
            if (
                leave_type.validity_start
                and leave_type.validity_stop
                and leave_type.validity_start > leave_type.validity_stop
            ):
                raise ValidationError(
                    _(
                        "End of validity period should be greater than start of validity period"
                    )
                )
    ...
    def _compute_valid(self):
        ...
        for holiday_type in self:
            if holiday_type.validity_start and holiday_type.validity_stop:
                holiday_type.valid = (dt < holiday_type.validity_stop) and (
                    dt > holiday_type.validity_start
                )
    ...
    def get_days_all_request(self):
        leave_types = sorted(
            self.search([]).filtered(
                lambda x: (
                    (not x.validity_stop or x.validity_stop >= fields.Date.today())
                    and (x.virtual_remaining_leaves or x.max_leaves)
                )
            ),
