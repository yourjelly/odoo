        ...
        for record in self:
            if record.requires_allocation == "yes":
                if record._context.get('default_date_from'):
                    ...
                    employee_id = [employee_id] if not isinstance(employee_id, list) else employee_id
                    leaves_count = record.get_employees_days(employee_id, date=record._context['default_date_from'])[employee_id[0]][record.id]
