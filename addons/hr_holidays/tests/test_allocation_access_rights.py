...
class TestAllocationRights(TestHrHolidaysCommon):
    ...
    def request_allocation(self, user, values={}):
        values = dict(values, **{"name": "Allocation", "number_of_days": 1,})
...
class TestAccessRightsHolidayManager(TestAllocationRights):
    ...
    def test_holiday_manager_both_validation(self):
        ...
        allocation.action_approve()
        self.assertEqual(
            allocation.state, "validate1", "It should have been validated one time"
        )
        allocation.action_validate()
        self.assertEqual(
            allocation.state, "validate", "It should have been completely validated"
        )
