...
class ExportGstrJson(ExportGstr):
    ...
    def prepare_gstr_json(self, gst_return):
        ...
        for gstr_return_section in GSTR_RETURN_SECTIONS:
            ...
            read_data = request.env[model].search_read(domain)
            if gstr_return_section == 'b2b':
                for partner_gstn, values in groupby(sorted(read_data, key= lambda k: k['partner_gstn']), lambda x: x['partner_gstn']):
                    invoice_datas=[]
                ...
                for place_of_supply, values in groupby(sorted(read_data, key= lambda k: k['place_of_supply']), lambda x: x['place_of_supply']):
                    invoice_datas=[]
                    for invoice_number, invoices in groupby(sorted(list(values), key=lambda k: k['invoice_number']),lambda x: x['invoice_number']):
                ...
                for exp_invoice_type, values in groupby(sorted(read_data, key= lambda k: k['exp_invoice_type']),lambda x: x['exp_invoice_type']):
                    invoice_datas = []
                    for invoice_number, invoices in groupby(sorted(list(values), key= lambda k: k['invoice_number']), lambda x: x['invoice_number']):
                ...
                for document_type, values in groupby(sorted(read_data, key= lambda k: k['document_type']), lambda x: x['document_type']):
                    document_datas=[]
                    serial_number = 1
            ...
            summary_type_json and json_data.update({gstr_return_section: summary_type_json})
    ...
    def set_values(self, keys, values, gstr_return_section, serial_number = 1):
        ...
        return {key: keys_data.get(key) or self.set_emty_value(keys_data.get(key)) for key in keys}
