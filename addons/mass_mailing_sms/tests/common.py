...
class MassSMSCase(SMSCase):
    ...
    def assertSMSTraces(
        ...
        for recipient_info, link_info, record in zip(
            ...
            self.assertTrue(
                len(trace) == 1,
                "SMS: found %s notification for number %s, (state: %s) (1 expected)"
                % (len(trace), number, state),
            )
            ...
            if check_sms:
                if state == "sent":
                    ...
                    error_code = (
                        ...
                        if state in ("exception", "ignored", "bounced")
                        else None
                    )
                    self.assertSMS(
                        partner,
            ...
            if link_info:
                ...
                for (url, is_shortened, add_link_params) in link_info:
                    ...
                    self.assertLinkShortenedText(
                        sms_sms.body, (url, is_shortened), link_params=link_params,
                    )
                    self.assertLinkShortenedText(
                        sms_sent["body"], (url, is_shortened), link_params=link_params,
                    )
