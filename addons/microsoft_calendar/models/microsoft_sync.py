...
class MicrosoftSync(models.AbstractModel):
    ...
    def _unlink_except_synchronized(self):
        ...
        if self.filtered("microsoft_id"):
            raise UserError(
                _(
                    "You cannot delete a record synchronized with Microsoft Calendar, archive it instead."
                )
            )
    ...
    def _sync_recurrence_microsoft2odoo(self, microsoft_events: MicrosoftEvent):
        ...
        for recurrent_master_id in set([x.seriesMasterId for x in recurrents]):
            recurrence_id = self.env["calendar.recurrence"].search(
                [("microsoft_id", "=", recurrent_master_id)]
            )
    ...
    def _update_microsoft_recurrence(self, recurrence_event, events):
        ...
        for recurrent_event in events_to_update:
            if recurrent_event.type == "occurrence":
                ...
                self.env["calendar.event"].browse(
                    recurrent_event.odoo_id(self.env)
                ).with_context(no_mail_to_attendees=True, mail_create_nolog=True).write(
                    dict(value, need_sync_m=False)
                )
    ...
    def _microsoft_insert(
        ...
        with microsoft_calendar_token(self.env.user.sudo()) as token:
            if token:
                ...
                self.write(
                    {"microsoft_id": microsoft_id, "need_sync_m": False,}
                )
