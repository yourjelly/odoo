...
class RecurrenceRule(models.Model):
    ...
    def _apply_recurrence(self, specific_values_creation=None, no_send_edit=False):
        ...
        detached_events = super()._apply_recurrence(
            specific_values_creation, no_send_edit
        )
        ...
        if not no_send_edit:
            for recurrence in self:
                values = recurrence._microsoft_values(
                    self._get_microsoft_synced_fields()
                )
                if not values:
                    continue
                values = values[0]
                if not recurrence.microsoft_id:
                    ...
                    recurrence._microsoft_patch(
                        microsoft_service, recurrence.microsoft_id, values
                    )
    ...
    def _microsoft_values(self, fields_to_sync):
        ...
        if not normal_event:
            return {}
        values = [
            normal_event._microsoft_values(
