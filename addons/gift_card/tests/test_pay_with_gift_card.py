...
class TestPayWithGiftCard(TestSaleGiftCardCommon):
    def test_paying_with_single_gift_card(self):
        gift_card = self.env["gift.card"].create({"initial_amount": 100,})
    ...
    def test_paying_with_multiple_gift_card(self):
        gift_card_1 = self.env["gift.card"].create({"initial_amount": 100,})
        gift_card_2 = self.env["gift.card"].create({"initial_amount": 100,})
        order = self.empty_order
        order.write(
            {
                "order_line": [
