...
class TestCrmCommon(TestSalesCommon, MailCase):
    ...
    def setUpClass(cls):
        ...
        cls.sales_team_1_m1.write(
            {"assignment_max": 45, "assignment_domain": False,}
        )
        cls.sales_team_1_m2.write(
            {"assignment_max": 15, "assignment_domain": False,}
        )
        ...
        cls.stage_team1_1 = cls.env["crm.stage"].create(
            {"name": "New", "sequence": 1, "team_id": cls.sales_team_1.id,}
        )
        cls.stage_team1_2 = cls.env["crm.stage"].create(
            {"name": "Proposition", "sequence": 5, "team_id": cls.sales_team_1.id,}
        )
        ...
        cls.stage_gen_1 = cls.env["crm.stage"].create(
            {"name": "Generic stage", "sequence": 3, "team_id": False,}
        )
        cls.stage_gen_won = cls.env["crm.stage"].create(
            {"name": "Generic Won", "sequence": 30, "team_id": False, "is_won": True,}
        )
...
class TestLeadConvertCommon(TestCrmCommon):
    ...
    def setUpClass(cls):
        ...
        cls.stage_team_convert_1 = cls.env["crm.stage"].create(
            {"name": "New", "sequence": 1, "team_id": cls.sales_team_convert.id,}
        )
    ...
    def _switch_to_multi_membership(cls):
        ...
        cls.sales_team_1_m2.write(
            {"assignment_max": 15, "assignment_domain": [("probability", ">=", 10)],}
        )
    ...
    def _switch_to_auto_assign(cls):
        ...
        cls.assign_cron.update(
            {"active": True, "interval_type": "days", "interval_number": 1,}
        )
