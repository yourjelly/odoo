        ...
        for lead in leads:
            if lead.id not in leads_done_ids:
                ...
                if self.company_id and self.company_id != lead_duplicates.company_id:
                    ...
                    leads_compatible_with_head_opportunity = lead_duplicates - leads_with_different_company
                    if len(leads_compatible_with_head_opportunity) < 2:
                        leads_with_different_company = lead_duplicates
                    else:
                        lead_duplicates = leads_compatible_with_head_opportunity
                    if len(leads_with_different_company) >= 1:
                        body = _('Leads (%s) could not be merged and have been skipped because it belong to different companies.',
                                 ', '.join(leads_with_different_company.mapped('name')))
                        leads_with_different_company._print_merging_leads_failed(body)
                    if len(leads_compatible_with_head_opportunity) < 2:
                        return {
