...
class CrmUpdateProbabilities(models.TransientModel):
    ...
    pls_start_date = fields.Date(required=False, default=_get_default_pls_start_date)   
    ...
    def action_update_crm_lead_probabilities(self):
        if self.env.user._is_admin():
            if self.pls_fields:
                ...
                self.env['ir.config_parameter'].sudo().set_param('crm.pls_fields', pls_fields_str)
            else:
                self.env['ir.config_parameter'].sudo().set_param('crm.pls_fields', "")
            if self.pls_start_date:
