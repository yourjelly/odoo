...
class AccountEdiFormat(models.Model):
    ...
    @api.model
    def _l10n_eg_eta_connect_to_server(self, request_data, request_url, method, access_token=False, production_enviroment=False):
        api_domain = access_token and self._l10n_eg_get_eta_token_domain(production_enviroment) or self._l10n_eg_get_eta_api_domain(production_enviroment)
        request_url = request_url % api_domain
        try:
    ...
    def _l10n_eg_edi_post_invoice_web_service(self, invoice):
        ...
        request_url = '%s/api/v1.0/documentsubmissions'
        request_data = {
    ...
    def _cancel_invoice_edi_eta(self, invoice):
        ...
        if access_data.get('error'):
            return access_data
        request_url = f'%s/api/v1/documents/state/{url_quote(invoice.l10n_eg_uuid)}/state'
        request_data = {
    ...
    def _l10n_eg_eta_get_access_token(self, invoice):
        ...
        secret = invoice.company_id.l10n_eg_client_secret
        access = '%s:%s' % (user, secret)
        user_and_pass = b64encode(access.encode()).decode()
        request_url = '%s/connect/token'
        request_data = {'body': {'grant_type': 'client_credentials'}, 'header': {'Authorization': f'Basic {user_and_pass}'}}
        response_data = self._l10n_eg_eta_connect_to_server(request_data, request_url, 'POST', access_token=True, production_enviroment=invoice.company_id.l10n_eg_production_env)
        if response_data.get('error'):
            return response_data
    ...
    def _l10n_eg_get_eta_invoice_pdf(self, invoice):
        ...
        if access_data.get('error'):
            return access_data
        request_url = f'%s/api/v1.0/documents/{url_quote(invoice.l10n_eg_uuid)}/pdf'
    ...
    def _l10n_eg_eta_prepare_invoice_lines_data(self, invoice, tax_data):
        ...
        for line in invoice.invoice_line_ids.filtered(lambda x: not x.display_type):
            ...
            price_subtotal_before_discount = self._l10n_eg_edi_round(abs(line.balance / (1 - (line.discount / 100))))
