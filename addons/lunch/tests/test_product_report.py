...
class TestLunchProductReport(TestsCommon):
    def test_product_available(self):
        self.assertTrue(
            self.env["lunch.product.report"].search([]),
            "There should be some record on lunch_product_report",
        )

    def test_order_in_report(self):
        pizza = self.env["lunch.product.report"].search(
            [("product_id", "=", self.product_pizza.id)], limit=1
        )
        ...
        new_pizza = self.env["lunch.product.report"].search(
            [
                ("product_id", "=", self.product_pizza.id),
                ("user_id", "=", pizza.user_id.id),
            ]
        )
