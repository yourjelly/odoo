...
class LunchProductCategory(models.Model):
    ...
    topping_ids_1 = fields.One2many(
        "lunch.topping", "category_id", domain=[("topping_category", "=", 1)]
    )
    topping_ids_2 = fields.One2many(
        "lunch.topping", "category_id", domain=[("topping_category", "=", 2)]
    )
    topping_ids_3 = fields.One2many(
        "lunch.topping", "category_id", domain=[("topping_category", "=", 3)]
    )
...
class LunchProduct(models.Model):
    ...
    def toggle_active(self):
        if self.filtered(
            ...
            raise UserError(
                _(
                    "The product category is archived. The user have to unarchive the category or change the category of the product."
                )
            )
        if self.filtered(
            ...
            raise UserError(
                _(
                    "The product supplier is archived. The user have to unarchive the supplier or change the supplier of the product."
                )
            )
