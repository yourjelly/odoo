...
class SaleOrderLine(models.Model):
    ...
    @api.onchange('product_packaging', 'product_uom_qty', 'product_uom')
    def _onchange_product_packaging(self):
        if self.product_packaging and self.product_uom_qty:
            newqty = self.product_packaging._check_qty(self.product_uom_qty, self.product_uom, "ceiling")
            if newqty != self.product_uom_qty:
                return {
                    'warning': {
                        'title': _('Warning'),
                        'message': _(
                            "This product is packaged by %(pack_size).2f %(pack_name)s. You should sell %(quantity).2f %(unit)s.",
                            pack_size=self.product_packaging.qty,
    ...
    def _prepare_procurement_values(self, group_id=False):
        ...
        values.update({
            ...
            'packaging_id': self.product_packaging,
        })
        return values
