...
class EventTrackController(http.Controller):
    ...
    def _event_tracks_get_values(self, event, tag=None, **searches):
        ...
        tracks_sudo_live = tracks_wdate.filtered(
            lambda track: track.is_published and track.is_track_live
        )
        tracks_sudo_soon = tracks_wdate.filtered(
            lambda track: track.is_published
            and not track.is_track_live
            and track.is_track_soon
        )
        tracks_by_day = []
    ...
    def _prepare_calendar_values(self, event):
        ...
        time_slots_by_tracks = {
            track: self._split_track_by_days(track, local_tz)
    ...
    def _event_agenda_get_tracks(self, event):
        ...
        if not request.env.user.has_group("event.group_event_manager"):
            tracks_sudo = tracks_sudo.filtered(
                lambda track: track.is_published or track.stage_id.is_accepted
            )
        return tracks_sudo
    ...
    def _event_track_page_get_values(self, event, track, **options):
        ...
        tracks_other = track._get_track_suggestions(
            restrict_domain=self._get_event_tracks_base_domain(track.event_id), limit=10
        )
    ...
    def event_track_proposal_post(self, event, **post):
        ...
        if post.get("add_contact_information"):
            ...
            if valid_contact_email or post.get("contact_phone"):
                ...
                raise exceptions.ValidationError(
                    _(
                        "Format Error : please enter a valid contact phone or contact email."
                    )
                )
        ...
        return request.redirect(
            "/event/%s/track_proposal/success/%s" % (event.id, track.id)
        )

    @http.route(
        ['/event/<model("event.event"):event>/track_proposal/success/<int:track_id>'],
        type="http",
        auth="public",
        methods=["GET"],
        website=True,
    ...
    def event_track_proposal_success(self, event, track_id):
        ...
        return request.render(
            "website_event_track.event_track_proposal", {"track": track, "event": event}
        )
