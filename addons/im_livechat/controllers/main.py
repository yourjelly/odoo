...
from odoo.addons.mail.tools.mail_guest import find_or_create_persona_for_channel, format_guest_cookie
...
class LivechatController(http.Controller):
    ...
    def get_session(self, channel_id, anonymous_name, previous_operator_id=None, chatbot_script_id=None, persisted=True, **kwargs):
        ...
        __, guest = find_or_create_persona_for_channel(channel, guest_name=self._get_guest_name(), post_joined_message=False)
        channel_info = channel._channel_info()[0]
        if guest:
            channel_info['guest_token'] = format_guest_cookie(guest)
        return channel_info
