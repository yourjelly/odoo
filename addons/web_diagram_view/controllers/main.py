...
class WebDiagramView(http.Controller):

    @http.route('/web_diagram/diagram/get_diagram', type='json', auth='user')
    def get_diagram_info(self, id, model, node, connector,
        ...
        graphs = ir_view.get_graph(int(id), model, node, connector, src_node,
        ...
        search_acts = node_act.search([('id', '=', id)])
        ...
        new_nodes = []
        new_connectors = []
        new_nodes.extend(value for key, value in nodes.items())
        new_connectors.extend(value for key, value in connectors.items())
        return dict(nodes=new_nodes,
                    conn=new_connectors,
                    display_name=name,
