...
class ResPartner(models.Model):
    ...
    def autocomplete(self, query, timeout=15):
        suggestions, _ = self.env["iap.autocomplete.api"]._request_partner_autocomplete(
            "search", {"query": query,}, timeout=timeout
        )
        if suggestions:
            results = []
    ...
    def enrich_company(self, company_domain, partner_gid, vat, timeout=15):
        response, error = self.env[
            ...
            "enrich",
            {"domain": company_domain, "partner_gid": partner_gid, "vat": vat,},
            timeout=timeout,
        )
    ...
    def read_by_vat(self, vat, timeout=15):
        vies_vat_data, _ = self.env[
            "iap.autocomplete.api"
        ]._request_partner_autocomplete("search_vat", {"vat": vat,}, timeout=timeout)
        if vies_vat_data:
