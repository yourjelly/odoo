...
class RFID(GPIODriver):
    ...
    def __init__(self):
        ...
        self.gpio_output('RST', 1)
        self.spi = spidev.SpiDev()
        self.spi.open(self.pins['SPI'].get('bus'), self.pins['SPI'].get('device'))
        self.spi.max_speed_hz = self.pins['SPI'].get('speed')
        GPIO.setup(self.get_pin('IRQ'), GPIO.IN, pull_up_down=GPIO.PUD_UP)
        GPIO.add_event_detect(self.get_pin('IRQ'), GPIO.FALLING, callback=self.irq_callback)
        self.init()
...
class RFIDIN(RFID):
    def pin_configuration(self):
        return {
            'RST': { 'pin': 22, 'signal': 'out', 'description': '' },
            'IRQ': { 'pin': 18, 'description': '' },
            'SPI': {
                'bus': 0,
                'device': 0,
                'speed': 1000000
...
class RFIDOUT(RFID):
    def pin_configuration(self):
        return {
            'RST': { 'pin': 33, 'signal': 'out', 'description': '' },
            'IRQ': { 'pin': 37, 'description': '' },
            'SPI': {
                'bus': 1,
                'device': 2,
                'speed': 1000000
