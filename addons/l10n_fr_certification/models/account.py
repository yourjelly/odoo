...
class AccountMove(models.Model):
    ...
    def _get_new_hash(self, secure_seq_number): #OK
    ...
    def _compute_hash(self, previous_hash): #OK
    ...
    def _compute_string_to_hash(self): #OK
    ...
    def write(self, vals):
        ...
        if has_been_posted:
            for move in self.filtered(lambda m: m.company_id._is_accounting_unalterable() and
                                                not (m.secure_sequence_number or m.inalterable_hash)):
    ...
    def button_cancel(self): # Useless
    ...
    @api.model
    def _check_hash_integrity(self, company_id): #OK
...
class AccountJournal(models.Model):
    ...
    def _onchange_restrict_mode_hash_table(self):
        if not self.restrict_mode_hash_table and self.company_id._is_accounting_unalterable():
    ...
    #     # restrict the operation in case we are trying to write a forbidden field
    #     for journal in self:
    #         if journal.company_id._is_accounting_unalterable(): # TODO flg: change condition here (if journal has hashed entries)
    #             if vals.get('restrict_mode_hash_table'):
    #                 field_string = journal._fields['restrict_mode_hash_table'].get_description(self.env)['string']
    #                 raise UserError(_("According to the French law, you cannot modify a journal in order for its posted data to be updated or deleted. Unauthorized field: %s.") % field_string)
