...
class ReportMembership(models.Model):
    ...
    def init(self):
        ...
        (SELECT
            ...
            CASE WHEN ml.state IN ('waiting', 'invoiced') THEN SUM(al.price_subtotal) ELSE 0 END AS tot_pending,
            CASE WHEN ml.state = 'paid' OR p.membership_state = 'old' THEN SUM(al.price_subtotal) ELSE 0 END AS tot_earned,
            ml.membership_id AS membership_id,
            p.company_id AS company_id
            FROM res_partner p
            LEFT JOIN membership_membership_line ml ON (ml.partner = p.id)
            LEFT JOIN account_move_line al ON (ml.account_invoice_line = al.id)
            LEFT JOIN account_move am ON (al.move_id = am.id)
