...
class ReturnPicking(models.TransientModel):
    ...
    product_return_moves = fields.One2many('stock.return.picking.line', 'wizard_id', 'Moves', compute='_compute_picking_dependancies', readonly=False)
    move_dest_exists = fields.Boolean('Chained Move Exists', compute='_compute_picking_dependancies')
    original_location_id = fields.Many2one('stock.location', compute='_compute_picking_dependancies')
    ...
    def _compute_picking_dependancies(self):
        for r in self:
            ...
            for move in r.picking_id.move_ids:
                ...
                product_return_moves_data.update(r._prepare_stock_return_picking_line_vals_from_move(move))
                product_return_moves.append((0, 0, product_return_moves_data))
            if r.picking_id and not product_return_moves:
                raise UserError(_("No products to return (only lines in Done state and not fully returned yet can be returned)."))
            if r.picking_id:
                r.product_return_moves = product_return_moves
                r.move_dest_exists = move_dest_exists
                r.original_location_id = r.picking_id.location_id.id
                location_id = r.picking_id.location_id.id
                if r.picking_id.picking_type_id.return_picking_type_id.default_location_dest_id.return_location:
                    location_id = r.picking_id.picking_type_id.return_picking_type_id.default_location_dest_id.id
