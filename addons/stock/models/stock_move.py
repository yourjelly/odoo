...
class StockMove(models.Model):
    ...
    orderpoint_id = fields.Many2one(
        "stock.warehouse.orderpoint", "Original Reordering Rule", check_company=True
    )
        ...
        for warehouse, moves in outgoing_unreserved_moves_per_warehouse.items():
            ...
            for move in moves:
                lines = [
                    ...
                    if l["move_out"] == move._origin
                    and l["replenishment_filled"] is True
                ]
                if lines:
    ...
    def _merge_moves(self, merge_into=False):
        ...
        for candidate_moves in candidate_moves_list:
            ...
            for k, g in groupby(
                sorted(candidate_moves, key=self._prepare_merge_move_sort_method),
                key=itemgetter(*distinct_fields),
            ):
                moves = (
                    self.env["stock.move"]
                    .concat(*g)
                    .filtered(lambda m: m.state not in ("done", "cancel", "draft"))
                )
    ...
    def _get_forecast_availability_incoming(self):
        self.ensure_one()
        warehouse = self.location_dest_id.warehouse_id
        self.forecast_availability = self.product_id.with_context(
            warehouse=warehouse.id, to_date=self.date
        ).virtual_available
        if self.state == "draft":
        ...
        for move_line in move_lines:
            ...
            if breaking_char in (move_line.lot_name or ""):
                ...
                move_lines_commands = self._generate_serial_move_line_commands(
                    split_lines[1:], origin_move_line=move_line,
                )
                if self.picking_type_id.show_reserved:
    ...
    def _assign_picking(self):
        ...
        for group, moves in grouped_moves:
            ...
            if picking:
                if any(
                    picking.partner_id.id != m.partner_id.id
                    or picking.origin != m.origin
                    for m in moves
                ):
    ...
    def _generate_serial_move_line_commands(self, lot_names, origin_move_line=None):
        ...
        if origin_move_line:
            ...
            location_dest = self.location_dest_id._get_putaway_strategy(
                self.product_id, quantity=1, packaging=self.product_packaging_id
            )
        move_line_vals = {
        ...
        for lot_name in lot_names:
            ...
            if move_lines:
                move_lines_commands.append(
                    (1, move_lines[0].id, {"lot_name": lot_name, "qty_done": 1,})
                )
    ...
    def _prepare_move_line_vals(self, quantity=None, reserved_quant=None):
        self.ensure_one()
        # apply putaway
        location_dest_id = self.location_dest_id._get_putaway_strategy(
            self.product_id, quantity=quantity or 0, packaging=self.product_packaging_id
        ).id
        vals = {
            "move_id": self.id,
            "product_id": self.product_id.id,
    ...
    def _update_reserved_quantity(
        ...
        for reserved_quant, quantity in quants:
            ...
            if to_update:
                ...
                uom_quantity = float_round(uom_quantity, precision_digits=rounding)
                uom_quantity_back_to_product_uom = to_update[
                    0
