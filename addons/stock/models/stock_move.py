...
class StockMove(models.Model):
        ...
        for move in self:
            ...
            if not move_lines:
                ...
                if float_compare(quantity_done, ml_quantity_done, precision_rounding=move.product_uom.rounding) != 0:
                    print('bypass cannot set done quantity error')
    ...
    def _compute_reserved_availability(self):
        ...
        for res in self.env['stock.move.line'].search_read([('move_id', 'in', self.ids)], ['move_id', 'product_qty']):
            result[res['move_id'][0]] += res['product_qty']
        for rec in self:
            rec.reserved_availability = rec.product_id.uom_id._compute_quantity(result.get(rec._origin.id or rec.id, 0.0), rec.product_uom, rounding_method='HALF-UP')
    ...
    def _set_quantity_done(self, qty):
        ...
        if vals['to_create']:
            self.env['stock.move.line'].create(vals['to_create'])
        if vals['to_write']:
