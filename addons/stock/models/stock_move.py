...
class StockMove(models.Model):
    ...
    def _push_apply(self):
        ...
        for move in self:
            ...
            if move.location_dest_id.company_id == self.env.company:
                rule = self.env['procurement.group']._search_rule(move.route_ids, move.packaging_id, move.product_id, warehouse_id, domain)
            else:
                rule = self.sudo().env['procurement.group']._search_rule(move.route_ids, move.packaging_id, move.product_id, warehouse_id, domain)
    ...
    def _adjust_procure_method(self):
        ...
        for move in self:
            ...
            rules = self.env['procurement.group']._search_rule(False, move.packaging_id, product_id, move.warehouse_id, domain)
            if rules:
