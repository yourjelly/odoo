...
class StockQuant(models.Model):
    ...
    inventory_quantity_set = (
        fields.Boolean()
    )  # Only used for UI purposes in the Inventory Adjustment page
    ...
    def _search_on_hand(self, operator, value):
        ...
        quant_ids = [
            l["id"] for l in self.env["stock.quant"].search_read(domain_loc, ["id"])
        ]
    ...
    def action_apply_inventory(self):
        ...
        quants_not_entered = self.filtered(
    ...
    def action_inventory_history(self):
        ...
        action = {
            ...
            "type": "ir.actions.act_window",
            "context": {"search_default_inventory": 1, "search_default_done": 1,},
            "domain": [
                ("product_id", "=", self.product_id.id),
    ...
    def _update_available_quantity(
        ...
        if quants:
            ...
            self._cr.execute(
                "SELECT id FROM stock_quant WHERE id IN %s LIMIT 1 FOR NO KEY UPDATE SKIP LOCKED",
                [tuple(quants.ids)],
            )
        ...
        if quant:
            quant.write(
                {"quantity": quant.quantity + quantity, "in_date": in_date,}
            )
        else:
            self.create(
                {
    ...
    def _get_quants_action(self, domain=None, extend=False):
        ...
        action.update(
            {"views": [(action["view_id"], "list"), (form_view, "form"),],}
        )
        if extend:
            action.update(
                {
            ...
            if quants:
                if not source_location_id:
                    ...
                    if recommended_location:
                        message = _(
                            "Serial number (%s) is not located in %s, but is located in location(s): %s. Source location for this move will be changed to %s",
                            lot_id.name,
                        ...
                        message = _(
                            "Serial number (%s) is not located in %s, but is located in location(s): %s. Please correct this to prevent inconsistent data.",
                            lot_id.name,
                            source_location_id.display_name,
                            ", ".join(sn_locations.mapped("display_name")),
                        )
...
class QuantPackage(models.Model):
    ...
    package_use = fields.Selection(
        [("disposable", "Disposable Box"), ("reusable", "Reusable Box"),],
        string="Package Use",
        default="disposable",
        required=True,
