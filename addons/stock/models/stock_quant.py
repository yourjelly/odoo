...
class StockQuant(models.Model):
    ...
    def _read_group_select(self, query, spec):
        if spec == 'inventory_quantity:sum' and self.env.context.get('inventory_report_mode'):
            return "NULL", []
        if spec == 'available_quantity:sum':
            quantity_expr, quantity_fnames = self._read_group_select(query, 'quantity:sum')
            reserved_quantity_expr, reserved_quantity_fnames = self._read_group_select(query, 'reserved_quantity:sum')
            return f'{quantity_expr} - {reserved_quantity_expr}', quantity_fnames + reserved_quantity_fnames
        if spec == 'inventory_quantity_auto_apply:sum':
            return self._read_group_select(query, 'quantity:sum')
        return super()._read_group_select(query, spec)
