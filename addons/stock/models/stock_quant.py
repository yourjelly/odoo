...
class StockQuant(models.Model):
    ...
    def action_apply_inventory(self):
        ...
        nb_quants_to_apply = len(self) - len(quants_not_entered)
        ctx = dict(self.env.context or {})
        ctx['default_quant_ids'] = self.ids
        if nb_quants_to_apply == 0:
            view = self.env.ref('stock.inventory_warning_nothing_to_apply_view', False)
            return {
                'name': _('No Quantities to Apply'),
                'type': 'ir.actions.act_window',
                'view_mode': 'form',
                'views': [(view.id, 'form')],
                'view_id': view.id,
        ...
        if self.env.context.get('apply_all'):
            ctx.update({
                'nb_quants_to_apply': nb_quants_to_apply,
                'quants_not_set_warning': nb_quants_to_apply != len(self),
            })
            view = self.env.ref('stock.stock_inventory_adjustment_name_form_view', False)
            return {
                'name': _('Inventory Adjustment Reference'),
                'type': 'ir.actions.act_window',
                'view_mode': 'form',
                'views': [(view.id, 'form')],
                'view_id': view.id,
    ...
    def _onchange_product_id(self):
        ...
        if product.tracking in ['lot', 'serial']:
            previous_quants = self.env['stock.quant'].search([('product_id', '=', product.id)], limit=1, order='create_date desc')
            if previous_quants:
            ...
            self.location_id = self.env['stock.warehouse'].search(
                [('company_id', '=', company_id)], limit=1)[0].in_type_id.default_location_dest_id
