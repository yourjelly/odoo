...
class Location(models.Model):
    ...
    warehouse_id = fields.Many2one(
        "stock.warehouse", compute="_compute_warehouse_id", recursive=True
    )
                    ...
                    if location.last_inventory_date:
                        ...
                        if days_until_next_inventory <= 0:
                            ...
                            location.next_inventory_date = (
                                ...
                                + timedelta(days=days_until_next_inventory)
                            )
                    else:
                        location.next_inventory_date = fields.Date.today() + timedelta(
    ...
    def _get_putaway_strategy(self, product, quantity=0, package=None, packaging=None):
        ...
        locations = self.env["stock.location"].search(
            [("id", "child_of", self.id), ("usage", "=", "internal")]
        )
        ...
        return (
            putaway_rules._get_putaway_location(
                product, quantity, package, qty_by_location
            )
            or self
        )
    ...
    def _check_product_storage(self, product, quantity, location_qty):
        ...
        if self.storage_category_id:
            ...
            if self.storage_category_id.allow_new_product == "empty" and any(
                float_compare(
            ...
            if (
                ...
                and self.quant_ids
                and self.quant_ids.product_id != product
            ):
                return False
    ...
    def _check_package_storage(self, product, package):
        ...
        if self.storage_category_id:
            ...
            if (
                self.storage_category_id.max_weight
                < self.forecast_weight + package.package_type_id.max_weight
            ):
                return False
            # check if only allow new product when empty
            if self.storage_category_id.allow_new_product == "empty" and any(
            ...
            if package_capacity:
                package_number = len(
                    self.quant_ids.package_id.filtered(
                        lambda q: q.package_type_id == package.package_type_id
                    )
                )
