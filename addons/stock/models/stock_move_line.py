...
class StockMoveLine(models.Model):
    ...
    location_id = fields.Many2one(
        "stock.location", "From", check_company=True, required=True
    )
    location_dest_id = fields.Many2one(
        "stock.location", "To", check_company=True, required=True
    )
        ...
        if self.result_package_id:
            if not self.id and self.user_has_groups(
                ...
                self.location_dest_id = self.location_dest_id._get_putaway_strategy(
                    self.product_id, package=self.result_package_id
                )
            ...
            new_move = self.env["stock.move"].create(
                {
                    ...
                    "product_uom_qty": 0
                    if move_line.picking_id and move_line.picking_id.state != "done"
                    else move_line.qty_done,
    ...
    def _action_done(self):
        ...
        for ml in mls_todo:
            if ml.product_id.type == "product":
                ...
                if (
                    not ml._should_bypass_reservation(ml.location_id)
                    and float_compare(
