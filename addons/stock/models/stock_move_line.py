...
class StockMoveLine(models.Model):
    ...
    def _onchange_result_package_id(self):
        if self.result_package_id:
            if not self.id and self.user_has_groups('stock.group_stock_multi_locations'):
                self.location_dest_id = self.move_id.location_dest_id._get_putaway_strategy(self.product_id, package=self.result_package_id) or self.location_dest_id
    ...
    def _onchange_product_id(self):
        if self.product_id:
            if not self.id and self.user_has_groups('stock.group_stock_multi_locations') and not self.result_package_id:
                ...
                self.location_dest_id = self.picking_id.location_dest_id._get_putaway_strategy(self.product_id, qty_done) or self.location_dest_id
            if self.picking_id:
    ...
    def _onchange_qty_done(self):
        ...
        if self.qty_done:
            qty_done = self.product_uom_id._compute_quantity(self.qty_done, self.product_id.uom_id)
            if not self.id and self.user_has_groups('stock.group_stock_multi_locations') and not self.result_package_id:
                self.location_dest_id = self.move_id.location_dest_id._get_putaway_strategy(self.product_id, qty_done) or self.location_dest_id
            if self.product_id.tracking == 'serial':
                qty_done = self.product_uom_id._compute_quantity(self.qty_done, self.product_id.uom_id)
