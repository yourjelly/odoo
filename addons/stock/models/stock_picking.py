...
class Picking(models.Model):
    ...
    move_line_nosuggest_ids = fields.One2many(
        "stock.move.line", "picking_id", domain=[("product_qty", "=", 0.0)]
    )
    ...
    def _compute_products_availability(self):
        ...
        pickings = self.filtered(
            lambda picking: picking.state not in ["cancel", "draft", "done"]
        ...
        for picking in pickings:
            ...
            if any(
                ...
                picking.products_availability_state = (
                    "late"
                    if picking.date_deadline and picking.date_deadline < forecast_date
                    else "expected"
                )
    ...
    def _check_backorder(self):
        ...
        for picking in self:
            ...
            if any(
                float_compare(
                    quantity_done[x], quantity_todo.get(x, 0), precision_digits=prec,
                )
                == -1
    ...
    def _attach_sign(self):
        ...
        self.message_post(
            attachments=[("%s.pdf" % filename, report[0])], body=message,
        )
        return True
