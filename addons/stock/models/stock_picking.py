...
class PickingType(models.Model):
    ...
    def _onchange_picking_code(self):
        if self.code == 'internal' and not self.env.user._has_group('stock.group_stock_multi_locations'):
            return {
                'warning': {
...
class Picking(models.Model):
    ...
    def _compute_show_lots_text(self):
        group_production_lot_enabled = self.env.user._has_group('stock.group_production_lot')
        for picking in self:
    ...
    def button_validate(self):
        ...
        if self.env.user._has_group('stock.group_reception_report'):
        ...
        if self.env.user._has_group('stock.group_production_lot'):
            ...
            for print_format in pickings_print_lot_label.picking_type_id.mapped("lot_label_format"):
                ...
                if action:
                    clean_action(action, self.env)
                    report_actions.append(action)
        if self.env.user._has_group('stock.group_tracking_lot'):
