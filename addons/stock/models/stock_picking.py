    ...
    def _check_backorder(self):
        ...
        for picking in self:
            if picking.picking_type_id.create_backorder != 'ask':
                continue
            quantity_todo = {}
            quantity_done = defaultdict(float)
            for move in picking.move_ids.filtered(lambda m: m.state != "cancel"):
                ...
                if move.picked:
                    quantity_done[move.product_id.id] += move.product_uom._compute_quantity(move.quantity, move.product_id.uom_id, rounding_method='HALF-UP')
            if any(
