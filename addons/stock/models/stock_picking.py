...
class Picking(models.Model):
    ...
    def _post_put_in_pack_hook(self, package_id):
        if package_id and self.picking_type_id.auto_print_package_label:
            report_actions = []
            if self.picking_type_id.package_label_to_print in ['pdf', 'both']:
                report_actions.append(self.env.ref("stock.action_report_quant_package_barcode_small").report_action(package_id.id))
            if self.picking_type_id.package_label_to_print in ['zpl', 'both']:
                report_actions.append(self.env.ref("stock.label_package_template").report_action(package_id.id))
            if report_actions:
                return self.picking_type_id._chain_reports(report_actions)
        return package_id
