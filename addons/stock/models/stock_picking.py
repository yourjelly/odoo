...
class PickingType(models.Model):
    ...
    group_procurements = fields.Boolean("Group Procurements In One Line", default=True, 
                                        help="When checked, it will group all procurements together")
...
class Picking(models.Model):
    ...
    def _prepare_pack_ops(self, quants, forced_qties):
        ...
        group_procurements = self.picking_type_id.group_procurements
        valid_quants = quants.filtered(lambda quant: quant.qty > 0)
        if group_procurements:
            _Mapping = namedtuple('Mapping', ('product', 'package', 'owner', 'location', 'location_dst_id', 'reservation_id'))
        else:
        ...
        for product, qty in forced_qties.items():
            if qty <= 0.0:
                continue
            key = _Mapping(product, self.env['stock.quant.package'], self.owner_id, self.location_id, computed_putaway_locations[product], False)
            qtys_grouped.setdefault(key, 0.0)
        ...
        for mapping, qty in qtys_grouped.items():
            ...
            val_dict = {
                ...
                'move_id': group_procurements and mapping.reservation_id or False,
                'pack_lot_ids': [
