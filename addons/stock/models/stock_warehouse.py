...
class Warehouse(models.Model):
    ...
    def _check_multiwarehouse_group(self):
        ...
        if cnt_by_company:
            max_cnt = max(cnt_by_company, key=lambda k: k[1])
            group_user = self.env.ref('base.group_user')
            group_stock_multi_warehouses = self.env.ref('stock.group_stock_multi_warehouses')
            group_stock_multi_locations = self.env.ref('stock.group_stock_multi_locations')
            if max_cnt[1] <= 1 and group_stock_multi_warehouses in group_user.implied_ids:
            ...
            if max_cnt[1] > 1 and group_stock_multi_warehouses not in group_user.implied_ids:
