...
class ProductTemplate(models.Model):
    ...
    description_pickingout = fields.Html(
    ...
    def _compute_quantities_dict(self):
        variants_available = {
            p["id"]: p
            for p in self.with_context(active_test=False).product_variant_ids.read(
    ...
    def action_open_quants(self):
        return (
            ...
            .product_variant_ids.filtered(lambda p: p.active or p.qty_available != 0)
            .action_open_quants()
        )
    ...
    def action_open_routes_diagram(self):
        ...
        if (
            ...
            return self.env.ref("stock.action_report_stock_rule").report_action(
                None,
                data={"product_id": products.id, "warehouse_ids": warehouse.ids,},
                config=False,
            )
        action = self.env["ir.actions.actions"]._for_xml_id(
