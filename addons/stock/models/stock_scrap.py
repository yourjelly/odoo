...
class StockScrap(models.Model):
    ...
    def _compute_location_id(self):
        groups = self.env['stock.warehouse']._read_group(
            [('company_id', 'in', self.company_id.ids)], ['company_id'], ['id:min'])
        locations_per_company = {
            company.id: stock_warehouse.lot_stock_id
            for company, stock_warehouse in groups
        }
        for scrap in self:
            if scrap.picking_id:
