...
class StockRule(models.Model):
    ...
    def _run_pull(self, procurements):
        ...
        for company_id, moves_values in moves_values_by_company.items():
            ...
            # Even if procurement_jit is disabled, We have to force the reservation of each moves
            # if the procure_method is 'mts_then_mto' to make sure nothing else take our available quantities
            confirmed_moves.filtered(lambda m: m.state == 'confirmed' and m.rule_id.procure_method == 'mts_then_mto' and m.procure_method == 'make_to_stock')._action_assign()
        return True
