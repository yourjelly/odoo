...
class StockPutawayRule(models.Model):
    ...
    def _populate_factories(self):
        company_ids = self.env.registry.populated_models['res.company'][:COMPANY_NB_WITH_STOCK]
        product_ids = self.env['product.product'].browse(self.env.registry.populated_models['product.product']).filtered(lambda p: p.type == 'consu' and p.is_trackable).ids
...
class StockWarehouseOrderpoint(models.Model):
    ...
    def _populate_factories(self):
        ...
        for suplierinfo in supplierinfos:
            ...
            if products and products[0].type == 'consu' and products[0].is_trackable:
