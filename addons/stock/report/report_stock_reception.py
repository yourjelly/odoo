...
class ReceptionReport(models.AbstractModel):
        ...
        for product_id, outs in products_to_outs.items():
            for out in outs:
                ...
                if not float_is_zero(qty_done, precision_rounding=product_uom.rounding):
                    sources_to_lines[source].append(self._prepare_report_line(qty_done, product_id, out, source, move_ins=moves_in))
    ...
    def action_assign(self, move_ids, qtys, in_ids):
        """ Links picking move(s) to the next step (i.e. move_ids in another picking)
        """
        for out, qty_to_link, ins in zip(move_ids, qtys, in_ids):
            ...
            if float_compare(out.product_qty, qty_to_link, precision_rounding=out.product_id.uom_id.rounding) == 1:
                # we are only assigning part of its demand => let's split it now to prevent reservation problems later
                # new move = the qty we're not assigning (and won't be visible in report)
