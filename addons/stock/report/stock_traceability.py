...
class StockTraceabilityReport(models.TransientModel):
    ...
    def _make_dict_move(self, level, parent_id, move_line, unfoldable=False):
        ...
        if move_line.picking_id.picking_type_code in ['incoming', 'outgoing']:
            location_source = picking_partner_name if move_line.picking_id.picking_type_code == 'incoming' else move_line.location_id.name
            location_destination = picking_partner_name if move_line.picking_id.picking_type_code == 'outgoing' else move_line.location_dest_id.name
        else:
