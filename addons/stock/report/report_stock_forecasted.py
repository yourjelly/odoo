    ...
    def _get_report_lines(
        ...
        def _reconcile_out_with_ins(
            ...
            for index, in_ in enumerate(ins):
                if float_is_zero(
                    in_["qty"], precision_rounding=out.product_id.uom_id.rounding
                ):
                    continue
                if (
                ...
                if float_is_zero(
                    demand, precision_rounding=out.product_id.uom_id.rounding
                ):
                    break
        ...
        for product in (ins | outs).product_id:
            ...
            for out in outs_per_product[product.id]:
                ...
                if not float_is_zero(
                    taken_from_stock, precision_rounding=product.uom_id.rounding
                ):
                ...
                if not float_is_zero(
                    demand, precision_rounding=product.uom_id.rounding
                ):
            ...
            for in_ in ins_per_product[product.id]:
                if float_is_zero(
                    in_["qty"], precision_rounding=product.uom_id.rounding
                ):
                    continue
