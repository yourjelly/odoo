...
class TestStockCommon(common.TestProductCommon):
    ...
    def _create_pack_operation(self, product, product_qty, picking_id, **values):
        PackOperation = self.PackOperation.sudo(self.user_stock_manager)
        vals = {
    ...
    def _create_move(self, product, src_location_id, dst_location_id, **values):
        ...
        move = Move.new({'product_id': product.id, 'location_id': src_location_id, 'location_dest_id': dst_location_id})
    ...
    def setUpClass(cls):
        ...
        cls.PackOperation = cls.env['stock.pack.operation']
