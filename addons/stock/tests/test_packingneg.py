...
class PackingNeg(TestStockCommon):
    ...
    def test_00_packing_neg_flow(self):
        ...
        self._create_move(self.product, self.supplier_location_id, self.stock_location_id, **{"product_uom_qty": 300, "picking_id": piking_in.id})

        # Confirm and assign picking and prepare partial
        piking_in.action_confirm()
        piking_in.action_assign()

        # Put 120 pieces on Packneg 1 (package), 120 pieces on Packneg 2 with lot A and 60 pieces on Packneg 3

        # create lot A
        lot_a = self.ProductionLot.create({'name': 'Lot neg', 'product_id': self.product.id})
        ...
        self._create_move(self.product, self.stock_location_id, self.customer_location_id, **{"product_uom_qty": 300.00, "picking_id": delivery_order_neg.id})
        ...
        self._create_move(self.product, self.supplier_location_id, self.stock_location_id, **{"product_uom_qty": 20.00, "picking_id": delivery_reconcile.id})
        ...
        neg_quants = self.env['stock.quant'].search([('product_id', '=', self.product.id), ('quantity', '<', 0), ('location_id.id', '!=', self.supplier_location_id)])
