...
class StockMove(TransactionCase):
    def test_in_1(self):
        ...
        self.assertEqual(move_line.product_reserved_qty, 0.0)
    ...
    def test_out_1(self):
        ...
        move_line = move1.pack_operation_ids[0]
        self.assertEqual(move_line.product_qty, 100.0)
        self.assertEqual(move_line.product_reserved_qty, 100.0)
        self.assertEqual(move_line.qty_done, 0.0)
        move_line.qty_done = 100.0
    ...
    def test_force_assign_1(self):
        """ Check that the behavior of `force_assign` method on a move is correct when the
        demand of a stock move is not in stock.
        """
        stock_location = self.env.ref('stock.stock_location_stock')
        supplier_location = self.env.ref('stock.stock_location_suppliers')
        uom_unit = self.env.ref('product.product_uom_unit')
    ...
    def test_force_assign_2(self):
        """ Check that the behavior of `force_assign` method on a move is correct when the
        demand of a stock move is partially in stock.
        """
        stock_location = self.env.ref('stock.stock_location_stock')
        supplier_location = self.env.ref('stock.stock_location_suppliers')
        uom_unit = self.env.ref('product.product_uom_unit')
        ...
        self.assertEqual(move_line.product_qty, 50.0)
        self.assertEqual(move_line.product_reserved_qty, 50.0)
        self.assertEqual(move1.reserved_availability, 50.0)
        ...
        self.assertEqual(move_line.product_qty, 100.0)
        self.assertEqual(move_line.product_reserved_qty, 50.0)
        # self.assertEqual(move1.reserved_availability, 100.0)  # FIXME: not sure it's the right behavior
