...
class StockMove(TransactionCase):
    ...
    def test_unreserve_3(self):
        ...
        # make some stock
        self.env['stock.quant'].increase_available_quantity(product1, stock_location, 2)
        self.assertEqual(len(self.env['stock.quant']._gather(product1, stock_location)), 1.0)
        self.assertEqual(self.env['stock.quant'].get_available_quantity(product1, stock_location), 2)
        ...
        self.assertEqual(self.env['stock.quant'].get_available_quantity(product1, stock_location), 0.0)
        quants = self.env['stock.quant']._gather(product1, stock_location)
    ...
    def test_link_assign_6(self):
        """ Test the assignment mechanism when four stock moves (2 sources, 2 dest) concerning
        multiple units of an untracked product. This particular test case simulates a two step
        receipts with backorder.
        """
        supplier_location = self.env.ref('stock.stock_location_suppliers')
        stock_location = self.env.ref('stock.stock_location_stock')
        uom_unit = self.env.ref('product.product_uom_unit')
    ...
    def test_use_unreserved_move_line_1(self):
        """ Test the assignment mechanism when validating a stocke move using a product reserved
        by another one.
        """
        stock_location = self.env.ref('stock.stock_location_stock')
        customer_location = self.env.ref('stock.stock_location_customers')
