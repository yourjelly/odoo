...
class TestSinglePicking(TestStockCommon):
    ...
    def test_recheck_availability_3(self):
        ...
        move_ids = move1.move_line_ids.sorted()
        self.assertEqual(move_ids[0].lot_id.id, lot1.id)
        self.assertEqual(move_ids[1].lot_id.id, lot2.id)
    ...
    def test_recheck_availability_4(self):
        ...
        self.assertEqual(len(move1.move_line_ids), 2)
        move_ids = move1.move_line_ids.sorted()
        self.assertEqual(move_ids[0].lot_id.id, serial1.id)
        self.assertEqual(move_ids[1].lot_id.id, serial2.id)
...
class TestStockUOM(TestStockCommon):
    ...
    def test_update_product_move_line_with_different_uom(self):
        ...
        move_ids_LtDA = self.env["stock.move.line"].search([
        ...
        reserved_on_move_ids_LtDA = sum(move_ids_LtDA.mapped('product_qty'))

        move_ids_GtDA = self.env["stock.move.line"].search([
        ...
        reserved_on_move_ids_GtDA = sum(move_ids_GtDA.mapped('product_qty'))

        # check that we do not reserve more (in the same UOM) than the quantity in stock
        self.assertEqual(float_compare(reserved_on_move_ids_LtDA, quant_LtDA.quantity, precision_digits=precision_digits), -1, "We do not reserve more (in the same UOM) than the quantity in stock")
        self.assertEqual(float_compare(reserved_on_move_ids_GtDA, quant_GtDA.quantity, precision_digits=precision_digits), -1, "We do not reserve more (in the same UOM) than the quantity in stock")

        # check that we reserve the same quantity in the ml and the quant
        self.assertTrue(float_is_zero(reserved_on_move_ids_LtDA - quant_LtDA.reserved_quantity, precision_digits=precision_digits))
        self.assertTrue(float_is_zero(reserved_on_move_ids_GtDA - quant_GtDA.reserved_quantity, precision_digits=precision_digits))
