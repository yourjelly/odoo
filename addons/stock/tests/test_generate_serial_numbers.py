...
class StockGenerate(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.location = cls.env["stock.location"].create(
            {"name": "Room A", "location_id": cls.warehouse.lot_stock_id.id,}
        )
        cls.location_dest = cls.env["stock.location"].create(
            {"name": "Room B", "location_id": cls.warehouse.lot_stock_id.id,}
        )
    ...
    def test_generate_03_raise_exception(self):
        ...
        form_wizard = Form(
            self.env["stock.assign.serial"].with_context(
                default_move_id=move.id, default_next_serial_number="code-xxx",
            )
        )
        wiz = form_wizard.save()
    ...
    def test_generate_04_generate_in_multiple_time(self):
        ...
        form_wizard = Form(
            self.env["stock.assign.serial"].with_context(default_move_id=move.id,)
        )
        # First assignment
