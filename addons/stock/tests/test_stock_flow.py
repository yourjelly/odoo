...
class TestStockFlow(TestStockCommon):
    ...
    def test_10_pickings_transfer_with_different_uom(self):
        ...
        productKG = self.ProductObj.create({'name': 'Product KG', 'uom_id': self.uom_kg.id, 'uom_po_id': self.uom_kg.id, 'type': 'consu',
            'is_trackable': True})
        picking_in = self.PickingObj.create({
            'picking_type_id': self.picking_type_in,
            'location_id': self.supplier_location,
    ...
    def test_20_create_inventory_with_packs_and_lots(self):
        ...
        packproduct = self.ProductObj.create({'name': 'Pack Product', 'uom_id': self.uom_unit.id, 'uom_po_id': self.uom_unit.id, 'type': 'consu',
            'is_trackable': True})
        lotproduct = self.ProductObj.create({'name': 'Lot Product', 'uom_id': self.uom_unit.id, 'uom_po_id': self.uom_unit.id, 'type': 'consu',
            'is_trackable': True})
    ...
    def test_picking_scheduled_date_readonlyness(self):
        ...
        product = self.env['product.product'].create({'name': 'Un petit coup de polish', 'type': 'consu',
            'is_trackable': True})
