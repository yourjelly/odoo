...
class TestStockFlow(TestStockCommon):
    ...
    def test_00_picking_create_and_transfer_quantity(self):
        ...
        # Check product A done quantity
        moves = self.MoveObj.search([('product_id', '=', self.productA.id), ('picking_id', '=', picking_in.id)])
        self.assertEqual(moves.product_uom_qty, 1.0, 'Wrong initial quantity for product A.')
        self.assertEqual(moves.quantity_done, 4.0, 'Wrong done quantity for product A.')
        # Check product B done quantity
        moves = self.MoveObj.search([('product_id', '=', self.productB.id), ('picking_id', '=', picking_in.id)])
        self.assertEqual(moves.product_uom_qty, 1.0, 'Wrong initial quantity for product B.')
        self.assertEqual(moves.quantity_done, 5.0, 'Wrong done quantity for product B.')
        # Check product C done quantity must be 7
        c_initial_qty = self.MoveObj.search([('product_id', '=', self.productC.id), ('picking_id', '=', picking_in.id)], limit=1).product_uom_qty
        self.assertEqual(c_initial_qty, 10.0, 'Wrong initial quantity of product C (%s found instead of 10)' % (c_initial_qty))
        c_done_qty = self.MoveObj.search([('product_id', '=', self.productC.id), ('picking_id', '=', picking_in.id)], limit=1).quantity_done
        self.assertEqual(c_done_qty, 7.0, 'Wrong done quantity of product C (%s found instead of 7)' % (c_done_qty))
        # Check product D initial and done quantities
        d_initial_qty = self.MoveObj.search([('product_id', '=', self.productD.id), ('picking_id', '=', picking_in.id)], limit=1).product_uom_qty
        self.assertEqual(d_initial_qty, 15.0, 'Wrong initial quantity of product D (%s found instead of 15)' % (d_initial_qty))
        d_done_qty = self.MoveObj.search([('product_id', '=', self.productD.id), ('picking_id', '=', picking_in.id)], limit=1).quantity_done
        self.assertEqual(d_done_qty, 7.0, 'Wrong done quantity of product D (%s found instead of 7)' % (d_done_qty))
    ...
    def test_10_pickings_transfer_with_different_uom(self):
        ...
        self.assertEqual(moves_DozA.quantity_done, 96, 'Wrong move quantity (%s found instead of 96)' % (moves_DozA.quantity_done))
        ...
        self.assertEqual(moves_SDozA.quantity_done, 1512, 'Wrong move quantity (%s found instead of 1512)' % (moves_SDozA.quantity_done))
        ...
        self.assertEqual(moves_SDozARound.quantity_done, 1584, 'Wrong move quantity (%s found instead of 1584)' % (moves_SDozARound.quantity_done))
        ...
        self.assertEqual(moves_kgB.quantity_done, 20, 'Wrong quantity in move (%s found instead of 20)' % (moves_kgB.quantity_done))
        ...
        self.assertEqual(moves_gB_kg.quantity_done, 0.526, 'Wrong move quantity (%s found instead of 0.526)' % (moves_gB_kg.quantity_done))
    ...
    def test_80_partial_picking_without_backorder(self):
        ...
        # Check that no cancelled move was created
        ...
        # Check that only the done quantity has been updated
        self.assertEqual(move_done.product_uom_qty, 10)
        self.assertEqual(picking.move_line_ids.qty_done, 4.0)
