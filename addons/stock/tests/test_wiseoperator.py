...
class TestWiseOperator(TestStockCommon):
    ...
    def test_00_wiseoperator(self):
        ...
        records = self.StockQuant.search([('product_id', '=', product_wise.id), ('quantity', '<', 0.0), ('location_id.id', '=', self.stock_location_id)])
        ...
        records = self.StockQuant.search([('product_id', '=', product_wise.id), ('location_id', '!=', self.supplier_location_id)])
