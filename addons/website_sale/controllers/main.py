...
class WebsiteSale(http.Controller):
    ...
    def _get_compute_currency(self, currency_from, currency_to):
        """ Return a lambda that computes a price from one currency to another. """
        company = request.website.company_id
        return lambda price: currency_from._convert(price, currency_to, company, fields.Date.today(), round=False)
    ...
    def shop(self, page=0, category=None, search='', min_price=0, max_price=0, ppg=False, **post):
        ...
        if search_product:
            ...
            prices = [tools.float_round(list_price * price_factor, 2) for list_price in search_product.mapped('list_price')]
            available_min_price = min(prices)
            available_max_price = max(prices)
            # This tackles the case where we switch to a list of products with different available
            # min / max prices than the ones set in the previous page.
            # In order to have logical results and not yield empty product lists, the price filter
            # is set to their respective available prices when the specified min exceeds the max,
            # and / or the specified max is lower than the available min.
            ...
            def _check_in_price_range(product):
                price_to_compare = product.price or product.lst_price
                return (not min_price or (float(min_price) <= price_to_compare)) and \
                       (not max_price or (price_to_compare <= float(max_price)))

            search_product = search_product.filtered(_check_in_price_range)
        else:
        ...
        values = {
            ...
            'min_price': float(min_price) if min_price else available_min_price,
            'max_price': float(max_price) if max_price else available_max_price,
        ...
        if (pl_id.selectable or pl_id == request.env.user.partner_id.property_product_pricelist) \
            ...
            if redirect_url:
                ...
                if min_price or max_price:
                    ...
                    previous_compute_currency = self._get_compute_currency(previous_price_list.currency_id, pl_id.currency_id)
                    args['min_price'] = min_price and str(previous_compute_currency(min_price))
                    args['max_price'] = max_price and str(previous_compute_currency(max_price))
