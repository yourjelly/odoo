...
class TestWebsiteSaleProductConfigurator(TestProductConfiguratorCommon, HttpCaseWithUserPortal, HttpCaseWithUserDemo):
    ...
    def test_02_variants_modal_window(self):
        ...
        self.assertEqual(new_order_line._get_short_description(), 'Short (TEST) (M always, M dynamic)')
        self.assertEqual(new_order_line.name, 'Never attribute size: M never\nNever attribute size custom: Yes never custom: TEST')
        self.assertEqual(new_order_line._get_full_description(), 'Short (TEST) (M always, M dynamic)\nNever attribute size: M never\nNever attribute size custom: Yes never custom: TEST')
