...
class Message(models.Model):
    ...
    def _compute_account_audit_log_preview(self):
        for message in self:
            ...
            tracking_value_ids = message.sudo().tracking_value_ids.filtered(lambda tracking: not tracking.field_groups or self.env.is_superuser() or self.env.user.has_group(tracking.field_groups))
