...
class ExemptedInGstReport(models.Model):
    _name = "exempted.in.gst.report"
    _description = "Exempted gst in supplied Statistics"
    _auto = False

    type_of_supply = fields.Selection([('intr', 'Inter-State supplies'),
                                    ('intra', 'Intra-State supplies')], "Type of supply")
            ...
            CREATE OR REPLACE VIEW %s AS (
                SELECT concat(sub.type_of_supply, '-', sub.invoice_month, '-', sub.company_id) as id,
                    ...
                    FROM ( SELECT
                         ...
                         FROM(SELECT
                                (CASE WHEN cp.state_id = p.state_id THEN 'intra' ELSE 'intr' END) as type_of_supply,
                                ail.id AS invoice_line_id,
                                ai.company_id,
                                cp.state_id AS company_state_id,
                                to_char(ai.date_invoice::timestamp with time zone, 'MM-YYYY'::text) AS invoice_month,
                                ...
                                FROM account_invoice_line ail
                                    ...
                                    LEFT JOIN ( SELECT  atax.id as id,
                                        ...
                                        GROUP BY atax.id,
                                            ...
                                            atax.tax_group_id,
                                            catax.tax_group_id) taxmin ON taxmin.a_invoice_line_id = ail.id
                                WHERE ai.state::text = ANY (ARRAY['open'::text, 'paid'::text]) and ai.type = 'in_invoice'
                                GROUP BY ail.id, ai.company_id,
                                    cp.state_id,
                                    ai.date_invoice, p.state_id, ai.type, p.composition) as ailtax
                ) as sub
                GROUP BY sub.type_of_supply, sub.company_id, sub.invoice_month)""" %(self._table, self.get_model_data('l10n_in','igst_group'),
                    self.get_model_data('l10n_in','cgst_group'),
                    self.get_model_data('l10n_in','sgst_group'),
