...
class StockPickingBatch(models.Model):
    ...
    def _compute_scheduled_date(self):
        """ scheduled_date = the earliest scheduled_date across all pickings. """
        self.scheduled_date = False
        for batch in self:
            dates_list = batch.picking_ids.mapped('scheduled_date')
            if dates_list:
                batch.scheduled_date = fields.Datetime.to_string(min(dates_list))
