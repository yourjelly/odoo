...
test("notifications grouped by notification_type", async (assert) => {
    const pyEnv = await startServer();
...
test("grouped notifications by document model", async (assert) => {
    ...
    patchWithCleanup(env.services.action, {
        doAction(action) {
            ...
            expect(JSON.stringify(action.views)).toBe(JSON.stringify([
                [false, "kanban"],
                [false, "list"],
                [false, "form"],
            ]))
            ...
            expect(JSON.stringify(action.domain)).toBe(JSON.stringify([["message_has_sms_error", "=", true]]))
        },
    });
