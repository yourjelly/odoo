...
class OnboardingController(http.Controller):
    ...
    def odoo_payments_return_from_redirect(self, account_code, account_holder_code, adyen_uuid, proxy_token):
        ...
        :param str account_holder_code: The account holder identifier from the submerchant db
        ...
        """
        draft_account_sudo = request.env['adyen.account'].sudo().search([
            ('account_holder_code', '=', account_holder_code),
            ('company_id', 'in', request.env.user.company_ids.ids),  # Must be accessible by current user
            ('merchant_status', '=', 'draft'),  # Makes no sense to update non draft accounts
        ])
        if draft_account_sudo:
            draft_account_sudo.with_context(update_from_adyen=True).write({
            ...
            _logger.warning("No draft account found matching given account holder code %s", account_holder_code)
