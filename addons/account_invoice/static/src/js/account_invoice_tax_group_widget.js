odoo.define('account.account_invoice_tax_group_widget', function (require) {
    "use strict";
    ...
    var TaxGroupCustomField = AbstractField.extend({
        ...
        _changeTaxValueByTaxGroup: function (taxGroupId, deltaAmount) {
            ...
            var line_id = self.record.data.invoice_tax_line_ids.data.find(elem => elem.data.tax_group_id && elem.data.tax_group_id.data.id === taxGroupId);

            // Trigger ORM
            self.trigger_up('field_changed', {
                dataPointID: self.record.id,
                changes: {line_ids: {operation: "UPDATE", id: line_id.id, data: {amount_tax: line_id.data.amount_tax + deltaAmount}}}, // account.invoice change
                initialEvent: {dataPointID: line_id.id, changes: {amount_tax: line_id.data.amount_tax}}, // account.invoice.tax change
            });
        },
        ...
        _render: function () {
            ...
            this.$el.html($(QWeb.render('AccountInvoiceTaxGroupWidgetTemplate', {
                lines: self.value,
    ...
    fieldRegistry.add('account_invoice_tax_group_widget', TaxGroupCustomField)
});
