...
class PaymentTransaction(models.Model):
    ...
    def _stripe_tokenize_from_notification_data(self, notification_data):
        ...
        if self.operation == 'online_direct':
            ...
            # Overwrite the PaymentMethod object retrieved by expanding the {Payment,Setup}Intent
            # request with the payment method details of the charge, as the former is empty in some
            # cases. E.g., when tokenizing a bancontact payment.
            payment_method = payment_intent['charges']['data'][0]['payment_method_details']
            if 'generated_sepa_debit' in payment_method[payment_method['type']]:
                # Save the ID of the payment method that was generated for the SEPA mandate.
                payment_method_id = payment_method[payment_method['type']]['generated_sepa_debit']
                mandate_id = payment_method[payment_method['type']]['generated_sepa_debit_mandate']
            else:
                mandate_id = payment_method[payment_method['type']].get('mandate')  # RBI e-mandate.
        else:  # 'validation'
            customer_id = notification_data['setup_intent']['customer']
            mandate_id = notification_data['setup_intent'].get('mandate')

        # Create the token from the notification and Stripe data.
        candidate_payment_details = [
            payment_method[payment_method['type']].get(details_key)
            for details_key in ('last4', 'iban_last4')
        ]
        payment_details = next((details for details in candidate_payment_details if details), None)
