...
class AccountEdiTestCommon(AccountTestInvoicingCommon):
    ...
    def setUp(self):
        super().setUp()
        # remove Administration groups from test user, as we want to test flow as
        # accounting manager only
        admin_groups = (self.env.ref('base.group_system') | self.env.ref('base.group_erp_manager'))
        self.env.user.groups_id -= admin_groups
