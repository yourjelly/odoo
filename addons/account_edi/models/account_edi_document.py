...
class AccountEdiDocument(models.Model):
    ...
    def _check_move_configuration(self):
        for document in self:
            ...
            if errors:
                # Errors are just informative at this point, _process_jobs will still be called on these documents
                document.error = self.env['account.edi.format']._format_error_message(_('Invalid configuration:'), errors)
                document.error_level = 'info'
            ...
            for document in documents:
                ...
                if move_result.get('attachment'):
                    ...
                    values = {
                        ...
                        # error_level default to 'error' => this document will not be processed again until forced retry or reset to draft
                        'error_level': move_result.get('error_level', DEFAULT_ERROR_LEVEL if 'error' in move_result else False)
                    }
                    ...
                    document.error_level = move_result.get('error_level', DEFAULT_ERROR_LEVEL) if document.error else False
