...
class AccountEdiDocument(models.Model):
    ...
    # Not Applicable: when there's no web service, 'to send', 'sent', 'to cancel', 'cancelled' make no sense
    state = fields.Selection([('to_send', 'To Send'), ('not_applicable', 'Not Applicable'), ('sent', 'Sent'), ('to_cancel', 'To Cancel'), ('cancelled', 'Cancelled')])
    ...
    def _prepare_jobs(self):
        ...
        * doc_type:       Are the moves of this job invoice or payments ?
        ...
        to_process = {}
        documents = self.filtered(lambda d: d.state in ('to_send', 'to_cancel', 'not_applicable') and d.blocking_level != 'error')
    ...
    def _process_job(self, documents, doc_type):
        ...
        def _postprocess_post_edi_results(documents, edi_result):
            ...
            for document in documents:
                ...
                if move_result.get('success') is True and document.state != 'not_applicable':
                    document.write({
                        'state': 'sent',
                        'error': False,
        ...
        if doc_type == 'invoice':
            if state in ['to_send', 'not_applicable']:
