...
class AccountMove(models.Model):
    ...
    def _add_edi_tax_values(
        self, results, grouping_key, serialized_grouping_key, tax_values
    ):
    ...
    def _prepare_edi_tax_details(
        self, filter_to_apply=None, grouping_key_generator=None
    ):
                ...
                tax_values_list.append(
                    {
                        ...
                        * invoice_line.company_currency_id.round(
                            tax_res["base"] / rate if rate else 0.0
                        ),
                        "tax_amount": sign
                        * invoice_line.company_currency_id.round(
                            tax_res["amount"] / rate if rate else 0.0
                        ),
        ...
        for payment in self:
            ...
            for edi_format in edi_formats:
                ...
                if edi_format._is_required_for_payment(payment):
                    ...
                    existing_edi_document.write(
                        {"state": False, "error": False, "blocking_level": False,}
                    )
        ...
        for move in posted:
            for edi_format in move.journal_id.edi_format_ids:
                ...
                if is_edi_needed:
                    ...
                    if existing_edi_document:
                        existing_edi_document.write(
                            {"state": "to_send", "attachment_id": False,}
                        )
                    else:
                        edi_document_vals_list.append(
                            {
    ...
    def button_cancel(self):
        ...
        self.edi_document_ids.filtered(lambda doc: not doc.attachment_id).write(
    ...
    def _prepare_edi_vals_to_export(self):
        ...
        res = {
            ...
            * (1 - (self.discount / 100.0)),
            "price_subtotal_before_discount": self.currency_id.round(
        ...
        res["price_discount"] = (
            res["price_subtotal_before_discount"] - self.price_subtotal
        )
