...
publicWidget.registry.ProductWishlist = publicWidget.Widget.extend(VariantMixin, {
    ...
    init: function (parent) {
        ...
        this.wishlistProductIDs = JSON.parse(
            JSON.stringify([sessionStorage.getItem("website_sale_wishlist_product_ids")]) || "[]"
        );
    },
    /**
    ...
    start: function () {
        ...
        if (this.el.querySelector("input.js_product_change")) {// manage "List View of variants"
            const checkedProductInput = this.el.querySelector("input.js_product_change:checked");
            // TODO-visp: Check this also
            $(checkedProductInput).trigger("change");
        } else {
            const inputProductId = this.el.querySelector("input.product_id");
            // TODO-visp: Check this also
