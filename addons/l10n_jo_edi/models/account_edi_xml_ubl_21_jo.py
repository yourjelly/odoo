...
def _aggregate_totals(vals):
    ...
    for line_val in vals['line_vals']:
        price_unit = _round_max_dp(line_val['price_vals']['price_amount'])
        quantity = _round_max_dp(line_val['line_quantity'])
        discount = _round_max_dp(line_val['price_vals']['allowance_charge_vals'][0]['amount'])
        ...
        if line_val['tax_total_vals']:
            ...
            total_tax_amount = _round_max_dp(sum(subtotal['tax_amount'] for subtotal in line_val['tax_total_vals'][0]['tax_subtotal_vals']))
...
class AccountEdiXmlUBL21JO(models.AbstractModel):
        ...
        for grouping_key, vals in taxes_vals['tax_details'].items():
            if grouping_key['tax_amount_type'] != 'fixed':
                subtotal = {
                    ...
                    'tax_amount': _round_max_dp(vals['raw_base_amount']) * vals['tax_category_percent'] / 100,
    ...
    def format_float(self, amount, precision_digits):
        ...
        rounded_amount = float_repr(_round_max_dp(amount), JO_MAX_DP).rstrip('0').rstrip('.')
