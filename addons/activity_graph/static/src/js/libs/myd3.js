d3.gantt = function() {
  ...
  var timeDomainStart = d3.timeDay.offset(new Date());
  var timeDomainEnd = d3.timeDay.offset(new Date());
  ...
  var initTimeDomain = function() {
    if (timeDomainMode === FIT_TIME_DOMAIN_MODE) {
      if (tasks === undefined || tasks.length < 1) {
        ...
        timeDomainEnd = d3.time.day.offset(new Date(),+5);
        return;
      }
 ...
 function initAxis() {
  ...
  function gantt(tasks) {
    ...
    svg.selectAll(".chart")
      ...
      tooltip.append('div').attr('class', 'taskStatus');
      svg.selectAll(".bar, .bar-running, .bar-killed, .bar-succeeded")
      .on('mouseover', function(d) {

        tooltip.style('display', 'inline-block');
        tooltip.style('opacity',10);
        tooltip.style('visibility',"visible");
        tooltip.text(d.status);
      })
