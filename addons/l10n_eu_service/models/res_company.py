...
class Company(models.Model):
    ...
    def _map_eu_taxes(self):
        ...
        oss_tax_groups = self.env["ir.model.data"].search(
            [("module", "=", "l10n_eu_service"), ("model", "=", "account.tax.group")]
        )
        for company in self:
            (
                ...
                for domestic_tax in taxes:
                    ...
                    if tax_amount and domestic_tax not in fpos.tax_ids.tax_src_id:
                        if not foreign_taxes.get(tax_amount, False):
                            tax_group_fid = "oss_tax_group_%s" % str(
                                tax_amount
                            ).replace(".", "_")
                            if not self.env["ir.model.data"].xmlid_to_object(
                                "l10n_eu_service.%s" % tax_group_fid
                            ):
    ...
    def _get_oss_account(self):
        self.ensure_one()
        if not self.env["ir.model.data"].xmlid_to_object(
            "l10n_eu_service.oss_tax_account_company_%s" % self.id
        ):
            sales_tax_accounts = (
                self.env["account.tax"]
