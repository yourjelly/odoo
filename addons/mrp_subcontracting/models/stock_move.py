...
class StockMove(models.Model):
    ...
    def _compute_show_subcontracting_details_visible(self):
        ...
        for move in self:
            if (
                move.is_subcontract
                and move._has_tracked_subcontract_components()
    ...
    def action_show_details(self):
        ...
        if self.is_subcontract and self._has_tracked_subcontract_components():
            ...
            action["context"].update(
                {"show_lots_m2o": self.has_tracking != "none", "show_lots_text": False,}
            )
        return action
    ...
    def action_show_subcontract_details(self):
        ...
        ctx = dict(
            self._context, search_default_by_product=True, subcontract_move_id=self.id
        )
        return {
