...
class StockPicking(models.Model):
    ...
    def _compute_display_action_record_components(self):
        for picking in self:
            ...
            if all(
                ...
                for subcontracted_move in component_sub_moves
            ):
                picking.display_action_record_components = False
                continue
            # Hide if all tracked product move line have a lot and all productions are in rigth state
            tracked_move_line = component_sub_moves.filtered(
                lambda sm: sm.has_tracking != "none"
            ).move_line_ids
            if all(
                sub_mo.state in ("to_close", "done")
                for sub_mo in subcontracted_productions
    ...
    def _get_subcontracted_productions(self):
        return self.move_lines.filtered(
            lambda move: move.is_subcontract
