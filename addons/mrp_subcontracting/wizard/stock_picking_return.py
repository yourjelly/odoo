...
class ReturnPicking(models.TransientModel):
    ...
    def _compute_subcontract_location_id(self):
        for record in self:
            ...
            )._get_property_stock_subcontractor()
    ...
    def _onchange_picking_id(self):
        ...
        if any(return_line.quantity > 0 and return_line.move_id.is_subcontract for return_line in self.product_return_moves):
            self.location_id = self.picking_id.partner_id.with_company(self.picking_id.company_id)._get_property_stock_subcontractor()
        return res
