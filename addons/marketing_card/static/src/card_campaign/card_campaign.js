        ...
        useEffect(
            () => {
                if (this.state.message || this.state.rewardUrl) {
                    const safeMessage = this.state.message instanceof markup("").constructor ? this.state.message : escape(this.state.message);
                    this.dialog.add(ConfirmationDialog, {
                        title: _t("Thank You!"),
                        body: renderToMarkup("marketing_card.ThanksMessage", {
                            message: markup(`<div>${safeMessage}</div>`), // avoids text-prewrap style
