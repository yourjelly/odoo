...
export class ImportDataView extends Component {
    setup() {
    ...
    async handleImport(isTest = true) {
        ...
        const { res, error } = await this.model.executeImport(
            isTest,
            this.totalSteps,
        ...
        if (error) {
            throw new Error("Error during import.", { cause: error });
        }
...
ImportDataView.template = "ImportDataView";
ImportDataView.nextId = 1;
ImportDataView.components = {
    ControlPanel,
    FileInput,
    ImportDataContent,
    ImportDataSidepanel,
...
registry.category("actions").add("import", ImportDataView);
