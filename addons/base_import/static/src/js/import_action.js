...
class OwlDataImport extends owl.Component {
    ...
    call_import(kwargs) {
        var fields = $('.oe_import_fields input.oe_import_match_field').map(function (index, el) {
            return $(el).select2('val') || false;
        }).get();
        var columns = $('.oe_import_grid-header .oe_import_grid-cell .o_import_header_name').map(function () {
    ...
    setup_encoding_picker() {
        ...
        // TODO: remove jquery dependancy
        jQuery('input.oe_import_encoding', document).select2({
            width: '50%',
    ...
    setup_separator_picker() {
        var data = [
            { id: ',', text: this.env._t("Comma") },
            { id: ';', text: this.env._t("Semicolon") },
            { id: '\t', text: this.env._t("Tab") },
            { id: ' ', text: this.env._t("Space") }
        ];
        // TODO: remove jquery dependancy
        jQuery('input.oe_import_separator', document).select2({
            width: '50%',
            data: data,
            query: dataFilteredQuery,
    ...
    setup_float_format_picker() {
        var data_decimal = [
            { id: ',', text: this.env._t("Comma") },
            { id: '.', text: this.env._t("Dot") },
        ];
        var data_digits = data_decimal.concat([{ id: '', text: _t("No Separator") }]);
        // TODO: remove jquery dependancy
        jQuery('input.oe_import_float_thousand_separator', document).select2({
            width: '50%',
            data: data_digits,
            query: dataFilteredQuery,
            initSelection: function ($e, c) {
        ...
        jQuery('input.oe_import_float_decimal_separator', document).select2({
            width: '50%',
            data: data_decimal,
            query: dataFilteredQuery,
            initSelection: function ($e, c) {
    ...
    setup_date_format_picker() {
        ...
        // TODO: remove jquery dependancy
        jQuery('input.oe_import_date_format', document).select2({
            width: '50%',
            data: data,
            query: dataFilteredQuery,
            initSelection: function ($e, c) {
...
const DataImportMixin = {
    ...
    getEnv() {
        return this.call('owl_import_service', 'getEnv');
    }
};
