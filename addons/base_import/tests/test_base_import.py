            ...
            self.env["base_import.import"]._get_mapping_suggestions(
                ...
                [
                    ...
                    {"name": "bob", "string": "Baz", "type": "text"},
                    {
                        ...
                        "string": "Qux",
                        "type": "many2one",
                        "fields": [{"name": "corge", "type": "text", "fields": []},],
                    },
                ],
            ),
            {
...
class TestPreview(TransactionCase):
    ...
    def test_csv_errors(self):
        ...
        result = import_wizard.parse_preview({"quoting": "foo", "separator": ",",})
        self.assertTrue("error" in result)

        result = import_wizard.parse_preview({"quoting": '"', "separator": "bob",})
        self.assertTrue("error" in result)
...
class TestBatching(TransactionCase):
    ...
    def test_recognize_batched(self):
        ...
        result = import_wizard.parse_preview(
            {"quoting": '"', "separator": ",", "has_headers": True, "limit": 100,}
        )
        self.assertIsNone(result.get("error"))
        self.assertIs(result["batch"], False)

        result = import_wizard.parse_preview(
            {"quoting": '"', "separator": ",", "has_headers": True, "limit": 5,}
        )
        self.assertIsNone(result.get("error"))
