...
class L10nInProductHsnReport(models.Model):
    ...
    def _from(self):
        from_str = """FROM account_move_line aml
            ...
            WHERE aa.internal_type = 'other' AND (aml.tax_line_id IS NOT NULL OR mt.account_tax_id IS NULL) AND (am.move_type IS NULL OR am.move_type in ('in_invoice', 'out_invoice'))
        """
        return from_str
