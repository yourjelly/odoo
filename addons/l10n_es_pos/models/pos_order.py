...
class PosOrder(models.Model):
    ...
    is_l10n_es_simplified_invoice = fields.Boolean(
        "Simplified invoice", compute="_compute_is_l10n_es_simplified_invoice"
    )
    l10n_es_unique_id = fields.Char(
        "Simplified invoice number", compute="_compute_l10n_es_unique_id"
    )

    @api.depends("account_move")
    def _compute_is_l10n_es_simplified_invoice(self):
        for order in self:
            order.is_l10n_es_simplified_invoice = bool(order.account_move)

    @api.depends("account_move")
    def _compute_l10n_es_unique_id(self):
        for order in self:
            order.l10n_es_unique_id = order.account_move.name
