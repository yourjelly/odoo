...
export class LayoutOption extends Component {
    ...
    getRow() {
        return this.props.editingElement.querySelector(".row");
    }
    isGrid() {
    ...
    setGridLayout() {
        const rowEl = this.props.editingElement.querySelector(".row");
        if (rowEl && rowEl.classList.contains("o_grid_mode")) {
            // Prevent toggling grid mode twice.
            return;
        }
        _toggleGridMode(this.props.editingElement.querySelector(".container"));
