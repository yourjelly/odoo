    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        if self.operation == "online_redirect":
            return_url = urls.url_join(
                base_url, OgoneController._hosted_payment_page_return_url
            )
            rendering_values = {
            ...
            rendering_values.update(
                {
                    ...
                    "api_url": self.acquirer_id._ogone_get_api_url(
                        "hosted_payment_page"
                    ),
                }
            )
            ...
            return_url = urls.url_join(
                base_url, OgoneController._flexcheckout_return_url
            )
            rendering_values = {
                "ACCOUNT_PSPID": self.acquirer_id.ogone_pspid,
                "ALIAS_ALIASID": payment_utils.singularize_reference_prefix(
                    prefix="ODOO-ALIAS"
                ),
    ...
    def _process_feedback_data(self, data):
        ...
        if feedback_type == "flexcheckout":
            ...
            if payment_status in const.PAYMENT_STATUS_MAPPING["pending"]:
                ...
                if self.token_id:
                    self.token_id.verified = (
                        True  # Validity of the token has been confirmed
                    )
                self._set_done()
            ...
            raise ValidationError(
                "Ogone: "
                + _("Received feedback data with unknown type: %s", feedback_type)
            )

    def _ogone_tokenize_from_feedback_data(
        self, card_number, acquirer_ref, is_one_shot_payment
    ):
        ...
        """
        token_name = (
            card_number or payment_utils.build_token_name()
