...
class CloudStorageSettings(models.TransientModel):
    ...
    cloud_storage_manual_delete = fields.Boolean(
        string="Manually Delete Cloud Storage Blobs",
        help='''Manually delete the blob from the cloud storage when the
        attachment is deleted from the server.''',
        config_parameter='cloud_storage_manual_delete',
    )

    def set_values(self):
        cloud_storage_configuration_before = self.env['cloud.storage.provider']._get_configuration()
        cloud_storage_manual_delete_before = self.env['ir.config_parameter'].get_param('cloud_storage_manual_delete')
        super().set_values()
        ...
        if cloud_storage_manual_delete_before != self.cloud_storage_manual_delete:
            self.env.ref('cloud_storage.ir_cron_cloud_storage_blobs_delete_action').active = self.cloud_storage_manual_delete
