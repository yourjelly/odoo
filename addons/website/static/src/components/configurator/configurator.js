...
const { App, Component, loadFile, onMounted, reactive, useEnv, useRef, useState, whenReady } = owl;
...
const getLoader = () => {
    if (!loader) {
        loader = stringToElement(renderToString("website.ThemePreview.Loader", { showTips: true }));
    }
    return loader;
};
...
const loadTemplates = async (urls) => {
    const templateStrings = await Promise.all(urls.map(loadFile));
    const templates = document.createElement("templates");
    for (const templateString of templateStrings) {
        const doc = new DOMParser().parseFromString(templateString, "text/xml");
        for (const child of doc.querySelectorAll("templates > *")) {
            templates.appendChild(child);
        }
    }
    return templates.outerHTML;
};
...
class Router {
    constructor() {
        ...
        window.onhashchange = () => this.updateLocation();
        window.onpopstate = () => this.updateLocation();
    }
    ...
    async start() {
        ...
        if (localState) {
            ...
            for (const feature of results.features) {
                this.features[feature.id] = { ...feature, selected: feature.module_state === "installed" };
...
const skipConfigurator = async () => {
    ...
    window.history.pushState({}, "", "/web#action=website.theme_install_kanban_action");
};
...
const applyConfigurator = async ({ store, router }, themeName) => {
    ...
    if (themeName !== undefined) {
        ...
        const selectedFeatures = Object.values(store.features)
