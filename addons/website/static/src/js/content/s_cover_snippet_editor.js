odoo.define('website.s_cover_snippet_editor', function (require) {
    'use strict';

    var core = require('web.core');
    var s_options = require('web_editor.snippets.options');
    require("website.s_cover_snippet_frontend");

    var QWeb = core.qweb;

    // COVER SNIPPET EDITOR
    ...
    s_options.registry.background.include({
        selectClass: function (previewMode, value, $li) {
            ...
            if (this.$target.hasClass("s_cover_snippet")) {
    ...
    s_options.registry.video_options = s_options.Class.extend({
        xmlDependencies: ['/website/static/src/xml/s_cover_snippet_modal.xml'],
        video_opt: function (previewMode, value, $li) {
            var self = this;
            self._s_media_block_animation().stop_video();
            self.$modal = $(QWeb.render("website.s_cover_snippet_modal"));
