...
var WysiwygMultizone = Wysiwyg.extend({
    ...
    start: function () {
        ...
        this._summernote.isDisabled = function () {
            return false;
        };

        this.$('.note-editable').addClass('o_not_editable').attr('contenteditable', false);
        this._getEditableArea().attr('contenteditable', true);
        this.$('[data-oe-readonly]').addClass('o_not_editable').attr('contenteditable', false);
        this.$('.oe_structure').attr('contenteditable', false).addClass('o_fake_not_editable');
        this.$('[data-oe-field][data-oe-type="image"]').attr('contenteditable', false).addClass('o_fake_not_editable');
        this.$('[data-oe-field]:not([contenteditable])').attr('contenteditable', true).addClass('o_fake_editable');
    },
    /**
     * @override
     */
    ...
    save: function () {
        ...
        return this.savingMutex.def.then(function () {
            return _super().then(function (_isDirty, html) {
                self._summernote.layoutInfo.editable.html(html);

                var $editable = self._getEditableArea();
                var $areaDirty = $editable.filter('.o_dirty');
                if (!$areaDirty.length) {
                    return false;
                }
                $areaDirty.each(function (index, editable) {
                    self.savingMutex.exec(self._saveEditable.bind(self, editable));
                });
                return self.savingMutex.def.then(function () {
                    return true;
                });
            });
        });
    ...
    _loadInstance: function () {
        ...
        return this._super().then(function () {
            ...
            self.$target.find('[id]').add(self.$target).each(function () {
                var $node = $(this);
                $node.attr('data-old-id', $node.attr('id')).removeAttr('id');
            });
            self.$('.note-editable:first').attr('id', id).addClass(className);
            self.selectorEditableArea = '.o_editable';
        });
    },
    /**
     * @private
    ...
    _saveEditable: function (editable) {
        var self = this;
        var recordInfo = this._getRecordInfo({
            target: editable,
        });
    ...
    _onDropImages: function (ev) {
        ...
        _.each(ev.data.promises, function (promise, index) {
