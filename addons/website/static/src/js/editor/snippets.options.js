...
options.userValueWidgetsRegistry['we-colorpicker'].include({
    async setValue() {
        ...
        const parentOptions = this.getParent().options;
        if (parentOptions.isWebsite && (compatibilityColorNames.includes(this._value) || this._ccValue)) {
            // The color palette automatically adjust the color preview
            // element with a class or a hardcoded value.
            // But when editing in website, a class is not enough, we need
            // a hardcoded value no matter what. This patch gets the value
            // from the iframe and applies it as a preview element.
            let ownerDocument;
            ...
            const value = this._ccValue || this._value;
            this.colorPreviewEl.style.backgroundColor = weUtils.getCSSVariableValue(value, computedStyles);
            this.colorPreviewEl.classList.remove(this.options.dataAttributes.colorPrefix + value);
        }
