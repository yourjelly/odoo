...
class TestPage(common.TransactionCase):
    def setUp(self):
        ...
        self.page_1 = Page.create({"view_id": self.base_view.id, "url": "/page_1",})

        self.page_1_menu = Menu.create(
            {"name": "Page 1 menu", "page_id": self.page_1.id, "website_id": 1,}
        )
...
class WithContext(HttpCase):
    def setUp(self):
        ...
        self.page = Page.create(
            {"view_id": base_view.id, "url": "/page_1", "is_published": True,}
        )
