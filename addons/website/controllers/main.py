...
class Website(Home):
    ...
    def website_configurator(self, step=1, **kwargs):
        ...
        if website_id.configurator_done is False:
            return request.render(
                "website.website_configurator", {"lang": request.env.user.lang}
            )
        else:
        ...
        for name, url, mod in current_website.get_suggested_controllers():
            if needle.lower() in name.lower() or needle.lower() in url.lower():
                ...
                suggested_controllers.append(
                    {"value": url, "label": "%s%s (%s)" % (icon, url, name),}
                )
    ...
    def get_dynamic_filter(
        ...
        return (
            dynamic_filter
            and dynamic_filter.render(template_key, limit, search_domain, with_sample)
            or ""
        )
    ...
    def get_dynamic_snippet_templates(self, filter_name=False):
        ...
        templates = (
            request.env["ir.ui.view"].sudo().search_read(domain, ["key", "name"])
        )
        return templates
    ...
    def theme_customize(self, enable=None, disable=None):
        ...
        self._get_customize_views(enable).filtered(lambda x: not x.active).write(
            {"active": True}
        )
    ...
    def theme_customize_bundle_reload(self):
        ...
        return {
            ...
            "website.assets_editor": request.env["ir.qweb"]._get_asset_link_urls(
                "website.assets_editor", options=context
            ),
        }
