...
class ModelPageController(Controller):
    ...
    def generic_manual_model(self, page_name_slugified=None, page_number=1, record_slug=None, **searches):
        ...
        if not layout_mode:
            ...
            layout_mode = request.env["website.controller.page"]._get_default_layout(view.id)
        ...
        if single_record_page:
            ...
            single_record_page = single_record_pages[0] if single_record_pages else False
        ...
        records = model_sudo.search(AND(domains), limit=self.pager_step, offset=self.pager_step * (page_number-1), order=searches["order"])
        render_context = {
