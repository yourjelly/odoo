...
class WebsiteControllerPage(models.Model):
    ...
    page_name = fields.Char(string="Page Name", help="The name of the page", required=True)
    ...
    page_type = fields.Selection(selection=[("listing", "Listing"), ("single", "Single record")],
        default="listing", string="Page Type", help="The type of the page. If set, it indicates whether the page displays a list of records or a single record")
    ...
    def _get_default_layout(self, view_id):
        return self.sudo().search_read([("view_id", "=", view_id)], ["default_layout"])[0]["default_layout"]
