...
from odoo.addons.http_routing.models.ir_http import url_localized
    ...
    def _post_processing_att(self, tagName, atts):
        ...
        if request:
            if name and name in atts:
                atts[name] = url_localized(atts[name])
            # Adapt background-image URL in the same way as image src.
            atts = self._adapt_style_background_image(atts, url_localized)
