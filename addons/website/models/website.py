...
class Website(models.Model):
    ...
    def configurator_init(self):
        ...
        try:
            result = self._website_api_rpc(
                "/api/website/1/configurator/industries", {"lang": self.env.user.lang}
            )
    ...
    def configurator_recommended_themes(self, industry_id, palette):
        ...
        return self._website_api_rpc(
            "/api/website/1/configurator/recommended_themes/%s" % industry_id, params
        )
    ...
    def configurator_apply(self, **kwargs):
        ...
        def set_features(selected_features):
            feature_ids = self.env["website.configurator.feature"].browse(
                selected_features
            )
            pages_views = {}
            ...
            for feature_id in feature_ids:
                if (
                    feature_id.type == "app"
                    and feature_id.module_id
                    and feature_id.module_id.state != "installed"
                ):
                ...
                if feature_id.type == "page" and feature_id.page_view_id:
                    result = self.env["website"].new_page(
                        ...
                        add_menu=True,
                        template=feature_id.page_view_id.key,
                    )
        ...
        custom_resources = self._website_api_rpc(
            "/api/website/1/configurator/custom_resources/%s"
            % kwargs.get("industry_id"),
            params,
        )
    ...
    <t t-call="website.layout">
        ...
        homepage_page = Page.search(
            [("website_id", "=", self.id), ("key", "=", standard_homepage.key),],
            limit=1,
        )
    ...
    def copy_menu_hierarchy(self, top_menu):
        def copy_menu(menu, t_menu):
            new_menu = menu.copy({"parent_id": t_menu.id, "website_id": self.id,})
    ...
    def new_page(
        ...
        view.with_context(lang=None).write(
            {"arch": template_record.arch.replace(template, key), "name": name,}
        )
