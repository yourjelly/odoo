...
class Website(models.Model):
    ...
    @tools.ormcache('path', 'qs', 'lang')
    # qs is only used for cache expr, while we use dict args as real value
    def _get_canonical_url_localized_cached(self, path, qs, args, lang):
    ...
    def _get_canonical_url_localized(self, lang, canonical_params):
        ...
        if request.endpoint:
            path = self._get_canonical_url_localized_cached(
                ...
                request.httprequest.query_string,  # for cachekey
