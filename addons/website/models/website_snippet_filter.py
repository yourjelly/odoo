...
class WebsiteSnippetFilter(models.Model):
    ...
    def render(self, template_key, limit, search_domain=None, with_sample=False):
        ...
        content = View._render_template(
            template_key, dict(records=records, is_sample=is_sample,)
        ).decode("utf-8")
        return [
            ET.tostring(el, encoding="utf-8")
            for el in ET.fromstring("<root>%s</root>" % content).getchildren()
        ]
            ...
            try:
                return (
                    self.action_server_id.with_context(
                        dynamic_filter=self, limit=limit, search_domain=search_domain,
                    )
                    .sudo()
                    .run()
                    or []
