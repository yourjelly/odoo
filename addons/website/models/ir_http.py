...
class Http(models.AbstractModel):
    ...
    def _generate_routing_rules(cls, modules, converters):
        ...
        for url, endpoint, routing in super(Http, cls)._generate_routing_rules(
            modules, converters
        ):
    ...
    def _serve_page(cls):
        ...
        if page and (request.website.is_publisher() or page.is_visible):
            ...
            response = request.render(
                page.view_id.id,
                {"deletable": True, "main_object": page,},
    ...
    def _get_error_html(cls, env, code, values):
        if code in ("page_404", "protected_403"):
            return (
                code.split("_")[1],
                env["ir.ui.view"]._render_template("website.%s" % code, values),
            )
