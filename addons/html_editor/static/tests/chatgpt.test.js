...
import { ChatGPTPlugin } from "../src/others/chatgpt_plugin";

import { MAIN_PLUGINS } from "@html_editor/plugin_sets";
import { DEFAULT_ALTERNATIVES_MODES } from "../src/others/chatgpt_alternatives_dialog";
...
test("ChatGPT dialog opens in prompt mode when selection is collapsed (with Powerbox)", async () => {
    const { editor } = await setupEditor("<p>te[]st</p>", { config: { Plugins: [...MAIN_PLUGINS, ChatGPTPlugin] } });
...
test("ChatGPT dialog opens in alternatives mode when selection is not collapsed (with toolbar)", async () => {
    const { editor } = await setupEditor("<p>te[s]t</p>", { config: { Plugins: [...MAIN_PLUGINS, ChatGPTPlugin] } });
...
test("ChatGPT alternatives dialog generates alternatives for each button", async () => {
    const { editor, el } = await setupEditor("<p>[test]</p>", { config: { Plugins: [...MAIN_PLUGINS, ChatGPTPlugin] } });
    let rpcIndex = 1;
    onRpc("/web_editor/generate_text", request => `Alternative #${rpcIndex++}`);
...
test("insert the response from ChatGPT prompt dialog", async () => {
    const { editor, el } = await setupEditor("<p>te[]st</p>", { config: { Plugins: [...MAIN_PLUGINS, ChatGPTPlugin] } });
    onRpc("/web_editor/generate_text", request => `
Here you go!
Enjoy :-)
    `);
    ...
    expect(getContent(el)).toBe(
        `<p>teHere you go!</p><p>Enjoy :-)[]st</p>`
    );
});

test("insert the response from ChatGPT alternatives dialog", async () => {
    const { editor, el } = await setupEditor("<p>t[es]t</p>", { config: { Plugins: [...MAIN_PLUGINS, ChatGPTPlugin] } });
    let rpcIndex = 1;
    onRpc("/web_editor/generate_text", request => `Alternative #${rpcIndex++}`);
