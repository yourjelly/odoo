...
describe("handleNewRecords", () => {
    ...
    test("undo/redo mutation apply with handleNewRecords resource", async () => {
        // When handling new records, handlers could themselves generate
        // mutations. Those mutations must be added in `currentStep`, otherwise
        // we will lose those mutation in some history manipulation. For
        // instance, if the next added step is reverted, the generated mutations
        // will be reverted.
        //
        // To test against that kind of bug, we hooks into normalize to signal
        // the next `handleNewRecords` to add a mutation (the adding of the
        // `handle` class).
        //
        // With the current implementation (at the time of writing this test),
        // this bug can only occurs if a normalized trigger the
        // `handleNewRecords` to generate mutation. The reason is that
        // `handleObserverRecords` is called twice in `addStep` so if any
        // mutations is generated by the first `handleNewRecords`, they would be
        // added to the step by the second call to `handleNewRecords`.
        let useCustomNormalize = false;
        const resources = {
            handleNewRecords: (records) => {
                ...
                if (
                    ...
                    record.target.className === "custom-normalized"
                ) {
                    editor.editable.children[0].classList.add(`handle`);
                }
            },
            normalize_handlers: () => {
                if (useCustomNormalize) {
                    editor.editable.children[0].classList.add("custom-normalized");
                    return;
                }
            },
        };
        ...
        useCustomNormalize = true;
        await insertText(editor, "b");
        useCustomNormalize = false;
        expect(getContent(editor.editable)).toBe(`<p class="custom-normalized handle">ab[]</p>`);

        await insertText(editor, "c");
        expect(getContent(editor.editable)).toBe(`<p class="custom-normalized handle">abc[]</p>`);
