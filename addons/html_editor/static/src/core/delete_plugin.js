...
export class DeletePlugin extends Plugin {
    ...
    */
    ...
    /**
     * Returns the next/previous position from [node, offset] for deletion.
     *
     * @param {Node} node
     * @param {number} offset
    ...
    findAdjacentPosition(node, offset, direction) {
        ...
        // Parameterize functions based on the direction.
        const findInTextNode = (
    ...
    isCursorAtStartOfElement(element, cursorNode, cursorOffset) {
        const [node] = this.findAdjacentPosition(cursorNode, cursorOffset, "backward");
        return !element.contains(node);
    }

    isCursorAtEndOfElement(element, cursorNode, cursorOffset) {
        const [node] = this.findAdjacentPosition(cursorNode, cursorOffset, "forward");
        return !element.contains(node);
    }
