...
class MoveNode extends Plugin {
    ...
    setup() {
        ...
        const avatarContainer = this._editor.mainAbsoluteContainer.querySelector(
        ...
        this._dragHelperContainer = this._editor.makeAbsoluteContainer(
        ...
        this._dropzoneHintContainer = this._editor.makeAbsoluteContainer(
        ...
        while (
            ...
            getComputedStyle(this._scrollableElement).overflowY !== "auto"
        ) {
        ...
        if (this._editor.document.defaultView !== window) {
            this._editor.addDomListener(
                this._editor.document.defaultView,
                "resize",
                this._updateHooks.bind(this)
            );
        }
    }
    destroy() {
    ...
    _updateHooks() {
        ...
        this._editableRect.width =
            this._editableRect.width - paddingLeft + (WIDGET_CONTAINER_WIDTH + 5);
    ...
    _updateAnchorWidgets(newAnchorWidget) {
        ...
        const listContainer =
            movableElement &&
            ancestors(movableElement, this._editable)
                .reverse()
    ...
    _getMovableElements() {
        return [...new Set([...this._editable.querySelectorAll(ALLOWED_ELEMENTS)])].filter((node) =>
            isNodeMovable(node)
        );
    }
    _getDroppableElements(draggableNode) {
        return this._getMovableElements().filter(
    ...
    _stopDropzones(movableElement) {
        ...
        if (this._currentDropHintElementPosition) {
            ...
            setSelection(movableElement, movableElement.childNodes.length);
...
registry.category("phoenix_plugins").add(MoveNode.name, MoveNode);
