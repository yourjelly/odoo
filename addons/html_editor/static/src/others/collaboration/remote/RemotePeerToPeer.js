...
export class RemotePeerToPeer extends RemoteInterface {
    ...
    handleNotification(dataPayload) {
        ...
        const payloadMethods = {
            rtc_data_channel_open: this.handleDataChannelOpen.bind(this),
            ptp_remove: this.handlePtpRemove.bind(this),
        };
        const method = payloadMethods[dataPayload.notificationName];
        if (method) {
            method(dataPayload);
            return;
        }
