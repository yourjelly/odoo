...
export class DeletePlugin extends Plugin {
    ...
    deleteElementBackward(params) {
        ...
        const responses = this.dispatch("DELETE_ELEMENT_BACKWARD:BEFORE", { ...params });
        if (responses.some((response) => response?.stop)) {
            return;
        }
        ...
        const shouldStop =
            ...
            this.deleteElementBackwardUnbreakableUnremovable();
    ...
    deleteElementBackwardUnbreakableUnremovable() {
        // @todo @phoenix specific to unremovable
        // if (!anchorOffset && isUnremovable(targetNode)) {
    ...
    deleteElementForward(params) {
        const responses = this.dispatch("DELETE_ELEMENT_FORWARD:BEFORE", { ...params });
        if (responses.some((response) => response?.stop)) {
            return;
        }
        ...
        const firstOutNode = getRightFirstOutNode(firstRightLeafNode, targetNode, targetOffset);
        ...
        const [leftNode, leftOffset] = leftPos(firstOutNode);
        if (firstOutNode && leftNode.tagName === "LI") {
            ...
            // @specific to li
            HTMLElement.prototype.oDeleteBackward.call(leftNode, leftOffset);
            return;
        }
        if (firstOutNode) {
            leftNode.oDeleteBackward(leftOffset);
            return;
        }
    }
...
export function getFirstRightLeafNode(node, offset) {
...
function getRightFirstOutNode(firstRightLeafNode, targetNode, targetOffset) {
    return findNode(
        rightLeaf(
