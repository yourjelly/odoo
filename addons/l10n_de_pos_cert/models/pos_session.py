...
class PosSession(models.Model):
    ...
    def _confirm_orders(self):
        orders = self.order_ids.filtered(lambda order: order.state == 'paid')
    ...
    def fiskaly_authentication(self):
        ...
        auth_response = requests.post(url + 'auth', json={
            'api_secret': self.config_id.company_id.fiskaly_secret,
            'api_key': self.config_id.company_id.fiskaly_key
        }, timeout=timeout)
    ...
    def _create_transaction_buyer(self, order):
        ...
        if order.partner_id:
            ...
            buyer['type'] = 'Kunde' if self.config_id.company_id == partner.company_id else 'Mitarbeiter'
    ...
    def _create_line_data(self, line, vat_definition_id):
        ...
        line_data = {
            ...
            'storno': False,
            'text': line.product_id.product_tmpl_id.name,
            'item': {
