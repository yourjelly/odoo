...
class SaleOrderLine(models.Model):
    ...
    def _get_delivered_qty(self):
        ...
        if bom and bom.type == 'phantom':
            ...
            move_product_dict = dict((product.id, 0.0) for product in moves.mapped('product_id'))
            ...
            for bom_line, data in lines:
                for move in moves:
                    if move.state == 'done' and move.product_id.id == bom_line.product_id.id:
                        move_product_dict[move.product_id.id] += move.product_uom._compute_quantity(move.product_uom_qty, bom_line.product_uom_id)
                ratio = move_product_dict[bom_line.product_id.id] / data['qty']
                min_transfer.append(ratio)
            if min_transfer:
                return min(min_transfer) * self.product_uom_qty
            else:
                return 0.0
