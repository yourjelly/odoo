...
% endif
    ...
    <span t-else="">Other Speaker</span>
        ...
        cls.base_qweb_templates = cls.env["ir.ui.view"].create(
            [
                {"name": "TestRender%d" % index, "type": "qweb", "arch": qweb_content,}
    ...
    def test_render_template_jinja(self):
        ...
        for source, expected in zip(self.base_jinja_bits, self.base_rendered):
            rendered = self.env["mail.render.mixin"]._render_template(
                source, partner._name, partner.ids, engine="jinja",
            )[partner.id]
            self.assertEqual(rendered, expected)
    ...
    def test_render_template_qweb_view(self):
        ...
        for source, expected in zip(
            ...
            rendered = (
                ...
                ._render_template(
                    source, partner._name, partner.ids, engine="qweb_view",
                )[partner.id]
                .decode()
            )
            self.assertEqual(rendered, expected)
    ...
    def test_template_rendering_impersonate(self):
        ...
        result = (
            self.env["mail.render.mixin"]
            .sudo()
            ._render_template_jinja(src, partner._name, partner.ids)[partner.id]
        )
...
<p>return value</p>"""
    ...
    def test_template_rendering_various(self):
        ...
        for engine in ["jinja", "qweb"]:
            ...
            self.assertEqual(
                expected, result if engine == "jinja" else result.decode()
            )  # tde: checkme
