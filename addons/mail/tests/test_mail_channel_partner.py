...
class TestMailChannelMembers(MailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.private_channel = cls.env["mail.channel"].create(
            {"name": "Secret channel", "public": "private", "channel_type": "channel",}
        )
    ...
    def test_channel_private_invite(self):
        ...
        with self.assertRaises(AccessError):
            self.private_channel.with_user(self.user_2).channel_invite(
        ...
        self.private_channel.with_user(self.user_1).channel_invite(
            [self.user_portal.partner_id.id]
        )
        channel_partners = self.env["mail.channel.partner"].search(
            [("channel_id", "=", self.private_channel.id)]
    ...
    def test_channel_group(self):
        ...
        with self.assertRaises(UserError):
            self.group_channel.with_user(self.user_1).channel_invite(
                [self.user_portal.partner_id.id]
            )
        channel_partners = self.env["mail.channel.partner"].search(
            [("channel_id", "=", self.group_channel.id)]
