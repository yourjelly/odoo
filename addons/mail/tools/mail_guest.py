...
def find_or_create_persona_for_channel(channel, guest_name, add_as_member=True, post_joined_message=True):
    """
    ...
    if not channel.env.user._is_public() and add_as_member:
        ...
        guest = find_or_create_guest_for_channel(
            channel=channel,
    ...
    return request.env.user.partner_id if not guest else None, guest


def find_or_create_guest_for_channel(channel, guest_name, country_id, timezone, add_as_member=True, post_joined_message=False):
...
def set_guest_cookie(guest):
    ...
    request.future_response.set_cookie(
        guest._cookie_name,
        format_guest_cookie(guest),
        httponly=True,
        expires=expiration_date,
    )


def format_guest_cookie(guest):
    ...
    """
    return f"{guest.id}{guest._cookie_separator}{guest.access_token}"
