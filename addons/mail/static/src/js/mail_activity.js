...
var ActivityLog = Widget.extend({
    ...
    update_timestamps: function () {
        var $el_create_date = $(this.$('.o_activity_timestamp')),
            datetime = moment.utc($el_create_date.data('datetime')).local();
        $el_create_date.text(datetime.fromNow()).attr('title', datetime.toString());
    },
});
