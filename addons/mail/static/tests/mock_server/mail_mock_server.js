...
async function mail_link_preview_hide(request) {
    ...
    const { link_preview_message_ids } = await parseRequestParams(request);
    const linkPreviewMessages = MailLinkPreviewMessage.browse(
        MailLinkPreviewMessage.search([["id", "in", link_preview_message_ids]])
    );
    for (const linkPreviewMessage of linkPreviewMessages) {
        ...
        BusBus._sendone(
            ...
            new mailDataHelpers.Store(
                MailMessage.browse(linkPreviewMessage.message_id)
            ).get_result()
        );
    }
}
