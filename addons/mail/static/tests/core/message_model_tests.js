...
QUnit.test("Message model properties", async (assert) => {
    ...
    assert.ok(env.services["mail.store"].discuss.inbox.messages.find((m) => m.id === message.id));
    assert.ok(env.services["mail.store"].discuss.starred.messages.find((m) => m.id === message.id));
