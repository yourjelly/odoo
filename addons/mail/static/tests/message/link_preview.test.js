...
test("auto layout with link preview list", async () => {
    ...
    pyEnv["mail.link.preview.message"].create({
...
test("No crash on receiving link preview of non-known message", async () => {
    ...
    const linkPreviewMessageId = pyEnv["mail.link.preview.message"].create({
        message_id: messageId,
        link_preview_id: linkPreviewId,
    });
    await start();
    ...
    rpc("/mail/link_preview/hide", { link_preview_message_ids: [linkPreviewMessageId] });
...
test("Delete all link previews at once", async () => {
    ...
    pyEnv["mail.link.preview.message"].create([
        { message_id: messageId, link_preview_id: linkPreviewId_1 },
        { message_id: messageId, link_preview_id: linkPreviewId_2 },
    ]);
    await start();
    await openDiscuss(channelId);
    await click(".o-mail-LinkPreviewCard button[aria-label='Remove']");
...
test("youtube and gdrive videos URL are embed", async () => {
    ...
    pyEnv["mail.link.preview.message"].create([
        { message_id: messageId_1, link_preview_id: linkPreviewId_1 },
        { message_id: messageId_2, link_preview_id: linkPreviewId_2 },
    ]);
    await start();
    await openDiscuss(channelId);
