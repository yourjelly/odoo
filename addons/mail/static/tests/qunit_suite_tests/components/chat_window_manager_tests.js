...
QUnit.module("mail", {}, function () {
    QUnit.module("components", {}, function () {
        ...
        QUnit.skipRefactoring("chat window new message: basic rendering", async function (assert) {
            assert.expect(10);
            ...
            assert.strictEqual(
                document.querySelectorAll(`.o_ChatWindow_header .o-mail-chat-window-header-name`)
                    .length,
                1,
                "should have name part in header"
            );
            assert.strictEqual(
                document.querySelector(`.o_ChatWindow_header .o-mail-chat-window-header-name`)
                    .textContent,
                "New message",
                "should display 'new message' in the header"
            );
            assert.strictEqual(
                document.querySelectorAll(`.o_ChatWindow_header .o_ChatWindowHeaderView_command`)
                    .length,
                1,
            ...
            assert.strictEqual(
                document.querySelectorAll(
                    `.o_ChatWindow_header .o_ChatWindowHeaderView_commandClose`
                ).length,
                1,
        ...
        QUnit.skipRefactoring("chat window new message: close", async function (assert) {
            assert.expect(1);
            ...
            async function (assert) {
                ...
                assert.strictEqual(
                    document.querySelector(
                        '.o_ChatWindow[data-visible-index="1"] .o-mail-chat-window-header-name'
                    ).textContent,
                    "Partner 131",
            ...
            async function (assert) {
                ...
                await afterEvent({
                    ...
                    func: () => {
                        ...
                        document
                            .querySelector(`.o_ChatWindow_newMessageFormInput`)
        ...
        QUnit.skipRefactoring("chat window: basic rendering", async function (assert) {
            assert.expect(15);
            ...
            assert.ok(chatWindow, "should have open a chat window of channel");
            assert.strictEqual(
            ...
            assert.strictEqual(
                chatWindowHeader.querySelectorAll(`:scope .o-mail-chat-window-header-name`).length,
                1,
                "should have thread name in header part"
            );
            assert.strictEqual(
                chatWindowHeader.querySelector(`:scope .o-mail-chat-window-header-name`)
                    .textContent,
                "General",
            ...
            assert.strictEqual(
                chatWindowHeader.querySelectorAll(`:scope .o_ChatWindowHeaderView_commandPhone`)
                    .length,
                1,
                "should have command to start an RTC call in audio mode"
            );
            assert.strictEqual(
                chatWindowHeader.querySelectorAll(`:scope .o_ChatWindowHeaderView_commandCamera`)
                    .length,
                1,
                "should have command to start an RTC call in video mode"
            );
            assert.strictEqual(
                chatWindowHeader.querySelectorAll(
                    `:scope .o_ChatWindowHeaderView_commandShowMemberList`
                ).length,
                1,
            ...
            assert.strictEqual(
                chatWindowHeader.querySelectorAll(
                    `:scope .o_ChatWindowHeaderView_commandShowCallSettingsMenu`
                ).length,
                1,
                "should have command to show the call settings menu"
            );
            assert.strictEqual(
                chatWindowHeader.querySelectorAll(`:scope .o_ChatWindowHeaderView_commandExpand`)
                    .length,
                1,
                "should have command to expand thread in discuss"
            );
            assert.strictEqual(
                chatWindowHeader.querySelectorAll(`:scope .o_ChatWindowHeaderView_commandClose`)
                    .length,
                1,
                "should have command to close chat window"
            );
            ...
            assert.ok(
                chatWindow
                    ...
                    .classList.contains("o_ThreadView"),
        ...
        QUnit.skipRefactoring("chat window: close on ESCAPE", async function (assert) {
            assert.expect(10);
        ...
        QUnit.skipRefactoring(
            ...
            async function (assert) {
                ...
                assert.containsN(
                    document.body,
                    ".o_ChatWindow .o-mail-composer-textarea",
                    2,
    ...
    `);
        ...
        .o_ChannelPreviewView[data-channel-id="${mailChannelId1}"]`);
            ...
            await afterNextRender(() =>
                triggerEvent(
                    chatWindow.querySelector(".o_ChatWindow .o-mail-composer-textarea"),
                    "keydown",
                    { key: "Tab" }
                )
            );
        ...
        QUnit.skipRefactoring(
            ...
            async function (assert) {
                ...
                await afterNextRender(() => {
                    document
                        .querySelector(
                            ".o_ChatWindow[data-visible-index='2'] .o-mail-composer-textarea"
                        )
                        .focus();
                });
                assert.strictEqual(
                ...
                assert.strictEqual(
                    document.querySelector(
                        ".o_ChatWindow[data-visible-index='1'] .o-mail-composer-textarea"
                    ),
                    document.activeElement,
                ...
                assert.strictEqual(
                    document.querySelector(
                        ".o_ChatWindow[data-visible-index='0'] .o-mail-composer-textarea"
                    ),
                    document.activeElement,
