...
class MailController(http.Controller):
    ...
    def read_subscription_data(self, follower_id):
        ...
        request.env[follower.res_model].browse(follower.res_id).check_access_rule(
            "read"
        )
    ...
    @http.route(
        "/mail/<string:res_model>/<int:res_id>/avatar/<int:partner_id>",
        type="http",
        auth="public",
    )
    ...
    def mail_init_messaging(self):
        values = {
            "needaction_inbox_counter": request.env[
                "res.partner"
            ].get_needaction_count(),
            ...
            "mention_partner_suggestions": request.env[
                "res.partner"
            ].get_static_mention_suggestions(),
            "shortcodes": request.env["mail.shortcode"]
            .sudo()
            .search_read([], ["source", "substitution", "description"]),
            "menu_id": request.env["ir.model.data"].xmlid_to_res_id(
            ...
            .mail_partner_format(),
            "public_partners": [
                partner.mail_partner_format()
                for partner in request.env.ref("base.group_public")
                .sudo()
