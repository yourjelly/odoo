...
class ThreadController(http.Controller):
    ...
    @http.route("/mail/thread/get_followers", methods=["POST"], type="jsonrpc", auth="public")
    def mail_thread_get_followers(self, id, thread_id, thread_model, limit=20, offset=0):
        domain = [
            ("res_id", "=", thread_id),
            ("res_model", "=", thread_model),
        ]
        data = request.env["mail.followers"].search(
                domain + [("partner_id", "!=", request.env.user.partner_id.id)], offset=offset, limit=limit + 1, order="name ASC"
            )
        followerListView = {
                "id": id,
        ...
        if not offset:
            self_follower = request.env["mail.followers"].search(domain + [("partner_id", "=", request.env.user.partner_id.id)],)
            followerListView["selfFollower"] = Store.one_id(self_follower)
            followerListView["followers"] = Store.many_ids(data)
            data = data + self_follower
        else:
