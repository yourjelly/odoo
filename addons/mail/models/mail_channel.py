...
from odoo import _, api, fields, models, modules, tools, SUPERUSER_ID
...
class Channel(models.Model):
    ...
    def channel_get(self, partners_to, pin=True):
        ...
        with registry(self.env.cr.dbname).cursor() as cr:
            ...
            self.flush()
            self.with_env(env).env.cr.execute("""
                SELECT P.channel_id
            ...
            result = self.with_env(env).env.cr.dictfetchall()
            if result:
                ...
                # create a new one
                channel = self.with_env(env).create({
