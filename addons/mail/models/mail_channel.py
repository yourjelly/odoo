...
class Channel(models.Model):
    ...
    channel_type = fields.Selection(
        [("chat", "Chat Discussion"), ("channel", "Channel")],
        string="Channel Type",
        default="channel",
    )
    ...
    # access
    uuid = fields.Char(
        "UUID", size=50, index=True, default=lambda self: str(uuid4()), copy=False
    )
    ...
    def _invite_check_access(self, partners):
        ...
        if any(channel.public == "groups" for channel in self):
            for channel in self.filtered(lambda c: c.public == "groups"):
                invalid_partners = [
                    ...
                    not in partner.mapped("user_ids.groups_id")
                ]
        ...
        if failed:
            raise UserError(
                _(
                    "Following invites are invalid as user groups do not match: %s",
                    ", ".join(
                        "%s (channel %s)" % (partner.name, channel.name)
                        for channel, partner in failed
                    ),
                )
            )
    ...
    def _alias_get_error_message(self, message, message_dict, alias):
        if alias.alias_contact == "followers" and self.ids:
            ...
            return False
        return super(Channel, self)._alias_get_error_message(
    ...
    def _notify_compute_recipients(self, message, msg_vals):
        ...
        if pids:
            ...
            self.env.cr.execute(
                sql_query, (email_from, list(pids), [author_id] if author_id else [],)
            )
    ...
    def _channel_message_notifications(self, message, message_format=False):
        ...
        for channel in self:
            notifications.append(
                [(self._cr.dbname, "mail.channel", channel.id), dict(message_format)]
            )
    ...
    def _get_channel_partner_info(self, all_partners, direct_partners):
        ...
        partner_infos = {
            partner["id"]: partner
            for partner in all_partners.sudo().read(["id", "name", "email"])
        }
        # add im _status for direct_partners
        direct_partners_im_status = {
            partner["id"]: partner
            for partner in direct_partners.sudo().read(["im_status"])
        }
    ...
    def channel_info(self, extra_info=False):
        ...
        all_partner_channel = self.env["mail.channel.partner"].search(
        ...
        direct_channel_partners = all_partner_channel.filtered(
            lambda pc: channel_dict[pc.channel_id.id].channel_type == "chat"
        )
        ...
        for channel in self:
            ...
            channel_partners = all_partner_channel.filtered(
                lambda pc: channel.id == pc.channel_id.id
            )
            ...
            if self.env.user and self.env.user.partner_id:
                ...
                partner_channel = channel_partners.filtered(
                    lambda pc: pc.partner_id.id == self.env.user.partner_id.id
                )
                if partner_channel:
        ...
        for session_state in self.env["mail.channel.partner"].search(domain):
            if not state:
                ...
                if session_state.fold_state == "open":
                    ...
                    state = "open"
            session_state.write(
                {"fold_state": state, "is_minimized": bool(state != "closed"),}
            )
            self.env["bus.bus"].sendone(
    ...
    def _execute_channel_pin(self, pinned=False):
        ...
        self.env["bus.bus"].sendone(
            (self._cr.dbname, "res.partner", self.env.user.partner_id.id),
            self.channel_info("unsubscribe" if not pinned else False)[0],
        )
        if channel_partners:
    ...
    def channel_seen(self, last_message_id=None):
        ...
        if self.channel_type == "chat":
            self.env["bus.bus"].sendmany(
                [[(self._cr.dbname, "mail.channel", self.id), data]]
            )
        else:
            data["channel_id"] = self.id
            self.env["bus.bus"].sendone(
                (self._cr.dbname, "res.partner", self.env.user.partner_id.id), data
            )
    ...
    def _set_last_seen_message(self, last_message):
        ...
        channel_partner.write(
            {"fetched_message_id": last_message.id, "seen_message_id": last_message.id,}
        )
