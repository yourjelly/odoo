...
class MailThread(models.AbstractModel):
    ...
    def _partner_find_from_emails_single(self, emails, avoid_alias=True, force_create=True):
        """ Shortcut version of '_partner_find_from_emails', two usages
          ...
          * MailThrad._partner_find_from_emails_single([..]) to use as a generic
        ...
        return self._partner_find_from_emails(
            {self: emails}, avoid_alias=avoid_alias, force_create=force_create
        )[self.id]

    def _partner_find_from_emails(self, records_emails, avoid_alias=True, force_create=True):
        """ Find or create partners based on emails. Result is contextualized
        based on records, calling '_get_customer_information' to populate
        ...
        for mail, partner in zip(emails_all, partners):
            ...
            for res_id in emails_key_res_ids[mail_key]:
                # use an "OR" because of potential duplicates input which should
                # not generate duplicates in returned recordset
