...
class LinkPreview(models.Model):
    ...
    def _create_link_previews(self, message):
        ...
        for url in set(urls):
            if (len(link_previews) >= 5):
                break
            link_previews |= self.env['mail.link.preview'].sudo()._create_link_preview(url, message.id)
    ...
    @api.model
    def _create_link_preview(self, url, message_id):
    ...
    def _get_link_preview_from_url(self, url):
        try:
            response = requests.head(url, timeout=3, headers=self.request_headers, allow_redirects=True)
    ...
    def _get_link_preview_from_html(self, url):
        response = requests.get(url, timeout=3, headers=self.request_headers, allow_redirects=True)
