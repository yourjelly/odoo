...
class Message(models.Model):
    ...
    * snailmail.letter (snailmail addon)
    ...
    def check_access_rule(self, operation):
        ...
        # trick: messages_to_check don't contain missing records
        messages_to_check = {
            values['id']: values
        ...
        model_doc_ids_msg_ids = defaultdict(lambda: defaultdict(list))
        for mid, message in messages_to_check.items():
            if (message.get('model') and message.get('res_id') and
                ...
                model_doc_ids_msg_ids[message['model']][message['res_id']].append(mid)

        for model, doc_ids_msg_ids in model_doc_ids_msg_ids.items():
            DocumentModel = self.env[model]
            if hasattr(DocumentModel, '_get_mail_message_access'):
                check_operation = DocumentModel._get_mail_message_access(doc_ids_msg_ids, operation)  # why not giving model here?
            else:
                check_operation = self.env['mail.thread']._get_mail_message_access(doc_ids_msg_ids, operation, model_name=model)
            documents = DocumentModel.browse(doc_ids_msg_ids)
            documents.check_access_rights(check_operation)
            for document in documents._filter_access_rules(check_operation):
                for mid in doc_ids_msg_ids.pop(document.id):
                    messages_to_check.pop(mid)
        ...
        if operation == 'create':
            ...
            for model, doc_ids_msg_ids in model_doc_ids_msg_ids.items():
                domain = [
                    ('res_model', '=', model),
                    ('res_id', 'in', list(doc_ids_msg_ids)),
                    ('partner_id', '=', self.env.user.partner_id.id),
                ]
                followers = self.env['mail.followers'].sudo().search_fetch(domain, ['res_id'])
                for follower in followers:
                    for mid in doc_ids_msg_ids[follower.res_id]:
                        messages_to_check.pop(mid)
