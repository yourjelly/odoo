...
from odoo.tools.rendering_tools import parse_inline_template, template_env_globals
...
class MailTemplate(models.Model):
    ...
    def _validate_reference(self, variable, expression, model=False):
        ...
        allowed_variables = {
            'object': self.env[self.env['ir.model'].browse(model).model or self.render_model],
            'env': self.env,
            'user': self.env.user
        }
        ...
        while i < len(refs):
            ...
            if isinstance(curr, models.BaseModel) and not refs[i] in curr._fields:
                # Stop at function as we do not know the return value of the function
                break
            elif isinstance(curr, models.BaseModel):
