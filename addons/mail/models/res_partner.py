...
class ResPartner(models.Model):
    ...
    def _find_or_create_from_emails(self, emails, additional_values=None,
        ...
        Optional additional values allow to customize the created partner when
        context allows to give more information. Data are given per normalized
        email as it is main criterion for this method.

        If an email is invalid but not void, the given value is used to find
        partners with same invalid email or create a new one with that value.
        It allows updating it afterwards. Notably with notifications resend it
        is possible to update emails, if only a typo prevents from having a
        real email for example.
