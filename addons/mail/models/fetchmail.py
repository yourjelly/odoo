...
class FetchmailServer(models.Model):
    ...
    server = fields.Char(string='Server Name', help="Hostname or IP of the mail server", readonly=[('state', '!=', 'draft')])
    port = fields.Integer(readonly=[('state', '!=', 'draft')])
    ...
    user = fields.Char(string='Username', readonly=[('state', '!=', 'draft')])
    password = fields.Char(readonly=[('state', '!=', 'draft')])
    ...
    priority = fields.Integer(string='Server Priority', readonly=[('state', '!=', 'draft')], help="Defines the order of processing, lower values mean higher priority", default=5)
