...
class PosOrder(models.Model):
    ...
    def _compute_l10n_es_tbai_state(self):
        for order in self:
            state = 'to_send' if order.l10n_es_tbai_is_required and not self.account_move else None
    ...
    def _process_saved_order(self, draft):
        ...
        if not self.to_invoice and self.amount_total > self.config_id.l10n_es_tbai_simplified_invoice_limit:
            raise UserError(_("Please create an invoice for an amount over %s.", self.config_id.l10n_es_tbai_simplified_invoice_limit))
