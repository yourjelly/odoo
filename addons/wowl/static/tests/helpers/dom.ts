...
const keyboardEventBubble = (args: any) =>
  Object.assign({}, args, { bubbles: true, keyCode: args.which });
const mouseEventMapping = (args: any) =>
  Object.assign({}, args, {
    bubbles: true,
    cancelable: true,
...
const mouseEventNoBubble = (args: any) =>
  Object.assign({}, args, {
    bubbles: false,
    cancelable: false,
    clientX: args ? args.pageX : undefined,
...
const noBubble = (args: any) => Object.assign({}, args, { bubbles: false });
const onlyBubble = (args: any) => Object.assign({}, args, { bubbles: true });
...
export type AdmissibleTarget = Component | HTMLElement | string;
export type Finder = number | string;

export function getNode(target: AdmissibleTarget): HTMLElement {
  let nodes: (Element | null)[];
  ...
  switch (typeof elFinder) {
    ...
    case "string": {
      const match = elements.find(
        (el) => (el as HTMLElement).innerText.trim().toLowerCase() === elFinder.toLowerCase()
      );
      if (!match) {
        throw new Error(
...
async function returnAfterNextAnimationFrame(): Promise<void> {
 ...
 * @returns {Promise}
 */
export async function triggerEvent(el: AdmissibleTarget, eventType: string, eventAttrs = {}) {
  let event: Event;
 ...
 * @returns {Promise}
 */
export async function triggerEvents(el: AdmissibleTarget, events: string[]) {
