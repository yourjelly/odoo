...
function openStudio(webClient) {
    return testUtils.dom.click(webClient.el.querySelector(".o_main_navbar .o_web_studio_navbar_item a"));
}
...
QUnit.module('web_studio', (hooks) => {
    hooks.beforeEach(() => {
        ...
        testConfig.serverData.menus = {
            ...
            11: { id: 11, children: [], name: "Partners (Action 4)", appID: 1, actionID: 4, xmlid: "menu_11" },
            12: { id: 12, children: [], name: "Partners (Action 3)", appID: 1, actionID: 3, xmlid: "menu_12" },
        ...
        testConfig.serverData.models.partner.fields.pony_id = {
            string: "Pony", type: "many2one", relation: "pony",
        };
        testConfig.serverData.models.pony.fields.partner_ids = {
            string: "Partners", type: "one2many", relation: "partner", relation_field: "pony_id",
        };
    ...
    QUnit.test('open Studio with act_window', async function (assert) {
        ...
        assert.containsOnce(webClient, '.o_kanban_record:contains(yop)',
        ...
        assert.containsNone(webClient, '.o_web_studio_client_action',
            "Studio should be closed");
        assert.containsOnce(webClient, '.o_kanban_view .o_kanban_record:contains(yop)',
    ...
    QUnit.test('open Studio with act_window and viewType', async function (assert) {
        ...
        await testUtils.dom.click(webClient.el.querySelector('.o_app[data-menu-xmlid=app_1]'));
        await legacyExtraNextTick();
        await testUtils.dom.click($(webClient.el).find('.o_menu_sections li:contains("Partners (Action 3)")'));
        await legacyExtraNextTick();
        ...
        await openStudio(webClient);
        assert.containsOnce(webClient, '.o_web_studio_client_action .o_web_studio_form_view_editor',
    ...
    QUnit.test('switch view and close Studio', async function (assert) {
        ...
        // click on tab "Views"
        await testUtils.dom.click(webClient.el.querySelector('.o_web_studio_menu .o_web_studio_menu_item a'));
        assert.containsOnce(webClient, '.o_web_studio_action_editor');

        // open list view
        await testUtils.dom.click(webClient.el.querySelector('.o_web_studio_views .o_web_studio_view_type[data-type=list] .o_web_studio_thumbnail'));
    ...
    QUnit.test('navigation in Studio with act_window', async function (assert) {
        ...
        assert.containsOnce(webClient, '.o_web_studio_client_action .o_web_studio_kanban_view_editor',
            "the kanban view should be opened");
        assert.strictEqual($(webClient.el).find('.o_kanban_record:contains(yop)').length, 1,
            "the first partner should be displayed");
        ...
        assert.strictEqual($(webClient.el).find('.o_list_view .o_data_cell').text(), "Twilight SparkleApplejackFluttershy",
    ...
    QUnit.test('open same record when leaving form', async function (assert) {
        ...
        await testUtils.dom.click($(webClient.el).find('.o_list_view tbody tr:first td:contains(Twilight Sparkle)'));
        await legacyExtraNextTick();
        ...
        assert.containsOnce(webClient, '.o_form_view',
            "the form view should be opened");
        assert.strictEqual($(webClient.el).find('.o_form_view span:contains(Twilight Sparkle)').length, 1,
    ...
    QUnit.test('open list view with sample data gives empty list view in studio', async function (assert) {
        ...
        testConfig.serverData.views['pony,false,list'] = `<tree sample="1"><field name="name"/></tree>`;
        ...
        assert.containsNone(webClient, '.o_list_table .o_data_row',
    ...
    QUnit.test('open Studio with editable form view and check context propagation', async function (assert) {
        ...
        const mockRPC = async (route, args) => {
            ...
            if (route === '/web/dataset/call_kw/partner/onchange') {
                assert.ok(!('default_type' in args.kwargs.context),
        ...
        await testUtils.dom.click(webClient.el.querySelector('.o_web_studio_form_view_editor .o_field_one2many'));
        await testUtils.dom.click(webClient.el.querySelector('.o_web_studio_form_view_editor .o_field_one2many .o_web_studio_editX2Many[data-type="form"]'));
