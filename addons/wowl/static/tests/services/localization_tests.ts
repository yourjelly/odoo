...
QUnit.test("formatDateTime", async (assert) => {
  ...
  const date = l10n.parseDateTime(isoDateStr) as DateTime;
...
QUnit.test("parseDateTime", async (assert) => {
  ...
  let date1 = l10n.parseDateTime(dateStr) as DateTime;
  ...
  date1 = l10n.parseDateTime(dateStr) as DateTime;
...
QUnit.test("parseDateTime (norwegian locale)", async (assert) => {
  ...
  const date1 = l10n.parseDateTime(dateStr) as DateTime;
...
QUnit.test("parseDate without separator", async (assert) => {
  ...
  // );
  // ---
  assert.equal((l10n.parseDate("1197") as DateTime).toFormat(dateFormat), "01.01/1197");
  assert.equal((l10n.parseDate("0131") as DateTime).toFormat(dateFormat), "01.01/0131");
  ...
  assert.equal(
    (l10n.parseDate("3101") as DateTime).toFormat(dateFormat),
    "31.01/" + DateTime.utc().year
  );
  assert.equal(
    (l10n.parseDate("31.01") as DateTime).toFormat(dateFormat),
    "31.01/" + DateTime.utc().year
  );
  assert.equal((l10n.parseDate("310197") as DateTime).toFormat(dateFormat), "31.01/1997");
  assert.equal((l10n.parseDate("310117") as DateTime).toFormat(dateFormat), "31.01/2017");
  assert.equal((l10n.parseDate("31011985") as DateTime).toFormat(dateFormat), "31.01/1985");
});
...
QUnit.test("parseDateTime without separator", async (assert) => {
  ...
  assert.equal(
    (l10n.parseDateTime("3101198508") as DateTime).toFormat(dateTimeFormat),
    "31.01/1985 08:00/00"
  );
  assert.equal(
    (l10n.parseDateTime("310119850833") as DateTime).toFormat(dateTimeFormat),
    "31.01/1985 08:33/00"
  );
  assert.equal(
    (l10n.parseDateTime("31/01/1985 08") as DateTime).toFormat(dateTimeFormat),
    "31.01/1985 08:00/00"
  );
});
...
QUnit.test("parse smart date input", async (assert) => {
  ...
  assert.strictEqual(
    (l10n.parseDate("+1d") as DateTime).toFormat(format),
    DateTime.local().plus({ days: 1 }).toFormat(format)
  );
  assert.strictEqual(
    (l10n.parseDateTime("+2w") as DateTime).toFormat(format),
    DateTime.local().plus({ weeks: 2 }).toFormat(format)
  );
  assert.strictEqual(
    (l10n.parseDate("+3m") as DateTime).toFormat(format),
    DateTime.local().plus({ months: 3 }).toFormat(format)
  );
  assert.strictEqual(
    (l10n.parseDateTime("+4y") as DateTime).toFormat(format),
    DateTime.local().plus({ years: 4 }).toFormat(format)
  );

  assert.strictEqual(
    (l10n.parseDate("+5") as DateTime).toFormat(format),
    DateTime.local().plus({ days: 5 }).toFormat(format)
  );
  assert.strictEqual(
    (l10n.parseDateTime("-5") as DateTime).toFormat(format),
    DateTime.local().minus({ days: 5 }).toFormat(format)
  );

  assert.strictEqual(
    (l10n.parseDate("-4y") as DateTime).toFormat(format),
    DateTime.local().minus({ years: 4 }).toFormat(format)
  );
  assert.strictEqual(
    (l10n.parseDateTime("-3m") as DateTime).toFormat(format),
    DateTime.local().minus({ months: 3 }).toFormat(format)
  );
  assert.strictEqual(
    (l10n.parseDate("-2w") as DateTime).toFormat(format),
    DateTime.local().minus({ weeks: 2 }).toFormat(format)
  );
  assert.strictEqual(
    (l10n.parseDateTime("-1d") as DateTime).toFormat(format),
