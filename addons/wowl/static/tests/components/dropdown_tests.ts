...
QUnit.test("can be styled", async (assert) => {
  ...
  const toggler = parent.el!.querySelector("button") as HTMLElement;
...
QUnit.test("payload received on item selection", async (assert) => {
  class Parent extends owl.Component {
    ...
    `;
    onItemSelected(ev: OwlEvent<any>) {
...
QUnit.test("multi-level dropdown: recursive template can be rendered", async (assert) => {
  ...
  parent = await mount(Parent, { env });
  assert.deepEqual(
    [...parent.el!.querySelectorAll<HTMLElement>("button,li")].map((el) => el.textContent),
    [
      "foo",
      "foo-0",
      "foo-00",
...
QUnit.test(
  ...
  async (assert) => {
    ...
    // Hover on THREE
    const three = parent.el!.querySelector<HTMLElement>(".three");
...
QUnit.test(
  ...
  async (assert) => {
    ...
    // Hover on BAR1
    const bar1 = parent.el!.querySelector<HTMLElement>(".bar1");
