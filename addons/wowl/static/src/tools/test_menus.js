...
(function (exports) {
    ...
    async function getNextMenu() {
        const menus = document.querySelectorAll(".o_menu_sections .o_dropdown_toggler, .o_menu_sections .o_dropdown_item");
        ...
        if (menu.classList.contains('o_dropdown_toggler')) {
            ...
            if (!menu.nextSibling) { // might already be opened if the last menu was blacklisted
        ...
        try {
            ...
            if (xmlId) {
                if (isEnterprise) {
                    ...
                    app = document.querySelector(`.o_navbar_apps_menu .o_dropdown_item[data-menu-xmlid="${xmlId}"]`);
                }
                if (!app) {
