import { SearchModel } from "../view_utils/search_model";
import { ModelBuilder, ReadGroupResult } from "../../services/model";
...
interface GraphModelLoadParams {
  fields: Fields;
  modelName: string;
}
...
export class GraphModel extends EventBus {
  ...
  constructor(modelService: ModelBuilder, searchModel: SearchModel) {
    super();
  ...
  get processedGroupBy(): string[] {
    ...
    //   return fieldName;
    // });
    return this.searchModel.groupBy.map((g) => g.toJSON()); // adapt in future
  }

  async load(params: GraphModelLoadParams) {
  ...
  sortDataPoints(dataPoints: DataPoint[]): DataPoint[] {
    if (
      this.searchModel.domains.length === 1 &&
      this.order &&
      this.mode !== "pie" &&
  ...
  async loadDataPoints() {
    ...
    this.searchModel.domains.forEach((domain, originIndex) => {
      proms.push(
