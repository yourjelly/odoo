...
import { GraphModel, Mode, MODES, Order, ORDERS, DateClasses, DataSet } from "./graph_model";
import { Fields, FieldDefinition } from "./types";
...
import { getGroupBy, GroupBy } from "../view_utils/group_by";
...
export class GraphView extends Component<GraphViewProps, OdooEnv> {
  static type = "graph";
  ...
  searchModel = this.metadata.search;
  model = new GraphModel(this.modelService, this.searchModel);

  async willStart() {
    await this.metadata.load();
    this.processMetaData();
    return this.model.load({ fields: this.fields, modelName: this.metadata.modelName });
  }
    ...
    if (node.nodeType === 1) {
      switch (node.tagName) {
        case "graph":
          ...
          // clean rng...
          if (this.props.state) {
            this.model.mode = this.props.state.mode;
          } else {
        ...
        case "field":
          ...
          if (isInitialMeasure) {
            ...
            // no validation on type???
            const groupBy = getGroupBy(fieldName, this.fields);
