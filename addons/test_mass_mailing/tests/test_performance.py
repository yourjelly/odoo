...
class TestMassMailPerformance(TestMassMailPerformanceBase):
    ...
    def test_send_mailing(self):
        ...
        with self.assertQueryCount(__system__=475, marketing=476):
        ...
        with self.assertQueryCount(__system__=71, marketing=68):
...
class TestMassMailBlPerformance(TestMassMailPerformanceBase):
    ...
    def test_send_mailing_w_bl(self):
        ...
        # runbot needs +2 compared to local
        with self.assertQueryCount(__system__=537, marketing=538):
            mailing.action_send_mail()
        ...
        self.assertEqual(len(mails), 12, 'Should not have auto deleted the blacklisted emails')

        cancelled_mail_count = self.env['mail.mail'].sudo().search([('mailing_id', '=', mailing.id), ('state', '=', 'cancel')])
        self.assertEqual(cancelled_mail_count, 12, 'Should not have auto deleted the blacklisted emails')
