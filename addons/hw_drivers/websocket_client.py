    ...
    def on_message(self, _ws, messages):
        ...
        for message in messages:
            ...
            if msg_type == 'iot_action':
                iot_box_identifier = msg_payload['iot_box_identifier']
                device_identifier = msg_payload['device_identifier']

                if iot_box_identifier == iot_mac:
                    if device_identifier in main.iot_devices:
                        start_operation_time = time.perf_counter()
                        _logger.debug(
                            "device '%s' action started with: %s",
                            device_identifier, pprint.pformat(msg_payload),
                        )
                        response = main.iot_devices[device_identifier].action(msg_payload)
                        _logger.info(
            ...
            elif msg_type != 'operation_confirmation':  # intended to be ignored
