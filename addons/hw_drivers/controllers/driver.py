...
class StatusController(http.Controller):
    ...
    def connect_box(self, token):
        ...
        image = '/var/www/False.jpg'
        if server == '':
            token = b64decode(token).decode('utf-8')
            url, token = token.split('|')
            home = os.getenv("HOME")
            if home:
                path = home + '/odoo/addons/point_of_sale/tools/posbox/configuration/connect_to_server.sh'
                try:
                    subprocess.check_output([path, url, '', token, 'noreboot'])
                    m.send_alldevices()
                    image = '/var/www/True.jpg'
