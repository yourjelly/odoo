...
class StatusController(http.Controller):
    ...
    def connect_box(self, token):
        ...
        image = os.path.join(home(), 'odoo/addons/hw_drivers/static/img/False.jpg')
        if server == '':
            token = b64decode(token).decode('utf-8')
            url, token = token.split('|')
            if home:
                path = os.path.join(home(), 'odoo/addons/point_of_sale/tools/posbox/configuration/connect_to_server.sh')
                try:
                    subprocess.check_call([path, url, '', token, 'noreboot'])
                    m.send_alldevices()
                    image = os.path.join(home(), 'odoo/addons/hw_drivers/static/img/True.jpg')
                except subprocess.CalledProcessError as e:
                    _logger.error('A error encountered : %s ' % e.output)
            else:
                _logger.warning('Home environment variable doesn\'t exist')
...
class Manager(Thread):
    ...
    def load_drivers(self):
        ...
        """
        path = os.path.join(home(), 'odoo/addons/hw_drivers/drivers')
