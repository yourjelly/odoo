...
class DisplayDriver(Driver):
    ...
    def run(self):
        while self.device_identifier != 'distant_display' and not self._stopped.is_set() and "pos-customer-display" not in self.url:
            time.sleep(60)
...
class DisplayController(http.Controller):
    ...
    def customer_facing_display(self):
        ...
        if data['action'] == 'open':
            self.ensure_display().update_url(f"{http.request.httprequest.remote_addr}/pos-customer-display/{data['access_token']}")
