...
class Users(models.Model):
    ...
    def totp_disable(self):
        ...
        return {
            ...
            "params": {
                "type": "warning",
                "message": _(
                    "Two-factor authentication disabled for user(s) %s", logins
                ),
    ...
    def totp_enable_wizard(self):
        ...
        w = self.env["auth_totp.wizard"].create({"user_id": self.id, "secret": secret,})
        return {
...
class TOTPWizard(models.TransientModel):
    ...
    qrcode = fields.Binary(
        attachment=False, store=True, readonly=True, compute="_compute_qrcode",
    )
