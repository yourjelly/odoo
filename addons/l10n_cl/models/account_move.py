...
class AccountMove(models.Model):
    ...
    def _l10n_cl_get_invoice_totals_for_report(self):
        ...
        values = self._l10n_cl_get_amounts()
        withholdings = self._l10n_cl_get_withholdings()
        if self.currency_id != self.env.ref('base.CLP') and not \
            self.l10n_latam_document_type_id._is_doc_type_export() and not \
                self.l10n_latam_document_type_id._is_doc_type_electronic_ticket():
            tax_totals['formatted_amount_total'] = formatLang(
                self.env, values['total_amount'], currency_obj=values['main_currency'])
            tax_totals['subtotals'][0]['amount'] = values['subtotal_amount_taxable']
            tax_totals['subtotals'][0]['formatted_amount'] = formatLang(
                self.env, values['subtotal_amount_taxable'], currency_obj=values['main_currency'])
        list_of_tax_groups = []
        for groups_by_subtotal in tax_totals['groups_by_subtotal'][_('Untaxed Amount')]:
            group_id = tax_lines.tax_line_id.filtered(lambda x: x.l10n_cl_sii_code == 14).tax_group_id.id
            if groups_by_subtotal['tax_group_id'] != group_id:
                continue
            groups_by_subtotal['formatted_tax_group_amount'] = formatLang(
                self.env, values['vat_amount'], currency_obj=values['main_currency'])
            groups_by_subtotal['formatted_tax_group_base_amount'] = formatLang(
                self.env, values['subtotal_amount_taxable'], currency_obj=values['main_currency'])
            list_of_tax_groups.append(groups_by_subtotal)
        for withholding in withholdings:
            list_of_tax_groups.append({
                'tax_group_name': withholding['tax_name'],
                'formatted_tax_group_amount': formatLang(
                    self.env, withholding['tax_amount'], currency_obj=values['main_currency']),
                'formatted_tax_group_base_amount': formatLang(
                    self.env, withholding['tax_base'], currency_obj=values['main_currency'])
            })
        tax_totals['groups_by_subtotal'][_('Untaxed Amount')] = list_of_tax_groups
        if values['subtotal_amount_exempt']:
            tax_totals['subtotals'].append({
                'amount': values['subtotal_amount_exempt'],
                'formatted_amount': formatLang(
                    self.env, values['subtotal_amount_exempt'], currency_obj=values['main_currency']),
                'name': _('Exempt Amount')})
            tax_totals['subtotals_order'].append(_('Exempt Amount'))
