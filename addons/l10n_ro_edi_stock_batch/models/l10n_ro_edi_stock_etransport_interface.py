...
class L10nRoEdiStockETransportInterface(models.Model):
    ...
    def _get_declarant_ref(self) -> str:
        ...
        return super()._get_declarant_ref()

    def _get_scheduled_date(self):
        # EXTENDS 'l10n_ro_edi_stock'
        self.ensure_one()
        if self.batch_id:
            return self.batch_id.scheduled_date.date()
        return super()._get_scheduled_date()

    def _get_commercial_partner(self):
        # EXTENDS 'l10n_ro_edi_stock'
        self.ensure_one()
        if self.batch_id:
            return self.batch_id.picking_ids[0].partner_id.commercial_partner_id
        return super()._get_commercial_partner()

    def _get_transport_partner(self):
        # EXTENDS 'l10n_ro_edi_stock'
        self.ensure_one()
        if self.batch_id:
            return self.batch_id.picking_ids[0].carrier_id.l10n_ro_edi_stock_partner_id
        return super()._get_transport_partner()
