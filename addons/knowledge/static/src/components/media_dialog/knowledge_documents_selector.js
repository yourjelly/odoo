...
import { DocumentSelector, createDocuments, documentSpecificClasses } from '@web_editor/components/media_dialog/document_selector';
...
patch(TABS, 'knowledge_media_dialog_tabs', {
    KNOWLEDGE_DOCUMENTS: {
        ...
        createElements: async (...args) => {
            const files = await createDocuments(...args);
            return files.map(file => {
                ...
                file.classList.add(...documentSpecificClasses);
