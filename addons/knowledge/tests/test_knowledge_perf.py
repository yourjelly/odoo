...
# TODO DBE: those tests are not meant to be merged. This is only to compare different access rules architectures
@tagged('knowledge_perf_tests')
class TestKnowledgePerf(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        for user in users[:5]:
            ...
            for i in range(0, 10):
                ...
                cls.level_0_articles |= article
                for m in range(1, 5):
                    if user != cls.users[((i+1)*m) % 50]:
                        article.with_user(user).invite_members(cls.users[((i+1)*m) % 50].partner_id, permissions[(m+i) % 3], send_mail=False)
    ...
    def test_perf(self):
        ...
        template = self.env['ir.qweb']._render('knowledge.knowledge_article_tree_template', tree_value)
        _logger.info("Display Home View for User 1: %s sec" % (start - datetime.now()).total_seconds())

        # Look for all the article a user has access to
        start = datetime.now()
        article_with_access = self.env["knowledge.article"].with_user(test_user).search([])
        _logger.info("Look for all the article a user has access to: %s sec" % (start - datetime.now()).total_seconds())
        ...
        _logger.info("Look for all the private article for a specified user - own_private_articles: %s sec" % (start - datetime.now()).total_seconds())
        start = datetime.now()
        test_user_private_articles = self.env["knowledge.article"].sudo().search([('owner_id', '=', test_user.id)])
        _logger.info("Look for all the private article for a specified user - test_user_private_articles: %s sec" % (start - datetime.now()).total_seconds())
        ...
        _logger.info("Show all the members and permissions of a specified article: %s sec" % (start - datetime.now()).total_seconds())
            ...
            for i in range(0, article_range):
                try:
                    ...
                    if i % 2 == 0: # some articles won't have specific members
                        for m in range(1, 5):
                            if create_users[u] != member_users[((i+1)*m) % 50]:
                                article.with_user(create_users[u]).invite_members(
                                    member_users[((i+1)*m) % 50].partner_id, permissions[(m+i) % 3], send_mail=False)
                except AccessError:
                    pass
                # Create private root articles
