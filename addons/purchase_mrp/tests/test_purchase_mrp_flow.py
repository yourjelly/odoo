...
class TestPurchaseMrpFlow(AccountTestInvoicingCommon):
    ...
    def test_bom_report_incoming_po(self):
        ...
        final_product_tmpl = self.env['product.template'].create({'name': 'Final Product', 'type': 'consu', 'is_trackable': True})
        component_product = self.env['product.product'].create({'name': 'Compo 1', 'type': 'consu', 'is_trackable': True})
