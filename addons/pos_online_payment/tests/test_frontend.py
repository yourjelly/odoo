...
class TestUi(AccountTestInvoicingCommon, OnlinePaymentCommon):
    ...
    def _open_session_fake_cashier_unpaid_order(self):
        ...
        create_result = self.env['sale.order'].with_user(self.pos_user).sync_from_ui([order_data])
        self.assertEqual(len(current_session.order_ids), 1)
        order_id = next(result_order_data for result_order_data in create_result['sale.order'] if result_order_data['pos_reference'] == order_pos_reference)['id']
