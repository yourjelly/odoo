...
class SaleTimesheetController(http.Controller):
    ...
    def _prepare_plan_values(self, domain):
        ...
        dashboard_toinvoice = sum([sol.currency_id.compute(sol.price_unit * (1 - (sol.discount or 0.0) / 100.0) * sol.qty_to_invoice, currency) for sol in so_lines]) + sum([i.currency_id.with_context(date=i.invoice_id.date_invoice).compute(i.price_unit * i.quantity, currency) for i in invoice_lines.filtered(lambda line: line.invoice_id.state == 'draft')])
        dashboard_values['money_amount']['to_invoice'] = round(dashboard_toinvoice, 1)
        dashboard_values['money_amount']['cost'] = sum(values['timesheet_lines'].mapped('amount'))
        dashboard_total = sum([dashboard_values['money_amount'][item] for item in dashboard_values['money_amount'].keys()])
        dashboard_values['money_amount']['total'] = round(dashboard_total, 2)
