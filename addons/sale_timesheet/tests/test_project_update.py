...
class TestProjectUpdateSaleTimesheet(TestProjectUpdate):
    ...
    def test_project_update_description_profitability(self):
        ...
        self.assertEqual(template_values['profitability']['previous_month'], (today + relativedelta(months=-1)).strftime('%B %Y'),
        ...
        self.assertEqual(template_values['profitability']['timesheet_trend'], "0.00", "Timesheet trend used in the template should be well defined.")
        ...
        self.assertEqual(template_values['profitability']['margin_percentage'], "0.00", "Margin percentage used in the template should be well defined")
        self.assertEqual(template_values['profitability']['billing_rate'], "0.00", "Billing rate used in the template should be well defined")

    def test_project_update_panel_profitability(self):
        panel_data = self.project_pigs.get_panel_data()

        self.assertEqual(len(panel_data['profitabilities']['data']), 4, "Panel data should contain 'profitabilities>data' entries")
        self.assertEqual(panel_data['profitabilities']['action'], "action_view_timesheet", "Panel data should contain 'profitabilities>action' entry")
