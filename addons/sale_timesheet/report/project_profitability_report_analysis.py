...
class ProfitabilityAnalysis(models.Model):
    ...
    def _main_table_alias(self):
        return "sub"

    def _select_field_list(self):
        main_table_fields = [
            'id', 'project_id', 'task_id', 'user_id', 'sale_line_id', 'analytic_account_id', 'partner_id', 'company_id', 'currency_id', 'sale_order_id',
            'order_confirmation_date', 'product_id', 'sale_qty_delivered_method', 'expense_amount_untaxed_to_invoice', 'expense_amount_untaxed_invoiced',
            'amount_untaxed_to_invoice', 'amount_untaxed_invoiced', 'timesheet_unit_amount', 'timesheet_cost', 'expense_cost', 'other_revenues',
            'downpayment_invoiced', 'line_date'
        ]
        field_list = [{'alias': field, 'column': self._main_table_alias() + '.' + field} for field in main_table_fields]
        field_list += [{
            'alias': 'margin',
            'columns': [
                {'column': self._main_table_alias() + '.expense_amount_untaxed_to_invoice', 'operator': '+'},
                {'column': self._main_table_alias() + '.expense_amount_untaxed_invoiced', 'operator': '+'},
                {'column': self._main_table_alias() + '.amount_untaxed_to_invoice', 'operator': '+'},
                {'column': self._main_table_alias() + '.amount_untaxed_invoiced', 'operator': '+'},
                {'column': self._main_table_alias() + '.other_revenues', 'operator': '+'},
                {'column': self._main_table_alias() + '.timesheet_cost', 'operator': '+'},
                {'column': self._main_table_alias() + '.expense_cost', 'operator': '+'},
            ]
        }]
        return field_list

    def _select(self):
        field_list = self._select_field_list()
        select = ""
        for field in field_list:
            if field.get('column'):
                select += field['column'] + ' AS ' + field['alias'] + ',\n'
            elif field.get('columns'):
                select += '('
                operator = False
                for column in field['columns']:
                    select += ((operator or column['operator'] == '-') and column['operator'] or '') + ' ' + column['column']
                    operator = True
                select += ') AS ' + field['alias'] + ',\n'
            else:
                raise NotImplementedError(_('The field list must have a column or columns key.'))
        select = select[:-2]
        return select

    def _main_table_query(self):
        return """
            SELECT
    ...
    def init(self):
        ...
        """ % (self._table, self._select(), self._main_table_query(), self._main_table_alias(), self._joins())
