...
class ProjectUpdate(models.Model):
    ...
    def _get_profitability_values(self, project):
        if not (
            self.user_has_groups("project.group_project_manager")
            and (
                project.analytic_account_id
                and project.allow_billable
        ...
        timesheets_this_month = self.env["project.profitability.report"].read_group(
            [("project_id", "=", project.id), ("line_date", ">=", start_of_month)],
            ["project_id", "timesheet_unit_amount"],
            ["project_id"],
        )
        ...
        timesheet_unit_amount = (
            timesheets_this_month
            and timesheets_this_month[0]["timesheet_unit_amount"]
            or 0.0
        )
        previous_timesheet_unit_amount = (
            timesheets_previous_month
            and timesheets_previous_month[0]["timesheet_unit_amount"]
            or 0.0
        )
        return {
            ...
            "timesheet_unit_amount": formatLang(
                self.env,
            ...
            "previous_timesheet_unit_amount": formatLang(
                self.env,
                project._convert_project_uom_to_timesheet_encode_uom(
                    previous_timesheet_unit_amount
                ),
            ...
            "timesheet_trend": formatLang(
                self.env,
                previous_timesheet_unit_amount > 0
                and ((timesheet_unit_amount / previous_timesheet_unit_amount) - 1) * 100
                or 0.0,
                digits=0,
            ),
            "costs": format_amount(
