...
class Project(models.Model):
    ...
    def _compute_warning_employee_rate(self):
        ...
        employees = self.env['account.analytic.line']._read_group(
            ...
            ['project_id'],
            ['employee_id:array_agg_distinct'],
        )
        dict_project_employee = {project.id: employee_ids for project, employee_ids in employees}
        for project in projects:
            project.warning_employee_rate = any(x not in project.sale_line_employee_ids.employee_id.ids for x in dict_project_employee.get(project.id, []))
