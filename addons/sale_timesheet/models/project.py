...
class Project(models.Model):
    ...
    def _compute_project_overview(self):
        ...
        for project in self.filtered(lambda p: not p.project_overview):
            project.project_overview = (
                project.allow_billable or project.allow_timesheets
            )
    ...
    def get_panel_data(self):
        ...
        return {**panel_data, "profitability_items": self._get_profitability_items()}
    ...
    def _get_profitability_items(self):
        ...
        data = []
        if self.allow_timesheets and self.user_has_groups(
    ...
    def get_profitability_common(self):
        ...
        timesheets = self.env["account.analytic.line"].read_group(
            [("project_id", "=", self.id)], ["so_line", "unit_amount"], ["so_line"]
        )
        ...
        return {
            "costs": profitability
            and profitability[0]["timesheet_cost"] + profitability[0]["expense_cost"]
            or 0.0,
            "margin": profitability and profitability[0]["margin"] or 0.0,
            "revenues": profitability
            and (
                profitability[0]["amount_untaxed_invoiced"]
                + profitability[0]["amount_untaxed_to_invoice"]
                + profitability[0]["expense_amount_untaxed_invoiced"]
                + profitability[0]["expense_amount_untaxed_to_invoice"]
                + profitability[0]["other_revenues"]
            )
            or 0.0,
            "timesheet_unit_amount": profitability
            and self._convert_project_uom_to_timesheet_encode_uom(
                profitability[0]["timesheet_unit_amount"]
            )
            or 0.0,
            "timesheet_percentage_billable": timesheet_billable
            and timesheet_billable / (timesheet_billable + timesheet_non_billable) * 100
            or 0.0,
        }
...
class ProjectTask(models.Model):
    ...
    analytic_account_id = fields.Many2one(
        "account.analytic.account", related="sale_order_id.analytic_account_id"
    )
    ...
    def _compute_analytic_account_active(self):
        ...
        for task in self:
            task.analytic_account_active = (
                task.analytic_account_active or task.analytic_account_id.active
            )
...
class ProjectTaskRecurrence(models.Model):
    ...
    def _get_recurring_fields(self):
        return ["analytic_account_id"] + super(
