...
class ProductTemplate(models.Model):
    ...
    service_upsell_warning = fields.Boolean(
        "Upsell Warning",
        help="The salesperson in charge will be assigned an activity informing him of an upselling opportunity once the selected threshold is reached.",
    )
    ...
    def _default_visible_expense_policy(self):
        ...
        return (
            visibility or super(ProductTemplate, self)._default_visible_expense_policy()
        )
        ...
        for product in self:
            ...
            if product.invoice_policy == "delivery":
                policy = (
                    ...
                    else "delivered_timesheet"
                )
            elif product.invoice_policy == "order" and (
                product.service_type == "timesheet" or product.type == "service"
            ):
        ...
        for product in self:
            ...
            if not policy and not product.invoice_policy == "delivery":
                ...
                product.service_type = (
                    "manual" if policy == "delivered_manual" else "timesheet"
                )
    ...
    def _onchange_type(self):
        ...
        if self.type == "service" and not self.invoice_policy:
            ...
            self.type == "consu"
            and not self.invoice_policy
            and self.service_policy == "ordered_timesheet"
        ):
            self.invoice_policy = "order"
        return res
