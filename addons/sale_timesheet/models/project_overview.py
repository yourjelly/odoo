...
class Project(models.Model):
    ...
    def _plan_get_employee_ids(self):
        aal_employee_ids = self.env["account.analytic.line"].read_group(
            [("project_id", "in", self.ids), ("employee_id", "!=", False)],
            ["employee_id"],
            ["employee_id"],
        )
    ...
    def _plan_prepare_values(self):
        ...
        dashboard_values = {
            ...
            "profit": {"invoiced": 0.0, "to_invoice": 0.0, "cost": 0.0, "total": 0.0,},
        }
            ...
            if not is_milestone:
                rows_sale_line[sale_line_row_key][-2] = (
                    sale_line.product_uom._compute_quantity(
                        sale_line.product_uom_qty, uom_hour, raise_if_failure=False
                    )
                    if sale_line
                    else 0.0
                )
        ...
        for row_key, row_employee in rows_employee.items():
            ...
            if sale_line_row_key not in rows_sale_line:
                ...
                rows_sale_line[sale_line_row_key] = (
                    [
                        {
                            "label": map_sol_names.get(sale_line.id)
                            if sale_line
        ...
        for row_key, row_sale_line in rows_sale_line.items():
            ...
            if sale_order_id not in rows_sale_order:
                rows_sale_order[sale_order_id] = (
                    [
                        {
                            "label": map_so_names.get(
                                sale_order_id, _("No Sales Order")
                            ),
    ...
    def _table_header(self):
        ...
        ts_months = sorted(
            [
                ...
                for i in range(0, DEFAULT_MONTH_RANGE)
            ]
        )  # M1, M2, M3
        ...
        header_names = (
            [_("Sales Order"), _("Before")]
            + [_to_short_month_name(date) for date in ts_months]
            + [_("Total"), _("Sold"), _("Remaining")]
        )
        ...
        result[-1]["tooltip"] = _(
            "What is still to deliver based on sold hours and hours already done. Equals to sold hours - done hours."
        )
        return result
    ...
    def _table_rows_get_employee_lines(self, data_from_db):
        ...
        ts_months = sorted(
            [
                initial_date - relativedelta(months=i, day=1)
                for i in range(0, DEFAULT_MONTH_RANGE)
            ]
        )  # M1, M2, M3
        default_row_vals = self._table_row_default()
        ...
        map_empl_names = {
            empl.id: empl.name
            for empl in self.env["hr.employee"].sudo().browse(employee_ids)
        }

        # extract rows data for employee, sol and so rows
        rows_employee = (
            {}
        )  # (so, sol, employee) -> [INFO, before, M1, M2, M3, Done, M3, M4, M5, After, Forecasted]
        for data in data_from_db:
            ...
            if row_key not in rows_employee:
                ...
                rows_employee[row_key] = [meta_vals] + default_row_vals[
                    :
                ]  # INFO, before, M1, M2, M3, Done, M3, M4, M5, After, Forecasted
    ...
    def _table_get_empty_so_lines(self):
        ...
        return (
            ...
            | set(
                ...
                .filtered(lambda sol: sol.is_service and not sol.is_expense)
                .ids
            ),
            set(so_lines.mapped("order_id").ids) | set(sale_order.ids),
        )
    ...
    def _plan_prepare_actions(self, values):
        ...
        if len(self) == 1:
            ...
            if self.env.user.has_group(
                ...
                task_order_line_ids = self.env["project.task"].read_group(
                    [("project_id", "=", self.id), ("sale_line_id", "!=", False)],
                    ["sale_line_id"],
                    ["sale_line_id"],
                )
                task_order_line_ids = [
                    ol["sale_line_id"][0] for ol in task_order_line_ids
                ]

            if self.env.user.has_group("sales_team.group_sale_salesman"):
                if (
                    self.pricing_type != "task_rate"
                    and self.allow_billable
            ...
            if self.env.user.has_group("sales_team.group_sale_salesman_all_leads"):
                to_invoice_amount = values["dashboard"]["profit"].get(
                    "to_invoice", False
                )  # plan project only takes services SO line with timesheet into account

                sale_order_ids = self.env["sale.order.line"].read_group(
                    [("id", "in", task_order_line_ids)], ["order_id"], ["order_id"]
                )
                sale_order_ids = [s["order_id"][0] for s in sale_order_ids]
                sale_order_ids = self.env["sale.order"].search_read(
                    [
            ...
            if sale_orders:
                ...
                if len(sale_orders) == 1:
                    so_action.update(
                        {
                            "action": self.env.ref(
                                "sale.action_sale_order_form_view"
                            ).sudo(),
...
def _to_action_data(
    model=None, *, action=None, views=None, res_id=None, domain=None, context=None
):
