...
class AccountAnalyticLine(models.Model):
    ...
    so_line = fields.Many2one(compute="_compute_so_line", store=True, readonly=False, domain="[('is_service', '=', True), ('is_expense', '=', False), ('state', 'in', ['sale', 'done']), ('order_partner_id', 'child_of', commercial_partner_id)]",
                                    help="Sales order item to which the time spent will be added, in order to be invoiced to your customer.\n"
                                         "If your employee has been explicitely linked to a sales order item on the project, this one is selected by default. Otherwise, the sales order item of the task is set. In the absence of a task, the one of the project is.\n"
