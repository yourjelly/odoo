...
class ProjectCreateSalesOrder(models.TransientModel):
    ...
    def _make_billable_at_project_rate(self, sale_order):
        ...
        timesheets_on_ticket = self.env.context.get('timesheets_on_ticket', [])
        for wizard_line in self.line_ids:
            ...
            if timesheets_on_ticket and not self.project_id.sale_line_id and not task_ids:
            ...
            if timesheets_on_ticket:
                search_domain = [('id', 'in', timesheets_on_ticket), ('so_line', '=', False)]
        ...
        if timesheets_on_ticket and self.project_id.sale_line_id and len(self.project_id.tasks) == 0 and len(self.line_ids) > 1:
    ...
    def _make_billable_at_employee_rate(self, sale_order):
        ...
        for map_entry in map_entries:
            ...
            timesheets_on_ticket = self.env.context.get('timesheets_on_ticket', [])
            if timesheets_on_ticket:
                search_domain.append(('id', 'in', timesheets_on_ticket))
            else:
