...
class AdyenProxyAuth(requests.auth.AuthBase):
    ...
    def __call__(self, request):
        h = hmac.new(
            self.adyen_account_id.proxy_token.encode("utf-8"), digestmod=hashlib.sha256
        )
        ...
        message = "%s|%s|%s|%s" % (
            ...
            json.dumps(
                url_decode(parsed_url.query), sort_keys=True
...
def odoo_payments_proxy_control(func, *args, **kwargs):
    ...
    account_id = (
        request.env["adyen.account"].sudo().search([("adyen_uuid", "=", adyen_uuid)])
    )
    if not account_id:
        raise Forbidden()

    secret = account_id.proxy_token.encode("utf8")
    msg_signature = request.httprequest.headers.get(
        "oe-signature"
    ...
    if int(msg_timestamp) + TIMEOUT < int(time.time()):
        _logger.debug(
            "HTTP request validation failed due to invalid timestamp for route %r",
    ...
    message = "%s|%s|%s|%s" % (
        ...
        json.dumps(
            url_decode(request.httprequest.query_string), sort_keys=True
        ),  # url query params sorted by key
