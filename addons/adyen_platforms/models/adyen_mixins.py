        ...
        for adyen_account in self:
            if vals.get("id_front"):
                ...
                adyen_account._upload_photo_id(
                    document_type,
                    adyen_account.id_front,
                    adyen_account.id_front_filename,
                )
            if vals.get("id_back") and adyen_account.id_type in [
                "ID_CARD",
                "DRIVING_LICENSE",
            ]:
                document_type = adyen_account.id_type + "_BACK"
                adyen_account._upload_photo_id(
                    document_type, adyen_account.id_back, adyen_account.id_back_filename
                )
            return res
    ...
    def _check_file_requirements(self, content):
        ...
        if mimetype not in ["image/jpeg", "image/png", "application/pdf"]:
            raise ValidationError(
                _("Allowed file formats for photo IDs are jpeg, jpg, pdf or png")
            )
        if file_size < (100 * 1024) or (
            ...
            raise ValidationError(
                _(
                    "Minimum allowed size for photo ID: 1 KB for PDF, 100 KB for other formats."
                )
            )
