...
class AdyenTransaction(models.Model):
    ...
    last_status_update = fields.Datetime(
        related="last_status_id.date", string="Last Status Update"
    )
    status_ids = fields.One2many(
        "adyen.transaction.status", "adyen_transaction_id", "Status History"
    )
    ...
    commercial_card = fields.Selection(
        [("yes", "Yes"), ("no", "No"), ("unknown", "Unknown"),], default="unknown"
    )
    ...
    _sql_constraints = [
        (
            "reference_unique",
            "unique(reference, capture_reference)",
    ...
    def _get_tx_from_notification(self, notification):
        ...
        if capture_reference:
            domain = expression.AND(
                [domain, [("capture_reference", "=", capture_reference)]]
            )

        if self:
            return self.filtered_domain(domain), reference, capture_reference
        return (
            self.env["adyen.transaction"].search(domain),
            reference,
            capture_reference,
        )
    ...
    def _handle_notification(self, data):
        adyen_uuid = data.get("additionalData", {}).get(
            "metadata.adyen_uuid"
        ) or data.get("adyen_uuid")
        account = (
            self.env["adyen.account"].sudo().search([("adyen_uuid", "=", adyen_uuid)])
        )
        if not account:
            _logger.warning("Received payment notification for non-existing account")
            return

        tx_sudo, reference, capture_reference = self.env[
            "adyen.transaction"
        ]._get_tx_from_notification(data)
        if not tx_sudo:
            tx_sudo = (
                self.env["adyen.transaction"]
                .sudo()
                .create(
                    {
    ...
    def _handle_authorisation_notification(self, notification_data):
        ...
        shopper_country_id = self.env["res.country"].search(
            [("code", "=", additional_data.get("shopperCountry"))]
        )
        commercial_card = additional_data.get("isCardCommercial", "unknown")
        card_country_id = self.env["res.country"].search(
            [
                (
                    "code",
                    "=",
                    additional_data.get(
                        "cardIssuingCountry", additional_data.get("issuerCountry")
                    ),
                )
            ]
        )
        ...
        self.write(
            {
                ...
                "total_amount": to_major_currency(
                    notification_data["amount"]["value"], currency_id.decimal_places
                ),
                "currency_id": currency_id.id,
                "date": parse(notification_data.get("eventDate"))
                .astimezone(UTC)
                ...
                if commercial_card in ("yes", "no", "unknown")
                else "unknown",
            }
        )
        self._trigger_sync()
    ...
    def _handle_fees_updated_notification(self, notification_data):
        ...
        currency_amount = self.env["res.currency"].search(
            [("name", "=", notification_data.get("totalAmount", {}).get("currency"))]
        )
        currency_fees = self.env["res.currency"].search(
            [("name", "=", notification_data.get("totalFees", {}).get("currency"))]
        )
        self.capture_reference = notification_data.get("captureReference")
        self.fees = to_major_currency(
            notification_data["totalFees"]["value"], currency_fees.decimal_places
        )
        self.fixed_fees = to_major_currency(
            notification_data["fixedFees"]["value"], currency_fees.decimal_places
        )
        self.variable_fees = to_major_currency(
            notification_data["variableFees"]["value"], currency_fees.decimal_places
        )
        self.merchant_amount = to_major_currency(
            notification_data["merchantAmount"]["value"], currency_amount.decimal_places
        )
        self.total_amount = to_major_currency(
            notification_data["totalAmount"]["value"], currency_amount.decimal_places
        )
    ...
    def _handle_refund_notification(self, notification_data):
        ...
        self.date = (
            parse(notification_data.get("eventDate"))
            .astimezone(UTC)
            .strftime(DEFAULT_SERVER_DATETIME_FORMAT)
        )
    ...
    def _handle_chargeback_notification(self, notification_data):
        ...
        self.adyen_account_id.message_post(
            body=_(
                "Transaction %s has been CHARGEBACK'ed: %s",
                self.description or self.reference,
                notification_data.get("reason"),
            ),
    ...
    def _create_missing_tx(self, account_id, transaction, **kwargs):
        currency_id = self.env["res.currency"].search(
            [("name", "=", transaction.get("amount", {}).get("currency"))]
        )
        amount = to_major_currency(
            transaction["amount"]["value"], currency_id.decimal_places
        )
        tx = self.create(
            {
                ...
                "total_amount": amount,
                "currency_id": currency_id.id,
                "date": parse(transaction.get("creationDate"))
                .astimezone(UTC)
                .strftime(DEFAULT_SERVER_DATETIME_FORMAT),
    ...
    def _trigger_sync(self):
        sync_cron = self.env.ref(
            "adyen_platforms.adyen_sync_cron", raise_if_not_found=False
        )
        if sync_cron:
    ...
    def _refund_request(self, amount=None):
        ...
        initial_amount = to_minor_currency(
            self.total_amount, self.currency_id.decimal_places
        )
        ...
        refund_data = {
            ...
            "date": str(self.date),
            "reference": "Refund of %s"
            % self.description,  # TODO generate unique reference
            ...
            .create(
                {
                    ...
                    "total_amount": to_major_currency(
                        res["totalAmount"]["value"], self.currency_id.decimal_places
                    ),
                    "fees": to_major_currency(
                        res["totalFees"]["value"], self.currency_id.decimal_places
                    ),
                    "variable_fees": to_major_currency(
                        res["totalFees"]["value"], self.currency_id.decimal_places
                    ),
                    "merchant_amount": to_major_currency(
                        res["merchantAmount"]["value"], self.currency_id.decimal_places
                    ),
...
class AdyenTransactionStatus(models.Model):
    ...
    adyen_transaction_id = fields.Many2one(
        "adyen.transaction", required=True, ondelete="cascade"
    )
...
class AdyenTransactionPayout(models.Model):
    ...
    def _create_missing_payout(self, account_id, transaction, **kwargs):
        ...
        bank_account_id = self.env["adyen.bank.account"].search(
            [
                (
                    "bank_account_uuid",
                    "=",
                    transaction.get("bankAccountDetail", {}).get("bankAccountUUID"),
                )
            ]
        )
        tx = self.create(
            {
                ...
                .strftime(DEFAULT_SERVER_DATETIME_FORMAT),
                "amount": to_major_currency(
                    transaction.get("amount", {}).get("value"),
