...
class AdyenShareholder(models.Model):
    ...
    shareholder_reference = fields.Char(
    ...
    def _compute_full_name(self):
        for adyen_shareholder_id in self:
            adyen_shareholder_id.full_name = (
                f"{adyen_shareholder_id.first_name} {adyen_shareholder_id.last_name}"
            )
    ...
    def create(self, values):
        ...
        shareholders = response["accountHolderDetails"]["businessDetails"][
            "shareholders"
        ]
        created_shareholder = next(
            shareholder
            for shareholder in shareholders
            if shareholder["shareholderReference"]
            == adyen_shareholder_id.shareholder_reference
        )
    ...
    def unlink(self):
        ...
        for shareholder_id in self:
            shareholder_id.adyen_account_id._adyen_rpc(
                "v1/delete_shareholders",
                {
    ...
    def _format_data(self, values):
        ...
        data = {
            ...
            "accountHolderDetails": {
                "businessDetails": {
                    "shareholders": [
                        {
                            "shareholderCode": values.get("shareholder_uuid")
                            or self.shareholder_uuid
                            or None,
                            "shareholderReference": values.get("shareholder_reference")
                            or self.shareholder_reference,
                            "address": {
                                "city": values.get("city") or self.city,
                                "country": country_id.code,
                                "houseNumberOrName": values.get("house_number_or_name")
                                or self.house_number_or_name,
                            ...
                            "personalData": {
                                "dateOfBirth": str(
                                    values.get("date_of_birth") or self.date_of_birth
                                ),
                            },
                        }
                    ]
        ...
        if document_number:
            data["accountHolderDetails"]["businessDetails"]["shareholders"][0][
                "personalData"
            ]["documentData"] = [{"number": document_number, "type": "ID",}]
