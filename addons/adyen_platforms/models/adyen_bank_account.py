...
class AdyenBankAccount(models.Model):
    ...
    bank_account_reference = fields.Char(
        "Reference", default=lambda self: uuid.uuid4().hex
    )
    ...
    kyc_status = fields.Selection(
        ADYEN_KYC_STATUS, compute="_compute_kyc_status", readonly=True
    )
    ...
    def create(self, values):
        ...
        adyen_account_id = self.env["adyen.account"].browse(
            values.get("adyen_account_id")
        )
        response = adyen_account_id._adyen_rpc(
        ...
        created_bank_account = next(
            ...
            if bank_account["bankAccountReference"]
            == adyen_bank_account_id.bank_account_reference
        )
    ...
    def write(self, vals):
        ...
        if not self.env.context.get("update_from_adyen"):
            self.adyen_account_id._adyen_rpc(
                "v1/update_account_holder", self._format_data(vals)
            )
        if "bank_statement" in vals:
            self._upload_bank_statement(
                vals["bank_statement"], vals["bank_statement_filename"]
            )
        return res
    ...
    def unlink(self):
        ...
        for bank_account_id in self:
            bank_account_id.adyen_account_id._adyen_rpc(
                "v1/delete_bank_accounts",
                {
    ...
    def _format_data(self, values=None):
        if values is None:
            values = {}
        adyen_account_id = (
            self.env["adyen.account"].browse(values.get("adyen_account_id"))
            if values.get("adyen_account_id")
            else self.adyen_account_id
        )
        country_id = (
        ...
        owner_country_id = (
            self.env["res.country"].browse(values.get("owner_country_id"))
            if values.get("owner_country_id")
            else self.owner_country_id
        )
        owner_state_id = (
            self.env["res.country.state"].browse(values.get("owner_state_id"))
            if values.get("owner_state_id")
            else self.owner_state_id
        )
        return {
            ...
            "accountHolderDetails": {
                "bankAccountDetails": [
                    {
                        "accountNumber": values.get("account_number")
                        or self.account_number
                        or None,
                        "accountType": values.get("account_type")
                        or self.account_type
                        or None,
                        "bankAccountReference": values.get("bank_account_reference")
                        or self.bank_account_reference,
                        "bankAccountUUID": values.get("bank_account_uuid")
                        or self.bank_account_uuid
                        or None,
    ...
    def _upload_bank_statement(self, content, filename):
        ...
        if mimetype not in ["image/jpeg", "image/png", "application/pdf"]:
            raise ValidationError(
                _(
                    "Allowed file formats for bank statements are jpeg, jpg, pdf or png. Received %r instead.",
                    mimetype,
                )
            )
        if file_size < (100 * 1024) or (
            file_size < 1024 and mimetype == "application/pdf"
        ):
            raise ValidationError(
                _(
                    "Minimum allowed size for bank statements: 1 KB for PDF, 100 KB for other formats."
                )
            )
        ...
        test_mode = (
            ...
            .sudo()
            .get_param("adyen_platforms.test_mode")
        )
        self.adyen_account_id._adyen_rpc(
            "v1/upload_document",
            {
