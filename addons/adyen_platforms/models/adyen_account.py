...
class AdyenAccount(models.Model):
    ...
    account_holder_code = fields.Char(
        "Account Holder Code", default=lambda self: uuid.uuid4().hex
    )

    company_id = fields.Many2one("res.company", default=lambda self: self.env.company)
    shareholder_ids = fields.One2many(
        "adyen.shareholder", "adyen_account_id", string="Shareholders"
    )
    bank_account_ids = fields.One2many(
        "adyen.bank.account", "adyen_account_id", string="Bank Accounts"
    )
    transaction_ids = fields.One2many(
        "adyen.transaction", "adyen_account_id", string="Transactions"
    )
    transactions_count = fields.Integer(compute="_compute_transactions_count")
    transaction_payout_ids = fields.One2many(
        "adyen.transaction.payout", "adyen_account_id"
    )
    ...
    adyen_kyc_ids = fields.One2many(
        "adyen.kyc", "adyen_account_id", string="KYC Checks", readonly=True
    )
    ...
    def default_get(self, fields):
        ...
        if not self.env.company.partner_id.is_company and {
            "last_name",
            "first_name",
        } & set(fields):
    ...
    def _compute_full_name(self):
        for adyen_account_id in self:
            if adyen_account_id.is_business:
                ...
                adyen_account_id.full_name = "%s %s" % (
                    adyen_account_id.first_name,
                    adyen_account_id.last_name,
                )
    ...
    def _compute_state(self):
        for account in self:
            if (
                account.account_status in ["inactive", "suspended"]
                and account.kyc_tier == 0
            ):
        ...
        for account in self.filtered("adyen_kyc_ids.status_message"):
            ...
            for kyc in account.adyen_kyc_ids.filtered("status_message"):
                ...
                checks.setdefault(doc_type, []).append(
                    {"document": kyc.document, "message": kyc.status_message,}
                )

            account.kyc_status_message = self.env["ir.qweb"]._render(
                "adyen_platforms.kyc_status_message", {"checks": checks}
            )
    ...
    def create(self, values):
        ...
        create_data["payoutSchedule"] = (
            ADYEN_PAYOUT_FREQUENCIES.get(
    ...
    def unlink(self):
        ...
        for adyen_account_id in self:
            adyen_account_id._adyen_rpc(
                "v1/close_account_holder",
    ...
    def _update_payout_schedule(self, payout_schedule):
        ...
        self._adyen_rpc(
            "v1/update_payout_schedule",
            {
                "accountCode": self.account_code,
                "metadata": {"adyen_uuid": self.adyen_uuid,},
                "payoutSchedule": {
                    "action": "UPDATE",
    ...
    def action_create_redirect(self):
        ...
        return_url = url_join(
            self.env.company.get_base_url(), "adyen_platforms/create_account"
        )
        onboarding_url = (
        ...
        return {
            ...
            "url": url_join(
                onboarding_url, "get_creation_token?return_url=%s" % return_url
            ),
        }
    ...
    def _format_data(self, values):
        fields = values.keys()
        data = {
            "accountHolderCode": values.get("account_holder_code")
            or self.account_holder_code,
        }
        holder_details = {}
        ...
        if (values.get("is_business") or self.is_business) and {
            "legal_business_name",
            "doing_business_as",
            "registration_number",
        } & fields:
            ...
            if "date_of_birth" in fields:
                holder_details["individualDetails"].setdefault("personalData", {})[
                    "dateOfBirth"
                ] = str(values["date_of_birth"])
    ...
    def _adyen_rpc(self, operation, adyen_data=None):
        ...
        try:
            req = requests.post(
                url_join(url, operation), json=payload, auth=auth, timeout=TIMEOUT
            )
            req.raise_for_status()
        except requests.exceptions.Timeout:
            raise UserError(
                _("A timeout occurred while trying to reach the Adyen proxy.")
            )
        except Exception:
            raise UserError(
                _("The Adyen proxy is not reachable, please try again later.")
            )
        response = req.json()
        ...
        if "error" in response:
            ...
            if name == "ValidationError":
                ...
                raise UserError(
                    _(
                        "We had troubles reaching Adyen, please retry later or contact the support if the problem persists"
                    )
                )
    ...
    def _handle_account_holder_status_change_notification(self, content):
        ...
        # Payout
        payout_allowed = (
            content.get("newStatus", {}).get("payoutState", {}).get("allowPayout", None)
        )
        ...
        if events:
            ...
            status_message = self.env["ir.qweb"]._render(
                "adyen_platforms.status_message",
                {"message": content.get("reason"), "reasons": reasons,},
            )
    ...
    def _handle_account_holder_verification_notification(self, content):
        ...
        document = "_".join(
            content.get("verificationType", "").lower().split("_")[:-1]
        )  # bank_account, identity, passport, etc.
        ...
        if bank_uuid:
            kyc = kyc.filtered(
                lambda k: k.bank_account_id.bank_account_uuid == bank_uuid
                or not k.bank_account_id
            )
        elif shareholder_uuid:
            kyc = kyc.filtered(
                lambda k: k.shareholder_id.shareholder_uuid == shareholder_uuid
                or not k.shareholder_id
            )
        else:
        ...
        if not kyc:
            ...
            if document == "bank_account" and bank_uuid:
                bank_account_id = (
                    self.env["adyen.bank.account"]
                    .sudo()
                    .search([("bank_account_uuid", "=", bank_uuid)])
                )
                additional_data["bank_account_id"] = bank_account_id.id
            if shareholder_uuid:
                shareholder_id = (
                    self.env["adyen.shareholder"]
                    .sudo()
                    .search([("shareholder_uuid", "=", shareholder_uuid)])
                )
    ...
    def _handle_account_updated_notification(self, content):
        ...
        if scheduled_date:
            self.next_scheduled_payout = (
                parse(scheduled_date).astimezone(UTC).replace(tzinfo=None)
            )
    ...
    def _handle_account_holder_payout(self, content):
        ...
        if status == "Failed":
            status_message = _(
                "Failed payout: %s", content["status"]["message"]["text"]
            )
            self.sudo().message_post(
                body=status_message, subtype_xmlid="mail.mt_comment"
            )
        ...
        for account in self:
            ...
            while next_page:
                ...
                for transaction in transactions:
                    create_date = (
                        parse(transaction.get("creationDate"))
                        .astimezone(UTC)
                        .replace(tzinfo=None)
                    )
                    ...
                    if status in ["Payout", "PayoutReversed"]:
                        reference = transaction.get("pspReference") or transaction.get(
                            "disputePspReference"
                        )
                        tx_sudo = account.transaction_payout_ids.sudo().filtered(
                            lambda t: t.reference == reference
                        )
                        if not tx_sudo:
                            self.env[
                                "adyen.transaction.payout"
                            ].sudo()._create_missing_payout(account.id, transaction)
                        else:
                            tx_sudo.status = status
                    else:
                        if status in ["Chargeback", "ChargebackReceived"]:
                            transaction["pspReference"] = transaction.get(
                                "disputePspReference"
                            )
                        (
                            tx_sudo,
                            _reference,
                            _capture_reference,
                        ) = account.transaction_ids.sudo()._get_tx_from_notification(
                            transaction
                        )
                        if not tx_sudo:
                            tx_sudo = (
                                ...
                                .sudo()
                                ._create_missing_tx(account.id, transaction)
                            )
    ...
    def _fetch_transactions(self, page=1):
        self.ensure_one()
        response = self._adyen_rpc(
            "v1/get_transactions",
            {
                "accountHolderCode": self.account_holder_code,
                "transactionListsPerAccount": [
                    {"accountCode": self.account_code, "page": page,}
                ],
            },
        )
...
class AdyenAccountBalance(models.Model):
    ...
    adyen_account_id = fields.Many2one(
        "adyen.account", required=True, ondelete="cascade"
    )
    ...
    def get_account_balance(self):
        ...
        balance_fields = {
            "balance": "balance",
            "onHoldBalance": "on_hold",
            "pendingBalance": "pending",
        }
        balances = (
            self.env["adyen.account.balance"]
            .sudo()
            .search([("adyen_account_id", "=", self.env.company.adyen_account_id.id)])
        )
        ...
        if not balances or any(b.write_date <= delta for b in balances):
            response = {}
            try:
                response = self.env.company.adyen_account_id._adyen_rpc(
                    "v1/account_holder_balance",
                    {
                ...
                _logger.warning(
                    _("Cannot update account balance, showing previous values: %s", e)
                )

            balances.write({f: 0 for f in balance_fields.values()})
            for total_balance, adyen_balances in response.get(
                "totalBalance", {}
            ).items():
                for balance in adyen_balances:
                    currency_id = self.env["res.currency"].search(
                        [("name", "=", balance.get("currency"))]
                    )
                    ...
                    bal[balance_fields.get(total_balance)] = to_major_currency(
                        balance.get("value", 0), currency_id.decimal_places
                    )
        ...
        return [
            {
                ...
                "payout_date": format_datetime(
                    self.env,
                    self.env.company.adyen_account_id.next_scheduled_payout,
                    dt_format="short",
                ),
