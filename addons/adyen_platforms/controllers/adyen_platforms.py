...
class AdyenPlatformsController(http.Controller):
    @http.route(
        "/adyen_platforms/create_account", type="http", auth="user", website=True
    )
    def adyen_platforms_create_account(self, creation_token):
        request.session["adyen_creation_token"] = creation_token
        return request.redirect(
            "/web?#action=adyen_platforms.adyen_account_action_create"
        )

    @odoo_payments_proxy_control
    @http.route(
        "/adyen_platforms/account_notification", type="json", auth="public", csrf=False
    )
    def adyen_platforms_notification(self):
        ...
        account = (
            request.env["adyen.account"]
            .sudo()
            .search([("adyen_uuid", "=", data["adyen_uuid"])])
        )
        if not account:
            _logger.error(
                "Received notification for non-existing account: %s", data["adyen_uuid"]
            )
            return
    ...
    @odoo_payments_proxy_control
    @http.route(
        "/adyen_platforms/transaction_notification",
        type="json",
        auth="public",
        csrf=False,
    )
