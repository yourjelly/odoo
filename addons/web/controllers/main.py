...
class ReportController(http.Controller):
    ...
    def report_routes(self, reportname, docids=None, converter=None, **data):
        ...
        elif converter == 'other':
            text = report.with_context(context).render_qweb_other(docids, data=data)[0]
    ...
    def report_download(self, data, token):
        ...
        try:
            if type == 'qweb-pdf':
                ext = 'pdf'
            elif type == 'qweb-other':
                ext = 'other'
            else:
                return
            reportname = url.split('/report/'+ext+'/')[1].split('?')[0]
            ...
            if docids:
                # Generic report:
                response = self.report_routes(reportname, docids=docids, converter=ext)
            else:
                # Particular report:
                data = url_decode(url.split('?')[1]).items()  # decoding the args represented in JSON
                response = self.report_routes(reportname, converter=ext, **dict(data))
