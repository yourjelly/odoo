                ...
                if False:  # self.debug and inherit_mode == self.EXTENSION_MODE:
                    for xpath in xpaths:
                        xpath.insert(
                            0,
                            etree.Comment(
                                " Modified by %s from %s " % (template_name, addon)
                            ),
                        )
...
class WebClient(http.Controller):
    ...
    @http.route(
        "/web/webclient/translations/<string:unique>", type="http", auth="public"
    )
...
class Binary(http.Controller):
    ...
    def content_common(
        ...
        **kw,
        ...
        return self._get_content_common(
            xmlid=xmlid,
            model=model,
            id=id,
            field=field,
    ...
    def content_image_backward_compatibility(self, model, id, field, resize=None, **kw):
        ...
        return self.content_image(
            model=model, id=id, field=field, width=width, height=height
        )
...
class ReportController(http.Controller):
    ...
    def report_barcode(
        self, type, value, width=600, height=100, humanreadable=0, quiet=1, mask=None
    ):
