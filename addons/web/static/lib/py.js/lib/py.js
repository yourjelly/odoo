...
(function (py) {
    ...
    py.evaluate = function (expr, context) {
        ...
        case '.':
            ...
            let first = py.evaluate(expr.first, context);
            if (expr.second.value === 'get' && py.PY_isInstance(first, py.object) && !first.get) {
                first = py.dict.fromJSON(first);
            }
            return py.PY_getAttr(first, expr.second.value);
        // numerical operators
        case '~':
