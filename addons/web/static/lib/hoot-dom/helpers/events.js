 ...
 * @param {EventListener} callback
 */
const unregisterListener = (target, type, callback) => {
    if (!listenerMap.has(target)) {
        return;
    }
    const listeners = listenerMap.get(target);
...
const _pointerDown = (target, eventInit) => {
    ...
    if (!isPrevented(events[0])) {
        ...
        const relatedEvent = dispatch(target, relatedType);
...
const _pointerUp = (target, eventInit) => {
    ...
    if (!events.some(isPrevented)) {
        ...
        events.push(dispatch(target, relatedType));
    }
 ...
 *  - [touch] `touchend`
 *  - `click`
 *  - `dblclick` if click is not prevented & current click count % 2 == 0
 *
 * @param {Target} target
 * @param {PointerOptions} [options]
 * @returns {Event[]}
...
export function dispatch(target, type, eventInit) {
    ...
    if (!event.defaultPrevented && typeof target[type] === "function") {
        switch (type) {
            case "blur":
            case "focus":
            case "submit":
...
export function watchListeners(target) {
    const restoreEventListeners = mockEventListeners(target);

    /**
     * @param {boolean} [cleanup=true]
     */
    return function checkListeners(cleanup = true) {
        if (!isInDOM(target)) {
            return;
        }
        const found = {};
        ...
        if (!cleanup && Object.keys(found).length) {
            console.warn(
                `${target.constructor.name} has event listeners left`,
