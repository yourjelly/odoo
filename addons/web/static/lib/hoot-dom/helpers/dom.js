...
const isNodeCssVisible = (node) => {
    ...
    return !parent || parent === getDefaultRoot() || isRootNode(parent) || isNodeCssVisible(parent);
};
...
const isNodeDisplayed = (node) => {
    return isInDOM(node) && (isRootNode(node) || Boolean(node.offsetParent));
};
...
const isNodeVisible = (node) => {
    ...
    if (!isNodeDisplayed(node) || !isNodeCssVisible(node)) {
        return false;
    }
 ...
 */
const isRootNode = (object) =>
    object && (object.window === object || R_ROOT_ELEMENT.test(object.nodeName || ""));
            ...
            for (const node of groupNodes) {
                ...
                for (let i = 0; i < targetNodes.length; i++) {
                    ...
                    for (const filter of filters) {
                        ...
                        returnNode = result;
                        pseudoReturningNode.push(filter.name);
                    }

                    if (pseudoReturningNode.length > 1) {
                        throw selectorError(
                            pseudoReturningNode[0],
                            `cannot use multiple pseudo-classes returning nodes (${and(
                                pseudoReturningNode
                            )})`
                        );
                    }
    ...
    .set("contains", (content) => {
        ...
        if (regex instanceof RegExp) {
            return function containsRegExp(node) {
                return regex.test(String(getNodeContent(node)));
            };
        } else {
