            ...
            <div class="hoot-results">
                <t t-foreach="filteredResults" t-as="result" t-key="result.id">
                    <t t-if="result.suite">
                        <button
                            type="button"
                            class="hoot-result text-nowrap d-flex align-items-center justify-content-between gap-1 w-100 px-2"
                            t-on-click="(ev) => this.toggleGroup(ev, result.suite.id)"
                        >
                            <span class="d-flex align-items-center">
                                <i
                                    class="hoot-icon fa fa-chevron-right d-flex justify-content-center"
                                    t-att-class="{ 'hoot-rotate-90': state.openGroups.includes(result.suite.id) }"
                                    style="font-size: 12px"
                                />
                                <span class="d-flex align-items-center overflow-hidden">
                                    <t t-foreach="result.suite.path" t-as="suite" t-key="suite.id">
                                        <strong
                                            class="text-nowrap p-1 d-none d-md-inline"
                                            t-att-class="{
                                                'hoot-text-muted': !suite_last,
                                                'hoot-text-primary': suite_last,
                                                'hoot-text-skip': suite.config.skip,
                                            }"
                                            t-esc="suite.name"
                                        />
                                        <t t-if="!suite_last">
                                        ...
                                        <t t-else="">
                                            (<strong class="hoot-text-primary" t-esc="result.suite.currentJobs.length" />)
                                        </t>
                                    </t>
                                </span>
                            </span>
                            <small
                                class="text-nowrap"
                                t-attf-class="hoot-text-{{ result.suite.config.skip ? 'skip' : 'muted' }}"
                                t-esc="formatTime(result.duration)"
                            />
                        </button>
                    </t>
