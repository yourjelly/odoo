 ...
 */
export class SearchPanel extends SearchComponent {
  setup() {
    this.state = useState({
      active: {},
      expanded: {},
  ...
  get sections() {
    return this.env.searchModel.getSections();
  }
  ...
  _toggleFilterGroup(filterId, { values }) {
    const valueIds = [];
    const checked = [...values.values()].every((value) => this.state.active[filterId][value.id]);
    values.forEach(({ id }) => {
      valueIds.push(id);
...
QWeb.registerComponent("SearchPanel", SearchPanel);
