 ...
 */
export class SearchBar extends SearchComponent {
  setup() {
    super.setup();
    ...
    this.autoCompleteSources = this.env.searchModel
  ...
  mounted() {
    this.env.searchModel.on("update", this, this.render);
  ...
  willUnmount() {
    this.env.searchModel.off("update", this);
  ...
  _createSource(searchItem) {
    const field = this.env.searchModel.fields[searchItem.fieldName];
  ...
  async _expandSource(source, shouldExpand) {
    ...
    if (shouldExpand) {
      ...
      const results = await this._orm(source.relation).call("name_search", domain, {
  ...
  _selectSource(source) {
    ...
    if (source.active) {
      ...
      this.env.searchModel.addAutoCompletionValues(source.searchItemId, autocompleteValue);
    }
  ...
  _onSearchKeydown(ev) {
    ...
    switch (ev.key) {
      ...
      case "ArrowLeft":
        ...
        //     this._focusFacet(this.env.searchModel.get("facets").length - 1);
        // }
        break;
      case "ArrowRight":
        // TODO
      ...
      case "Backspace":
        if (!this.state.inputValue.length) {
          // TODO
          // const facets = this.env.searchModel.getFacets();
          // if (facets.length) {
      ...
      case "Enter":
        if (!this.state.inputValue.length) {
          // TODO
          // this.env.searchModel.dispatch('search');
          break;
        }
      /* falls through */
      case "Tab":
...
QWeb.registerComponent("SearchBar", SearchBar);
