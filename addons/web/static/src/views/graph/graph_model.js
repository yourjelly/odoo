...
import { GROUPABLE_TYPES, rankInterval } from "../search/search_utils";
...
import { ORM } from "../../services/orm_service";
  ...
  normalize(metaData) {
    ...
    for (const fieldName in fields) {
      ...
      if (!["id", "__count"].includes(fieldName) && field.store === true) {
        if (
          (!fieldModif.invisible && ["integer", "float", "monetary"].includes(field.type)) ||
