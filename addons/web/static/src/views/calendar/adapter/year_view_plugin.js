...
function createYearView(FullCalendar) {
  const { Calendar, createElement, EventApi, memoizeRendering, View } = FullCalendar;

  return class YearView extends View {
    constructor() {
      super(...arguments);
      this.months = null;
      this.renderSubCalendarsMem = memoizeRendering(
        this.renderSubCalendars,
        this.unrenderSubCalendars
      );
      this.events = [];
    }
...
let plugin = null;
export function createYearViewPlugin(FullCalendar) {
  plugin =
    plugin ||
    FullCalendar.createPlugin({
      views: {
        odooYearView: {
          class: createYearView(FullCalendar),
          duration: { years: 1 },
          defaults: {
            fixedWeekCount: true,
          },
