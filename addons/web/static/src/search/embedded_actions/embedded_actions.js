...
export class EmbeddedActionsDropdown extends Component {
    ...
    async _saveNewAction(ev) {
        ...
        this.props.embeddedActions.push(enrichedNewEmbeddedAction);
        const embeddedActionIdStr = embeddedActionId[0].toString();
        visibleEmbeddedActions[embeddedActionIdStr] = true;
        const order = this.props.embeddedActions.map((el) => el.id);
        ...
        this.state.currentEmbeddedAction = enrichedNewEmbeddedAction;
        this.state.newActionName = `${newActionName} Custom`;
    }

    getDropdownClass(action) {
        const isSelected =
            (!this.env.isSmall && this._checkValueLocalStorage(action)) ||
            (this.env.isSmall && this.state.currentEmbeddedAction?.id === action.id)
                ? "selected"
                : "";
        const isClickable =
    ...
    _onShareCheckboxChange() {
        this.state.newActionIsShared = !this.state.newActionIsShared;
    }
    ...
    async _deleteEmbeddedAction(action) {
        const { visibleEmbeddedActions, embeddedActions, currentEmbeddedAction } = this.state;
        ...
        this.props.embeddedActions = embeddedActions.filter(({ id }) => id !== action.id);
...
export class EmbeddedActions extends Component {
    static template = "web.EmbeddedActions";
    static components = { EmbeddedActionsDropdown };
    static props = {
        embeddedActions: Array,
    ...
    setup() {
        ...
        this.state = useState({
            ...
            visibleEmbeddedActions:
                (this.env.config.embeddedActions?.length > 0 &&
                    JSON.parse(browser.localStorage.getItem(this.embeddedActionsVisibilityKey))) ||
                {},
        ...
        onMounted(async () => {
            if (this.props.embeddedActions?.length > 0) {
                // If there is no visible embedded actions, the current action (if it exists) is put by default
                if (!Object.keys(this.state.visibleEmbeddedActions).includes("false")) {
    ...
    onClickShowEmbedded() {
        if (this.state.showEmbedded) {
            browser.localStorage.removeItem(this.embeddedVisibilityKey);
        } else {
            browser.localStorage.setItem(this.embeddedVisibilityKey, true);
        }
        this.state.showEmbedded = !this.state.showEmbedded;
        browser.localStorage.setItem("showEmbeddedActions", this.state.showEmbedded);
    }
    ...
    _checkValueLocalStorage(action) {
        const actionIdStr = action.id.toString();
        return this.state.visibleEmbeddedActions[actionIdStr];
    }
    ...
    _setVisibility(actionId) {
        ...
        if (this.state.visibleEmbeddedActions[actionIdStr]) {
            delete this.state.visibleEmbeddedActions[actionIdStr];
        } else {
            this.state.visibleEmbeddedActions[actionIdStr] = true;
        }
        browser.localStorage.setItem(
            this.embeddedActionsVisibilityKey,
            JSON.stringify(this.state.visibleEmbeddedActions)
        );
    }
    ...
    async onEmbeddedActionClick(action) {
        this.env.config.setEmbeddedActions(this.props.embeddedActions);
        const { active_id, active_model } = this.env.searchModel.globalContext;
        const actionContext = action.context ? makeContext([action.context]) : {};
        const context = {
            ...actionContext,
            ...
            parent_action_embedded_actions: this.props.embeddedActions,
    ...
    _sortEmbeddedActions(order) {
        this.props.embeddedActions = this.props.embeddedActions.sort((a, b) => {
    ...
    _sortEmbeddedActionDrop({ element, previous }) {
        const order = this.props.embeddedActions.map((el) => el.id).filter((el) => el !== false);
