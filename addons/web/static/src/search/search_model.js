...
export async function processSearchViewDescription(
    ...
    return { defaultValues, fields, irFilters, preSearchItems, sections };
}
    ...
    if (node.nodeType === 1) {
        switch (node.tagName) {
            case "search":
                ...
                break;
            case "searchpanel":
                // the search panel children have a 'field' tag and musth be
                // processed together (see warning in createSectionsFromArch)
                createSectionsFromArch(node.childNodes, data);
                break;
            case "group":
                pushGroup(data);
                for (let child of node.childNodes) {
                ...
                if (node.hasAttribute("context")) {
                    ...
                    try {
                        ...
                        if (groupBy) {
                            ...
                            const [fieldName, defaultInterval] = makeContext(
                                context
                            ).group_by.split(":");
...
function createSectionsFromArch(nodes, data) {
    ...
    for (const node of nodes) {
        ...
        const section = {
            ...
            description: attrs.string || data.fields[attrs.name].string,
        ...
        if (type === "category") {
            section.activeValueId = data.defaultValues[attrs.name];
        ...
        data.sections.push([section.id, section]);
    }
...
export class SearchModel extends Model {
    ...
    async load(config) {
        ...
        // View component could do something similar for __exportState__ and use some
        // state storage?
        this.__saveParams__ = config.__saveParams__ || new CallbackRecorder();
    ...
    async createNewFavorite(params) {
        ...
        const fns = this.__saveParams__.callbacks;
...
SearchModel.services = ["orm", "user", "view"];
