...
/**
 * Function to use as an event handler to replay the incoming event after all
 * the lazy JS has been loaded. Note that blocking the incoming event is left
...
function waitLazy() {
    ...
    const loadingEffectButtonEls = [...mainEl.querySelectorAll(BUTTON_HANDLER_SELECTOR)]
        // We target all buttons but we allow to disable the effect by adding
        // a specific class if needed. Note that if some non-lazy loaded code
        // is adding an event handler on some buttons, it means that if they do
        // not have that class, they will show a loading effect and not do
        // anything until lazy JS is loaded anyway. This is not ideal,
        // especially since this was added as a stable fix/imp, but this is a
        // compromise: on next page visits, the cache should limit to effect of
        // the lazy loading anyway.
        .filter(el => {
    ...
    for (const formEl of document.querySelectorAll('form:not(.o_no_wait_lazy_js)')) {
        registerLoadingEffectHandler(formEl, 'submit', blockFunction);
    }
}
/**
