...
var ListRenderer = BasicRenderer.extend({
    ...
    _renderView: async function () {
        ...
        if (this.state.count > 0 || !this.noContentHelp) {
            ...
            const $table = $('<table>').addClass('o_list_table table table-sm table-hover table-striped');
        ...
        this.el.innerHTML = "";
        this.$el.removeClass('o_list_optional_columns');
        ...
        if (tableWrapper) {
            ...
            if (this.optionalColumns.length) {
                this.$el.addClass('o_list_optional_columns');
            ...
            if (this.selection.length) {
                const $checked_rows = this.$('tr').filter((index, el) => {
                    return this.selection.includes($(el).data('id'));
                });
