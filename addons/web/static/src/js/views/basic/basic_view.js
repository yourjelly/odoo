...
var BasicView = AbstractView.extend({
    ...
    _processView: function (fieldsView, type) {
        fieldsView.arch = this._postprocess_fvg(fieldsView.arch, fieldsView.fields);
        // write the field name inside the description for fields in view
        for (var fieldName in fieldsView.fields) {
            fieldsView.fields[fieldName].name = fieldName;
        }
        fieldsView.type = type;
        this._processFieldsView(fieldsView);
    },
    /**
