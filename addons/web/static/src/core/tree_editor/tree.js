 ...
 * @param {boolean} [negate=false]
 * @returns {import("@web/core/tree_editor/domain_tree").Condition|ComplexCondition}
 */
 ...
 * @returns {Tree}
 */
function _expressionTreeFromAST(ast, options, negate = false) {
    if (isNot(ast)) {
        return _expressionTreeFromAST(ast.right, options, !negate);
    }
    ...
    if (ast.type === 14) {
        ...
        for (const subAST of subASTs) {
            const child = _expressionTreeFromAST(subAST, options, options.distributeNot && negate);
            addChild(tree, child);
        }
        return tree;
    }
    ...
    if (ast.type === 13) {
        ...
        return _expressionTreeFromAST(newAST, options, negate);
    }
...
function _expressionFromTree(tree, options, isRoot = false) {
    ifElseCase: if (tree.type === "connector" && tree.value === "|" && tree.children.length === 2) {
        // check if we have an "if else"
        const isSimpleAnd = (tree) =>
            tree.type === "connector" && tree.value === "&" && tree.children.length === 2;
        if (tree.children.some((c) => !isSimpleAnd(c))) {
            break ifElseCase;
        }
        ...
        for (let i = 0; i < 2; i++) {
            ...
            for (let j = 0; j < 2; j++) {
                ...
                if (str1 === `not ${str2}` || `not ${str1}` === str2) {
                    // do smth smarter. this is very fragile
 ...
 * @returns {ExpressionTree} the conditions better expressed as atomic expressions become atomic expressions
 */
...
/**
 * @param {ExpressionTree} expressionTree
 * @returns {ExpressionTree} without atomic expressions
 */
function removeComplexConditions(expressionTree) {
    if (expressionTree.type === "condition") {
        return { ...expressionTree };
    }
    if (expressionTree.type === "complex_condition") {
        const ast = parseExpr(expressionTree.value);
        return {
            type: "condition",
    ...
    return {
        ...expressionTree,
        children: expressionTree.children.map((child) => removeComplexConditions(child)),
    };
}
...
export function treeFromExpression(expression, options = {}) {
    ...
    const expressionTree = _expressionTreeFromAST(ast, options);
    return createBetweenOperators(expressionTree);
}
 ...
 */
export function expressionFromTree(expressionTree, options = {}) {
    const simplifiedTree = removeBetweenOperators(expressionTree);
...
export function expressionFromDomain(domain, options = {}) {
    ...
    return expressionFromTree(createComplexConditions(tree), options);
}
...
export function domainFromExpression(expression, options = {}) {
    const expressionTree = treeFromExpression(expression, options);
    return domainFromTree(expressionTree);
}
...
export function domainFromTree(tree) {
    const simplifiedTree = removeBetweenOperators(removeComplexConditions(tree));
    const domainAST = {
        type: 4,
