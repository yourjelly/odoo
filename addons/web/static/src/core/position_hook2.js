 ...
 * @property {(popperElement: HTMLElement, target: HTMLElement, options)} [shouldCompute]
...
export function usePosition(refName, getTarget, getOptions = () => DEFAULTS) {
    ...
    useEffect(() => {
        ...
        let options = getOptions(el, target);
        if (!options) {
            // No compute needed
            return;
        }
        options = { ...DEFAULTS, ...options };
        const iframe = getIFrame(target);
        compute = () => reposition(target, el, iframe, options);
