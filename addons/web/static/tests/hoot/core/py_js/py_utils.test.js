...
describe("@web", "core", "py_js", "py_utils", () => {
    describe("formatAST", () => {
        test("basic values", () => {
            expect(checkAST("1", "integer value")).toBe(true);
            expect(checkAST("1.4", "float value")).toBe(true);
            expect(checkAST("-12", "negative integer value")).toBe(true);
        ...
        test("simple arithmetic", () => {
            ...
            expect(checkAST("a // 10", "integer division")).toBe(true);
        });

        test("boolean operators", () => {
            expect(checkAST("True and False", "boolean operator")).toBe(true);
            expect(checkAST("True or False", "boolean operator or")).toBe(true);
            expect(checkAST("(True or False) and False", "boolean operators and and or")).toBe(
                true
            );
        ...
        test("context_today", () => {
            expect(checkAST(`context_today().strftime("%Y-%m-%d")`, "context today call")).toBe(
                true
            );
        });
        ...
        test("escaping support", () => {
            expect(evaluateExpr(String.raw`"\x61"`)).toBe("a", "hex escapes");
            expect(evaluateExpr(String.raw`"\\abc"`)).toBe(String.raw`\abc`, "escaped backslash");
