...
function parseArch(arch, options = {}) {
    const parser = new CalendarArchParser({
        arch,
        fields: "fields" in options ? options.fields : FAKE_FIELDS,
        date: "date" in options ? options.date : FAKE_DATE,
...
QUnit.todo("fieldMapping", (assert) => {
    assert.ok(false);
});

QUnit.todo("fieldNames", (assert) => {
    assert.ok(false);
});

QUnit.todo("filtersInfo", (assert) => {
    assert.ok(false);
});

QUnit.todo("formViewId", (assert) => {
...
QUnit.todo("popoverFields", (assert) => {
    assert.ok(false);
});
