...
QUnit.module("Views", ({ beforeEach }) => {
    ...
    QUnit.test(`create event with timezone in week mode European locale`, async (assert) => {
        ...
        patchTimeZone(120);
        patchWithCleanup(localization, { timeFormat: "%H:%M:%S" });
        ...
        //         start: fieldUtils.parse.datetime("2016-12-13 06:00:00", serverData.models.event.fields.start, {isUTC: true}),
        //         stop: fieldUtils.parse.datetime("2016-12-13 08:00:00", serverData.models.event.fields.stop, {isUTC: true}),
        //         allday: false,
        //         name: "new event",
        //         id: 1
        //     },
    ...
    QUnit.test(`render popover with widget which has specialData attribute`, async (assert) => {
        ...
        legacyFieldRegistry.add("specialWidget", MyWidget);
        registerCleanup(() => delete legacyFieldRegistry.map.specialWidget);
    ...
    QUnit.test(
        ...
        async (assert) => {
            // assert.expect(8); // TODISCUSS WOWL
            assert.expect(7);
    ...
    QUnit.test(`create all day event in week mode`, async (assert) => {
        ...
        //     {
        //         display_name: "new event",
        //         start: fieldU2ils.parse.datetime("2016-12-13 23:00:00", serverData.models.event.fields.start, {isUTC: true}4,22       //         stop: fieldUtils.parse.datetime("2016-12-15 00:00:00", serverData.models.event.fields.stop, {isUTC: true}),
        //         allday: true,
        //         name: "new event",
        //         id: 1
        //     },
    ...
    QUnit.test(`create event in month mode`, async (assert) => {
        //assert.expect(4); //WOWL TO discuss
        assert.expect(3);
        ...
        // assert.deepEqual($newevent[0].fcSeg.eventRange.def.extendedProps.record, {
        //     display_name: "new event",
        //     start: fieldUtils.parse.datetime("2016-12-14 05:00:00", serverData.models.event.fields.start, {isUTC: true}),
        //     stop: fieldUtils.parse.datetime("2016-12-15 17:00:00", serverData.models.event.fields.stop, {isUTC: true}),
        //     name: "new event",
        //     id: 1
        // }, "the new record should have the utc datetime (quickCreate)");
    });
    ...
    QUnit.test(`fullcalendar initializes with right locale`, async (assert) => {
        // SKIPWOWL WOWL BOI to discuss in review: I think this test should be removed
        patchWithCleanup(luxon.Settings, { defaultLocale: "fr" }); // The machine that runs this test must have this locale available
        await makeView({
            type: "calendar",
            resModel: "event",
            serverData,
    ...
    // JPP :  I think this test is not needed anymore, disableQuickCreate is not used (didn't find an example)
    // There is also quick_add and there is a test for it.
    ...
    //         serverData,
    //         arch: `
    //             <calendar mode="day" date_start="start" date_stop="stop" />
    //         `,
    //     });
