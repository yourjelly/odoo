...
import { CalendarYearRenderer } from "@web/views/calendar/renderer/year/calendar_year_renderer";
...
QUnit.test("click on a date without events", async (assert) => {
    assert.expect(2);
...
QUnit.test("click on a date with events", async (assert) => {
    assert.expect(1);
