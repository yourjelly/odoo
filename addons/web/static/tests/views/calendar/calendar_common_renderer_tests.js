...
import { CalendarCommonRenderer } from "@web/views/calendar/renderer/common/calendar_common_renderer";
...
QUnit.test("Day: check event", async (assert) => {
    const calendar = await start({ model: { scale: "day" } });
    assert.containsOnce(calendar.el, ".o-calendar--event");
    assert.hasAttrValue(calendar.el.querySelector(".o-calendar--event"), "data-event-id", "1");
});

QUnit.todo("Day: hover event", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "day" } });
});
...
QUnit.todo("Day: move event from all day slot to timed slot", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "day" } });
});

QUnit.todo("Day: move event from timed slot to timed slot", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "day" } });
});

QUnit.todo("Day: move event from timed slot to all day slot", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "day" } });
});

QUnit.todo("Day: resize event, increase time", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "day" } });
});

QUnit.todo("Day: resize event, decrease time", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "day" } });
});
...
QUnit.todo("Week: click all day slot", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "week" } });
});

QUnit.todo("Week: select range on same day", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "week" } });
});

QUnit.todo("Week: select range on different day", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "week" } });
});

QUnit.todo("Week: check event", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "week" } });
});

QUnit.todo("Week: hover event", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "week" } });
});

QUnit.todo("Week: click on event", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "week" } });
});

QUnit.todo("Week: move event from all day slot to timed slot on same day", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "week" } });
});

QUnit.todo("Week: move event from timed slot to timed slot on same day", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "week" } });
});

QUnit.todo("Week: move event from timed slot to all day slot on same day", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "week" } });
});

QUnit.todo("Week: move event from all day slot to timed slot on different day", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "week" } });
});

QUnit.todo("Week: move event from timed slot to timed slot on different day", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "week" } });
});

QUnit.todo("Week: move event from timed slot to all day slot on different day", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "week" } });
});

QUnit.todo("Week: resize event, increase time on same day", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "week" } });
});

QUnit.todo("Week: resize event, decrease time on same day", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "week" } });
});

QUnit.todo("Week: resize event, increase time on different day", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "week" } });
});

QUnit.todo("Week: resize event, decrease time on different day", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "week" } });
});

QUnit.todo("Month: check week numbers", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "month" } });
});

QUnit.todo("Month: check dates", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "month" } });
});

QUnit.todo("Month: click date", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "month" } });
});

QUnit.todo("Month: select range", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "month" } });
});

QUnit.todo("Month: check event", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "month" } });
});

QUnit.todo("Month: hover event", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "month" } });
});

QUnit.todo("Month: click on event", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "month" } });
});

QUnit.todo("Month: move event", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "month" } });
});

QUnit.todo("Month: resize event, increase time", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "month" } });
});

QUnit.todo("Month: resize event, decrease time", async (assert) => {
    assert.ok(false);
    // const calendar = await start({ model: { scale: "month" } });
});
