...
QUnit.assert.checkLegend = function (graph, expectedLegendLabels, message) {
    expectedLegendLabels = expectedLegendLabels instanceof Array ?
                                expectedLegendLabels :
                                [expectedLegendLabels];
    var legend = graph.renderer.chart.generateLegend();
    var actualLegendLabels = $(legend).find('li').toArray().map(node => node.textContent);
    this.pushResult({
...
QUnit.assert.checkLabels = function (graph, expectedLabels) {
    expectedLabels = expectedLabels instanceof Array ?
                        expectedLabels :
                        [expectedLabels];
...
QUnit.module('Views', {
    ...
    QUnit.test('use a many2one as a measure should work (without groupBy)', async function (assert) {
        ...
        assert.containsOnce(graph, 'div.o_graph_canvas_container canvas',
                    "should contain a div with a canvas element");
        assert.checkLabels(graph, 'Total');
    ...
    QUnit.test('Undefined should appear in bar, pie graph but not in line graph', async function (assert) {
        ...
        assert.strictEqual(graph.renderer.chart.data.labels.indexOf('Undefined'), -1);

        await testUtils.dom.click(graph.$buttons.find('.o_graph_button[data-mode=bar]'));
        assert.ok(graph.renderer.chart.data.labels.indexOf('Undefined') >= 0);
    ...
    QUnit.test('Undefined should appear in bar, pie graph but not in line graph with multiple groupbys', async function (assert) {
        ...
        await testUtils.dom.click(graph.$buttons.find('.o_graph_button[data-mode=pie]'));
        var labels = graph.renderer.chart.data.labels
        assert.ok(labels.filter(label => /Undefined/.test(label)).length >= 1);
    ...
    QUnit.module('Graph View: comparison mode', {
        beforeEach: async function () {
            ...
            this.data.foo.records.push({id: 9, foo: 48, bar: false, product_id: 41, color_id: 7, date: "2016-12-0"});
    ...
    // QUnit.skip('test tooltips callbacks', async function (assert) {
    //     assert.expect(1);
    ...
    //     var tooltipCallbacks = chart.options.tooltips.callbacks;
    ...
    //     assert.strictEqual(label, 'My dataset: 20');


    //     // check tooltip (2nd method)
    ...
    //     // // does not work here!!!! How should we compute clientY?
    //     //     clientY: 0,
    //     // });
    //     // canvas.dispatchEvent(evt);
    //     // var tooltip = chart.tooltip;

    //     // assert.deepEqual(tooltip._view, {
    //     //     // Body
    //     //     bodyFontColor: '#fff',
    //     //     _bodyAlign: 'left',
    //     //     bodySpacing: 2,
