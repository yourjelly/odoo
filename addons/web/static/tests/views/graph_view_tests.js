...
function onGraphSaveParams(graph) {
    /** @todo DAM */
    console.warn("Not the right way --> review way to save params in favorite menu");
    return graph.saveParams();
}
...
QUnit.module("Views", (hooks) => {
    ...
    QUnit.test(
        ...
        async function (assert) {
            assert.expect(4);
            // do we want to keep a context of that form?
            const graph = await makeView(
                { serverData },
                {
                    type: "graph",
                    modelName: "foo",
                    arch: `<graph string="Gloups"><field name="product_id"/></graph>`,
                    views: [[false, "search"]],
                }
            );
            assert.deepEqual(onGraphSaveParams(graph), {
                context: {
                    graph_mode: "bar",
    ...
    QUnit.test("reload graph with correct fields", async function (assert) {
        ...
        const graph = await makeView(
            ...
            },
            {
                type: "graph",
                modelName: "foo",
                arch: `<graph><field name="foo" type="measure"/></graph>`,
                views: [[false, "search"]],
            }
        );
        await toggleFilterMenu(graph);
        ...
        async function (assert) {
            ...
            const graph = await makeView(
                { serverData },
                {
                    type: "graph",
                    modelName: "foo",
                    arch: `<graph><field name="product_id" type="measure"/></graph>`,
                }
            );
            checkLabels(assert, graph, ["Total"]);
