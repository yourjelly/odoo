...
import { createWebClient, doAction } from "../../actions/helpers";
import { fakeTitleService, makeFakeLocalizationService, makeFakeRouterService, makeFakeUIService, makeFakeUserService } from "../../helpers/mock_services";
...
async function clickOnDataset(graph) {
  ...
  await triggerEvent(chart.canvas, "click", {
...
QUnit.module("Views", (hooks) => {
  ...
  QUnit.test("simple bar chart rendering with no data", async function (assert) {
    assert.expect(4);
    testConfig.serverData.models.foo.records = [];
    const graph = await makeView(
      { testConfig },
      {
        type: "graph",
  ...
  QUnit.test("displaying line chart with only 1 data point", async function (assert) {
    ...
    // point is displayed.
    testConfig.serverData.models.foo.records = testConfig.serverData.models.foo.records.slice(0, 1);
    const graph = await makeView(
      { testConfig },
      {
        type: "graph",
  ...
  QUnit.test("pie chart rendering (no data, several domains)", async function (assert) {
    assert.expect(11);
    testConfig.serverData.models.foo.records = [{ product_id: 37, bar: true }];
    const graph = await makeView(
      { testConfig },
      {
        type: "graph",
    ...
    checkTooltip(assert, graph, { lines: [{ label: "True group / xphone", value: "1" }] }, 0, 0);
    checkTooltip(assert, graph, { lines: [{ label: "False group / No data", value: "0" }] }, 1, 1);
  });
  ...
  QUnit.test("process simple arch (no field tag)", async function (assert) {
    assert.expect(2);
    const fields = testConfig.serverData.models.foo.fields;
  ...
  QUnit.test("correctly uses graph_ keys from the context", async function (assert) {
    assert.expect(8);
    const recs = testConfig.serverData.models.foo.records;
  ...
  QUnit.test("correctly uses graph_ keys from the context (at reload)", async function (assert) {
    assert.expect(10);
    testConfig.serverData.views["foo,false,search"] = `
      <search>
  ...
  QUnit.test("differentiate many2one values with same label", async function (assert) {
    assert.expect(1);
    testConfig.serverData.models.product.records.push({ id: 39, display_name: "xphone" });
    testConfig.serverData.models.foo.records.push({ id: 18, product_id: 39 });
    const graph = await makeView(
      { testConfig },
      {
        type: "graph",
  ...
  QUnit.test("not use a many2one as a measure by default", async function (assert) {
    ...
    await toggleMenu(graph, "Measures");
    assert.deepEqual(
      [...graph.el.querySelectorAll(".o_cp_bottom_left li.o_menu_item")].map((el) => el.innerText),
      ["Foo", "Revenue", "Count"]
    );
  });
  ...
  QUnit.test(
    ...
    async function (assert) {
      assert.expect(1);
      testConfig.serverData.models.foo.fields.bouh = {
        string: "Bouh",
        type: "integer",
        store: true,
        group_operator: "sum",
  ...
  QUnit.test('graph view "graph_measure" field in context', async function (assert) {
    ...
    checkLegend(assert, graph, "Product");
    assert.strictEqual(getYAxeLabel(graph), "Product");
    checkTooltip(assert, graph, { title: "Product", lines: [{ label: "Total", value: "2" }] }, 0);
  });

  QUnit.test('"graph_measure" in context is prefered to measure in arch', async function (assert) {
    assert.expect(6);
    const graph = await makeView(
      { testConfig },
      {
  ...
  QUnit.test(
    ...
    async function (assert) {
      ...
      await toggleMenu(graph, "Measures");
      try {
        findItem(graph, "Revenue");
      } catch (e) {
        assert.ok(true, `"Revenue" can not be found in the "Measures" menu`);
      }
    }
  );
  ...
  QUnit.test(
    ...
    async function (assert) {
      assert.expect(6);
      testConfig.serverData.views["foo,364,list"] = `<list/>`;
      testConfig.serverData.views["foo,29,form"] = `<form/>`;
  ...
  QUnit.test("graph view sort by measure", async function (assert) {
    ...
    testConfig.serverData.models.product.records.push({ id: 38, display_name: "zphone" });
    testConfig.serverData.models.foo.records[7].product_id = 38;
  ...
  QUnit.test("graph view sort by measure for multiple grouped data", async function (assert) {
    ...
    testConfig.serverData.models.product.records.push({ id: 38, display_name: "zphone" });
    testConfig.serverData.models.foo.records[7].product_id = 38;
    testConfig.serverData.models.foo.records.splice(
      0,
      4,
  ...
  QUnit.test("empty graph view with sample data", async function (assert) {
    ...
    assert.hasClass(graph.el.querySelector(".o_graph_canvas_container"), "o_sample_data_disabled");
  ...
  QUnit.test("reload chart with switchView button keep internal state", async function (assert) {
    assert.expect(3);
    testConfig.serverData.views["foo,false,list"] = `<list/>`;
