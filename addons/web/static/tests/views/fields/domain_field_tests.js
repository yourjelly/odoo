...
import {
    addNewRule as DomainSelector_addNewRule,
    clearNotSupported as DomainSelector_clearNotSupported,
    editValue as DomainSelector_editValue,
    getConditionText as DomainSelector_getConditionText,
    getCurrentValue as DomainSelector_getSelectedValue,
...
QUnit.module("Fields", (hooks) => {
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            assert.strictEqual(
                DomainSelector_getSelectedValue(target),
                "uid",
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            assert.equal(DomainSelector_getSelectedValue(target), "context_today()");

            await DomainSelector_clearNotSupported(target);
    ...
    QUnit.test("basic domain field usage is ok", async function (assert) {
        ...
        assert.containsOnce(target, DomainSelector_SELECTORS.addNewRule);

        // Clicking on the button should add the [["id", "=", "1"]] domain, so
        // there should be a field selector in the DOM
        await DomainSelector_addNewRule(target);
        ...
        await DomainSelector_editValue(target, 2);
    ...
    QUnit.test("domain field: manually edit domain with textarea", async function (assert) {
        ...
        await editInput(target, DomainSelector_SELECTORS.debugArea, "[['id', '<', 40]]");
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            await editInput(target, DomainSelector_SELECTORS.debugArea, "[['abc', '=', 1]]");
            // the count should not be re-computed when editing with the textarea
            assert.strictEqual(
                target.querySelector(".o_domain_show_selection_button").textContent.trim(),
                "2 record(s)"
            ...
            await editInput(target, DomainSelector_SELECTORS.debugArea, "[['abc']]");
            assert.verifySteps([]);
    ...
    QUnit.test("domain field: edit domain with dynamic content", async function (assert) {
        ...
        await doAction(webClient, 1);
        assert.strictEqual(
            target.querySelector(DomainSelector_SELECTORS.debugArea).value,
            rawDomain
        );
        ...
        await editInput(target, DomainSelector_SELECTORS.debugArea, rawDomain);
        assert.strictEqual(
            target.querySelector(DomainSelector_SELECTORS.debugArea).value,
            rawDomain
        );
    ...
    QUnit.test("domain field with 'inDialog' options", async function (assert) {
        ...
        assert.strictEqual(DomainSelector_getConditionText(target), "ID = 1");
    });
    ...
    QUnit.test("invalid value in domain field with 'inDialog' options", async function (assert) {
        ...
        await click(target, `.modal ${DomainSelector_SELECTORS.addNewRule}`);
        await editInput(target, DomainSelector_SELECTORS.debugArea, "[(0, '=', expr)]");
        await click(target, ".modal-footer .btn-primary");
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            await editInput(target, DomainSelector_SELECTORS.debugArea, "[['id', '!=', False]]");
