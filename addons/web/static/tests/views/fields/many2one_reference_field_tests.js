...
QUnit.module("Fields", (hooks) => {
    ...
    QUnit.test("Many2OneReferenceField in form view", async function (assert) {
        ...
        await makeView({
            ...
            resModel: "partner",
            resId: 1,
            arch: '<form><field name="res_id" widget="many2one_reference_dynamic"/></form>',
            mockRPC(route, args) {
    ...
    QUnit.test("Many2OneReferenceField in list view", async function (assert) {
        await makeView({
            ...
            resModel: "partner",
            resId: 1,
            arch: '<list><field name="res_id" widget="many2one_reference_dynamic"/></list>',
        });
    ...
    QUnit.test("Many2OneReferenceField with no_open option", async function (assert) {
        await makeView({
            ...
            resModel: "partner",
            resId: 1,
            arch: `<form><field name="res_id" widget="many2one_reference_dynamic" options="{'no_open': 1}"/></form>`,
        });
    ...
    QUnit.test("Many2OneReferenceField: unset, model_field not in view", async function (assert) {
        await makeView({
            type: "form",
            serverData,
            resModel: "partner",
        ...
        // should be readonly as we can't infer the model
        assert.containsNone(target.querySelector(".o_field_widget[name=res_id]"), "input");
    });

    QUnit.test(
        "Many2OneReferenceField edition: unset, model_field in view",
        async function (assert) {
            ...
            await makeView({
                ...
                <form>
                    <field name="model"/>
                    <field name="res_id" widget="many2one_reference_dynamic"/>
                </form>`,
    ...
    QUnit.test(
        "Many2OneReferenceField edition: set, model_field not in view",
        async function (assert) {
            assert.expect(3);
            ...
            await makeView({
                ...
                mockRPC(route, { method, args }) {
                    if (method === "web_save") {
                        assert.deepEqual(args, [[1], { res_id: 14 }]);
                    }
                },
            });
    ...
    QUnit.test("Many2OneReferenceField with no_create option", async function (assert) {
        await makeView({
            ...
            resModel: "partner",
            resId: 1,
            arch: `<form><field name="res_id" widget="many2one_reference_dynamic" options="{'no_create': 1}"/></form>`,
        });
