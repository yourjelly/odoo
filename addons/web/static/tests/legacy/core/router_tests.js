...
QUnit.test("can parse an search with 2 key/value pairs", (assert) => {
    assert.deepEqual(parseSearchQuery("?a=1&g=2"), { a: 1, g: 2 });
});
...
QUnit.test("stateToUrl encodes URI compatible strings", (assert) => {
    let route = {};
    assert.strictEqual(stateToUrl(route), "/home");

    route = { a: "11", g: "summer wine" };
    assert.strictEqual(stateToUrl(route), "/home?a=11&g=summer%20wine");

    route = { g: "2", c: "", e: "kloug,gloubi" };
    assert.strictEqual(stateToUrl(route), "/home?g=2&c=&e=kloug%2Cgloubi");
});
...
QUnit.test("test the help utils history.back and history.forward", async (assert) => {
    ...
    router.pushState({ k1: 1 });
    await nextTick();
    assert.deepEqual(browser.location.href, "http://lordofthering/home?k1=1");

    router.pushState({ k2: 2 });
    await nextTick();
    assert.deepEqual(browser.location.href, "http://lordofthering/home?k1=1&k2=2");

    router.pushState({ k3: 3 }, { replace: true });
    await nextTick();
    assert.deepEqual(browser.location.href, "http://lordofthering/home?k3=3");
    ...
    await nextTick();
    assert.deepEqual(browser.location.href, "http://lordofthering/home?k4=3");
...
QUnit.test("parse an url with hash (key/values)", async (assert) => {
    ...
    assert.strictEqual(browser.location.pathname, "/home");
});
