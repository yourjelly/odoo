...
test("filter by a date field using period works even in January", async () => {
    ...
    const searchBar = await mountWithSearch(SearchBar, {
        ...
        searchViewArch: `
            <search>
                <filter string="Date" name="some_filter" date="date_field" default_period="month_-1"/>
            </search>
        `,
...
test("filter with date attribute set as search_default", async () => {
    ...
    await mountWithSearch(SearchBar, {
        ...
        searchViewArch: `
            <search>
                <filter string="Date" name="date_field" date="date_field" default_period="month_-1"/>
            </search>
        `,
...
test("filter with multiple values in default_period date attribute set as search_default", async () => {
    ...
    await mountWithSearch(SearchBarMenu, {
        ...
        searchViewArch: `
            <search>
                <filter string="Date" name="date_field" date="date_field" default_period="year_0,year_-1"/>
            </search>
        `,
        context: { search_default_date_field: true },
    });
