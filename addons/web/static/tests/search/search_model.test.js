...
test("parsing one filter tag with default_period date attribute", async () => {
    const model = await createSearchModel({
        searchViewArch: `
            <search>
                <filter name="date_filter" string="Date" date="date_field" default_period="year_0,year_-1"/>
            </search>
        `,
    });
    ...
    expect(sanitizeSearchItems(model)).toEqual([
        {
            defaultGeneratorIds: ["year_0", "year_-1"],
            description: "Date",
            domain: "[]",
...
test("parsing date filter with startMonth, endMonth, startYear, endYear attributes", async () => {
    const model = await createSearchModel({
        searchViewArch: `
            <search>
                <filter 
...
test("parsing date filter with sub filters", async () => {
    const model = await createSearchModel({
        searchViewArch: `
            <search>
...
test("toggle a date filter", async () => {
    ...
    model.toggleDateFilter(filterId, "year_0");
    expect(model.domain).toEqual([]);
});

test("toggle a custom filter in a date filter", async () => {
    mockDate("2019-01-06T15:00:00");
    const model = await createSearchModel({
        searchViewArch: `
            <search>
...
test("toggle a custom filter in a date filter with a domain", async () => {
    mockDate("2019-01-06T15:00:00");
    const model = await createSearchModel({
        searchViewArch: `
            <search>
