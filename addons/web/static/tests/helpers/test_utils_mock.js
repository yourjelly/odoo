 ...
 *
 * @param {Component} Component the Component class to set the env on
 * @param {Object} [params]
 ...
 * @param {boolean} [params.mockSRC=false] if true, will redirect src calls to
 *   the mock server
 * @param {MockServer} [mockServer]
 * @returns {function} a clean up function, must be called at the end of the test
 */
async function setMockedOwlEnv(Component, params, mockServer) {
    params = params || {};
    ...
    // instantiate a Bus if not provided
    let destroyBus;
    let bus = params.bus;
    if (!bus) {
        bus = new Bus();
    ...
    const envBusTrigger = bus.trigger;
    const busTrigger = function () {
        core.bus.trigger(...arguments);
        envBusTrigger.call(bus, ...arguments);
    };
    ...
    let done = false;
    while (!done) {
        const serviceName = _.findKey(servicesToDeploy, function (Service) {
        ...
        if (serviceName) {
            ...
            const service = services[serviceName] = new Service(env);
    ...
    // patch session (for legacy widgets)
    ...
    // patch config (for legacy widgets)
    let initialConfig;
    if ('config' in params) {
    ...
    // patch translate params (for legacy widgets)
...
async function addMockEnvironment(widget, params) {
    ...
    const cleanUp = await setMockedOwlEnv(owl.Component, params, mockServer);
...
return {
    ...
    setMockedOwlEnv: setMockedOwlEnv,
    intercept: intercept,
    patchDate: patchDate,
    patch: patch,
    unpatch: unpatch,
