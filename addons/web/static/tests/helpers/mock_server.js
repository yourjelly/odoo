...
export class MockServer {
  ...
  _mockSearchPanelSelectMultiRange(model, [fieldName], kwargs) {
    ...
    if (field.type === "many2many") {
      const comodelRecords = this.mockSearchRead(field.relation, [comodelDomain, fieldNames], {
        limit,
      });
      ...
      for (const record of comodelRecords) {
        ...
        let inImage;
        if (enableCounters || !expand) {
          const searchDomain = new Domain([...modelDomain, [fieldName, "in", record.id]]).toList();
          ...
          const searchCountDomain = new Domain([...searchDomain, ...localExtraDomain]).toList();
          if (enableCounters) {
    ...
    if (field.type === "many2one") {
      ...
      if (!expand) {
        ...
        comodelDomain = new Domain([...comodelDomain, ["id", "in", imageElementIds]]).toList();
      }
      const comodelRecords = this.mockSearchRead(field.relation, [comodelDomain, fieldNames], {
        limit,
      });
