...
QUnit.module('relational_fields', {
    ...
    QUnit.test('fieldmany2many tags with color: rendering and edition', function (assert) {
        ...
        assert.ok(form.$('.badge:contains(silver)').length,
        ...
        assert.strictEqual(form.$('.o_field_many2manytags > .badge').length, 3,
            "should contain 3 tags");
        assert.ok(form.$('.o_field_many2manytags > .badge:contains("red")').length,
            "should contain newly added tag 'red'");
        assert.ok(form.$('.o_field_many2manytags > .badge[data-color=8]:contains("red")').length,
        ...
        $input = form.$('.o_field_many2manytags .badge[data-id=13] > a'); // selects 'red' tag
        $input.click(); // opens the colorpicker dropdown
        assert.ok(form.$('.o_field_many2manytags .badge[data-id=13] .o_colorpicker .custom-checkbox'), "should have a checkbox in the colorpicker dropdown menu");

        var $checkBox = form.$('.o_field_many2manytags .badge[data-id=13] .o_colorpicker .custom-checkbox input');
        assert.notOk($checkBox.is(':checked'), "should have unticked checkbox in colorpicker dropdown menu");

        $checkBox.mousedown();
        $input = form.$('.o_field_many2manytags .badge[data-id=13]'); // refresh
        assert.equal($input.data('color'), "0", "should become transparent when toggling on checkbox");

        $input.find('a').click();
        $checkBox = form.$('.o_field_many2manytags .badge[data-id=13] .o_colorpicker .custom-checkbox input'); // refresh
    ...
    QUnit.test('fieldmany2many tags: update color', function (assert) {
        ...
        assert.ok(form.$('.badge:first()').is('.o_tag_color_0'),
            'first tag color should be 0');

        // Update the color in readonly
        form.$('.badge:first() > a').click();
        ...
        form.$buttons.find('.o_form_button_edit').click();
        form.$('.badge:first() > a.dropdown-toggle').click();
