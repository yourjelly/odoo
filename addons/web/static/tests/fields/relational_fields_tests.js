...
QUnit.module('relational_fields', {
    ...
    QUnit.only('one2many with multiple UPDATE onchanges', function (assert) {
        assert.expect(6);

        this.data.partner.records[0].turtles = [1, 2];
        this.data.partner.onchanges = {
            turtles: function (obj) {
                // to [1, 4] the server returns [1, 1]
                obj.turtles = [
                    [5],
                    [1, 1, {
                        display_name: "leonardo",
        ...
        var form = createView({
            ...
            mockRPC: function (route, args) {
                if (args.method === 'onchange') {
                    assert.deepEqual(args.args[1].turtles, [
                        [1, 1, {display_name: "leonardo", turtle_int: 1}],
                        [4, 2, false],
                    ], "should correctly send 1 and 4 commands");
                }
                return this._super.apply(this, arguments);
            },
            // debug: 1,
        ...
        assert.strictEqual(form.$('.o_field_one2many .o_data_row').length, 2,
            "o2m should contain 2 rows");
        assert.strictEqual(form.$('.o_field_one2many .o_data_row').text(), "leonardo0donatello9",
            "o2m list should correctly display the records");

        // update the first row
        form.$('.o_field_one2many .o_data_row:first .o_data_cell:eq(1)').click();
        form.$('.o_field_one2many .o_data_row:first .o_data_cell:eq(1) input').val('1').trigger('input');
        form.$('.o_field_char[name="foo"]').click();
        assert.strictEqual(form.$('.o_field_one2many .o_data_row').text(), "leonardo0donatello9",
            "o2m list should correctly display the records after onchange1");

        // update the first row again
        form.$('.o_field_one2many .o_data_row:first .o_data_cell:eq(1)').click();
        form.$('.o_field_one2many .o_data_row:first .o_data_cell:eq(1) input').val('1').trigger('input');
        form.$('.o_field_char[name="foo"]').click();
        assert.strictEqual(form.$('.o_field_one2many .o_data_row').text(), "leonardo0donatello9",
            "o2m list should correctly display the records after onchange2");
