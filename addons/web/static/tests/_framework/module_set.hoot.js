...
const runTests = async () => {
    ...
    const modTotal = performance.now() - startTime;
    const s = moduleSetCount === 1 ? "" : "s";
    console.debug(`Module loader:`, moduleSetCount, `module set${s} loaded`, {
        avg: ((modTotal / moduleSetCount) << 0) + "ms",
        total: (modTotal << 0) + "ms",
    });
