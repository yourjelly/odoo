        ...
        switch (field.type) {
            ...
            case "datetime": {
                if (Array.isArray(v1)) {  // [raw date string, format date]
                    ...
                    v1 = getDateNumberSortingValue(r1, fieldNameSpec);
                    v2 = getDateNumberSortingValue(r2, fieldNameSpec);
                }
                break;
            }
            case "reference":
...
export class Model extends Array {
    ...
    _web_read_group(domain, groupby, aggregates, having, offset, limit, order) {
        ...
        for (const record of records) {
            ...
            for (const groupbySpec of groupby) {
                const [fieldName] = safeSplit(groupbySpec, ":");
                const value = formatFieldValue(this._fields, groupbySpec, record[fieldName]);
                recordGroupValues[groupbySpec] = value
            }
            const valueKey = JSON.stringify(recordGroupValues);
    ...
    read_progress_bar(domain, groupBy, progressBar) {
        ...
        const groups = this._web_read_group(domain, [groupBy], []);
