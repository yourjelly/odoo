...
describe("time", () => {
    test("strftime", () => {
        expect(check("time.strftime('%Y')", (d) => String(d.getFullYear()))).toBeTruthy();
        expect(
            check("time.strftime('%Y') + '-01-30'", (d) => String(d.getFullYear()) + "-01-30")
        ).toBeTruthy();
        expect(check("time.strftime('%Y-%m-%d %H:%M:%S')", formatDateTime)).toBeTruthy();
    });
});

describe("datetime.datetime", () => {
    test("datetime.datetime.now", () => {
        expect(check("datetime.datetime.now().year", (d) => d.getUTCFullYear())).toBeTruthy();
        expect(check("datetime.datetime.now().month", (d) => d.getUTCMonth() + 1)).toBeTruthy();
        expect(check("datetime.datetime.now().day", (d) => d.getUTCDate())).toBeTruthy();
        expect(check("datetime.datetime.now().hour", (d) => d.getUTCHours())).toBeTruthy();
        expect(check("datetime.datetime.now().minute", (d) => d.getUTCMinutes())).toBeTruthy();
        expect(check("datetime.datetime.now().second", (d) => d.getUTCSeconds())).toBeTruthy();
    });
    ...
    test("to_utc in october with winter/summer change", () => {
        mockDate("2021-10-17 10:00:00", [+1, +2]);
...
describe("datetime.date", () => {
    test("datetime.date.today", () => {
        expect(check("(datetime.date.today()).strftime('%Y-%m-%d')", formatDate)).toBeTruthy();
    });
    ...
    test("basic operations with dates", function () {
        ...
        expect(evaluateExpr("a + day == date(2002, 3, 3)", ctx)).toBeTruthy();
        expect(evaluateExpr("day + a == date(2002, 3, 3)", ctx)).toBeTruthy(); // 5
        expect(evaluateExpr("a - day == date(2002, 3, 1)", ctx)).toBeTruthy();
        expect(evaluateExpr("-day + a == date(2002, 3, 1)", ctx)).toBeTruthy();
        expect(evaluateExpr("a + week == date(2002, 3, 9)", ctx)).toBeTruthy();
        expect(evaluateExpr("a - week == date(2002, 2, 23)", ctx)).toBeTruthy();
        expect(evaluateExpr("a + 52*week == date(2003, 3, 1)", ctx)).toBeTruthy(); // 10
        expect(evaluateExpr("a - 52*week == date(2001, 3, 3)", ctx)).toBeTruthy();
        expect(evaluateExpr("(a + week) - a == week", ctx)).toBeTruthy();
        expect(evaluateExpr("(a + day) - a == day", ctx)).toBeTruthy();
        expect(evaluateExpr("(a - week) - a == -week", ctx)).toBeTruthy();
        expect(evaluateExpr("(a - day) - a == -day", ctx)).toBeTruthy(); // 15
        expect(evaluateExpr("a - (a + week) == -week", ctx)).toBeTruthy();
        expect(evaluateExpr("a - (a + day) == -day", ctx)).toBeTruthy();
        expect(evaluateExpr("a - (a - week) == week", ctx)).toBeTruthy();
        expect(evaluateExpr("a - (a - day) == day", ctx)).toBeTruthy();
...
describe("relativedelta absolute : period is singular", () => {
    ...
    test("type of date + relative delta", () => {
        ...
        expect(evaluateExpr(expr1) instanceof PyDate).toBeTruthy();
    });
});
...
describe("misc", () => {
    test("context_today", () => {
        expect(check("context_today().strftime('%Y-%m-%d')", formatDate)).toBeTruthy();
    });

    test("today", () => {
        expect(check("today", formatDate)).toBeTruthy();
    });

    test("now", () => {
        expect(check("now", formatDateTime)).toBeTruthy();
    });
