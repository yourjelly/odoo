...
QUnit.module("Components", (hooks) => {
    ...
    QUnit.test("support properties", async (assert) => {
        ...
        serverData.models.product.records[1].definitions = [
            { name: "xpad_prop_1", string: "P3", type: "many2one", comodel: "partner" },
        ];
        ...
        assert.strictEqual(getCurrentPath(target), "Properties > P2");
        ...
        assert.strictEqual(getCurrentPath(target), "Properties > P3");
        assert.deepEqual(getOperatorOptions(target), ["=", "!=", "is set", "is not set"]);
    });
