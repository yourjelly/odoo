...
test("retrieving the stored action when open the new window", async () => {
    const originalAction = JSON.stringify({
        id: 1,
        name: "Partners Action 1",
        res_model: "partner",
        type: "ir.actions.act_window",
    ...
    browser.sessionStorage.setItem("current_action", originalAction);

    redirect("/odoo/action-1");
    // Create the web client. It should execute the stored action.
    await mountWebClient();

    // Check the `new_tab_action` was used
    const loadedAction = { ...getService("action").currentController.action };
    delete loadedAction.controllers;
