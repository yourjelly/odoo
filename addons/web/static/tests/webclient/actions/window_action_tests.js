...
QUnit.module("ActionManager", (hooks) => {
    ...
    QUnit.test("keynav: can't restore previous action if form is invalid", async function (assert) {
        serverData.models.partner.fields.foo.required = true;
        ...
        // empty the foo field -> make record invalid
        const input = target.querySelector(".o_field_widget[name=foo] input");
        input.value = "";
        await triggerEvent(input, null, "input");
        // trigger the breadcrumbs hotkey
        triggerHotkey("alt+b");
        await nextTick();
        assert.containsNone(target, ".o_list_view");
        assert.containsOnce(target, ".o_form_view");
