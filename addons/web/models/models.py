...
class Base(models.AbstractModel):
    ...
    def read_progress_bar(self, domain, group_by, progress_bar):
        ...
        records_values = self.search_read(
            domain or [], [progress_bar["field"], group_by]
        )
        ...
        for record_values in records_values:
            ...
            if field_type == "selection":
                group_by_value = (
                    selection_labels[group_by_value]
                    if group_by_value in selection_labels
                    else False
                )
            ...
            if type(group_by_value) == tuple:
                group_by_value = group_by_value[
                    1
                ]  # FIXME should use technical value (0)
    ...
    def _search_panel_field_image(self, field_name, **kwargs):
        ...
        model_domain_image = self._search_panel_domain_image(
            field_name, model_domain, enable_counters and no_extra, set_limit and limit,
        )
    ...
    def _search_panel_domain_image(
        ...
        domain = AND([domain, [(field_name, "!=", False)],])
    ...
    def search_panel_select_range(self, field_name, **kwargs):
        ...
        extra_domain = AND(
            [kwargs.get("category_domain", []), kwargs.get("filter_domain", []),]
        )
    ...
    def search_panel_select_multi_range(self, field_name, **kwargs):
        ...
        if field.type == "many2one":
            if enable_counters or not expand:
                extra_domain = AND([extra_domain, kwargs.get("group_domain", []),])
                domain_image = self._search_panel_field_image(
                    field_name,
                    model_domain=model_domain,
                    extra_domain=extra_domain,
            ...
            if not expand:
                image_element_ids = list(domain_image.keys())
                comodel_domain = AND(
                    [comodel_domain, [("id", "in", image_element_ids)],]
                )
            comodel_records = Comodel.search_read(
                comodel_domain, field_names, limit=limit
            )
...
class ResCompany(models.Model):
    ...
    def _get_asset_style_b64(self):
        ...
        company_styles = template_style._render({"company_ids": company_ids,})
