...
class TestSyncGoogle2Odoo(TestSyncGoogle):
    ...
    def test_cancelled(self):
        ...
        gevent = GoogleEvent([{"id": google_id, "status": "cancelled",}])
        self.sync(gevent)
    ...
    def test_recurrence_write_with_outliers(self):
        ...
        middle_event.write(
            {"start": datetime(2021, 2, 22, 16, 0, 0), "need_sync": False,}
        )
        ...
        event = self.env["calendar.event"].create(
            {
                ...
                "partner_ids": [
                    (6, 0, [self.env.user.partner_id.id, other_user.partner_id.id],)
                ],  # current user is attendee
            }
        )
