...
class TestProjectUpdateHrTimesheet(TestProjectUpdate):
    def _create_timesheet(self):
        self.employee = self.env["hr.employee"].create(
            {"name": "Randy Heart", "company_id": self.env.company.id}
        )
    ...
    def test_project_update_form_people(self):
        ...
        self.assertTrue(
            "People" in update.description, "The description should contain 'People'."
        )
    ...
    def test_project_update_form_no_people(self):
        ...
        self.assertFalse(
            "People" in update.description,
            "The description should not contain 'People'.",
        )
    ...
    def test_project_update_description_people(self):
        ...
        self.assertEqual(
            template_values["people"]["uom"],
            "hours",
            "The timesheet uom is well given to template",
        )
        self.assertTrue(
            template_values["people"]["is_uom_hour"],
            "Company default timesheet uom is hours",
        )
        self.assertEqual(
            len(template_values["people"]["activities"]),
            1,
            "The number of recorded timesheet activities is 1",
        )
        self.assertEqual(
            template_values["people"]["activities"][0]["name"],
            "Randy Heart",
            "The number of recorded timesheet activities is 1",
        )
