...
class AccountAnalyticLine(models.Model):
    ...
    project_id = fields.Many2one(
        ...
        help="Project on which you spent your time. The 'timesheets' feature needs to be enabled on the project for it to be selectable.")
    user_id = fields.Many2one(compute='_compute_user_id', store=True, readonly=False)
    employee_id = fields.Many2one('hr.employee', "Employee", domain=_domain_employee_id, context={'active_test': False},
        help="Employee who performed this activity. Define a 'timesheet cost' on the employee to track the cost of their time.\n"
             "Be careful, one user can be linked to multiple employees(one per company).")
