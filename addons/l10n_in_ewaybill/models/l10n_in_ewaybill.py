...
class Ewaybill(models.Model):
    ...
    def _compute_is_incoming(self):
        for ewaybill in self:
            ewaybill.is_incoming = ewaybill._is_incoming()
    ...
    def _is_overseas(self):
        ...
        return partner.l10n_in_gst_treatment in ('overseas', 'special_economic_zone')
    ...
    def _check_lines(self):
        ...
        if (
            lines_with_no_hsn := [
                ...
                if line.product_id and not line.l10n_in_hsn_code
            ]
        ):
            error_message.append(_("Please set HSN code in below invoice lines: \n%s", '\n'.join(lines_with_no_hsn)))
    ...
    def _l10n_in_ewaybill_stock_handle_zero_distance_alert_if_present(self, response_data):
