...
class TestEventPerformance(EventPerformanceCase):
    ...
    def test_event_create_single_notype_website(self):
        ...
        with self.profile(collectors=['sql', 'cache']), freeze_time(self.reference_now), self.assertQueryCount(event_user=242):  # tef 228 / com 234
            self.env.cr._now = self.reference_now  # force create_date to check schedulers
            event_values = dict(
                self.event_base_vals,
                website_menu=True
        ...
        # sometimes one additionnal query for _get_company_ids not in cacheµ
