...
class TestEventSecurity(TestEventCommon):
    ...
    def test_event_access_event_registration(self):
        ...
        self.assertEqual(registration.event_id, self.event_0, 'Registration users should be able to read')
    ...
    def test_implied_groups(self):
        ...
        # Registration User
        self.assertFalse(
            self.user_eventregistration.has_group('event.group_event_manager'),
            'The event registration group must not imply the event user manager')
        self.assertFalse(
            self.user_eventregistration.has_group('event.group_event_user'),
            'The event registration group must not imply the event user group')

    def test_multi_companies(self):
        """Test the ACLs with multi company.

        We should not be able to see records in others companies.
        """
