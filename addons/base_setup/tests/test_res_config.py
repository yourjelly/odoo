...
class TestResConfig(TransactionCase):
    ...
    def test_multi_company_res_config_group(self):
        ...
        self.assertTrue(user in self.env.ref('base.group_multi_currency').sudo().user_ids)
        ...
        self.assertTrue(new_user in self.env.ref('base.group_multi_currency').sudo().user_ids)

        # Case 2: Disable a group
        default_values.update({'group_multi_currency': False})
        ResConfig.create(default_values).execute()
        self.assertTrue(user not in self.env.ref('base.group_multi_currency').sudo().user_ids)
        ...
        self.assertTrue(new_user not in self.env.ref('base.group_multi_currency').sudo().user_ids)
