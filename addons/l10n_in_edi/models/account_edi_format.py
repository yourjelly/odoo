...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_in_prepare_edi_tax_details(self, move, in_foreign=False):
        def l10n_in_grouping_key_generator(tax_values):
            ...
            line_code = "other"
            if tax_line.tax_tag_ids in self.env.ref("l10n_in.tax_tag_cess"):
                if tax_line.tax_line_id.amount_type != "percent":
                    line_code = "cess_non_advol"
                else:
                    line_code = "cess"
            elif tax_line.tax_tag_ids in self.env.ref("l10n_in.tax_tag_state_cess"):
                if tax_line.tax_line_id.amount_type != "percent":
                ...
                for gst in ["cgst", "sgst", "igst"]:
                    if tax_line.tax_tag_ids in self.env.ref("l10n_in.tax_tag_%s"%(gst)):
                        line_code = gst
            return {
