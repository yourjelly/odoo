...
def load_marked_modules(env, graph, states, progressdict, report,
    ...
    while True:
        ...
        if not module_list:
            break
        graph.add_modules(env.cr, module_list)
...
def load_modules(registry, _force_demo=False, status=None, update_module=False):
    ...
    with registry.cursor() as cr:
        ...
        if 'base' in tools.config['init']:
            cr.execute("UPDATE ir_module_module SET state = %s WHERE name = %s AND state = %s", ('to install', 'base', 'installed'))
