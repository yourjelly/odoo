...
class Registry(Mapping):
    ...
    def init(self, db_name):
        ...
        self.__caches_groups = {}
        for cache_name, cache in self.__caches.items():
            self.__caches_groups.setdefault(cache_name.split('.')[0], []).append(cache)
    ...
    def clear_cache(self, *cache_names):
        ...
        for cache_name in cache_names:
            for cache in self.__caches_groups[cache_name]:
                cache.clear()
    ...
    def clear_all_caches(self):
        ...
        ``tools.ormcache``.
        """
        for cache_name, caches in self.__caches_groups.items():
            for cache in caches:
                cache.clear()
            self.cache_invalidated.add(cache_name)
    ...
    def cache_invalidated(self):
        ...
        return getattr(self._invalidation_flags, 'cache', set())
    ...
    def setup_signaling(self):
        ...
        with self.cursor() as cr:
            ...
            # must be reloaded.
            # The `base_cache_signaling_...` sequence indicates when all caches must
            # be invalidated (i.e. cleared).
            sequence_names = ('base_registry_signaling', *(f'base_cache_signaling_{cache_name}' for cache_name in self.__caches_groups))
    ...
    def get_sequences(self, cr):
        cache_sequences_query = ', '.join([f'base_cache_signaling_{cache_name}' for cache_name in self.__caches_groups])
        cache_sequences_values_query = ',\n'.join([f'base_cache_signaling_{cache_name}.last_value' for cache_name in self.__caches_groups])
        cr.execute(f"""
        ...
        cache_sequences = dict(zip(self.__caches_groups, cache_sequences_values))
            ...
            # Check if the model caches must be invalidated.
                ...
                for cache_name, cache_sequence in self.cache_sequences.items():
                    ...
                    if cache_sequence != expected_sequence:
                        ...
                        for cache in self.self.__caches_groups[cache_name]: # don't call clear_cache to avoid signal loop
                            cache.clear()
