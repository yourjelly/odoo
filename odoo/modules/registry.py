...
class Registry(Mapping):
    ...
    def get_trigger_tree(self, fields: list, select=bool):
        ...
        """
        return self._field_depends_graph.trigger_tree(fields, select)
    ...
    def _discard_fields(self, fields: list):
        ...
        self._field_depends_graph.discard(fields)
    ...
    def is_modifying_relations(self, field):
        """ Return whether ``field`` causes some relation to be modified. """
        try:
            return self._is_modifying_relations[field]
        except KeyError:
            result = field.relational or self.field_inverses[field] or any(
    ...
    @lazy_property
    def _field_depends_graph(self):
        """ This property is the graph of field dependencies.  It is a directed
        graph with optionally labelled edges, where the nodes consist of field
        objects and empty nodes (falsy objects).
        """
        graph = Graph()
        ...
        for Model in self.models.values():
            ...
            for field in Model._fields.values():
                ...
                with suppress(*exceptions):
                    for dependency in OrderedSet(field.resolve_depends(self)):
                        *path, to_field = dependency
                        graph.add_depends(field, path, to_field)
