...
class BaseModel(metaclass=MetaModel):
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def export_data(self, fields_to_export):
        ...
        :rtype: dict
        """
        if not (self.env.is_admin() or self.env.user._has_group('base.group_allow_export')):
    ...
    def check_field_access_rights(self, operation, field_names):
        ...
        if not field_names:
            field_names = [name for name, field in self._fields.items() if field.has_groups_access(self.env)]
        else:
            # Unknown (or virtual) fields are considered accessible because they will not be read and nothing will be written to them.
            invalid_fields = [name for name in field_names if name in self._fields and not self._fields[name].has_groups_access(self.env)]
            if invalid_fields:
                ...
                if request and request.session.debug:
                    def format_groups(field):
                        ...
                        return _("allowed for groups %s",
                            ', '.join(anyof
                            .sorted(lambda g: g.id)
    ...
    def _fetch_field(self, field):
        ...
        if self._context.get('prefetch_fields', True) and field.prefetch:
            fnames = [
                ...
                if f.has_groups_access(self.env)
            ]
            ...
            if field.store:
                ...
                for dotname in self.pool.field_depends[field]:
                    ...
                    if (not dep_field.store) or (dep_field.prefetch is True and
                        dep_field.has_groups_access(self.env)
                    ):
