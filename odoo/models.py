...
%(fields_list)s""") % {
    ...
    def unlink(self):
        ...
        for sub_ids in cr.split_for_in_conditions(self.ids):
            ...
            # mark fields that depend on 'records' to recomputed after 'records'
            # has been deleted (like updating a sum of lines after deleting one
            # line)
            with self.env.protecting(self._fields.values(), self):
                records.modified(self._fields, before=True)

            cr.execute(f'DELETE FROM "{self._table}" WHERE id IN %s', (sub_ids,))
