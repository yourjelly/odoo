...
try:
    ...
    class Config:
        def parse(self, option_name, value):
            if option_name != 'addons_path':
                raise ValueError("only addons_path is supported")
            return value.split(',')  # the paths must be resolved already
        def __getitem__(self, item):
            if item != 'addons_path':
                raise KeyError("only addons_path is supported")
            return []
...
class UpgradeCode(Command):
    ...
    def __init__(self):
        ...
        self.parser.add_argument(
            '--addons-path',
            type=functools.partial(config.parse, 'addons_path'),
