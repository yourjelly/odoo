    ...
    def parse(self):
        ...
        while stack:
            ...
            # -------------------------------------------------
                ...
                assert field.relational, "Invalid use of the where domain operator on a non-relational field."
                # active_test False to keep the behavior of subqueries where the active field isn't considered.
                right = comodel.with_context(active_test=False)._search(right, order='id')
                push((left, 'in', right), model, alias)
