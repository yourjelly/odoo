...
def normalize_leaf(element):
    ...
    if isinstance(right, bool) and operator in ('in', 'not in'):
        warnings.warn(f"The domain term '{(left, original, right)}' should use the '=' or '!=' operator.", UserWarning)
        operator = '=' if operator == 'in' else '!='
    if isinstance(right, (list, tuple)) and operator in ('=', '!='):
        warnings.warn(f"The domain term '{(left, original, right)}' should use the 'in' or 'not in' operator.", UserWarning)
        operator = 'in' if operator == '=' else 'not in'
    return left, operator, right


def check_leaf(element, internal=False):
    if element in DOMAIN_OPERATORS or element in (True, False):
        return

    try:
        left, operator, _right = element
    except (TypeError, ValueError):
        raise ValueError(f"Invalid leaf {element}") from None

    if isinstance(left, str) and left:
        if operator in TERM_OPERATORS:
            return
        if internal and operator in INTERNAL_TERM_OPERATORS:
            return
    raise ValueError(f"Invalid leaf {element}")
