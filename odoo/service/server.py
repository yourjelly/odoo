...
class ThreadedServer(CommonServer):
    ...
    def process_limit(self):
        ...
        for thread in threading.enumerate():
            if not thread.daemon and getattr(thread, 'type', None) not in ('websocket', 'cron'):
...
class GeventServer(CommonServer):
    ...
    def process_limits(self):
        ...
        if self.ppid != os.getppid():
            _logger.warning("WebSocket Parent changed: %s", self.pid)
            restart = True
        memory = memory_info(psutil.Process(self.pid))
        if config['limit_memory_soft'] and memory > config['limit_memory_soft']:
            _logger.warning('WebSocket virtual memory limit reached: %s', memory)
            restart = True
        if restart:
            # suicide !!
