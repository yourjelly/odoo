...
class TestESLint(lint_case.LintCase):
    ...
    def test_eslint(self):
        """ Test that there are no eslint errors in all javascript files """
    ...
    def test_whitelist_eslint(self):
        """ Test that there are no eslint errors in whitelist javascript files """
        ...
        eslintrc_path = get_resource_path('test_lint', 'tests', 'whitelist_eslintrc')
        ignore_path = get_resource_path('test_lint', 'tests', 'whitelist_ignore')

        _logger.info('Testing %s js files', len(files_to_check))
        # https://eslint.org/docs/user-guide/command-line-interface
        cmd = [eslint, '--no-eslintrc', '-c', eslintrc_path, '--ignore-path', ignore_path, '--quiet'] + files_to_check
        process = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False)
        self.assertEqual(process.returncode, 0, msg=process.stdout.decode())