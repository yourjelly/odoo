...
class test_float_field(ImporterCase):
    ...
    def test_nonsense(self):
        ...
        self.assertEqual(
            ...
            [
                message(
                    u"'foobar' does not seem to be a number for field 'Value'",
                    field_name="Value",
                )
            ],
        )
...
class test_required_string_field(ImporterCase):
    ...
    def test_ignore_excess_messages(self):
        ...
        for m in result["messages"][:-1]:
            ...
            self.assertEqual(
                m["message"], u"Missing required value for the field 'Value' (value)"
            )
...
class test_m2o(ImporterCase):
    ...
    def test_by_names(self):
        ...
        self.assertEqual(
            ...
            [
                message(
                    u"Found multiple matches for value 'export.integer:42' in field 'Value' (2 matches)",
                    type="warning",
                )
            ],
        )
    ...
    def test_name_create_enabled_m2o(self):
        ...
        self.assertEqual(
            ...
            [
                message(
                    u"No matching record found for name '101' " u"in field 'Value'",
                    moreinfo=moreaction(res_model="export.integer"),
                    field_name="Value",
                    field_path=["value"],
                    field_type="name",
...
class test_o2m(ImporterCase):
    ...
    def test_name_get(self):
        s = (
            u"Java is a DSL for taking large XML files and converting them "
            u"to stack traces"
        )
    ...
    def test_name_create_enabled_m2o_in_o2m(self):
        ...
        self.assertEqual(
            ...
            [
                message(
                    u"No matching record found for name '101' " u"in field 'Value/M2O'",
                    moreinfo=moreaction(res_model="export.integer"),
                    field_name="Value",
...
class test_realworld(SavepointCaseWithUserDemo):
    ...
    def test_o2m_subfields_fail_by_implicit_id(self):
        ...
        result = Model.with_context(import_file=True).load(
            ["child/child1/parent_id"], [["5"],],
        )
        self.assertEqual(
            result["messages"],
            [
                message(
                    u"No matching record found for name '5' in field 'Child/Child1/Parent'",
                    field="child",
...
class test_inherits(ImporterCase):
    ...
    def test_create_no_parent(self):
        r = self.import_(["id", "value_parent", "value"], [["xxx.child", "0", "1"],])
        ...
        self.assertEqual(
            rec._get_external_ids()[rec.id], ["xxx.child"],
        )
        self.assertEqual(
    ...
    def test_create_parent_no_xid(self):
        ...
        r = self.import_(
            ["id", "parent_id/.id", "value"], [["xxx.child", str(parent.id), "1"],]
        )
        rec = self.env[self.model_name].browse(r["ids"])
        self.assertEqual(rec.value_parent, 0)
    ...
    def test_create_parent_with_xid(self):
        ...
        self.assertEqual(
            rec.parent_id._get_external_ids()[rec.parent_id.id], ["xxx.parent"],
        )
    ...
    def test_create_parent_by_xid(self):
        ...
        r = self.import_(
            ["id", "parent_id/id", "value"], [["xxx.child", "xxx.parent", "1"],]
        )
        rec = self.env[self.model_name].browse(r["ids"])
        self.assertEqual(rec.value_parent, 0)
        self.assertEqual(rec.parent_id, parent)
    ...
    def test_update_parent_no_xid(self):
        parent = self.env["export.inherits.parent"].create({"value_parent": 0})
        child = self.env[self.model_name].create({"parent_id": parent.id, "value": 1,})
        ...
        self.assertEqual(
            child._get_external_ids()[child.id], ["xxx.child"],
        )
        self.assertEqual(
    ...
    def test_update_parent_with_xid(self):
        ...
        self.assertEqual(
            parent._get_external_ids()[parent.id], ["xxx.parent"],
        )
