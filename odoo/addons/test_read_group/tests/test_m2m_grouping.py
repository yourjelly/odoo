...
class TestM2MGrouping(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.tasks = cls.env["test_read_group.task"].create(
            [
                ...
                {"name": "Donkey Kong", "user_ids": [Command.set([])],},  # no user
            ]
        )
    ...
    def test_base_users(self):
        ...
        user_by_tasks = self.users.read_group(
            domain=[], fields=["name:array_agg"], groupby=["task_ids"],
        )
        self.assertEqual(
            user_by_tasks,
            [
    ...
    def test_base_tasks(self):
        ...
        task_by_users = self.tasks.read_group(
            domain=[], fields=["name:array_agg"], groupby=["user_ids"],
        )
        self.assertEqual(
            task_by_users,
            [
