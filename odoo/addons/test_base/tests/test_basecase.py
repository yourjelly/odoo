...
class TestSingleTransactionCase(common.SingleTransactionCase):
    ...
    def test_00(self):
        ...
        self.env['test_base.model'].create({'name': 'test_per_class_teardown'})
        test_base_models = self.env['test_base.model'].search([('name', '=', 'test_per_class_teardown')])
        self.assertEqual(1, len(test_base_models), "Test base record not found.")
    ...
    def test_20a(self):
        ...
        test_base_id, _ = self.env['test_base.model'].name_create('Mr Blue')
...
class TestTransactionCase(common.TransactionCase):
    ...
    def test_00(self):
        """ Create a test base record. """
        test_base_models = self.env['test_base.model'].search([('name', '=', 'test_per_class_teardown')])
        self.assertEqual(0, len(test_base_models), "Test base record found.")
        self.env['test_base.model'].create({'name': 'test_per_class_teardown'})
        test_base_models = self.env['test_base.model'].search([('name', '=', 'test_per_class_teardown')])
        self.assertEqual(1, len(test_base_models), "Test base record not found.")
    ...
    def test_20a(self):
        ...
        test_base_id, _ = self.env['test_base.model'].name_create('Mr Yellow')
