...
class TestACL(TransactionCaseCommon):
    ...
    def test_field_visibility_restriction(self):
        ...
        TestbaseModel = self.env['test_base.model'].sudo(self.user_test)

        # Verify the test environment first
        original_fields = TestbaseModel.fields_get([])
        form_view = TestbaseModel.fields_view_get(False, 'form')
        ...
        self._set_field_groups(TestbaseModel, 'parent_id', GROUP_SYSTEM)

        fields = TestbaseModel.fields_get([])
        form_view = TestbaseModel.fields_view_get(False, 'form')
        view_arch = etree.fromstring(form_view.get('arch'))
    ...
    def test_field_crud_restriction(self):
        ...
        test_base_model = self.env['test_base.model'].search([], limit=1)[0].sudo(self.user_test)

        # Verify the test environment first
        has_group_system = self.user_test.has_group(GROUP_SYSTEM)
        self.assertFalse(has_group_system, "`demo` user should not belong to the restricted group")
        self.assert_(test_base_model.read(['color']))
        self.assert_(test_base_model.write({'color': 1}))

        # Now restrict access to the field and check it's forbidden
        self._set_field_groups(test_base_model, 'color', GROUP_SYSTEM)

        with self.assertRaises(AccessError):
            test_base_model.read(['color'])
        with self.assertRaises(AccessError):
            test_base_model.write({'color': 1})
    ...
    def test_fields_browse_restriction(self):
        ...
        test_base_model = TestbaseModel.search([], limit=1)
        test_base_model.name
        # ... except if they are restricted
        with self.assertRaises(AccessError):
            with mute_logger('odoo.models'):
                test_base_model.parent_id
    ...
    def test_view_create_edit_button_invisibility(self):
        ...
        o2m_group_system = self.env['test_base.m2o.group.system'].sudo(self.user_test)
        base_restricted_view = o2m_group_system.fields_view_get(False, 'form')
    ...
    def test_m2o_field_create_edit_invisibility(self):
        ...
        test_base_model = self.env['test_base.model'].sudo(self.user_test)
        test_base_view = test_base_model.fields_view_get(False, 'form')
        view_arch = etree.fromstring(test_base_view['arch'])
        field_node = view_arch.xpath("//field[@name='m2o_group_system_id']")
        self.assertTrue(len(field_node), "m2o_group_system_id field should be in test_base_model from view")
        for method in methods:
...
class TestIrRule(TransactionCaseCommon):
    ...
    def test_ir_rule(self):
        model_test_base_model = self.env.ref('test_base.model_test_base_model')
        ...
        # read as test user the test_base.model (one blank domain)
        TestbaseModel = self.env['test_base.model'].sudo(self.user_test)
        test_base_models = TestbaseModel.search([])
        self.assertTrue(test_base_models, "Test user should see some records.")
        ...
        # read as test user with domains [] and blank
        test_base_models = TestbaseModel.search([])
        self.assertTrue(test_base_models, "Test user should see some records.")
        ...
        # read the test_base as test user
        test_base_models = TestbaseModel.search([])
        self.assertTrue(test_base_models, "Test user should see some records.")
        ...
        # read as test user (exercising the global company rule)
        test_base_models = TestbaseModel.search([])
        self.assertTrue(test_base_models, "Test user should see some records.")
        ...
        # check that test user still sees test_base, because group-rules are OR'ed
        test_base_models = TestbaseModel.search([])
        self.assertTrue(test_base_models, "Test user should see some records.")

        # create a new group with test user in it, and a complex rule
        ...
        # read the test_base again as test user, which should give results
        test_base_models = TestbaseModel.search([])
        self.assertTrue(test_base_models, "Test user should see records even with the combined rules.")

        # delete global domains (to combine only group domains)
        self.env['ir.rule'].search([('groups', '=', False)]).unlink()

        # read the test_base as test user (several group domains, no global domain)
        test_base_models = TestbaseModel.search([])
        self.assertTrue(test_base_models, "Test user should see some records.")
