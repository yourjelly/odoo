...
class TestDomainOptimize(TransactionCase):
    ...
    def test_condition_build(self):
        dom = Domain('a', '=', 1)
        self.assertEqual((dom.field_expr, dom.operator, dom.value), ('a', 'in', OrderedSet([1])))
        self.assertIsInstance(dom.value, OrderedSet)

        # equal with a collection becomes an 'in'
        dom = Domain('a', '=', [1, 2])
        self.assertEqual((dom.field_expr, dom.operator, dom.value), ('a', 'in', OrderedSet([1, 2])))
        self.assertIsInstance(dom.value, OrderedSet)
    ...
    def test_condition_optimize_like(self):
        ...
        self.assertEqual(
            Domain('name', '=like', '')._optimize(model),
            Domain('name', 'in', [False])._optimize(model),
