...
class TestFields(TransactionCaseWithUserDemo):
    ...
    def test_13_inverse_with_unlink(self):
        ...
        company.write(
            {"country_id": country2.id, "child_ids": [(2, child_company.id)],}
        )
    ...
    def test_70_x2many_write(self):
        ...
        discussion.important_messages = [
            Command.create({"body": "What is the answer?", "important": True,})
        ]
    ...
    def test_80_copy(self):
        ...
        def count(msg):
            ...
            return Translations.search_count(
                [("name", "=", "test_new_api.message,label"), ("res_id", "=", msg.id),]
            )
    ...
    def test_91_binary_svg_attachment(self):
        ...
        record = (
            ...
            .create({"name": "Test without attachment", "image_attachment": SVG,})
        )
        attachment = self.env["ir.attachment"].search(
            [
                ("res_model", "=", record._name),
    ...
    def test_94_image(self):
        ...
        record.write(
            {"image": image_h, "image_128": image_h,}
        )
        ...
        record.write(
            {"image": image_w, "image_128": image_w,}
        )
...
class TestSelectionOndeleteAdvanced(common.TransactionCase):
    ...
    def test_ondelete_unexisting_policy(self):
        class Foo(models.Model):
            ...
            my_selection = fields.Selection(
                selection_add=[("random", "Random stuff"),], ondelete={"random": "poop"}
            )
    ...
    def test_ondelete_default_no_default(self):
        class Foo(models.Model):
            ...
            my_selection = fields.Selection(
                selection_add=[("corona", "Corona beers suck"),],
    ...
    def test_ondelete_required_null_explicit(self):
        class Foo(models.Model):
            ...
            my_selection = fields.Selection(
                selection_add=[("brap", "Brap"),], ondelete={"brap": "set null"}
            )
    ...
    def test_ondelete_required_null_implicit(self):
        class Foo(models.Model):
            ...
            my_selection = fields.Selection(selection_add=[("boing", "Boyoyoyoing"),])
...
def update(model, *fnames):
    ...
    return 'UPDATE "{}" SET {} WHERE id IN %s'.format(
        model._table, ", ".join('"{}" = %s'.format(column) for column in columns),
    )
