...
class TestJavascriptAssetsBundle(FileTouchable):
    ...
    def _any_ira_for_bundle(self, type, lang=None):
        ...
        bundle = self.jsbundle_name if type == 'js' else self.cssbundle_name
    ...
    def test_16_ltr_and_rtl_css_access(self):
        ...
        css_bundles = self.env['ir.attachment'].search([
            ('url', '=like', '/web/content/%-%/{0}%.{1}'.format(self.cssbundle_name, 'css'))
        ])
    ...
    def test_19_css_in_debug_assets(self):
        ...
        self.assertIn('/test_assetsbundle/static/src/css/test_cssfile1/rtl/{0}.css'.format(self.cssbundle_name), content)

        # there should be assets(css) created in debug mode as user's lang direction is rtl
        css_bundle = self.env['ir.attachment'].search([
            ('url', '=', '/test_assetsbundle/static/src/css/test_cssfile1/rtl/{0}.css'.format(self.cssbundle_name))
        ])
...
class TestAssetsBundleWithIRAMock(FileTouchable):
    ...
    def _get_asset(self):
        files, remains = self.env['ir.qweb']._get_asset_content(self.stylebundle_name, {})
...
class TestAssetsManifest(AddonManifestPatched):
    ...
    def test_11_include(self):
        ...
        self.env['ir.asset'].create({
            ...
            'target': 'test_assetsbundle.manifest6',
        })
        view._render()
    ...
    def test_13_include_circular(self):
        ...
        self.env['ir.asset'].create({
            ...
            'directive': 'include',
            'bundle': 'test_assetsbundle.irasset_include1',
            'target': 'test_assetsbundle.irasset_include2',
        })
        self.env['ir.asset'].create({
            'name': 'test_jsfile4',
            'directive': 'include',
            'bundle': 'test_assetsbundle.irasset_include2',
            'target': 'test_assetsbundle.irasset_include1',
        })
        # LPE Fixme: find an exception type
    ...
    def test_13_2_include_recursive_sibling(self):
        ...
        self.env['ir.asset'].create({
            ...
            'directive': 'include',
            'bundle': 'test_assetsbundle.irasset_include2',
            'target': 'test_assetsbundle.irasset_include3',
        })
        self.env['ir.asset'].create({
            'name': 'test_jsfile4',
            'directive': 'include',
            'bundle': 'test_assetsbundle.irasset_include2',
            'target': 'test_assetsbundle.irasset_include4',
        })
        self.env['ir.asset'].create({
            'name': 'test_jsfile4',
            'directive': 'include',
