...
class TestSelectRangeMulti(odoo.tests.TransactionCase):
    ...
    def test_many2one(self):
        ...
        result = self.SourceModel.search_panel_select_multi_range(
            "tag_id", enable_counters=True, expand=True,
        )
        self.assertEqual(
            result["values"],
            [
        ...
        result = self.SourceModel.search_panel_select_multi_range(
            "tag_id", enable_counters=True, expand=True, limit=2,
        )
        self.assertEqual(
        ...
        result = self.SourceModel.search_panel_select_multi_range(
            "tag_id", expand=True, group_by="folder_id",
        )
        self.assertEqual(
            result["values"],
            [
        ...
        result = self.SourceModel.search_panel_select_multi_range(
            "tag_id", expand=True, search_domain=[["id", "in", [r1_id, r2_id]]],
        )
        self.assertEqual(
            result["values"],
            [
        ...
        result = self.SourceModel.search_panel_select_multi_range(
            "tag_id", enable_counters=True, group_by="folder_id",
        )
        self.assertEqual(
            result["values"],
            [
        ...
        self.assertEqual(
            result["values"], [{"__count": 2, "display_name": "Tag 1", "id": t1_id,},]
        )

        # counters, no expand, no group_by, and limit
        result = self.SourceModel.search_panel_select_multi_range(
            "tag_id", enable_counters=True, limit=2,
        )
        self.assertEqual(
            result, SEARCH_PANEL_ERROR,
        )
        ...
        result = self.SourceModel.search_panel_select_multi_range(
            "tag_id", search_domain=[["id", "in", [r1_id, r2_id]]],
        )
        self.assertEqual(result["values"], [{"display_name": "Tag 1", "id": t1_id,},])

        # no counters, no expand, no group_by, and comodel domain
        result = self.SourceModel.search_panel_select_multi_range(
            "tag_id", comodel_domain=[["id", "in", [t2_id, t3_id]]],
        )
        self.assertEqual(result["values"], [{"display_name": "Tag 2", "id": t2_id,},])
    ...
    def test_many2many(self):
        ...
        # counters, expand, and no group_by
        result = self.SourceModel.search_panel_select_multi_range(
            "tag_ids", enable_counters=True, expand=True,
        )
        self.assertEqual(
            result["values"],
            [
        ...
        # no counters, expand, and group_by (many2one case)
        result = self.SourceModel.search_panel_select_multi_range(
            "tag_ids", expand=True, group_by="folder_id",
        )
        self.assertEqual(
            result["values"],
            [
        ...
        result = self.SourceModel.search_panel_select_multi_range(
            "tag_ids", expand=True, search_domain=[["id", "in", [r1_id, r2_id]]],
        )
        self.assertEqual(
            result["values"],
            [
        ...
        # counters, no expand, no group_by, and search_domain
        result = self.SourceModel.search_panel_select_multi_range(
            "tag_ids", enable_counters=True, search_domain=[["id", "=", r2_id]],
        )
        self.assertEqual(
            result["values"], [{"__count": 1, "display_name": "Tag 1", "id": t1_id,},]
        )

        # counters, no expand, no group_by, and category_domain
        result = self.SourceModel.search_panel_select_multi_range(
            "tag_ids", enable_counters=True, category_domain=[["id", "=", r2_id]],
        )
        self.assertEqual(
            result["values"],
            [
        ...
        result = self.SourceModel.search_panel_select_multi_range(
            "tag_ids", group_by="folder_id", search_domain=[["id", "=", r2_id]],
        )
        self.assertEqual(
            result["values"],
            [
        ...
        result = self.SourceModel.search_panel_select_multi_range(
            "tag_ids", search_domain=[["id", "=", r2_id]],
        )
        self.assertEqual(result["values"], [{"display_name": "Tag 1", "id": t1_id,},])

        # no counters, no expand, no group_by, and search_domain
        result = self.SourceModel.search_panel_select_multi_range("tag_ids", limit=2,)
        self.assertEqual(
            result, SEARCH_PANEL_ERROR,
        )
    ...
    def test_selection_empty(self):
        result = self.SourceModel.search_panel_select_multi_range("state", expand=True,)
        self.assertEqual(
            result["values"],
