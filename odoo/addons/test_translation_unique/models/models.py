...
class TestTranslationUniqueModel1(models.Model):
    ...
    def create_unique_index_for_translated_field(self, fname):
        ...
        for lang, __ in self.env['res.lang'].get_installed():
            index_name = f'unique_index_{self._table}_{fname}_{lang.lower()}'
            tools.create_unique_index(self.env.cr, index_name, self._table, [f"""(("{fname}"->'{lang}'))"""])

    def get_unique_field_value(self, value):
        search_pattern = value + r'_copy_[%]'
        re_pattern = value + r'(_copy_\[(\d+)\]|)$'
