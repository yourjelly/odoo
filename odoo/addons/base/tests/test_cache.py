...
class TestCache(TransactionCase):
    def test_cache_performance(self):
        cache = self.env.cache
        env = self.env
        field_translate = self.env['res.partner.category']._fields['name']
        field_no_translate = self.env['res.partner.category']._fields['color']
        records = self.env['res.partner.category'].browse((i for i in range(1, 100000)))

        def test_update(field):
            value = {'en_US': '1'} if field.translate else 1
            field.update_cache(records, itertools.repeat(value), dirty=False)

        def test_get_values(field):
            return
            list(field.get_cache_values(records))

        def test_get_records_different_from(field):
            return
            context_key = env.cache_key(field)
            value = {'en_US': '1'} if field.translate else 1
            field.get_ids_different_from(records, value)

        def test_get_missing_ids(field):
            return list(field.get_cache_miss_ids(records))

        def test_read(field):
            for record in records:
                record[field.name]

        for field in [field_no_translate, field_translate]:
            for test in [test_get_missing_ids, test_update, test_get_values, test_get_records_different_from, test_get_missing_ids, test_read]:
                t = timeit.timeit("test(field)", number=10, globals={'test': test, 'field': field})
                print(f'time for {test.__name__}, {field}: {t}')
