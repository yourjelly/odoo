...
class TestGraph(BaseCase):
    def _test_graph_order(
        ...
        with patch('odoo.modules.graph.Graph.update_from_db'),\
                patch('odoo.modules.module.get_manifest', manifests.get):
            graph = Graph()
            graph_legacy = GraphLegacy()

            for modules in modules_list:
                graph.add_modules(None, modules)
                graph_legacy.add_modules(None, modules)

            names = list(g.name for g in graph)
...
class TestGraphAll(TransactionCase):
    ...
    def test_graph_order_all(self):
        ...
        with patch('odoo.modules.graph.Graph.update_from_db'):
            time0 = time.time()
            graph.add_modules(self.cr, modules)
            names = list(g.name for g in graph)
            time1 = time.time()
