...
class TestReports(odoo.tests.TransactionCase):
    ...
    def test_report_with_some_resources_reloaded_from_attachment(self):
        ...
        user1 = self.env["res.partner"].create({
            "name": "Rodion Romanovich Raskolnikov"
        })
        user2 = self.env["res.partner"].create({
            "name": "Dmitri Prokofich Razumikhin"
        })
        user3 = self.env["res.partner"].create({
            "name": "Porfiry Petrovich"
        })
        self.assert_report_creation(report.report_name, (user1 + user2 + user3), user2)

    def test_report_with_some_resources_reloaded_from_attachment_with_multiple_page_report(self):
        """
