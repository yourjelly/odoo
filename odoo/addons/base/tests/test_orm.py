        ...
        for model in self.env.registry.values():
            for field in model._fields.values():
                if field.company_dependent and field.type == 'many2one':
                    for field_name_, field_ in self.env[field.comodel_name]._fields.items():
                        self.assertFalse(
                            field_.type == 'many2one' and field_.ondelete == 'cascade',
                            (f'when a row for {field_.comodel_name} is deleted, a row for {field_.model_name} '
                             f'may also be deleted for sake of on delete cascade field {field_}, which may '
                             f'cause MissingError for a company dependent many2one field {field} in the future. '
                             f'Please override the unlink method of {field_.comodel_name} and do the ORM on '
                             f'delete cascade logic and remove/override the ondelete="cascade" of {field_}')
                        )
