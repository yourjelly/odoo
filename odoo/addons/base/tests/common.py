...
class SavepointCaseWithUserDemo(TransactionCase):
    ...
    def _load_partners_set(cls):
        cls.partner_category = cls.env["res.partner.category"].create(
            {"name": "Sellers", "color": 2,}
        )
        cls.partner_category_child_1 = cls.env["res.partner.category"].create(
            {"name": "Office Supplies", "parent_id": cls.partner_category.id,}
        )
        cls.partner_category_child_2 = cls.env["res.partner.category"].create(
            {"name": "Desk Manufacturers", "parent_id": cls.partner_category.id,}
        )
            ...
            [
                {
                    ...
                    "child_ids": [
                        Command.create({"name": "Sheila Ruiz",}),  # 'Willie Burke',
                        Command.create({"name": "Wyatt Howard",}),  # 'Ron Gibson',
                        Command.create({"name": "Austin Kennedy",}),  # Tom Ruiz
                    ],
                },
                {
                    ...
                    "child_ids": [
                        Command.create({"name": "Liam King",}),  # 'Douglas Fletcher',
                        Command.create(
                            {"name": "Craig Richardson",}  # 'Floyd Steward',
                        ),
                        Command.create({"name": "Adam Cox",}),  # 'Addison Olson',
                    ],
                },
                {
                    ...
                    "child_ids": [
                        Command.create({"name": "Pedro Boyd",}),  # Edwin Hansen
                        Command.create(
                            {
                        ...
                        Command.create({"name": "Leona Shelton",}),  # 'Soham Palmer',
                        Command.create({"name": "Scott Kim",}),  # 'Oscar Morgan',
                    ],
                },
                {
                    ...
                    "child_ids": [
                        Command.create({"name": "Louella Jacobs",}),  # 'Billy Fox',
                        Command.create({"name": "Albert Alexander",}),  # 'Kim Snyder',
                        Command.create({"name": "Brad Castillo",}),  # 'Edith Sanchez',
                        Command.create(
                            {"name": "Sophie Montgomery",}  # 'Sandra Neal',
                        ),
                        Command.create({"name": "Chloe Bates",}),  # 'Julie Richards',
                        Command.create(
                ...
                },
                {
                    ...
                    "child_ids": [
                        Command.create({"name": "carole miller",}),  # 'Toni Rhodes',
                        Command.create({"name": "Cecil Holmes",}),  # 'Gordon Owens',
                    ],
                },
                {
                    ...
                    "child_ids": [
                        ...
                        Command.create({"name": "Clinton Clark",}),  # 'Nicole Ford',
                        Command.create({"name": "Howard Bryant",}),  # 'Colleen Diaz',
                    ],
                },
                {
                    ...
                    "child_ids": [
                        Command.create({"name": "Mark Webb",})  # 'Lorraine Douglas',
                    ],
                },
                {
