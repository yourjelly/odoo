...
class SearchCase(TransactionCase):
    ...
    def check_consistency(self, expected, searched, filtered, domain):
        tests = {
            'expected': True,
            'searched': all(a == b for a, b in zip(expected, searched)),
            'filtered': all(a == b for a, b in zip(expected, filtered)),
        }
        ...
        self.assertEqual(searched, expected, message.format(wrong='searched', right=right, domain=domain))
        self.assertEqual(filtered, expected, message.format(wrong='filtered', right=right, domain=domain))

    def execute_test(self, domain, expected):
        inverted_domain = []
        skip_count = 0
        for leaf in domain:
            pop_count = 2 if skip_count == 0 and leaf == '!' else 0
            inverted_domain.extend(['!', leaf] if skip_count == 0 else [leaf])
            inverted_domain = inverted_domain[:len(inverted_domain) - pop_count]
            skip_count += 2 if leaf in ['&', '|'] else 1 if leaf == '!' else -1
...
class TestOne2ManyBase:
    ...
    def test_00_equals(self):
        domain = [(self.path, '=', self.get_children(count=1, index=0).id)]
    ...
    def test_01_not_equals(self):
        ...
        domain = [(self.path, '!=', self.get_children(count=2, index=0).id)]
    ...
    def test_04_all_equals_false(self):
        domain = [(self.path, 'all =', False)]
        self.assertEqual(self.search(domain), self.parents[0])

    def test_05_all_not_equals_false(self):
        domain = [(self.path, 'all !=', False)]
        self.assertEqual(self.search(domain), self.parents)
...
class TestMany2ManyBase:
    ...
    def test_00_equals(self):
        domain = [(self.path, '=', self.children[0].id)]
        self.execute_test(domain, self.get_parents(self.children[0]))

    def test_01_not_equals(self):
        domain = [(self.path, '!=', self.children[0].id)]
        self.execute_test(domain, self.get_parents(self.children[1]) + self.get_parents(self.children[2]))
    ...
    def test_06_in(self):
        domain = [(self.path, 'in', self.children[1:3].ids)]
        self.execute_test(domain, self.get_parents(self.children[1]) + self.get_parents(self.children[2]))

    def test_06_not_in(self):
        domain = [(self.path, 'not in', self.children[1:3].ids)]
        self.execute_test(domain, self.get_parents(self.children[0]))
...
class TestSubfield:
    ...
    def test_02_equals(self):
        domain = [(self.path, '=', self.params[0])]
    ...
    def test_04_all_equals(self):
        domain = [(self.path, 'all =', self.params[0])]
        self.assertEqual(self.search(domain), self.get_parents(m=0))

    def test_05_all_not_equals(self):
        domain = [(self.path, 'all !=', self.params[0])]
        self.assertEqual(self.search(domain), self.get_parents(n=0))
...
class TestNestedOne2ManySubfield(SearchCase, TestSubfield):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
