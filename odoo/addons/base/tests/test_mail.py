...
class EmailConfigCase(TransactionCase):
    ...
    def test_email_from_rewrite(self):
        ...
        email_from, return_path = get_email_from("admin@test.example.com")
        self.assertEqual(
            email_from, '"admin@test.example.com" <email_force@domain.com>'
        )
        self.assertEqual(return_path, "email_force@domain.com")

        email_from, return_path = get_email_from('"Admin" <admin@test.example.com>')
        self.assertEqual(
            email_from, '"Admin (admin@test.example.com)" <email_force@domain.com>'
        )
        self.assertEqual(return_path, "email_force@domain.com")

        # We always force the email FROM (notification email contains a name part)
        set_param(
            "mail.force.smtp.from", '"Your notification bot" <email_force@domain.com>'
        )
        set_param("mail.dynamic.smtp.from", False)
        set_param("mail.catchall.domain", "odoo.example.com")
        ...
        self.assertEqual(
            email_from,
            '"Admin (admin@test.example.com)" <email_force@domain.com>',
        ...
        self.assertEqual(
            email_from,
            '"Admin (admin@test.example.com)" <notification@odoo.example.com>',
        ...
        set_param(
            ...
            '"Your notification bot" <notification@odoo.example.com>',
        )
        set_param("mail.catchall.domain", "odoo.example.com")
