...
class TestHasGroup(TransactionCase):
    ...
    def test_env_uid(self):
        ...
        self.assertTrue(
            Partner.env.user._has_group(self.group0),
            "the test user should belong to group0"
        )
        self.assertFalse(
            Partner.env.user._has_group(self.group1),
            "the test user should *not* belong to group1"
        )

    def test_record(self):
        self.assertTrue(
            self.test_user._has_group(self.group0),
            "the test user should belong to group0",
        )
        self.assertFalse(
            self.test_user._has_group(self.group1),
    ...
    def test_portal_creation(self):
        ...
        self.assertTrue(
            portal_user._has_group(self.grp_portal_xml_id),
            "The portal user should belong to '%s'" % self.grp_portal_xml_id,
        )
        self.assertTrue(
            portal_user._has_group(grp_test_portal_xml_id),
            "The portal user should belong to '%s'" % grp_test_portal_xml_id,
        )
        self.assertTrue(
            portal_user._has_group(grp_test_internal2_xml_id),
            "The portal user should belong to '%s'" % grp_test_internal2_xml_id
        )
        self.assertFalse(
            portal_user._has_group(self.grp_internal_xml_id),
    ...
    def test_has_group_cleared_cache_on_write(self):
        ...
        def populate_cache():
            self.test_user._has_group('test_user_has_group.group0')
