...
class TestXMLID(TransactionCase):
    ...
    def test_create_external_id_with_space(self):
        model = self.env["res.partner.category"]
        data_list = [
            {"xml_id": "test_convert.category_with space", "values": {"name": "Bar"},}
        ]
...
class TestIrModel(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.env["x_bananas"].create(
            [
                {"x_name": "Banana #1", "x_length": 3.14159, "x_color": 9,},
                {"x_name": "Banana #2", "x_length": 0, "x_color": 6,},
                {"x_name": "Banana #3", "x_length": 10, "x_color": 6,},
            ]
        )
