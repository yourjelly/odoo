...
class TestViewInheritance(ViewCase):
    ...
    def test_get_inheriting_views(self):
        ...
        self.assertEqual(
            self.view_ids["A21"]._get_inheriting_views(), self.get_views(["A21"]),
        )
        self.assertEqual(
...
class TestApplyInheritanceMoveSpecs(ViewCase):
    ...
    def test_move_after(self):
        ...
        self.assertEqual(
            self.base_arch,
            E.template(
                E.div(), E.div({"class": "target"}), E.p("Content", {"class": "some"}),
            ),
        )
        self.apply_spec(self.wrapped_arch, spec)
        self.assertEqual(
...
class TestViews(ViewCase):
    ...
    def test_view_inheritance_text_after(self):
        ...
        self.assertEqual(view["type"], "form")
        self.assertEqual(
            view["arch"], '<form string="F">(<div/>a<p/>b<p/>c)</form>',
        )
    ...
    def test_view_inheritance_text_before(self):
        ...
        self.assertEqual(view["type"], "form")
        self.assertEqual(
            view["arch"], '<form string="F">(a<p/>b<p/>c<div/>)</form>',
        )
    ...
    def test_modifiers(self):
        ...
        _test_modifiers(
            '<field name="a" invisible="1" required="0"/>', {"invisible": True},
        )
        _test_modifiers(
            '<field name="a" invisible="0" required="1"/>', {"required": True},
        )
        _test_modifiers(
            ...
            self.View.create(
                {
                    ...
                    "model": "ir.ui.view",
                    "arch": arch
                    % ("", '<field name="noupdate"/><field name="fake_field"/>'),
                }
            )
    ...
    def assertValid(self, arch, name="valid view"):
        self.View.create(
            {"name": name, "model": "ir.ui.view", "arch": arch,}
        )

    def assertInvalid(self, arch, expected_message=None, name="invalid view"):
        with self.assertRaises(ValidationError) as catcher, mute_logger(
...
class ViewModeField(ViewCase):
    ...
    def testChangeInheritOfPrimary(self):
        ...
        """
        base1 = self.View.create({"inherit_id": None, "arch": "<qweb/>",})
        base2 = self.View.create({"inherit_id": None, "arch": "<qweb/>",})
        view = self.View.create(
...
class TestViewCombined(ViewCase):
    ...
    * Apply inheriting views on top
    ...
    def test_primary_after_extensions(self):
        ...
        a = self.View.create({"model": "a", "arch": "<qweb><a/></qweb>",})
        b = self.View.create(
