...
class TestResCurrency(TransactionCase):
    ...
    def test_currency_cache(self):
        ...
        # update the (cached) rate used in the previous query
        ...
        # create a new rate for the date used in the previous query
        ...
        # repeat _convert call
        # the cached conversion rate is invalid due to the new rate -> query
        with self.assertQueryCount(1):
            self.assertEqual(currencyA._convert(
                from_amount=100,
                to_currency=currencyB,
