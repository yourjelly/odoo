...
class TestBase(TransactionCase):
    ...
    def test_60_read_group(self):
        ...
        groups_data = res_users.with_context(active_test=False).read_group([], groupby=['login'], order='login DESC')
        ...
        groups_data = res_users.read_group(domain, groupby=['login'], order='login DESC', limit=3, offset=3)
        ...
        groups_data = res_users.read_group(domain, aggregates=['color:sum'], groupby=['function', 'login'])
        ...
        for group_data in groups_data:
            self.assertIn('color:sum', group_data, "Aggregated data for the column 'color' is not present in read_group return values")
            self.assertEqual(group_data['color:sum'], 3, "Incorrect sum for aggregated data for the column 'color'")

        # group on inherited char field, reverse order
        groups_data = res_users.read_group(domain, aggregates=['color:sum'], groupby='name', order='name DESC')
        ...
        groups_data = res_users.read_group(domain, groupby='color')
        ...
        groups_data = res_users.read_group(domain, groupby=['name', 'color'], order='name DESC')
        self.assertEqual([g['name'] for g in groups_data], ['Nab', 'Eve', 'Bob', 'Alice'], 'Incorrect ordering of the list')
        ...
        groups_data = res_users.read_group(domain, groupby='name', aggregates=['color:sum'], order='color:sum DESC, name')
        ...
        self.assertEqual([g['__count'] for g in groups_data], [1, 2, 1, 2], 'Incorrect number of results')

        # group on inherited date column (res_partner.date) -> Year-Month, default ordering
        groups_data = res_users.read_group(domain, groupby=['date:month'])
        self.assertEqual(len(groups_data), 4, "Incorrect number of results when grouping on a field")
        ...
        self.assertEqual([g['__count'] for g in groups_data], [1, 1, 1, 3], 'Incorrect number of results')

        # group on inherited date column (res_partner.date) specifying the :year -> Year default ordering
        groups_data = res_users.read_group(domain, groupby=['date:year'])
        ...
        self.assertEqual([g['__count'] for g in groups_data], [2, 4], 'Incorrect number of results')

        # group on inherited date column (res_partner.date) -> Year-Month, custom order
        groups_data = res_users.read_group(domain, groupby=['date:month'], order='date:month DESC')
        self.assertEqual(len(groups_data), 4, "Incorrect number of results when grouping on a field")
        ...
        self.assertEqual([g['__count'] for g in groups_data], [3, 1, 1, 1], 'Incorrect number of results')

        # group on inherited many2one (res_partner.title), default order
        groups_data = res_users.read_group(domain, aggregates=['color:sum'], groupby=['title'])
        self.assertEqual(len(groups_data), 2, "Incorrect number of results when grouping on a field")
        ...
        self.assertEqual([g['__count'] for g in groups_data], [4, 2], 'Incorrect number of results')
        self.assertEqual([g['color:sum'] for g in groups_data], [10, -1], 'Incorrect aggregation of int column')

        # group on inherited many2one (res_partner.title), reversed natural order
        groups_data = res_users.read_group(domain, aggregates=['color:sum'], groupby=['title'], order="title desc")
        self.assertEqual(len(groups_data), 2, "Incorrect number of results when grouping on a field")
        # m2o is returned as a (id, label) pair
        self.assertEqual([(title_sir.id, 'Sir...'), (title_lady.id, 'Lady...')], [g['title'] for g in groups_data], 'Incorrect ordering of the list')
        self.assertEqual([g['__count'] for g in groups_data], [2, 4], 'Incorrect number of results')
        self.assertEqual([g['color:sum'] for g in groups_data], [-1, 10], 'Incorrect aggregation of int column')

        # group on inherited many2one (res_partner.title), multiple orders with m2o in second position
        groups_data = res_users.read_group(domain, aggregates=['color:sum'], groupby=['title'], order="color:sum desc, title desc")
        self.assertEqual(len(groups_data), 2, "Incorrect number of results when grouping on a field")
        # m2o is returned as a (id, label) pair
        ...
        groups_data = res_users.read_group(domain, aggregates=['color:sum'], groupby=['title'], order='color:sum')
        self.assertEqual(len(groups_data), 2, "Incorrect number of results when grouping on a field")
        # m2o is returned as a (id, label) pair
