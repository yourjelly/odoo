...
from odoo.tools.safe_eval import safe_eval, const_eval, expr_eval, CodeChecker, _MATH_NODES, _ALLOWED_NODES
...
class TestSafeEval(BaseCase):
    ...
    def test_05_call_checker_injection(self):
        ...
        self.assertEqual(
            self.inject(case2),
            "__call_checker(__CheckerWrapper(foo, __type_checker).bar, baz, qux=quux)",
        )
    ...
    def test_06_attr_injection(self):
        ...
        self.assertEqual(
            self.inject(code1),
            "__CheckerWrapper(foo, __type_checker).bar",
        )

        self.assertEqual(
            self.inject(code2),
            "__CheckerWrapper(__CheckerWrapper(foo, __type_checker).bar, __type_checker).baz",
        )

        self.assertEqual(
            self.inject(code3),
            "__call_checker(__CheckerWrapper(foo, __type_checker).bar, baz)",
        )

        self.assertEqual(
            self.inject(code4),
            "__call_checker(__CheckerWrapper(foo, __type_checker).bar, baz, qux=__CheckerWrapper(quux, __type_checker).quuz)",
        )
    ...
    def test_11_unmodifiable_sandbox_globals(self):
        # Check that the sandboxed globals are not modifiable
        # Inside of a server action, it's possible to create another ir.action.server, and have full control over this action
        # execution context. With some manipulations, it's possible to modify the sandboxed globals and bypass all the security checks.

        with self.assertRaises(NameError, msg="You cannot use '__CheckerWrapper' as a key in a context dictionary."):
            safe_eval("", globals_dict={"__CheckerWrapper": 0}, mode="exec")

        with self.assertRaises(NameError, msg="You cannot use '__call_checker' as a key in a context dictionary."):
            safe_eval("", globals_dict={"__call_checker": 0}, mode="exec")

        with self.assertRaises(NameError, msg="You cannot use '__SAFE_TYPES' as a key in a context dictionary."):
            safe_eval("", globals_dict={"__SAFE_TYPES": 0}, mode="exec")

        with self.assertRaises(NameError, msg="You cannot use '__check_format' as a key in a context dictionary."):
            safe_eval("", globals_dict={"__check_format": 0}, mode="exec")
    ...
    def test_12_use_of_denied_ast_nodes(self):
        ...
        with self.assertRaises(ValueError, msg="ast.Call is not allowed."):
            safe_eval("foo()", ast_subset=_MATH_NODES)
