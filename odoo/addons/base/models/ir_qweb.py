    ...
    * to customize ``t-field`` rendering, subclass ``ir.qweb.field`` and
    ...
    def _compile_directive_call_assets(self, el, options):
        ...
        space = (
            ...
            and el.getprevious().tail
            or el.getparent().text
        )
        sep = u"\n" + space.rsplit("\n").pop()
        return [
            ast.Assign(
                ...
                value=ast.Call(
                    ...
                    keywords=[
                        ...
                        ast.keyword(
                            "lazy_load", self._get_attr_bool(el.get("lazy_load", False))
                        ),
                    ...
                    ast.If(
                        ...
                        orelse=[
                            ...
                            ast.If(
                                ...
                                body=[
                                    self._append(ast.Name(id="content", ctx=ast.Load()))
                                ],
                                orelse=[],
                            ),
                            self._append(ast.Str(u"</")),
    ...
    def _generate_asset_nodes_cache(
        ...
        return self._generate_asset_nodes(
            bundle, options, css, js, debug, async_load, defer_load, lazy_load, media
        )
    ...
    @tools.ormcache_context(
        "bundle", 'options.get("lang", "en_US")', keys=("website_id",)
    )
