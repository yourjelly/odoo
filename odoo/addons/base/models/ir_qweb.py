...
The generated code uses ``_has_view_groups`` Odoo method from
``ir.ui.view`` model.
...
class IrQWeb(models.AbstractModel):
    ...
    * to customize ``t-field`` rendering, subclass ``ir.qweb.field`` and
    ...
    def _compile_directive_groups(self, el, compile_context, level):
        ...
        The code will contain the condition `if self.env['ir.ui.view']._has_view_groups(groups)`
        ...
        code = self._flush_text(compile_context, level)
        code.append(indent_code(f"if self.env['ir.ui.view']._has_view_groups({groups!r}):", level))
