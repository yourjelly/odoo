...
class IrModelAccess(models.Model):
    ...
    def _group_xmlid_with_access(self, model_name, access_mode='read'):
        ...
        """
        assert access_mode in ('read', 'write', 'create', 'unlink'), 'Invalid access mode'
        # access = self.env['ir.model.access'].search([(f'perm_{access_mode}', '=', True), ('model_id.model', '=', model_name)])
        model = self.env['ir.model']._get(model_name)
        access = self.env['ir.model.access'].search([(f'perm_{access_mode}', '=', True), ('model_id', '=', model.id)])
        if not access:
            return tuple()
        if any(not acc.group_id for acc in access):
            return None
        return tuple(access.group_id.get_external_id().values())
