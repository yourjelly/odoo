...
* if False, the view currently does not extend its parent but can be enabled
    ...
    def _postprocess_access_rights(self, tree):
        ...
        """
        Group = self.env['res.groups']._define_group()
    ...
    def _postprocess_view(self, node, model_name, editable=True,
        ...
        if view_groups is None:
            view_groups = ~self.env['res.groups']._define_group()
        ...
        while stack:
            ...
            if node.get('groups'):
                node_info['view_groups'] &= self.env['res.groups']._define_group(node.get('groups'))
    ...
    def _postprocess_tag_field(self, node, name_manager, node_info):
        ...
        if field:
            if field.groups:
                node_info['model_groups'] &= self.env['res.groups']._define_group(field.groups)
            if (
    ...
    def _validate_view(self, node, model_name, view_type=None, editable=True, full=False,
        ...
        while stack:
            ...
            if node.get('groups'):
                ...
                node_info['view_groups'] &= self.env['res.groups']._define_group(str_group)
...
class NameManager:
    ...
    def _get_field_groups(self, name):
        ...
        if not field and name not in self.available_names and name not in self.field_info:
            intersect = self.Group.no_one()
        elif field and field.groups:
            intersect &= self.Group.define(field.groups)
    ...
    def check(self, view):
        ...
        for name, node in self.must_exist_groups.items():
            if self.Group.get_id(name) is None:
    ...
    def get_missing_fields(self):
        ...
        for name, groups_uses in self.used_fields.items():
            ...
            for used_groups, (use, node) in groups_uses.items():
                ...
                available_combined_groups = self.Group.no_one()
            ...
            missing_groups = self.Group.no_one()
