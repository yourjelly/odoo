        ...
        if field:
            ...
            if field.relational:
                ...
                if isinstance(domain, str):
                    ...
                    name_manager.must_have_fields(node, vnames, ('domain', domain))
            context = node.get('context')
            if context:
                vnames = get_expression_field_names(context)
                name_manager.must_have_fields(node, vnames, ('context', context))
...
class NameManager:
    ...
    def __init__(self, model, parent=None, parent_field=None):
        ...
        self.mandatory_fields = collections.defaultdict(dict)  # {field_name: {'groups': '(use, node)}}
        self.mandatory_parent_fields = collections.defaultdict(dict)  # {field_name: {'groups': (use, node)}}
    ...
    def has_field(self, node, name, info=frozendict()):
        groups = self._get_field_groups(name) + self._get_node_groups(node)
    ...
    def must_have_fields(self, node, names, use, groups=None):
        if groups is None:
            groups = self._get_field_groups(node.get('name') if node.tag == 'field' else None) + self._get_node_groups(node)
        for name in names:
            if name == 'id':
                continue
    ...
    def _get_node_groups(self, node):
        ...
        """
        return tuple(reversed([tuple(n.get('groups').split(',')) for n in chain([node], node.iterancestors()) if n.get('groups')]))

    def _get_field_groups(self, name):
        """ Return the list of xml_ids of groups which have been granted the read access.
            If the xml_ids of groups is a tuple is empty, there is no group that can satisfy.
            If the xml_ids of groups is None, it is not necessary to belong to a group to have access.
           :rtype: tuple(tuple | None)
        """
        groups = []
        ...
        for manager, name in model_fields:
            groups.append(manager.model.env['ir.model.access']._group_xmlid_with_access(manager.model._name, 'read'))
            field = manager.model._fields.get(name)
            if field:
                if field.groups:
                    groups.append(tuple(field.groups.split(',')))
            elif name not in self.available_names:
                groups.append(tuple())
    ...
    def check(self, view):
        ...
        for name, (missing_groups, reasons) in self.get_missing_fields().items():
            ...
            if not field and name not in self.available_names:
                error_msg = _(
                ...
                has_groups = self._get_field_groups(name)
                msg = [_("Field %(name)r is restricted by groups without matching with the common mandatory groups.\n"
                         ...
                         "The mandatory groups take care of the view (and hierarchy), the description field groups and model read access.\n"
                         "The field %(name)r (%(has_groups)s) is used by this following elements (and their groups):",
                            name=name,
                            has_groups=_('Only super user has access') if tuple() in has_groups else ' > '.join([','.join(g) for g in has_groups if g]) or '-',
                    )]
                for item_groups, use, node in reasons:
                    ...
                    msg.append(
                        _("   %(node)s    (%(groups)s)",
                            ...
                            groups=(
                                _('Free access') if item_groups == (None,) or not item_groups else
                                ' > '.join([','.join([
                                        (_('Only super user has access') if group is False else group)
                                        for group in groups if group is not None
                                        ])
                                    for groups in item_groups
                                ])
                            ),
                        )
                    )
            ...
            if error_msg:
                if self.model.pool._init and not config['test_enable'] and not getattr(threading.current_thread(), 'testing', False):
                    ...
                    view._log_view_warning(error_msg, node)
                else:
                    view._raise_view_error(error_msg)
    ...
    def get_missing_fields(self):
        ...
        """
        implied_map = {}
        def is_implied_group(group, parent_group):
            if group == parent_group:
                return True
            if group == 'base.group_no_one' or parent_group == 'base.group_no_one':
                # group session dependent
                return False
            if parent_group is None or group is False:
                return False
            if group is None or parent_group is False:
                return True
            if parent_group not in implied_map:
                implied_groups = self.model.env.ref(parent_group if parent_group[0] != '!' else parent_group[1:]).trans_implied_ids.get_external_id().values()
                implied_map[parent_group] = tuple([parent_group] + [implied_group if parent_group[0] != '!' else '!' + implied_group for implied_group in implied_groups])
            return group in implied_map[parent_group]

        def add_sentinel_group(item):
            res = []
            for groups in item:
                if groups is None:
                    res.append((None,))
                    ...
                    res.append(groups)
                elif all(group[0] == '!' for group in groups):
                    res.append((None,) + groups)
                else:
                    res.append(groups)
            return res
        ...
        # mandatory_data = {
        #   'field_a': [
        #       ( [(E,F), (G,), (A,B), (B1,)], use, node ),
        #       ( [(E,F), (G,), (A,B), (B2,)], use, node ),
        #   ],
        #   'field_b': [
        #       ( [(E,F), (G,), (A,B), (A,C)], use, node ),
        #   ],
        # }
        #
        # data = {
        #   'field_a': [
        #       ('available', [(E,F), (G,), (A,B), (A,C)]),   # field present in view
        #       ('available', [(E,F), (G,), (A,B), (B,)]),
        #       ('model',     [(E,F), (G,), (A,B)]),          # field on the model
        #   ],
        #   'field_b': [
        #       ('model',     [(E,F)] ),                      # field on the model
        #   ],
        # }
        mandatory_data = collections.defaultdict(list)
        data = collections.defaultdict(list)
        for name, groups_uses in self.mandatory_fields.items():
            for groups, (use, node) in groups_uses.items():
                mandatory_data[name].append((add_sentinel_group(groups), use, node))

            for available_groups in self.available_fields.get(name, {}).get('groups', []):
                data[name].append(('available', add_sentinel_group(available_groups)))

            data[name].append(('model', add_sentinel_group(self._get_field_groups(name))))

        missing_fields = {}
        for name, mandatory_items in mandatory_data.items():
            errors = []
            mandatory = []
            for mandatory_item in mandatory_items:
                match = [
                    ...
                    if mandatory_item[0] == item[1] or
                        all(
                            any(
                                # only super user has access, pass this step
                                False in mandatory_groups or
                                any(  # each groups must have a least one group contained in mandatory groups
                                    any(
                                        is_implied_group(group, mandatory_group)
                                        for mandatory_group in mandatory_groups
                                    )
                                    for group in groups if not group or group[0] != '!'
                                ) and
                                all(  # each groups must have all negative groups contained in mandatory groups
                                    any(
                                        is_implied_group(group, mandatory_group)
                                        for mandatory_group in mandatory_groups
                                    )
                                    for group in groups if group and group[0] == '!'
                                ) and
                                all(  # each mandatory groups must have a at least one complete subset in available groups
                                    len(mandatory_groups) == 1
                                    or
                                    mandatory_group in groups
                                    or
                                    any(
                                        is_implied_group(group, mandatory_group)
                                        for group in groups
                                    )
                                    for mandatory_group in mandatory_groups if not mandatory_group or mandatory_group[0] != '!'
                                )
                                for mandatory_groups in mandatory_item[0]
                            )
                            for groups in item[1]
                    )]
                if not any(item[0] == 'model' for item in match):
                    # No match possible using only access right and groups on the field.
                    errors.append(mandatory_item)
                    continue

                if any(item[0] == 'available' for item in match):
                    # At least one field in view match match with the mandatory combinations.
                    continue

                mandatory.append(mandatory_item)
            ...
            # Find the minimum necessary group combination for the missing field
            all_mandatory_groups = set(chain(*[mandatory_item[0] for mandatory_item in mandatory])) - {(False,), (None,)}
            missing_groups = []
            for groups in all_mandatory_groups:
                if groups in field_groups:
                    continue
                if groups in missing_groups:
                    continue
                if all(
                    any(
                        all(
                            any(is_implied_group(group, m_group) for group in groups)
                            for m_group in m_groups
                        )
                        for m_groups in item_groups
                    )
                    for item_groups, *_ in mandatory
                ):
                    if None in groups:
                        # remove sentinel
                        groups = tuple(group for group in groups if group and group[0] == '!')
                    if groups:
                        missing_groups.append(groups)

            missing_fields[name] = (missing_groups, mandatory)
