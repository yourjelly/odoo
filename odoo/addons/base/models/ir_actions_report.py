...
class IrActionsReport(models.Model):
    ...
    def _run_wkhtmltopdf(
        ...
        def delete_temporary_file(temporary_file_path):
            try:
                os.unlink(temporary_file_path)
            except OSError:
                _logger.error('Error when trying to remove file %s', temporary_file_path)
        ...
        with ExitStack() as stack:
            ...
            if request and request.db:
                ...
                stack.callback(delete_temporary_file, cookie_jar_file_path)
            ...
            if header:
                ...
                stack.callback(delete_temporary_file, head_file_path)
                files_command_args.extend(['--header-html', head_file_path])
            if footer:
                foot_file_fd, foot_file_path = tempfile.mkstemp(suffix='.html', prefix='report.footer.tmp.')
                with closing(os.fdopen(foot_file_fd, 'wb')) as foot_file:
                    foot_file.write(footer.encode())
                stack.callback(delete_temporary_file, foot_file_path)
            ...
            for i, body in enumerate(bodies):
                ...
                stack.callback(delete_temporary_file, body_file_path)

            pdf_report_fd, pdf_report_path = tempfile.mkstemp(suffix='.pdf', prefix='report.tmp.')
            os.close(pdf_report_fd)
            stack.callback(delete_temporary_file, pdf_report_path)
