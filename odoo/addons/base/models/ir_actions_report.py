...
class IrActionsReport(models.Model):
    ...
    report_type = fields.Selection(
        [("qweb-html", "HTML"), ("qweb-pdf", "PDF"), ("qweb-text", "Text"),],
        required=True,
        default="qweb-pdf",
    ...
    def _prepare_html(self, html):
        ...
        if not bodies:
            body = b"".join(
                lxml.html.tostring(c, encoding="utf-8")
    ...
    def barcode(
        ...
        try:
            width, height, humanreadable, quiet = (
                int(width),
                int(height),
    ...
    def _post_pdf(self, save_in_attachment, pdf_content=None, res_ids=None):
        ...
        if len(streams) == 1:
            ...
            try:
                ...
                raise UserError(
                    _("One of the documents, you try to merge is encrypted")
                )
    ...
    def _render_qweb_pdf(self, res_ids=None, data=None):
        ...
        if isinstance(self.env.cr, TestCursor):
            return self_sudo.with_context(context)._render_qweb_html(
                res_ids, data=data
            )[0]
    ...
    def _get_rendering_context(self, docids, data):
        ...
        if report_model is not None:
            ...
            data.update(
                {"doc_ids": docids, "doc_model": self_sudo.model, "docs": docs,}
            )
