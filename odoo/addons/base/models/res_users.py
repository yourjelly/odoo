...
class Users(models.Model):
    ...
    def _default_groups(self):
        default_user_id = self.env["ir.model.data"].xmlid_to_res_id(
    ...
    def _action_show(self):
        ...
        if len(self) > 1:
            ...
            action.update(
                {"view_mode": "form", "views": [[view_id, "form"]], "res_id": self.id,}
            )
        return action
...
class APIKeys(models.Model):
    ...
    def init(self):
        ...
        """.format(
                table=self._table, index_size=INDEX_SIZE
            )
        )
    ...
    def _generate(self, scope, name):
        ...
        ip = request.httprequest.environ["REMOTE_ADDR"] if request else "n/a"
        _logger.info(
            "API key generated: scope: <%s> for '%s' (#%s) from %s",
            scope,
            self.env.user.login,
            self.env.uid,
            ip,
