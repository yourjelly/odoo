...
from odoo.tools import is_html_empty, frozendict, lazy_property, SQL, SetDefinitions
...
class ResGroups(models.Model):
    ...
    all_user_ids = fields.Many2many('res.users', compute='_compute_all_user_ids', recursive=True, string='Users and implied users')
    ...
    * A group "computer developer employee" will imply that he is an employee group, a user group,
    ...
    all_implied_ids = fields.Many2many('res.groups', string='Transitive and recursive closure', recursive=True,
        compute='_compute_all_implied_ids', search='_search_all_implied_ids')
    ...
    def _search_all_implied_ids(self, operator, value):
        ...
        if isinstance(value, str):
            value = [group_definitions.get_id(value)]
        elif isinstance(value, int):
            value = [value]
        ids = [*value, *group_definitions.get_subset_ids(value)]
        return [('id', operator, ids)]

    @api.depends('user_ids', 'implied_ids', 'implied_ids.user_ids')
    def _compute_all_user_ids(self):
        group_definitions = self.env['res.groups']._get_group_definitions()
        for g in self:
            g.all_user_ids = g.user_ids + self.browse(group_definitions.get_subset_ids(g.ids)).mapped('all_user_ids')
...
class ResUsers(models.Model):
    ...
    group_ids = fields.Many2many('res.groups', 'res_groups_users_rel', 'uid', 'gid', string='Groups', default=lambda s: s._default_groups())
    view_show_technical_groups = fields.Boolean(string="Show technical groups", store=False)
    view_implied_group_ids = fields.Many2many('res.groups', compute='_compute_view_implied_group_ids', string="Implied groups")
    ...
    def _compute_view_implied_group_ids(self):
        group_definitions = self.env['res.groups']._get_group_definitions()
        for user in self:
            user.view_implied_group_ids = user.group_ids.all_implied_ids.ids
            user.view_disjoint_group_ids = list(group_definitions.get_disjoint_ids(user.group_ids.all_implied_ids.ids))
            if user.view_show_technical_groups:
                user.view_visible_implied_group_ids = user.view_implied_group_ids
            else:
                user.view_visible_implied_group_ids = user.view_implied_group_ids.filtered(lambda g: g.category_id.visible)
    ...
    def _compute_accesses_count(self):
        for user in self:
            groups = user.group_ids.all_implied_ids
    ...
    def _check_one_user_type(self):
        ...
        for user in self:
            ...
            if group_definitions.get_disjoint_ids(group_ids).intersection(group_ids):
    ...
    def write(self, values):
        ...
        if old_groups:
            ...
            if added_groups:
                internal_users = self.env.ref('base.group_user').user_ids - self
    ...
    def _get_group_ids(self):
        ...
        self.ensure_one()
        return self.group_ids.all_implied_ids._ids
    ...
    def action_show_groups(self):
        ...
        return {
            ...
            'domain': [('id', 'in', self.group_ids.all_implied_ids.ids)],
            'target': 'current',
        }
