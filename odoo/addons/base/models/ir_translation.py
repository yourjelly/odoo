...
class IrTranslation(models.Model):
    ...
    def translate_fields(self, model, id, field=None, view=False):
        ...
        if view:
            domain = ['&', '&', ('res_id', '=', id), ('name', '=like', model + ',' + field), ('lang', 'in', [l.code for l in lang])]
            ...
            if fld.related:
                try:
                    ...
                    if record:
                        related_domain = make_domain(fld, record)
                        related_domain += [('lang', 'in', [l.code for l in lang])]
                        domain = ['|'] + domain + related_domain
                except AccessError:
                    pass

            action['view_id'] = self.env.ref('base.view_translation_dropdown_tree').id
