...
class Environment(Mapping[str, typing.Any]):
    ...
    _protected: StackMap[Field, OrderedSet[IdType]]
    ...
    def flush_query(self, query: SQL) -> None:
        ...
        for field in fields_to_flush:
            assert field.model_name is not None
...
class Cache:
    ...
    def __init__(self):
        ...
        # in `_data`
        self._dirty: defaultdict[Field, OrderedSet[IdType]] = defaultdict(OrderedSet)
    ...
    def insert_missing(self, records: BaseModel, field: Field, values: Iterable) -> None:
        ...
        if field.translate:
            if env.context.get('prefetch_langs'):
                ...
                langs: OrderedSet[str] = OrderedSet(installed + ['en_US'])
        ...
        for field, field_cache in self._data.items():
            ...
            if field in depends_context:
                for context_keys, inner_cache in field_cache.items():
                    context = dict(zip(depends_context[field], context_keys))  # type: ignore
