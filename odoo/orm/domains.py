...
class Domain:
    ...
    field_expr: str | None = None  # ease checks whether the domain is a condition
...
class DomainNary(Domain):
    ...
    def _optimize(self, model: BaseModel) -> Domain:
        ...
        # optimize children
        children: Iterator[Domain] | Iterable[Domain] | Domain = (c._optimize(model) for c in self.children)
        while True:
...
class DomainCondition(Domain):
    ...
    """
    __slots__ = ('_field', 'field_expr', 'operator', 'value')
    _field: Field | None
    field_expr: str
    operator: str
    value: typing.Any
    ...
    def new(cls, field_expr, operator, value) -> DomainCondition:
        ...
        if isinstance(value, SQL) and operator not in ('any', 'not any', 'in', 'not in'):
            ...
            if field_expr not in {'groups_id', 'user_ids'}:
    ...
    def __iter__(self):
        ...
        if operator == 'any' and isinstance(value, DomainCondition):
            ...
            yield (f"{field_expr}.{field_b}", operator, value)
            return
    ...
    def register(optimization: Callable[[type[DomainNary], BaseModel, list[DomainCondition]], Iterable[Domain]]):
        def optimizer(cls, model, conditions: Iterable[Domain]):
            for (field_name, apply_operator), conds in itertools.groupby(
                conditions, lambda c: (c.field_expr, True) if c.field_expr and c.operator in operators else ('', False)  # type: ignore
            ):
                if (
                    apply_operator
...
def _operator_equal_if_value(leaf, _):
    ...
    if not leaf.value:
        return _TRUE_DOMAIN
    return DomainCondition(leaf.field_expr, 'in', OrderedSet([leaf.value]))
...
def _operator_different(leaf, _):
    ...
    warnings.warn("Operator '<>' is deprecated, use '!=' directly", DeprecationWarning)
    return DomainCondition(leaf.field_expr, '!=', leaf.value)
...
def _operator_equal_as_in(leaf, _):
    ...
    if not isinstance(value, COLLECTION_TYPES) and not value:
        value = False
    return DomainCondition(leaf.field_expr, 'in', [value])
...
def _operator_nequal_as_not_in(leaf, _):
    ...
    if not isinstance(value, COLLECTION_TYPES) and not value:
        value = False
    return DomainCondition(leaf.field_expr, 'not in', [value])
...
def _optimize_id_any_condition(leaf, _):
    ...
    """
    if leaf.field_expr == 'id' and isinstance(domain := leaf.value, Domain):
...
def _optimize_in_set(leaf, _):
    ...
    if isinstance(value, OrderedSet):
        return leaf
    if isinstance(value, COLLECTION_TYPES):
        return DomainCondition(leaf.field_expr, leaf.operator, OrderedSet(value))
...
def _optimize_any_domain(leaf, model):
    ...
    try:
        ...
        raise ValueError(f"Cannot determine the relation for {leaf.field_expr}")
    ...
    if domain is value:
        return leaf
    return DomainCondition(leaf.field_expr, leaf.operator, domain)
...
def _optimize_like_str(leaf, model):
    ...
    if not value:
        ...
        if leaf._field.relational or '=' in leaf.operator:
            return DomainCondition(leaf.field_expr, 'not in' if result else 'in', OrderedSet([False]))
        return Domain(result)
    if isinstance(value, (str, SQL)):
        # accept both str and SQL
        return leaf
    return DomainCondition(leaf.field_expr, leaf.operator, str(value))
...
def _optimize_relational_name_search(leaf, model):
    ...
    return DomainCondition(leaf.field_expr, any_operator, value)
...
def _optimize_in_boolean(leaf, model):
    ...
    if value is leaf.value:
        return leaf
    return DomainCondition(leaf.field_expr, leaf.operator, value)
...
def _optimize_type_date(leaf, _):
    ...
    if leaf.operator.endswith('like') or "." in leaf.field_expr:
        return leaf
    operator = leaf.operator
    ...
    if value == leaf.value:
        return leaf
    return DomainCondition(leaf.field_expr, operator, value)
...
def _optimize_type_datetime(leaf, _):
    ...
    if is_day and operator == '>':
        ...
        try:
            ...
            return DomainCondition(leaf.field_expr, 'not in', OrderedSet([False]))
        operator = '<'
...
def _operator_hierarchy(leaf, model):
    ...
    if comodel._name == model._name:
        if leaf.field_expr != 'id':
            parent = leaf.field_expr
        if field.type == 'many2one':
