...
PdfFileReader.__init__ = lambda self, stream, strict=True, warndest=None, overwriteWarnings=True: old_init(
...
class OdooPdfFileWriter(PdfFileWriter):
                ...
                NameObject("/Params"): DictionaryObject(
                    {
                        NameObject("/CheckSum"): createStringObject(
                            hashlib.md5(attachment["content"]).hexdigest()
                        ),
    ...
    def addAttachment(self, fname, fdata):
        ...
        if self._root_object.get("/Names") and self._root_object["/Names"].get(
            ...
            new_attachment = self._create_attachment_object(
                {"filename": fname, "content": fdata}
            )
