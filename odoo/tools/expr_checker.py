...
def __ast_default_check_type(method, value):
    ...
    safe_type = (
        str, bytes, float, complex, int, bool, type(None),
        tuple, list, set, dict,
...
def __ast_default_check_call(func, check_type, *args, **kwargs):
    if func.__name__ == "get" and hasattr(func, "__self__") and type(func.__self__) == dict:
        return func(*args, **kwargs)

    for arg in (*args, *kwargs.values()):
        check_type("arguments", arg)

        if args and ("." in func.__qualname__ and not hasattr(args[0], func.__name__) or (
                hasattr(type(args[0]), func.__name__) and func != getattr(type(args[0]), func.__name__))) and hasattr(
                func, "__self__") and type(func.__self__) not in {dict, OrderedDict, list, JSON}:
