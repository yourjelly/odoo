...
def cleanup_csv(header, rows):
    for i, column in enumerate(header):
        if column == 'user_type_id':
            column = "account_type"
        elif column in ('tag_ids', 'country_id'):
            column = column + '/id'
        header[i] = '"' + str(column).replace('"', '\\"') + '"'
    for i, row in enumerate(rows):
        for j, value in enumerate(row):
            converted_account_type = convert_account_type(value)
            if converted_account_type is not None:
                value = converted_account_type
...
def convert_account_type(value):
    mapping = {
        ...
        "account.data_account_type_revenue": "revenue",
